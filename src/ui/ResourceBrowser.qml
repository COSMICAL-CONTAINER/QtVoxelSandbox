...
Item {
    ...
    readonly property var mobModel: [
        ...
        { mobType: 20, name: "洞穴蜘蛛" } // t1012③ 洞穴蜘蛛（§9 区隔命名；生物图鉴条目 + 生物蛋 0x25E 映射；蜘蛛同模 0.7× 蓝染——预览缩放经 mobPreviewScale 20 分支）
    ...
    //   机制等价 MC 1.0；坐姿几何走 MobModel sitPose（t878② 已就绪），狼/豹猫驯服态视觉 = 项圈环带
    //   几何（t986 起 MobModel collarVisible + materials[1] 项圈红——旧 t831/t920/t963 overlay 盒退役），
    ...
    function mobFallbackColor(t) {
        switch (t) {
            ...
            case 20: return "#1c3a52" // t1012③ CaveSpider 暗蓝染（与 Main.qml 蓝染体色 / 蛋生成色同源）
    ...
    function mobPreviewScale(t) {
        ...
        if (t === 17) return 0.95 // t782/t818/t968 燃烬者头+4棒全模型 1.30 高 ×1.14 宽（[-0.68,0.62]（t968 棒 Y 交错）/半径 0.57；t968 头 0.7³→0.42³）→ 0.95 撑满可辨不溢框
        if (t === 19) return 1.5 // t952 小蹒跚者全模型 ~0.97 高（[-0.45,0.515]）幼体小 → 放大可辨（同蠹虫放大口径）
        if (t === 20) return 1.05 // t1012③ 洞穴蜘蛛（蜘蛛同模 0.7×：预览放大补偿小体型可辨；全模型 0.43×0.7≈0.30 高 → ×1.05 观感同蜘蛛 ×0.75 档）
    ...
    function mobPreviewCentY(t) {
        switch (t) {
            ...
            case 20: return 0.06  // t1012③ 洞穴蜘蛛 [-0.30,0.13] 同模 ×0.7 → 上提 0.06 居中主体
            ...
            case 17: return 0.03 // t782/t968 头+棒跨 [-0.68, 0.62]（体心 -0.03 → 上提居中；棒 Y 交错对称近居中微调）
        }
        return 0
    }
                                ...
                                View3D {
                                    ...
                                    Node { // t966 pitch 父（生物分支）：恒铰链俯仰层；position 上提 = 俯仰绕生物自身
                                        ...
                                        Node { // t966 yaw 子：自转/拖拽转台层
                                            ...
                                            Model {
                                                id: mobPreviewModel
                                                // t986 驯服项圈环带：狼/豹猫驯服态（mobTamedActive 单源拨杆，同
                                                //   review0830 #23 单源口径）→ MobModel 几何追加裸颈段四薄板
                                                //   围合一圈项圈（mobmodel.cpp t986；替代旧 t920/t963 overlay 盒）。
                                                readonly property bool mobCollarActive: root.mobTamedActive
                                                // t986 材质按 subset 1 归属条件化：羊（毛茸态）subset 1 = 羊头 →
                                                //   [身, 羊头]；狼/豹猫驯服态 subset 1 = 项圈环带 → [身, 项圈红]；
                                                //   其余单段 → [身]。subset→materials 按下标配对，数组不能静态共用
                                                //   （狼项圈错吃羊头材质 = 项圈变毛块）。未驯服狼/豹猫无项圈 subset
                                                //   → materials[1] 不被消费。
                                                readonly property bool sheepHeadActive: root.selectedMobFromSection === 3 && root.sheepSkinHeadActive
                                                property PrincipledMaterial mobBodyMat: PrincipledMaterial {
                                                    lighting: PrincipledMaterial.NoLighting
                                                ...
                                                property PrincipledMaterial mobHeadMat: PrincipledMaterial {
                                                ...
                                                property PrincipledMaterial mobCollarMat: PrincipledMaterial {
                                                    // t986 subset 1 项圈：纯色项圈红（Main.qml rgba(0.76,0.16,0.16)
                                                    // 同值）；图鉴不做昼夜灰阶 / 受击红闪（纯色预览，同其他 overlay
                                                    // 眼/腿约定——承旧 t920 overlay 材质语义）。无贴图 → 环带恒红，
                                                    // pack 命中与否无关。
                                                    lighting: PrincipledMaterial.NoLighting
                                                    baseColor: "#c22828"
                                                }
                                                materials: sheepHeadActive ? [mobBodyMat, mobHeadMat]
                                                    : (mobCollarActive ? [mobBodyMat, mobCollarMat] : [mobBodyMat])
                                                geometry: MobModel {
                                                    // t986 驯服项圈环带：狼/豹猫驯服态（mobTamedActive）→ 几何追加
                                                    //   项圈（subset 1 = mobCollarMat 项圈红）；未驯服 false 单段绘制。
                                            ...
                                            //   t946 坐姿随移→t987 新链（Main.qml 成对契约）：尾根 = 站姿位绕坐姿
                                            //   根锚（t987 前爪着地点 (-0.42,-0.24)）旋 24.4° = (0,-0.148,0.564)
                                            //   + 垂尾搭地 35°+75°=110°（机制等价 MC 坐狼垂尾）。
                                            Node {
                                                visible: root.selectedMobType === 10
                                                position: root.mobTamedActive && root.mobSitPreview
                                                          ? Qt.vector3d(0, -0.15, 0.56) : Qt.vector3d(0, 0.16, 0.38)
                                            ...
                                            // t920/t963 驯服狼·猫红项圈 → t986 整删 overlay：旧「单横扁盒」心
                                            //   z=-0.30 埋进头盒 z 范围（只露两侧凸块 = 用户「两个红点」根因）；
                                            //   项圈改由 MobModel 几何裸颈段四薄板围合一圈（上方 mobCollarActive →
                                            //   collarVisible + materials[1]），与游戏内 delegate 同一几何源
                                            //   （t782 三消费端同源，不再各自复刻 overlay 盒；review0830 #23 单源
                                            //   口径由 mobTamedActive 拨杆承袭）。
                                            ...
                                            //   t946 坐姿眼随移→t987 新链（Main.qml 成对契约）：坐姿头心 (0,0.097,-0.195)
                                            //   + 眼偏移净 8° 随头旋 → (±0.08, 0.16, -0.39)。
                                            Model {
                                                visible: root.selectedMobType === 10 && root.selectedMobPackSrc === ""
                                                geometry: UnitCube {}
                                                position: root.mobTamedActive && root.mobSitPreview
                                                          ? Qt.vector3d(-0.08, 0.16, -0.39) : Qt.vector3d(-0.08, 0.16, -0.61)
                                            ...
                                            //   「驯服猫不走 pack 判据」）；t946 坐姿眼随移→t987 新链（与狼同修）：
                                            //   坐姿头心 (0,0.100,-0.135) + 眼偏移净 8° 随头旋 → (±0.07, 0.15, -0.28)。
                                            Model {
                                                ...
                                                geometry: UnitCube {}
                                                position: root.mobTamedActive && root.mobSitPreview
                                                          ? Qt.vector3d(-0.07, 0.15, -0.28) : Qt.vector3d(-0.07, 0.15, -0.53)
