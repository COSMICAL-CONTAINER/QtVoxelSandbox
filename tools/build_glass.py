...
边框 / 透明度迭代史（用户逐轮目视调定，改前必读）：
  ...
  t993 再增实一档（用户「透明度还是太透明」）：**材质 opacity 0.30→0.45**（Main.qml glassChunkComp，
    双钉同上）——只动材质轴，本脚本贴图零改动（无需重跑重生）；0.45 同时收口 t899 起手持玻璃立方
    （heldCubeIsGlass 分支恒 0.45）与世界段的漂移（玻璃三消费端同值）。
pack 态瓦片（包内 glass.png）不适用（玻璃合法半透），本调整仅程序态自绘；
材质 opacity 为 pack-on/off 两态共用（pack 只覆写 tile 68 像素）→ t993 增实两态同调。
...
def draw_edge(c):
    ...
    粗框 → 收回单圈、深度取两档之间偏浅（棱线可辨但不抢面）。透明度轴由材质 opacity 承担（t993 起回 0.45，
    见文件头迭代史），棱深只管「框感」。"""
