...
### 🅺 下界传送门无限尺寸（t848）
**t848** 传送门无限拓展：t806 的 2..4 宽/3..5 高上限取消——**轴向 for 循环一直检测**黑曜石长度（用户明示「可以无限延长」；上限注释说明与 MC 23×23 的取舍=按用户要求不设限）；实测复现 5 长×4 高点不着（t806 声称 4..5 支持但用户实测 5×4 失败——真 bug 定位）；**共用边框门**：两门中间共用一竖列黑曜石时各自可点燃（框检测从「独占框」放宽为「共享柱可复用」）。
