...
### 🅵 TNT 与性能（t996-t997）
**t996** TNT 引燃后方块不清除：点燃 TNT 后**原方块没清除**——持续闪烁动画不停、**方块变成贴图、人物可以穿过去**。引燃链应把原 TNT 格置 Air，由 primed 实体接管闪烁渲染与碰撞；现状卡在中间态（疑 primed 实体化失败落入渲染 fallback）。验收：点燃瞬间原格清空、可穿行；闪烁动画只存在于 primed 实体、随爆炸终止；爆炸/伤害/掉落链零回归；阴性轮钉「置 Air」调用（回退 → 原格残留复现 → 复原）。
**t997** 大量 TNT 同爆性能：t933（单爆炸级联风暴）修复后，用户 9-01 @ 35d5a72 实测**放很多 TNT 爆炸仍很卡（15FPS，66.7ms/frame，sim 仅 4.63ms）**——多 TNT 同爆场景不在此前修复覆盖内。用 t934 交付的 F3 render-side 行（`gpu`/`prep`/`present`/`(N)` 样本数/`act ct`）在复现 rig 归因卡顿源 → 修（节流/合批/限并爆/爆炸波分期）或登记判读结论 + 治理项。验收：复现 rig 量化归因逐项有数；有修复则前后帧数据对比，无修复则登记结论与治理路径。
