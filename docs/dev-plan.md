...
**t978** 生物复制体泄漏：新建世界进去生物生成**瞬间位置出现一个复制的投影**；游玩时间久了**地上全是静止的、只有贴图的复制生物**堆着不动（用户原话「大问题」）。F3 佐证 `mobs 1/36 items 0/71`：C++ 侧只 1 只活体但槽池 36 高水位 = 复制体在 QML delegate/监视器层而非实体层。疑面（先证伪再修，不锁死）：t935 槽监视器指纹差分下死槽/复用槽 delegate 可见性（死槽不再 bump → 隐藏不触发？）、spawn 槽复用瞬间新旧内容交叠、跨世界槽池残留。验收：spawn 瞬间零双影；长玩 + 大量生成/死亡后场上静止复制体 = 0；换世界后无残留；t935 的 bump/fan 收口经济学不回退。
