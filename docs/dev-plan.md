...
| t285 | ✅ | **蜘蛛**：快速移动（可爬墙；昼伏夜出）**〔t1044 勘误：本行「可爬墙 ✅」在 t285 当时与实现矛盾（仅落地快速移动 + 1 格越障跳），自 t1044 落地（parity 裁-2）起表述方成立〕** | t281 | Entities（spider climb/fast） + textures |
...
**t973** 生物格放方块检测：生物占据的格子**不能放置方块**（防活埋）——放置预检加实体占用门。 ✅✅（457acf7）占用判定单一权威 = 新增 `EntityManager::mobOccupiesCell(bx,by,bz) const` **只读查询**（Game→Entities 向下依赖，hostileNearby/mobTypeCountNear 先例；非 Q_INVOKABLE C++ 侧查询）：写入格**整格 AABB** 与任一**活体 mob** 碰撞盒（pos±per-type halfW/halfH，与 mobAabbHitsSolid/findMobHit 同一张半宽表）**严格相交** → 拒。选 **AABB 相交**而非格中心点：大盒生物可跨格缘（铁傀儡 halfW 0.6 / 夜行者 halfH 1.4），点判恰漏「盒压邻格而中心不在」的活埋；MC 同为实体碰撞盒×放置盒相交判定（preventEntitySpawning 语义）。**严格比较**（相触不算重叠）：mob 脚底正好站格顶 / 身侧贴邻格壁不拒邻格。**排除面清单**（逐条落定）：dead（hp≤0 濒死/死亡动画帧）不拒；掉落物（ItemEntityManager 域）/箭/浮标/雪球/引燃 TNT/下落方块等非 Mob kind 不拒（MC「物品可以盖」）；矿车/船在各自 Manager 不在查询域（骑乘矿车旁放置不受干扰）；桶倒流体不走本链（桶分支在门前早 return——MC 桶可对生物倒，伤害面归伤害系统），只拦方块；种子/树苗/浆果种植走专用分支不经本门（植物无碰撞不活埋）；创造/生存同规则（预检不读模式）；玩家自埋面由既有 overlapsPlayerAABB 管不动。**预检序位** = 通用放置预检层：「目标格必须空」(t198) 与「不与玩家重叠」(t146) 之后、方块族专项预检（火把 t114 为首）之前——族专项答「方块放这合不合法」、本门答「格子被不被活体占」，两问正交；写入格全覆盖：目标格 + 门上格 + 床头格（与玩家重叠双格查对称），且 t163(b) 同格合并 / t262 邻格合并 / t505 雪层堆叠三个**先于通用链的写入位**同门（合并=写入格碰撞盒扩到整格/增高，活体站本格即活埋）。拒绝 = no-op return（不挥不消耗，同其它预检拒绝口径）。骑乘组合（t952 小僵尸骑鸡）：骑手/载具两盒逐实体独立判，撞任一即拒。**探针 P-t973**（矩阵 392→393，P-t945 真玩家全链 rig：loadSavedState→tick 射线→placeBlock）：(a) 空格对照成功+挥手（链路通证，(b) 的拒非射线落空 no-op）；(b) Survival 蹒跚者**实读占用格**放置 → 拒（格 id 不变+栈不消耗+不挥手+hitBlock 钉预检链真到达）；(c) 掉落物占格成功（同场活体僵尸在可达域外 ≥3 格——唯一差异=占用者身份）；(d) damageEntity 致死帧同格成功（槽仍 alive）；(e) tryMount 骑地面矿车、从座位实读眼位瞄车旁带 → 放置成功+骑乘不断；(f) 源码钉（门调用行 / Entities 排除面 `alive && kind==Mob && !dead` / 门/床双格查 / 合并位门 / 序位语句钉——torchHostOk 锚，`==Torch` 文本在 placeState 计算段更早出现不能作锚）。**rig 防漂移**（lessons t836/t897）：僵尸只与 (b)(d) 同场——(b) 紧跟 spawn（1 aim tick 位移 ≤kChaseSpeed×dt钳0.05=0.14 << 格余量 0.2，盒恒在出生格内），(d) 时已 dead 冻结；(c)(e) 目标格离可达域 ≥3 格；per-leg delta 计数（栈/挥手基线）免疫前腿级联。**阴性轮**：摘除通用门主调用行 → 恰 t973 FAIL（392/1——石头落进活体蹒跚者格内+消耗+挥手，用户症状复现；源码钉同红如预告），复原 → **393 PASS / 0 FAIL**（基线腿 t960 已知 flake 复跑清）。编译零警告（dxcompiler 豁免）；exe 冒烟 18s 存活零 TypeError。待目视（实机复现类）：站僵尸旁往它身上放方块被拒、对空地照常；骑矿车时沿路放方块不受干扰；掉落物上可直接盖方块。
**t974** 保存退出偶发未保存：有概率重进是上一次存档点——保存时序/异步写完成前的退出竞态。 ✅✅（4964ffc）**调查判决**：保存链（savePlayerData → saveAll → saveProgress）全部是同步 SQLite 写（commit 返回即落盘），链上**无 deferred / 分帧写**——「异步写完成前退出」假设证伪。真实概率窗口有二：①**退出存档是 fire-and-forget**——Main.qml 三个写盘调用返回值全部弃置，任一瞬态失败（外部进程瞬持 .sqlite 文件锁：杀软 / 索引器 / 同步盘；磁盘满）即 saveAll 单事务整回滚，库中留**上一次存档**、退出照常进行——回滚保旧档与症状逐字吻合（「重进是上一次存档点」）；②**窗口关闭路径从未落盘**——playing 态直接关窗（X / Alt+F4）无任何保存点，进程即退，自上次保存退出后的进度全丢；用户分不清本次走哪条退出路径 → 观感「偶发」。**修法（写完成核验后才拆链，双退出路径全覆盖）**：Main.qml 三写收口为 `runExitSave()`（返三写与）——saveAndExitToWorldList 核验结果、失败幂等重试一次（全量重写语义：saveAll DELETE+INSERT / 两 upsert OR REPLACE；两次调用间同步 JS 串行无 tick 插入，快照不漂移）、仍败则 toast「存档写入失败，本次进度未保存」+ console.warn 显式留痕绝不静默；完成门置于 `coverGrabPending = true` **之前**（「写盘在退出前同步完成」链序）；新增 `onClosing` 兜底走同一条 runExitSave 链 + closeWorld 后再放行关闭（onCoverGrabbed / finishExitToWorldList 本就以 isOpen() 守门，中途态提前关库按降级路径安全收尾）；WorldStore 新增 `saveOkCount` Q_PROPERTY——每次成功持久化恰 +1（saveAll 在 commit 成功门后、两 upsert 在 exec 成功后），失败绝不计数 = 与 false 返回值同源互证的行为级写完成观测面。**原子性核验**：saveAll 单事务（DELETE 全量 + INSERT，chunks 主键 cx/cz）→ 失败/中断恒回滚到上一完好档，半写永不可见（PLAN §2-L 事务性设计兑现确认；无 autosave 通道，无双写竞态面）。**探针 P-t974**（矩阵 393→394，真 WorldStore + 真 3×3 chunk 世界 + PID 后缀临时库）：(a) 行为腿——退出存档 → **立即 closeWorld** → 重开重载 → 标记块 / 玩家位 / 背包栈 / 进度全为新档（非上一次存档点）；(b) 写完成计数——三写恰 +3；(c) 竞态窗口阳性腿（确定性复现）——第二连接 BEGIN EXCLUSIVE 瞬持库写锁（野外=杀软/索引器/同步盘）→ 三写**全部失败且可检测**（false + 计数零动）→ 释放后重读旧档完好（回滚保档 = 症状库侧形态 + 半写不可见实证）→ 重试三写成功 +3（QML 重试语义的 C++ 级验证）；(d) 源码钉——runExitSave 门（核验→重试→toast）先于 coverGrabPending、onClosing 同链 + 关库、计数器契约（Q_PROPERTY + 恰 3 处 bump、saveAll 在 commit 门后）。**阴性轮**：QML 完成门回退 fire-and-forget → 恰 t974 FAIL（393/1）；复原 → **394 PASS / 0 FAIL**。`-Wall -Wextra -fsyntax-only` 两触碰 TU 审计零新告警（测试 TU 16 条既有告警全在本 diff 行号之前）；exe 冒烟 `root objects after load: 1` 零 TypeError。待目视（实机复现类）：退出存档失败时 toast 文案出现（需人为锁库，常规路径不可见）；关窗重进进度保留。
**t975** 创造拿取语义再反转（用户 8-28 定稿）：**左键=拿一组**、**右键=只拿一个**（t896 的左键 1 个被翻案）。 ✅✅（c426c67）**逐入口清单（全库核）**：创造拿取面唯一 = Inventory.qml 调色板格（无限源）；其余槽位面（Inventory 护甲/合成/主栏/hotbar 行、AnvilUI、EnchantingTableUI、SurvivalInventory）左/右键走 InventoryOps 生存共享语义（左整组拾取=有限源消耗 / 右拿半，非创造拿取）、中键走 t653①/t896/t956 复制面——**全部零触碰**。**修法**：调色板左右两键收敛新 `paletteTake(id, takeCount, toggleReturnOnSameId)` 单一入口（Inventory.qml:519），分支结构 = t896 前左键原样（t632 预设书 / t318 同格 toggle / t136-t292-t356 换拿归还虚空），仅数量随键位分传：左 = `maxStackSize(modelData)` 整组（工具/桶/护甲经单一权威天然 1 件，t33 口径）、右 = 1 件；新增调色板 per-slot 右键 TapHandler（t181/t166d 按下不动抓/越阈值 DragHandler 接管的共存模式）。**右键不接 t318 toggle**（归还意图只挂主拿取键）——否则「右键连点拿一个」被「点原格归还」吞成拿/还振荡；右键同格 = **续拿 +1**（cap = maxStackSize 单一权威，满组静默 no-op），异格 = 换拿 1 件。**与中键复制语义界线**：中键 = 纯复制（不受 t318 toggle、元数据面同 copyStackToCursor 口径），左键 = 拿取（同格触发 t318 归还）；调色板无限源下「拿一组」与「复制一组」数量等价，差异仅在 toggle 与元数据保真——中键面零触碰保留。**探针 P-t975**（矩阵 394→395）：(a) 源码钉（paletteTake 三段结构 / 左 maxStackSize+toggle、右 Qt.RightButton 邻近段 1+无 toggle / 无限源拿取字面恰 2 处〔入口+中键〕/ paletteTake 调用恰 2 处——散写第三处即红）；(b) 行为腿 = t874/t956 装配法（源树 Inventory.qml 临时目录直载 + 真 Hotbar VM + CharacterPreview3D 桩 + wrapper 供 window/cursorTracker 作用域），returnHeldToVoidRequested 接宿主同义 sink（Main.qml 直连 `heldBlock=0` 行为级镜像），九腿：①左键一组 64 ②左键同格 t318 归还虚空 ③右键空手 1 件 ④右键连点 +1=2 ⑤右键满组 cap no-op ⑥右键异格换拿 1 件 ⑦左键异格换拿整组 ⑧工具左键「一组」=1 ⑨预设附魔书（哨兵）= 0x227×1 带锐锋 I 附魔；itemTaken/voidReturn 发射计数精确钉（拿取发、归还/满组 no-op 不发）。**阴性轮**：Inventory.qml 回退 t896 形 → 恰 P-t975 FAIL（两轮稳定；t960/t882 已知 flake 轮换复跑清）→ 复原 **395 PASS / 0 FAIL**。P-t896 合法演化（P-t949(d) 先例）：被翻案的左键 1 钉移交 P-t975，保留 t896 仍拥有的面（中键整组复制 / Math.min 绝迹 / maxStackSize 权威）。编译零警告；exe 冒烟 15s 存活、root objects 1、零 TypeError。待目视（实机复现类）：创造调色板左键整组上手、右键 1 件、右键连点逐个累加、中键复制照旧。
**t976** 耐久条显示回归修（t931 后反了）：已消耗耐久的镐子背包里**不显示**耐久条（hover 能看到耐久掉了）——t931 修复引入的回归（显示条件写反或绑定面漏）。 ✅✅（0b89af1）**调查判决**：显示条件本身没反——t931「满耐久隐、受损后持续显」语义正确，引擎级 rig（真面板 QML × 真 Hotbar：预置损伤 / 创建后到达 / live damageSelectedItem / 创造面板 tab 切换）五面全绿；真根因在**实机 qmlcachegen AOT 面**——DurabilityBar.qml 单独成编译单元，aotstats 证组件内 `visible` 是整条耐久链上**唯一被静态 AOT 编译的链接**（codegenResult 0），组件内 width/color 因跨对象属性静态不可解析自动回退解释执行（codegenResult 2）——显隐决策隔着「面板单元绑定写 curDur/maxDur → 组件单元 AOT 绑定再读」两跳跨单元链实机不重算：tooltip 走信号处理器直读 VM 恒新（「hover 能看到耐久掉了」）、条恒隐。t498「进背包无耐久显示、只在 hover tooltip 显」同症（该轮为护甲加 cur/max 数字文本），t498 数字 / t640② 组件 / t931 语义三次迭代每次解释器侧验证全绿而实机仍隐，即此 AOT/解释分叉同根。**修法（决策上收，不动 t931 语义）**：五个使用点（Inventory 生存 tab 主栏 / hotbar 行 / 护甲槽 + SurvivalInventory 主栏 / hotbar 行）显隐决策上收面板 delegate 单元，qml-touch 三轮表达形式触碰 revision 并参与返回值：`visible: { const _r = root.hotbar.<mainRevision|slotRevision|armorRevision>; return _r >= 0 && mDur > 0 && cDur > 0 && cDur < mDur }`——与图标/数量同款实机已实证形状，重建 aotstats 证新绑定回退解释执行（codegenResult 2）；`curDur < maxDur`（t931 满耐久隐）逐字保留；组件内 visible 保留作语义兜底 + 组件头新增**使用契约**（逐槽必须自显 revision-touching visible 及根因说明）。**探针 P-t976**（矩阵 395→396）：(a) 源码钉——三 revision 变体逐字钉 Inventory 恰 1/1/1 次、SurvivalInventory 恰 1/1 次 + 组件头 t976 契约段 + 组件内 t931 语义行保留；(b) 行为腿（t874/t956/t975 装配法：真 SurvivalInventory.qml × 真 Hotbar × InvSlot 文件面）：①预置损伤镐 hotbar 行 200/250、主栏 150/250 → 条 visible 且彩段宽 ∝ 比例（27.2/20.4 @barW 34）②满耐久 250/250 → 隐（t931 语义保持）③创建后到达 60/250 → 显（revision 驱动重算）④live damage：选中满耐久镐 damageSelectedItem×30 → 220/250 由隐转显（签名不与腿① 相撞）⑤空槽条恒隐 + 总条数 40（4 armorDurBar 内联 + 27 主栏 + 9 hotbar 行）。**阴性轮**：撤 SurvivalInventory 主栏 visible 上收行（回退组件内隐式决策）→ 恰 P-t976 FAIL（395/1）→ 复原 **396 PASS / 0 FAIL**。编译零警告（dxcompiler 豁免）；exe 冒烟 15s 存活零 TypeError。待目视（实机复现类，须用户在新版本戳 exe 上确认后关单）：背包受损镐显耐久条且随损耗变短/变红、满耐久无条；HUD 1-9 行为不变。
**t977** 切换物品栏物品名浮显：切槽显示当前物品名——血量/饱食度**上方中间**、白字；**附魔物品显示详情**（名称+耐久度+换行+逐条附魔带等级）；改名物品显示改名后名字（多附魔工具区分用）。一定时长淡出。 ✅✅（dc5e55e）组装单一权威 = 新增 `Hotbar::slotDetailText(slot)` Q_INVOKABLE（QML 只消费字符串——**AOT 教训 t976/t177 落实**：文本组装不上 QML 绑定，呈现层经信号处理器直读；事件级 = 仅切槽读一次，无每帧直发信号）：空槽 → 空串（不显；显「空手」从简不做，口径登记）；普通物品单行「名称」（nameAt 走 t477 customName 优先 → 改名物品显改名后名字，多附魔工具据此区分）；附魔物品 = 「名称」+换行+「耐久: cur/max」行+换行+逐条「附魔名 罗马等级」（enchantListText t590 同款；耐久行按 t349「有无耐久」判、护甲段 toolMaxDurability=0 时另查 ArmorRegistry——hotbar 槽可持护甲件；附魔书有附魔无耐久行、带耐久无附魔单行名，两行互不依赖）。呈现 = 新组件 `HeldItemNameFlash.qml`（Main.qml 挂 vitalsBar 上方 8px 居中，与 t508 下船提示同行位——同显叠字从简不互斥，登记取舍；Creative 亦显、观察者 active=false 连带不显）：切槽 → 立即全不透明显（无淡入）→ holdMs **2000** 停留 → fadeMs **300** 淡出；连续切槽 restart 天然防抖只显最后槽；lastShownSlot 守卫拦「同槽内容变更」（拾取入手/铁砧改名/附魔/工具破损清槽都会对选中槽补发 selectedSlotChanged——非切槽不显）；active=false（菜单/读档 applyPlayerState 灌 selectedSlot 早于 appState 切 playing）只重同步基线 → 进世界恢复不闪上局持物名；Timer 属 UI chrome（toast 同族）不 gate worldRunning（t884 清点口径豁免，登记）。白字黑描边自绘（§9 override (a)），零 MC 资产/专名。**探针 P-t977**（矩阵 396→397）：(a) 组装权威行为腿（真 Hotbar 直调逐字断言）：附魔+改名镐恰 4 行「挖掘者/耐久: 200/250/锐锋 III/效率 II」/ 普通方块单行名 / 空槽空串 / 铁胸甲「耐久: 120/max」护甲耐久行 / 附魔书「附魔书\n锐锋 V」无耐久行；(b) QML 行为腿（真 HeldItemNameFlash.qml × 真 Hotbar rig，headless 可达——组件纯 QtQuick，Timer/NumberAnimation 走引擎动画时钟）：创建基线静默 / 切槽文本=当前物品名且立即 op==1 / 附魔镐 QML 消费面==C++ 权威输出逐字 / 空槽切立即隐 / 连续切槽计时重置（t0 切 A、t0+140 切 B，t0+370 采样仍全不透明）+ t0+750 淡出收尾 op==0 文本保持 / holdMs-fadeMs 探针短节拍（300/100，生产默认由源码钉钉住）；(c) 源码钉：Main.qml 挂载块（vitalsBar 上方 8px 居中 + hotbar 注入 + active 游玩态门）+ 组件白字 / hold 2000 / fade 300 默认常量 + onSelectedSlotChanged 直读 Q_INVOKABLE + lastShownSlot 守卫 + 非活跃基线重同步 + 「立即显」行**双行锚钉** holdTimer.restart()（裸计数钉会撞头注释字面——首轮 pin 假红教训）。**阴性轮**：撤 flash() 的 holdTimer.restart()（计时重置回退）→ 恰 P-t977 FAIL（396/1，qml 腿 fade-end 红 = 名字永驻不淡出）→ 复原 **397 PASS / 0 FAIL**（首轮 t882 已知 flake 复跑自清）。编译零警告（dxcompiler 豁免）；exe 冒烟 18s 存活 root objects 1 零 TypeError。待目视（实机复现类）：切槽/滚轮显名 2s 淡出、附魔工具显名称+耐久+逐条附魔等级、改名工具显改名名、空槽不显、进世界不闪名。
...
## R19.18 用户实测复盘批六（t978-t997，20 项；2026-09-01 立项）

**立项背景**：R19.17 + review0830/review0831 全闭后第六轮实测（用户 @ 35d5a72）。四条主线：① **生物复制体泄漏（用户原话「大问题」）**——spawn 瞬间位置出现复制投影 + 长时间游玩满地静止「只有贴图」的复制生物，F3 `mobs 1/36` 高水位死槽为头号嫌疑面（时间线上 t935 槽位指纹差分 8-28 刚落地，须最先证伪）；② 矿车铁轨三连（V 形谷底振荡 / 转弯+上坡互斥 / 平行吸附与站轨选块）+ 仙人掌两修（其一为**口径翻案**：仙人掌旁放轨用户口径 = 能放，不掉仙人掌）；③ 狼/豹猫生态三修（项圈 / 坐姿 / 战斗 AI 越障跳）+ 查看器两修（生物蛋移除 = t967 局部翻案 / 方块默认视角回归俯视 = t966 回归疑）；④ **大量 TNT 同爆仍 15FPS**（t933 单爆炸风暴修复不覆盖多 TNT 场景，sim 仅 4.63ms——大头在渲染/同步侧）+ TNT 引燃后方块不清除（闪烁残影 / 贴图化 / 可穿人）。

**F3 快照存档（用户 2026-09-01 17:22 @ 35d5a72，大量 TNT 爆炸后、夜间、创造模式）**：
- `FPS:15, 66.7 ms/frame, 4.63 ms sim`（模拟侧只占 7%——卡顿不在 tick，在渲染/同步等待，t934 交付的 render-side 行正好归因）
- `mesh: culled terrain verts: 1508528 tris: 754264`、`chunks:10x10=100 render r=3 window 42/100`（爆炸后表面破碎，顶点量暴涨）
- `entities: mobs 1/36 items 0/71 orbs 0/3`（场上仅 1 活 mob 而 36 槽高水位、0 活物品而 71 槽高水位——t978 复制体泄漏的槽位形态）
- `pos 46.7,66.6,108.4 yaw -36 pitch -60`、`time 20:35 day 0`（深夜）

### 🅰 生物（t978-t980）⚠️ t978 最高优先（用户原话「大问题」）
**t978** 生物复制体泄漏：新建世界进去生物生成**瞬间位置出现一个复制的投影**；游玩时间久了**地上全是静止的、只有贴图的复制生物**堆着不动（用户原话「大问题」）。F3 佐证 `mobs 1/36 items 0/71`：C++ 侧只 1 只活体但槽池 36 高水位 = 复制体在 QML delegate/监视器层而非实体层。疑面（先证伪再修，不锁死）：t935 槽监视器指纹差分下死槽/复用槽 delegate 可见性（死槽不再 bump → 隐藏不触发？）、spawn 槽复用瞬间新旧内容交叠、跨世界槽池残留。验收：spawn 瞬间零双影；长玩 + 大量生成/死亡后场上静止复制体 = 0；换世界后无残留；t935 的 bump/fan 收口经济学不回退。 ✅✅（1b5b43c）**静态审计判决：既有路径无自发冻结面**（P-t935 已钉槽级 bump 语义、六处 releaseSlot 调用点审计各带 alive 守卫、指纹契约对 Main.qml 全部 At() 访问器逐项核账无缺字段）→ 修法定为**三层防御性收口**而非单一病灶：① releaseSlot 幂等守卫 `if (!e.alive) return;`——重复释放会把同一索引入 free list 两次，之后 LIFO 两次 spawn 共写一槽（后者 std::move 无声覆盖前者 + liveCount 漂移）= 槽池簿记损坏面，守卫钉死「free list 恒无重复索引」结构不变量；② clearAll **死槽纠正 bump**——有监视器（=有 delegate 在看）的死槽恰 +1：其指纹早在释放 notify 已对齐死态、常规 notify 永不再 bump、冻结 delegate 无常规自愈面，跨世界清场是唯一必经的整池刷新点，在此强制自愈；无监视器死槽零 bump（无观众零成本）保 t935 经济学；**实现教训：纠正循环必须在释放循环之前**——放其后会对「本次 clearAll 刚释放的槽」纠正+差分各 +1 双 bump（实机 diag monRevB 观测 (2,2,3) 与 bump 模型逐槽吻合后修正），顺序约束已钉进 for 行注释；③ Main.qml mob delegate visible 追加 `entityManager.count` 触碰（NOTIFY 同为 entitiesChanged）——mon.revision 仍是主依赖（本槽可见态变化才重算其余 ~49 绑定，t935 收口不动），count 触碰兜「监视器/依赖因任何机制失灵」的冻结面：死槽 delegate 最迟下一拍 notify 重读 aliveAt=false 归位（成本每槽每 emit 1 次 bool Q_INVOKABLE，对照 t935 消掉的 50 绑定/槽可忽略）；`entityManager.revision` 恒绝迹、`mon.revision` 迁移面 ≥100 不减（P-t935(d) 钉保绿）。**探针 P-t978**（矩阵 397→398，真 EntityManager+真监视器非复写）：(a) 双 removeEntityAt 后两 spawn 必得两个不同活槽 + liveCount==2 精确；(b) 三槽全建监视器→预杀槽 1→clearAll 全槽恰 (r+1,r+1,r+1)、LIFO 复用槽 2 恰再 +1、未复用死槽零额外 bump；(c) 三修复串源码钉 + t935 迁移面钉。**阴性轮**：三修复体全撤 → 恰 P-t978 FAIL（397/1，diag a/b/b2/c 全 false——同轮 t897 偶发红〔探针注释自证 flake，非本域〕复跑清）→ 恢复 **398 PASS / 0 FAIL**。零警告（dxcompiler 豁免）；exe 冒烟 18s 存活 root objects 1 零 TypeError。待目视（实机复现类）：新建世界 spawn 瞬间无双影；长玩后地上无静止复制体堆积。
**t979** 猪两修：① 猪死亡掉落**皮革**（错——对照 MC 1.0 掉落表：生猪排；若火焰致死走熟排则核既有火焰链口径）；② 猪在**水位 1/8、1/4**（水格高度的 1/8 与 1/4，脚踝深、身子远未没入）居然会**淹死**——明明一直在努力往上跳还是淹死。溺水判定应按身体淹没程度（水位高度 vs 口鼻/眼部高度一类的单一权威），不能脚位格沾水就窒息计时；浅水 + 跳跃应能脱困。验收：1/8、1/4 水位猪零溺水伤且能跳出；没顶深水溺水链仍有效；掉落表修正。 ✅✅（9d9b3e9）**根因**：t828 溺水头判 + t923 浮面浮力 + resting 破除**三处同式整格布尔**（floor(pos.y+halfH·0.8) 处 blockAt==Water）无视水位档——猪 halfH=0.45 口鼻线 0.81<1 格，站脚踝水洼口鼻线仍落脚位水格 → 误判没顶 15s 溺亡；「努力往上跳还淹死」=同误判驱动的浅水浮力空跳。掉落侧 t473 给猪加皮革 ×1 对照 MC 1.0 掉落表为错（皮革属牛）。**修法**：新谓词 `mobSnoutSubmergedInWater`（口鼻线 vs 格内实际液面连续比较；水源满格液面=格顶与旧判在源块水完全等价 → t828/t923 既有探针零变化）收口三消费点；Main.qml 猪分支去皮革只掉生猪排 ×1-2，burned→熟排火焰链核对不变。**玩家侧不同病登记不修**：眼高 1.62>1 格站立时眼位格恒异于脚位格，浅水结构性免疫。**探针 P-t979**（矩阵 408→409）：1/8、1/4 水位零溺水伤+跳出 / 盖顶水槽（矮桥洞语义，口鼻钉水面下——开放水洼腿不敏感：猪数秒走出即清计时器，本轮实测改判别腿）满格没顶照常溺 / 掉落表腿 / 源码钉。**阴性轮**：回退为旧整格布尔 → 恰 P-t979 独红（408/1，盖顶水槽 4 头溺亡复现）→ 复原 **409/0**。rig 硬化（本轮踩坑记档）：没顶水柜补四周全高壁防漂浮漂移假红、脱困腿改粘性「曾离开」标记防折返误判。零警告；冒烟零 TypeError。
**t980** 鱿鱼陆地生存：鱿鱼在**陆地上保持行走步态且不掉血不死**。两修：陆上窒息掉血至死（MC 1.0 语义——离水持续受伤）；陆上移动语义不再是行走步态（挣扎/缓动）。验收：陆上面持续掉血可至死亡；水中游动/漂浮行为零回归。 ✅✅（b83d4b2）**根因**：窒息块整体豁免鱿鱼且无替代链（离水不死）+ 搁浅分支委托 aiWander（正常行走步态）。**修法**：成对反向谓词 `mobBodyAboveWaterSurface`（体底线 vs 液面，半浸水洼不算离水）→ 1s 宽限后 1HP/s 至死（复用 damageEntity/mobDied 链、回水止血、计时器独占 mobAir/mobDrown 字段——溺水块对鱿鱼跳过）；aiSquid 搁浅改间歇挣扎（1.6s 周期随机换向、前 0.6s 半速蠕动后 1.0s 摊歇）。**单一权威设计（与 t979 成对）**：同用「采样线 vs 格内液面」比较式 family——口鼻线（空气呼吸者：溺水+浮面+resting 三消费同源）/ 体底线（水生者反向锚），源码钉 ≥4/≥2 处防外散。**探针 P-t980**（矩阵 409→410）：陆上 1HP/s 至死精确 + 水中零回归 + 步态退化钉 + 权威源码钉。**阴性轮**：双刀一轮 → 恰 P-t980 独红（窒息链短路 hp@12s=10 + 搁浅回退 aiWander maxSpd=1.0）→ 复原 **410/0**。rig 硬化：水井补侧壁防鱿鱼漂出井口假红。零警告；冒烟零 TypeError。待目视：鱿鱼搁浅挣扎掉血至死、水中游动如常。

### 🅱 矿车与铁轨（t981-t985）
**t981** V 形轨谷矿车振荡：矿车放在 V 形铁轨（两侧斜坡相对成谷）上滑落到底部时**卡住、来回振荡**最后才停下。谷底应一次平滑减速停驻（或按速度通过），不允许往复振荡。注意 review0831 #28「embedded-cart sampling-lost freeze escapes the exemption contract」登记项同域——本任务实证其是否即此症并一并清算。验收：V 谷场景矿车滑底一次停驻、无往复振荡；平轨/常规坡道行为零回归。 ✅✅（7c5f993）**根因两症**：① 无电 V 形凹谷格物理全量保守零耗散——下坡半幅 kCartSlopeGravity 加速/上坡半幅同量减速，t863① 失速反溜（-0.5）与 t909② 静置闸 kick（+1.0）在两壁停驻点反复再供能 → 滑落谷底后两壁间极限环往复；② review0831 #28 实证为**独立第二症**（振荡空场即复现、无需嵌入方块）：stepCartAlongRail 受阻三分法 !sampled 分支无条件回退+清速、不看 embeddedAtEntry 豁免 → 嵌入车遇失联子步被永久打回子步起点，违背函数自身豁免契约。修法：① 新增 tryValleyBottomCapture（滑行/骑行 coasting 两半边共用）：|speed| ≤ kCartValleyEscape（=重力终端 10，boost 12.8 可穿透）进 V 谷格即按「制动到谷心」速度律（a0=v²/2d 每 tick 自校正）一次平滑减速停驻谷心（谷心梯度 0 = 静置闸稳定不动点）；金轨刹车/boost 优先不扩面；② !sampled 分支补 haveFreePos 门：非嵌入回退、嵌入车豁免延续直至脱出；两处矛盾头注释同步修正。**探针 P-t981**（矩阵 400→401）：谷一次停驻反转数==0 + 终位谷心 ±0.06 + 60tick 静止守卫 / 通电 boost ~11.6 穿谷而过 / 骑行嵌入车越格 / 源码钉。**阴性轮**：两修回退 → 腿 a 反转 18 次（极限环实锤）+ 腿 c 冻结，恰 P-t981 FAIL（400/1）→ 恢复 **401/0**。零警告；exe 冒烟 17s 零 TypeError。待目视（实机复现类）：V 谷矿车一次停驻谷心、boost 车穿谷。
**t982** 铁轨转弯+上坡互斥（用户口径：**一格铁轨绝对不可同时转弯和上坡，只能做到一点**）：现在构造坡上转弯场景，**转弯贴图被拉伸 45° 兼作上坡**。修轨道形状判定——turn 与 ascend 互斥（坡上转弯处按 MC 口径落成平转弯段或直上坡段之一）；斜坡段几何/贴图不再借用转弯贴图拉伸。验收：坡顶接转弯场景逐格形状符合互斥表；无 45° 拉伸贴图；直线坡/平转弯各自零回归。 ✅✅（04a72e1）**根因**：railConnections 规则①（t709 臂高放宽后）允许带坡臂垂直对成弯——mesher 拐角 quad 沿坡臂整边抬 1.0（armLift）= 转弯贴图拉伸 45° 兼作上坡；矿车 railRiseAt 同面 45° 爬坡过弯（物理与渲染同一违例几何）。修法：规则①立用户铁律「一格铁轨绝对不可同时转弯和上坡」——两臂同层（各自三高探针层差均 0）才许弯，任一臂带坡即禁弯、落成坡臂轴向直坡段（坡度优先于转弯，垂直臂弃连后自行重算为独立 stub）；规则⑤转辙器 T 分支弯道端恰一端带坡时选平端落弯；t709 坡臂拐角放宽被本铁律推翻（注释即契约同步改写）。**探针 P-t982**（矩阵 401→402）：坡顶/坡底接转弯逐格 shape=直坡段、同层垂直对照常拐角零回归、骑行行为级（停驻坡顶格心不被拽上坡臂）、源码钉互斥两行；P22 review#3 腿随规格改版平拐角路由。几何/视觉规则按口径豁免阴性轮（行为+源码双钉）。零警告；exe 冒烟 17s 零 TypeError。待目视（实机复现类）：坡上转弯处只显直坡或平弯其一、无 45° 拉伸。
**t983** 铁轨平行吸附 + 站轨选块：① 三种铁轨**平行放置时吸附作用怪异**、「完全不遵守规则」（连接判定对齐 MC：平行相邻轨不互连，只有端点相对才连接）；② **站在铁轨上朝前放置前向铁轨**：准星已指向目标位置却选不中——实际选中的是**脚底下的铁轨**。选块必须指哪指哪（疑非完整方块命中盒用整格 AABB 抢命中，准星视觉所指与命中盒不一致）。review0830-B #12「under-line parallel-track tradeoff」登记项在本任务翻案清算。验收：平行轨互不吸附改按 MC 连接规则；站轨上前向放置命中准星所指格；探针钉射线命中优先级。 ✅✅（25bb2d6）**双规格三症一域**：① 平行吸附——railConnections 规则②③加「显式轴（bit5）只连轴上臂、纯侧臂 0 连接、轴偏守恒」，规则①平地拐角加平行拒连，规则④「任取单端救垂直 stub」级联退役；② 站轨选块——t938 HitRail 整格口径废除（位+fullCell 特判移除），选体与相机对轨统一走 raycastAABBs 2/16 薄板 sub-AABB（瞄板选中、瞄上部空气穿透）；③ clampShift 链可达闸改目标层精确解，头顶并行线不再劫持。**验证实战**：首轮矩阵 7 FAIL = 前棒 WIP 两处真回归被收尾轮揪出（拐角平行判定只读 bit5——增量铺设瞬态邻居持单垂直连接时 bit5=0 被误判平行拒连拆角，改「连接位严格平行于自轴或无连接且 bit5 显式同轴」；fresh 双垂直单臂 tie-break 误改 X 先，恢复 Z 先）→ 复跑 403/0。**探针 P-t983**（矩阵 402→403）+ P-t938 改版 + review28c/t943/review0830-B 钉同步。**阴性轮**（合并一轮）：平行腿恰红（HEAD 形平行轨被拽成 NS 互连对=用户症状复现）+ 站轨腿源码钉红（行为腿 (d) 结构性未红已如实分账：过滤器镜像修后 updateRaycast，HEAD 特判按 filter 位触发不可达）+ 同域钉连带恰红 → 恢复 **403/0**，t981/t982 探针全程绿。零警告；exe 冒烟零 TypeError。#12 登记项就此翻案清算完毕。待目视（实机复现类）：平行轨互不吸附、站轨上能选中准星前方目标格。
**t984** 仙人掌旁放铁轨（口径翻案，用户 9-01 原话：「我的口径是能放下来，而不是仙人掌会掉落，你之前一直都做错了」）：铁轨邻接仙人掌应**能放下来**，仙人掌**不掉落**。仙人掌破坏校验只应被**完整实体方块**的水平邻接触发（MC 语义）；铁轨/火把等非完整方块不触发。连带核与 t503（仙人掌 worldgen/挖除链）口径一致。验收：仙人掌四邻放铁轨成功、仙人掌不动；完整方块邻接仍照旧触发破坏掉落。 ✅✅（b9dcb25）**根因**：checkCactusOnEdit ④ 邻接坍落门槛「非空 id != Air」把轨族/火把/压力板等非完整方块全当触发源（t911 旧钉「轨贴仙人掌即毁柱」即此口径的产物）。**修法**：门槛收紧为「id != Air && isSolid(id)」——与 t503 worldgen 仙人掌柱 4 邻守卫同一谓词同源（其注释自 t445 起即声明同源契约，本修使代码真正兑现），worldgen/放置/挖除三路径口径一致；t911 旧钉翻案、t945 玩家路径探针期望翻转（(g) 石头对照腿保留）。**探针 P-t984**（矩阵 403→404）六段：轨三变体+火把四邻 / 木石压力板 / 石头·沙对照 / 挖除链失撑 / isSolid 源码双钉。**阴性轮**：回退 ④ 门控一行 → 恰 t911/t945/P-t984 三红（401/3）→ 恢复 **404/0**。零警告；exe 冒烟零 TypeError。待目视（实机复现类）：仙人掌旁放轨不碎柱。
**t985** 仙人掌底接触阴影：仙人掌放在沙子上**没占满一个平面格子（这是对的）**，但**底部沙子与仙人掌接触的部分整片变成阴影**。修不满格方块对支撑面的 AO/光照——矮窄侧面不该把支撑面顶面压出大片阴影。验收：沙上仙人掌正下方及周围沙面无异常阴影带；其它不满格方块（铁轨/按钮/雪层）支撑面同查同修。 ✅✅（644af3a）**归因（rig 实测）= 光照 opacity 非 AO**：AO/PCF 侧无恙（t849/t850 已把仙人掌排除出 heightmap，columnTopSurfaceY==沙顶钉在）；病灶在 `lightOpacity(Cactus)==15`（满格遮光）——天光种子列在仙人掌格截断 + BFS 进入衰减 max(1,15)=15 → 仙人掌格天光恒 0 → 沙顶面立方面光恰朝仙人掌格采 0 = 整面压进暗部。**修法**：`case Cactus: return 0`（不遮光；与贴图 solidify 语义解耦，注释同步），setBlock 链 recomputeLightAround 保证受影响格即时重算。**探针 P-t985**（矩阵 404→405）五腿：仙人掌格 skyLightAt==15 / 沙顶面亮度腿 / 石头对照腿（满格满遮 sky==0 零回归）/ 铁轨·火把·石压板·雪层不满格族 sky 15 防漂移钉 / 源码钉。首跑 t960 负载型 flake 复跑清 → **405/0**。豁免阴性轮（视觉/光照类，行为+对照双钉）。零警告（dxcompiler 豁免）；exe 冒烟 17s 零 TypeError。待目视（实机复现类）：沙上仙人掌底部无接触阴影带。

### 🅲 狼与豹猫（t986-t988）
**t986** 项圈一圈：狼/豹猫驯服/未驯服时**脖子上的项链完全不明显**——「差不多看到两个红点的样子」，用户要**围绕脖子一圈的项链**。先查 pack 资产/材质包内有无现成 collar 贴图区（wolf 类 entity 贴图），有则贴图驱动；无则几何环带程序自绘（IP 门：不抄 MC 资产）。驯服/未驯服显隐语义按现状保留。验收：驯服狼/豹猫脖子可见完整一圈项圈、远看可辨；未驯服态语义正确。 ✅✅（645d118）**方案=几何环带程序自绘**（比贴图更强 IP 姿态：pack 内虽存在 wolf_collar/cat_collar.png 但项目代码零引用，本实现纯几何零新增资产文件）：MobModel 裸颈段（body 前缘与头后缘之间）四薄板围合环带、独立 subset 纯色项圈红渲染，`collarVisible` 属性仅 mobType 10/11 读、狼/豹猫×坐/站 4 发射点；旧 t831/t963「两红点」overlay 盒整删；Main.qml/ResourceBrowser.vue 图鉴拨杆 + 条件化材质数组（防狼项圈错吃羊头材质），刷怪笼迷你恒野生零回归（t782 三消费端同源）。**探针 P-t986**（矩阵 405→406）(a)-(e)：环带几何顶点腿（18° 旋转质心手算逐位吻合）+ 驯服显隐 + 未驯服腿 + 同源钉 + **IP 门自证钉**（三实现文件对 wolf_collar/cat_collar 零引用）。豁免阴性轮（视觉类，行为+源码双钉）。零警告；exe 冒烟零 TypeError。待目视（实机复现类）：驯服狼/豹猫脖子一圈红项圈远看可辨。
**t987** 坐姿返修（兔子病）：狼/豹猫**趴下（坐）姿态完全像兔子**——后脚直接多出好长一节接触地面的部分、腿凭空长高一节，「完全不像趴下的样子」。修坐姿肢体变换（四足坐姿：臀部落地、后腿前折、前腿立撑），消灭穿地长腿。验收：坐态臀部着地、后腿折叠贴身、无穿地伸出段；站/走/坐切换无跳变。 ✅✅（1ad3d02 合并提交，见下）**根因**：旧 t946「臀锚后仰」坐姿根锚 (-0.14,0.36) 落在臀部自身上——18° 后仰臀部几乎不沉（躯干底仅沉 0.013）而胸顶抬 0.39，坐高全靠前腿盒加长 0.34→0.54 + 臀下 0.42 大腿柱 + 贴地长爪板 = 用户「兔子病」（后脚多出一节 + 腿长高一节）。**修法**：根锚移到**前爪着地点**（狼 (-0.42,-0.24)/豹猫 (-0.40,-0.20)，站姿前腿占位不变量），躯干绕其后仰 24.4°/26.6°（tanθ 解析解臀底角精确触地）；前腿与站姿同盒立撑（顶埋胸补肩窝）；后腿折叠为侧埋矮臀块+贴地细爪板；头净俯仰 8°；sitRot 链派生纪律保持，QML 三消费端同链解析值随移。**探针 P-t987**（顶点级坐姿肢体 + 站态对照零回归）。豁免阴性轮（视觉类）。**与 t988 同提交**：两探针同在测试文件，拆分必产生中间红提交，违背提交后全绿铁律（偏差如实入档）。
**t988** 狼战斗 AI 缺陷：狼帮我打僵尸时 **AI 还是不会走路**（应主动接近目标），且**遇到要跳跃才能上的格子不会跳、卡在那里**。修 tamed wolf 攻击/跟随态的移动驱动 + 越障跳（review0830-C #14 修的是咬范围内禁跳——本任务是其反面：移动途中 1 格障碍必须跳）。验收：指狼打僵尸，狼奔跑接近、途中 1 格台阶自动跳上、抵达后撕咬；平地跟随/待机零回归。 ✅✅（1ad3d02 合并提交，见 t987 条）**根因**：#14 压跳门旧形纯 `distXZ > kAttackRange`——异层目标进咬击带后探跳被压死 = 狼隔 1 格台阶卡死不上台（用户症状）。**修法**：chase lambda 增 tdy 参数（4 调用点全接），门收窄为 `distXZ > kAttackRange || std::abs(tdy) > 0.5f`——「同层可咬」才压跳，#14 行为保留。**探针 P-t988**（矩阵 406→408 合并增）三腿：平地接近咬 / 台阶跳上 / 台沿异层爬升（真实注册链）；P-t946 连续域 + P-t986 项圈位 + P-r0830C(d) 门形源码钉同步演化。**阴性轮（方法论偏差如实记录）**：回退门形/短路 tdy 均无法打红 P-t988——插桩实测 leg(c) 点火发生在 distXZ=1.642>1.6（咬击击退把僵尸推远→距离子句放行），P-t988(c) 对门形行为不敏感；门形回归拦截由 **P-r0830C(d) 源码钉承担**：门文本回退 → 恰 P-r0830C FAIL（407/1，d 腿红 hits=11 lift=0）→ 恢复 **408/0**。零警告；exe 冒烟零 TypeError。待目视（实机复现类）：狼打僵尸会接近、1 格台阶会跳。

### 🅳 资源查看器（t989-t990）
**t989** 移除生物蛋（t967 局部翻案，用户 9-01 口径：「生物蛋纯属多余，上面已经有生物的查看了」；且小僵尸蛋与别的蛋**风格不统一**）：查看器**整段移除**生物蛋分区/入口（t967「生物蛋并入生物类」决策按「用户后来的话优先于旧任务结论」翻案），三分类归属、单选中收口、预览链随之同步。验收：查看器无生物蛋面；三分类/单选中/预览全链零回归（P-t967 探针按 P-t949(d) 先例合法演化）。 ✅✅（b34a549）**移除面**：蛋分区表头+格段、categoryOfEntry 蛋条目（0 档仅存语义登记）、蛋预览路径（selectedMobType 收敛生物段直读）、paletteModel 蛋段（新 `Hotbar::isSpawnEgg` Q_INVOKABLE 透传 RecipeRegistry::mobTypeForSpawnEgg 单一权威，QML 蛋 id 字面量镜像表整表退役）。**边界守住**：图鉴 17 条 mobModel 保留、Inventory.qml 调色板蛋分区不动、右键生成/中键复制不动（用户只嫌查看器蛋多余）。**探针 P-t989**（矩阵 410→411）：注释感知剥离器后钉蛋键代码绝迹（14 蛋 hex 键+mobTypeForEgg+eggEntries）+ rig 腿（paletteModel 并集==block+matEntries、选蛋路径不存在、分类/收口照旧）。**探针演化三处**：P-t967 蛋腿翻转为断言蛋不存在（P-t949(d) 先例）、P-review0830-23 蛋腿翻转（蛋路径绝迹+驯服残留清零）、P-t952 蛋映射钉（case 0x25D 绝迹，C++ 权威钉保绿）。UI/视觉域豁免阴性轮（源码钉+rig 双钉）。零警告；冒烟零 TypeError。待目视：查看器左栏无蛋分区。
**t990** 方块默认视角回归俯视：查看器方块默认视角**从俯视变成仰视了**（用户记得之前不是这样）。git -S 定位回归源（t966 pitch 父拆层的 -22 基偏疑翻号/丢失）后修复——默认应**自上向下俯视**；t877/t966 拖拽定律与四相位行为不动。验收：打开查看器任一方块默认俯视；拖拽交互零回归（P-t966 行为腿全绿不改判）。 ✅✅（a5aee23）**git -S 考古判决：基偏从未变号**（t458 初版 -22 → t599 加 userPitch → t820 翻拖拽符号 → t877 回退 → t966 8eef0f4 拆层逐字搬 -22）；真凶是 **t966 拆层本身**——旧单节点图 Ry(yaw)·Rx(pitch) 中 −22° 作用于模型局部系，自转带动倾角 → 顶/底面交替可见（「见顶面」半相位为真）；拆成世界系 pitch 父后同一 −22° 恒定 = top 法线 z 分量 sin(−22°)<0 恒背相机 = **全自转相位恒见底面**（用户观感如实归因于节点层级变化，非翻号）。**修法**：四分支（整立方/床/异形/生物同族一致）基偏 −22°→+22°（恒定俯视 3/4）；−35 yaw 基偏与 t877 定稿拖拽定律逐字不动。**P-t966 行为腿全绿不改判**（上拖→更俯视两基偏号下方向一致，前/侧/背相位手算验证），其基偏计数钉合法演化 -22→+22==4。**探针 P-t990**（矩阵 411→412）：源码钉（+22 四支、−22 形绝迹、定律行逐字）+ rig 读回真场景图（恰 4 个裸 pitch 父 x==22 逐支钉 + BlockCube top 法线 z∈(0.30,0.45) 号+量级双钉、全相位恒成立、bottom 恒负、上拖更俯）。豁免阴性轮（UI/视觉域）。零警告；冒烟零 TypeError。待目视（实机复现类）：打开任一方块默认俯视 3/4。

### 🅴 杂项（t991-t995）
**t991** 栅栏几何对齐 MC：木栅栏/石头栅栏「太难受了、完全不符合预期」——**先对照 MC 的样子**再修：木栅栏 = 中心柱（约 1.5 格高）+ 朝连接方向横杆；石栅栏（墙）= 中心柱 + 顶部凸缘 + 低连接杆；相邻栅栏/贴墙自动连接。世界渲染与查看器预览同源修（一处修两面）。验收：孤立栅栏 = 单柱形态；相邻栅栏横杆连接、贴方块面连接；与门/栅栏门衔接自然。 ✅✅（7a6d6bf）**MC 1.0 对照逐项差**：旧柱 0.4 见方×1.0 高 + 0.4 厚板单杆 → 木栅栏改 4/16 柱×1.5 格高 + 每向两道 2/16 横杆（y 6-9/16、12-15/16，格边→柱面）；CobbleFence 拆独立 case = 8/16 柱 + 1px 顶檐外挑 + 低连接拱（y 10-15/16、6/16 截面）；itemshapegeometry 查看器预览同源改（木 9 盒 yShift -0.75、墙 6 盒新形）；selection/raycast/solidTopOffset 随视觉同步（碰撞 1.5/0.4 见方不动，越障语义零改动）。P-t801 合法演化为 P-t991+P-t991b、P-t925 栅栏行随动（墙 144 顶点）。豁免阴性轮（几何域行为+源码钉）。零警告；冒烟零 TypeError。待目视：木栅栏双横杆、石墙顶檐低拱、连接自然。
**t992** 按钮几何统一：石头/木头按钮**放地上和放墙上大小不统一——以墙上为准统一**；且墙上形态**比例也不协调**（自查对照 MC 按钮比例修）。验收：同种按钮地面/墙面尺寸一致（墙上口径为基准）；比例协调；按下/弹起动画（如有）零回归。 ✅✅（07924e6）**根因**：mechBoxes 按钮墙装 6 宽×2 高细横条（比例失调）与地装 6×6 footprint 两套尺寸。**修法**：统一 MC 比例 6/16 宽×4/16 高×厚 2/16（按下态仅压薄 1/16），墙面钮 y 6..10 居中、地面 footprint 6×4——五安装面同一张 6×4 钮脸；三消费端（mesher/raycast/查看器）共用 mechBoxes 自动同源。**探针 P-t992**（矩阵 413→414）：厚度轴/脸 4×6 统一钉 + 五面位置钉 + 源码钉；P9/t925 旧钉零回归。豁免阴性轮。零警告；冒烟零 TypeError。待目视：地面/墙面按钮同尺寸、比例协调。
**t993** 玻璃更实：玻璃方块**透明度还是太透明**——调「更实」一档（边框/面配比或面透明度调整，保持可透视但实体感明显）。pack-on/pack-off 两态同调。验收：玻璃墙实体感明显提升、仍可透视后方景物（用户目视口径）。 ✅✅（93ef8ff）**根因**：世界玻璃段材质 opacity 0.30（t899 增透遗留）且与手持玻璃 0.45 漂移。**修法（杠杆论证）**：pack-on 只覆写 tile 68 像素不触材质 → **材质轴是唯一满足「pack 两态同调」的杠杆**（贴图级调整只达 pack-off）——世界段 opacity 0.30→0.45 三消费端同值收口；贴图保持全不透（同 water 模式契约），不碰 alphaCutoff/Mask、零透明排序新面；build_glass.py 迭代史双钉更新。**探针 P-t993**（矩阵 414→415）：材质解析 0.45∈(0,1) + tile 68 逐像素 alpha==255 + pack 映射钉。豁免阴性轮。零警告；冒烟零 TypeError。待目视（实机复现类）：玻璃实体感明显提升仍可透视。
**t994** 刷怪笼迷你生物贴图：刷怪笼放生物蛋进去后**能看到生物在笼内悬浮飘动（对）——但只有模型没有贴图、纯灰色**。修笼内迷你 mob 的贴图绑定（t782 三消费端同源纪律：一处修，delegate/图鉴/笼三面一致）。验收：笼内迷你生物带正确贴图旋转飘动；世界内 mob / 图鉴预览贴图零回归。 ✅✅（2f4f055）**根因（git -S 考古至 t786 原稿 65433c3）**：贴图查表属性声明在 **miniMobBob** 上、消费端却引用 **miniMobSpin**.miniPackTex（不存在）→ QML 绑定静默 undefined → `undefined !== null` 恒真 → packTextured 恒真 + baseColorMap=undefined 纯灰 + 眼层恒隐；蛋→笼 mobType 映射链（C++ 权威）本身健全。**修法**：Main.qml 五处消费端引用改指 miniMobBob + 作用域契约注释。**探针 P-t994**（矩阵 415→416）：破坏形态字面量绝迹钉 + 修正行 verbatim + 全 14 蛋 mobTypeForSpawnEgg→spawnerStateForMob→spawnerMobTypeForState 回程钉 + 三消费端共享 MobModel 钉。豁免阴性轮。零警告；冒烟零 TypeError。待目视：笼内迷你生物带贴图飘动。
**t995** 地底结构对照 MC：地底各种遗迹「现阶段只能说有影子，但是不太像」——继续**对照 MC 的生成方式**返修：先盘点本项目已有地底结构 → 逐个对照 MC 布局（房间形态/走廊走向/出现频率/战利品位）→ 差距清单 → 首轮修正。大项允许本任务交付「对照报告 + 首批修正」并把后续分批登记。 ✅✅（afc848d）**盘点**：5 结构生成器（地牢 t392/426/786 / 废弃矿井 t484/565 / 沙漠神殿 t485 / 丛林神殿 t486 / 要塞 t487/564/713/759）。**对照总评**：影子成立，不像在形态随机度/房间类型/内容密度（地牢尺寸恒定无苔石单箱、矿井无洞穴蜘蛛笼小室、要塞无裂纹石砖变体储藏龛空置、沙漠神殿偏小无竖井、丛林神殿单层单箱）。**对照报告**：docs/underground-structures-comparison-2026-09-03.md（含分批计划：批 2 矿井蛛笼小室+多箱+要塞龛箱；批 3 沙漠神殿 21×21+竖井+裂纹石砖新 id；批 4 要塞 piece 化+丛林多层塔；频率校准批）。**首批修正（全落地牢，矩阵 416→419）**：P-t995a 苔石混砌（Cobble+MossyCobble，地板苔率 50%>墙 25%，叉积式方向比实测 49%/25%）/ P-t995b 内空随机 5/7（r bit28/29 独立、与笼型位零耦合，11 净样两尺寸齐）/ P-t995c 每间 1-2 箱（bit30 ~50% 对角双箱独立填池）；rig=t786 同款 5 seed×128² 世界池，t786/t759/t393 worldgen 探针零回归。豁免阴性轮（生成参数域，源码+行为双钉）。零警告；冒烟零 TypeError。

### 🅵 TNT 与性能（t996-t997）
**t996** TNT 引燃后方块不清除：点燃 TNT 后**原方块没清除**——持续闪烁动画不停、**方块变成贴图、人物可以穿过去**。引燃链应把原 TNT 格置 Air，由 primed 实体接管闪烁渲染与碰撞；现状卡在中间态（疑 primed 实体化失败落入渲染 fallback）。验收：点燃瞬间原格清空、可穿行；闪烁动画只存在于 primed 实体、随爆炸终止；爆炸/伤害/掉落链零回归；阴性轮钉「置 Air」调用（回退 → 原格残留复现 → 复原）。 ✅✅（86db03f）**根因（打火石点燃 TNT 整条分支缺失）**：t492 Bug B 移除了 TNT 右键直接点燃（当时注释「本项目无打火石」），t724 补打火石分支时没补 TNT 分叉 → 打火石右键 TNT 落进 t843 fallback 地面火路径：邻格 setBlock(Fire) 而 TNT 原格永不清、PrimedTnt 实体永不建——用户看到的「闪烁永续/贴图化/可穿行」中间态即此（闪烁面=地面火+方块共存渲染，碰撞消失=原格实际已非固态语义的渲染残留）。修：打火石分支补 TNT 分叉——原格置 Air + spawnPrimedTnt 原位接管闪烁与碰撞（与红石点燃路径同口径）。**探针 P-t996**（矩阵 398→399）：rig 真 World+真 EntityManager：点燃瞬间原格 Air / primed 实体在场 / fuse 递减 / 到点爆炸 / 实体清理全链行为钉 + 引燃入口覆盖。**阴性轮**：回退「置 Air」→ 原格残留腿恰红 → 复原。零警告；exe 冒烟零 TypeError。待目视（实机复现类）：打火石点 TNT 原格即清、闪烁属 primed 实体、随爆炸终止。
**t997** 大量 TNT 同爆性能：t933（单爆炸级联风暴）修复后，用户 9-01 @ 35d5a72 实测**放很多 TNT 爆炸仍很卡（15FPS，66.7ms/frame，sim 仅 4.63ms）**——多 TNT 同爆场景不在此前修复覆盖内。用 t934 交付的 F3 render-side 行（`gpu`/`prep`/`present`/`(N)` 样本数/`act ct`）在复现 rig 归因卡顿源 → 修（节流/合批/限并爆/爆炸波分期）或登记判读结论 + 治理项。验收：复现 rig 量化归因逐项有数；有修复则前后帧数据对比，无修复则登记结论与治理路径。 ✅✅（396d742）**归因（rig 实测落盘）**：t933 修的是单爆炸级联风暴，多 TNT 同帧到期 = N× 联合盒 reflood + N× worldChanged QML 扇出同帧叠加——rig 36 同引信 TNT：pre-fix 同 tick 36 爆、refloodD 36 / wc 36、引爆 tick 墙钟 1.42ms vs 静默 tick 0.009ms（~150 倍）；用户 F3 sim 4.63ms 占 7% → 真实场景大头在渲染/同步侧被 N 次扇出逐帧放大（GUI 侧 present/GPU 归因须实机 F3 判读，已登记待用户复测）。**修法=爆炸波分期**：`kMaxTntDetonationsPerTick=4` + 预算耗尽引信重挂 `kDetonationWaveRegroupSec=1/60s`（entitymanager.h 常量族紧邻 kChainFuseSec；链式 TNT 1.2s 错峰引信语义不动、总破坏量不变、只影响同帧到期簇）。**连带收益**：分期后 36 爆 refloodD/wc 36→21——后波爆炸球整落先波弹坑，destroySphereSilent 空 destroyed 早退免费去重重叠盒；单 tick 峰值引爆 36→4、maxTickMs 1.42→0.41。**探针 P-t997**（矩阵 399→400）：(a) 分期行为——任意 tick 引爆数 ≤ 预算 + 全部 36 实爆零 primed 残留；(b) 归因计数落盘——`wc==refloodD ∈ [1,totalDet]` 不变式（pre-fix 36/36 实测记进探针头注释；分期去重使恒等 36 不成立，主控验收认可此演化）；(c) 源码钉预算常量+钳制行 `e.fuse = kDetonationWaveRegroupSec`。**阴性轮**：修复体不在场（WIP 原态）→ 恰 P-t997 FAIL（399/1，(a)(c) 腿红，maxPerTick 36）→ 落修复 **400 PASS / 0 FAIL**。rig 工程教训（记档）：探针基线读数须在搭台 setBlock 之后（每格 setBlock 自发 worldChanged/reflood）；kRigY 39-44 丛林冠层铺满致首跑假红，选址三级兜底；Windows 矩阵日志必须 QT_FORCE_STDERR_LOGGING=1。零警告；exe 冒烟 17s 零 TypeError。待目视（实机复现类）：大量 TNT 同爆 F3 帧时间对比 + render-side 行读数（t934 判读指引）。

### 📎 R19.18 范围与顺序
t978-t997（20 项）。**建议顺序：t978 复制体泄漏最高优先（用户原话「大问题」；t935 回归嫌疑须最早点证或排除）→ t996/t997 TNT 批（功能硬 bug + 性能归因，用户实测数据在手）→ t981-t985 矿车铁轨组 → t986-t988 狼豹猫组 → t979/t980 生物组其余 → t989/t990 查看器 → t991-t995 杂项**。每项独立 commit + dev-plan ✅✅；全部完成后只读 review + 统计报告。实机复现类（t978/t990/t993/t996/t997 等）产出须用户在新版本戳 exe 口头确认后关单。

## R19.19 地底结构逐方块还原批 + 隔墙有眼（t998-t1004，7 项；2026-09-03 立项，同日用户批准开工「全部跑完吧」）

**测试跑法硬性口径（用户 2026-09-03，所有本批命令必须遵守）**：`QT_QPA_PLATFORM_PLUGIN_PATH=D:/Qt/6.11.1/mingw_64/plugins/platforms QT_QPA_PLATFORM=offscreen QT_FORCE_STDERR_LOGGING=1 QT_ASSUME_STDERR_HAS_CONSOLE=1 ./build/redstone_matrix_test.exe > matrix_run.log 2>&1; echo EXIT=$?`——offscreen 下 QCursor::setPos 空操作，测试不再劫持用户真实指针（代码守卫已入库 244f5cf：窗口不可见/未激活时 grab/pollMouse 不碰指针、release 按 overrideCursor 非空配对）。冒烟同理加同前缀。

**立项背景**：用户 9-03 口径：「地底结构刷怪笼以及末地要塞还有废弃矿坑等结构**上网探索完如何生成的之后就直接做出来**，我想要的是**逐方块还原**的那种，而不是随便敷衍了事的」+ 新成就**「隔墙有眼」**（进入到要塞结构里触发）。承接 t995 对照报告（docs/underground-structures-comparison-2026-09-03.md）的后续分批计划并升级为「逐方块忠实」口径。

**联网考据来源（minecraft.wiki，2026-09-03 抓取）**：Monster_Room / Mineshaft（+/Structure 子页由各任务实现工自行精读）/ Stronghold / Desert_pyramid / Jungle_pyramid（+/Structure 子页）。考据要点存档：
- **地牢**：外墙宽×长 = 7 或 9（内空 5/7）、总高 6（内空 4）；**地板逐块 25% 圆石 / 75% 苔石**，墙/顶普通圆石；**刷怪笼仅僵尸 50% / 骷髅 25% / 蜘蛛 25%（无苦力怕！）**；箱子 = 2 个各 3 次尝试（0-2 箱，贴墙规则：目标块为空气且四邻恰一实心），偶成双箱；墙脚 1-5 个 2 高豁口通洞穴；每 chunk 14 次尝试（项目有限世界按现有 24 网格×10% 经济保留，登记）。
- **矿井**：3×3 巷道（现有 ✓）；木板+栅栏支撑**每 4 格一道**；起点厅 10×10 拱顶四向 1-4 出口；交叉口 = 5×5 **双层**木柱支撑；斜坡巷道对角下切；**洞穴蛛网室**（蛛网密集藏刷怪笼）；箱=轨道上的运输矿车（项目无该实体→登记偏差：箱落地轨旁）；铁轨不连续（残缺感）；火把部分巷道装饰。
- **要塞**：piece 链式生成——起始螺旋梯 → 五向交叉 → 加权随机房间链 ≤50 间（传送门房保证生成且距起点 ≥5 间、图书馆 0-2 间距起点 ≥4）；**传送门房**：704 石砖 + 39 铁栏杆 + 15 岩浆 + 8 石砖楼梯 + 12 传送门框环（每框 10% 预嵌眼）+ 1 银鱼笼，铁栏杆格栅入口；**石砖变体逐块随机：普通 45% / 苔石砖 30% / 裂纹石砖 20% / 怪物蛋 5%**；图书馆（大双联 233 书架 2 箱 7 梯 / 小 161 书架 1 箱）；监狱厅（2 牢房 20 铁栏杆 2 铁门）；储藏室（双层、圆石芯、梯子、箱）；箱走廊（1 箱）；走廊 5 长 50% 侧出口；填充走廊 16-48。世界 128 座 8 环 → 项目有限世界保留 1 座近中心（登记）。
- **沙漠神殿**：21×21 底、正面安卡纹样 + 两侧后上方副入口 + 顶窗；**1.8 前纹样用羊毛（橙/蓝）**——项目有色羊毛 63-77，按时代口径用羊毛零新 id（现代为陶瓦，登记）；地面风玫瑰棋盘 + 中心蓝块；蓝块正下方石压板接 3×3 TNT×9；暗渠藏 4 箱。
- **丛林神殿**：3 层苔石建筑；底层 3 拉杆谜题（3 刻纹石砖上，正确组合 → 活塞门开 → 主层箱显露）；2 条绊线发射器陷阱（走廊中段 + 转角箱前，藤蔓遮蔽，2-14 箭）；2 箱。**活塞/绊线/刻纹石砖项目均缺**→ 登记偏差（见下）。

**登记偏差清单（实现前用户可否决）**：
1. 洞穴蜘蛛：无该 mobType → 矿井蛛网室刷怪笼用 MobSpider（项目 7 号），登记；可选后续加蓝色变种
2. 运输矿车：无该实体 → 矿井箱落地轨旁，登记
3. 陶瓦：无 → 沙漠神殿纹样用橙/蓝羊毛（时代口径反而更忠实），登记
4. 活塞+绊线：无 → 丛林神殿拉杆谜题改为**三拉杆序开门**（正确组合触发局部开墙显箱，开墙机制用现有方块事件链实现）；绊线陷阱改为压力板触发发射器（走廊位次按考据摆），登记
5. 要塞数量：有限世界 160×160 → 保留 1 座近中心，登记
6. 刻纹砂岩：无 → 沙漠神殿用现有 Sandstone/CutSandstone 家族替代，登记

### 🅰 结构新方块包（t998）
**t998** 新增三方块：**苔石砖 MossyStoneBrick / 裂纹石砖 CrackedStoneBrick / 铁栏杆 IronBars**（要塞逐方块还原的前置依赖）。id+注册表 def+程序贴图（克隆 tools/build_stone_brick.py 的 numpy 定种子模式；苔石砖=石砖+苔痕覆盖、裂纹石砖=石砖+裂纹线、铁栏杆=竖条栅格；走 build_atlas.py TILES 追加+tileIndex 接线，**顺序契约**：与 chunkgeometry tileIndex 对齐防渗色）+ 铁栏杆异形几何（PartialBlockGeometry 薄柱+横板，类栅栏族先例；含连接判定：相邻铁栏杆/贴方块面连接）+ 创造调色板/图鉴入口 + 挖掘/掉落口径（铁栏杆需镐？对照项目金属系惯例）。验收：三方块世界可放可挖、贴图正确、铁栏杆连接呈现正确；图鉴预览正确。探针 P-t998：def/tileIndex/几何三面钉 + 贴图 tile 逐像素腿（苔/裂覆盖特征在场）+ 调色板入口钉。 ✅✅（e8c1bca，review0904 回填）id 140-142 **enum 尾部追加**（存档契约）+ ShapeIronBars=10 新形；三程序贴图 tile 181-183（定种子 numpy：苔=石砖同 seed 487 基底+苔斑〔逐像素 diff 1/256〕/裂=暗裂纹线〔近黑 26 px 可分离〕/铁栏杆=周期 4 竖条亮度带+alpha 恒 255〔薄杆面整张压缩采样下透明列会致面消隐，刻意以暗缝底达成同读感——脚本头注登记〕）+ AtlasTileCount 184 + kMcBlockId 98/98/101 + 编译期越界 static_assert；铁栏杆几何=2/16 中心柱满格高+四向横板 y[7/16,9/16]、连接判定 isCollidable∨isFullCube（栅栏族 R1 口径）、碰撞 4/16 柱盒与选中/射线十字条带分离（t991 口径）；调色板/图鉴入口（isPackDerivedIconFamily Glass 先例）+ 挖掘需镐金属口径。**探针 P-t998**（矩阵 419→420）：def/tileIndex 钉 + 贴图逐像素三腿 + 几何顶点腿（孤立单柱/四向横板/贴 Stone 面）+ 调色板钉；t879 图集宽度钉 181→184 随动。**阴性轮豁免（几何/视觉类，行为+源码+像素三钉）——review0904 维度2 判豁免成立；补最小突变阴性轮登记为后续批可选项**。零警告；offscreen 冒烟零 TypeError。

### 🅱 地牢逐方块校准（t999）
**t999** 地牢对齐考据口径：地板逐块 25% 圆石/75% 苔石（现 50%）、墙/顶普通圆石（现墙 25% 苔石→改普通；**t995a 探针合法演化**：断言翻转为地板 75% 苔>墙 0 苔）/ 刷怪笼加权池 **Shambler50/Bones25/Spider25，Stalker 退出池**（**t786 加权分布腿合法演化**）/ 箱子改 MC 尝试规则（2 箱 × 3 次尝试，贴墙恰一实心邻，偶双箱；**t995c 演化**：1-2 箱腿改为按新规则的分布断言）/ **新增墙脚 1-5 个 2 高豁口**通向空气（挖向邻近空气/洞穴，项目有限世界找不到空气邻域时可全封登记）。尺寸（内空 5/7×4）与 24 网格×10% 频率不动。验收：全部考据口径逐项落地。探针 P-t999：5-seed rig 逐项（地板苔率窗、墙零苔、豁口数 1-5、刷怪笼加权、箱尝试分布）+ 演化探针全绿。 ✅✅（d72229c，review0904 回填）**四口径落地**（wallBlock 拆分地板/墙：75% 苔地板+零苔墙顶 / kDungeonSpawnerWeights{128,64,64} Stalker 退役〔state 位布局零动，t786 编解码腿保绿〕/ 箱 2 位×3 次尝试贴墙恰一实心邻〔Chest 无双箱合并态——相邻保持独立单箱**登记**；0 箱为 MC 少见态〕/ 豁口 `1+(r>>14&7)%5` 最近空气柱 BFS〔曼哈顿 ≤12、先扫后挖、Bedrock/Water/Lava/Chest/Spawner 拒挖〕，rig 实测 10/11 房有豁口 91%、全封 1 间登记）。**探针演化**：t995a 翻转（地板苔窗 [60,90]+墙零苔+旧双率 lambda 绝迹钉）/ t995c 翻转（0-2 箱分布+贴墙核+旧角位/对角位/bit30 绝迹）/ t786 加权池腿翻案（rig 96²→128² 对齐同 seed 池——96² 池 ~8 笼对 Stalker 退出**无判别力**，首轮阴性实证零抽出漏红；删单世界主导序粗检噪声腿）/ **P-t999 新增**（→421）：豁口 run∈[0,5]+>80% 房有豁口+向外穿透无盲洞+联合面+源码钉三锚。**阴性轮**（地板回退 50%+笼池回退 4 槽合并）→ 恰 t786/t995a/t999 三红（418/3，落盘 matrix_neg_t999.log）→ 复绿 **421/0**。零警告；offscreen 冒烟零 TypeError。

### 🅲 隔墙有眼（t1000）
**t1000** 新成就**「隔墙有眼」**：玩家**进入要塞结构区域**解锁（用户口径原话「进入到末地要塞的结构里面去了大概这也」；标准 MC 同名进度触发是跟随末影之眼——项目无该物品，按用户口径落地）。触发链：placeStronghold 显式记录要塞 bounds（placeAt :7622 处已有 cx,cy,cz → 常量足迹 ±22/±(墙高+1) 写入 world 字段 + Q_INVOKABLE getter；读档反推路径同步回写 bounds 或由 portal 坐标+常量推导——二选一以最小改动定）；PlayerController tick 内做「进入 bounds」边沿检测，**一次性事件信号**（dispenserFired 同款事件级，禁每帧直发）→ Main.qml Connections 路由 → `progress.onEnteredStronghold()` → unlock（定义进 achievementDefs 表：名「隔墙有眼」+ 描述 + 图标用末地传送门框系图标）→ toast。持久化沿用现有 achievements map（零额外工作）。验收：新世界进要塞触发 toast+成就列表点亮；存退重进不重复触发；无要塞世界（理论不可能，防御）不触发。探针 P-t1000：bounds 记录/反推一致性腿 + rig 玩家进入触发腿（真 PlayerController+progress）+ 边沿一次性钉 + 源码钉。 ✅✅（ea3322e）**bounds 选型 = portal 坐标反解足迹闭区间**（水平 cx±22/cz±22 含墙环、竖直 [cy, cy+9] 含地板顶板），**零新增序列化字段**——单一真相源 = m_strongholdPortal*，worldgen 记录与读档 rebind（扫框架簇反推）两路径均先于玩家 tick；placeStronghold 的 kHalf/kWallH 字面量上收类常量 kStrongholdHalf/WallH 与判定同源引用。**触发链**：PlayerController tickImpl 脚底判定 false→true 上升沿一次性信号 `enteredStronghold`（守卫 m_insideStronghold，setWorld+finishWorldLoad 双重置点；位于 !m_captured 早退**之前**——同水/岩浆态先例）→ Main.qml Connections 路由 → progress.onEnteredStronghold() → 定义 `{entered_stronghold, 无父, 隔墙有眼, 发现了藏在地底深处的要塞, EndPortal}` unlock（首行已解锁幂等）→ toast；持久化走现有 achievements map。**探针 P-t1000**（矩阵 421→422）：行为腿（界外 tick 静默 / 进界恰发一次 / 界内重 tick 安静 / 出界重进重发 + toast 幂等 / finishWorldLoad 重武装守卫）/ bounds 一致性（环格/顶板格含 y 域）/ 读档反推腿（save→load 后 bounds 仍判对零重 toast）/ 源码钉。**阴性轮**：撤边沿守卫 → 恰 P-t1000 红（entered 12 vs 守卫 3，行为+源码钉双红）→ 复绿 **422/0**。**本批 offscreen 测试跑法首批全量验证通过**（含插件路径前缀）。零警告；offscreen 冒烟 16s 零 TypeError。

### 🅳 矿井逐方块重建（t1001）
**t1001** 废弃矿井对齐考据：起点厅 10×10 拱顶（四向 1-4 出口）替代现 7×7×4 交叉洞室；巷道支撑**木板+栅栏每 4 格一道**（部分缺失随机）；5×5 双层木柱交叉口（新 piece）；斜坡巷道对角下切（现 L 形折线改造）；**新增洞穴蛛网室 piece**（蛛网高密度围 1 刷怪笼，state=MobSpider 按偏差 1）；铁轨残缺不连续（挖段）；箱落地轨旁（偏差 2）；火把部分巷道。频率经济保留（36 网格×40%，登记）。验收：piece 化生成（起点厅+巷道+交叉口+斜坡+蛛网室按考据组合）。探针 P-t1001：5-seed rig（巷道截面 3×3、支撑间距 4、起点厅 10×10 拱顶、蛛网室存在+蛛网密度+笼 state、轨残缺率窗、箱位）+ 源码钉 piece 表。矿井/沙漠/丛林**当前零矩阵探针**，本探针从零新建（t995 rig 模式）。 ✅✅（57270c0）**/Structure 子页补课结论（实现工自抓，首次 60s 超时重试成功）**：支撑=栅栏柱+木板封顶（子页无文字间隔→采批头每 4 格）；起点厅子页 ~8×11 石地板无拱顶图→采批头 10×10 拱顶；**两处口径分歧：交叉口子页 3×3+三层角柱 vs 批头 5×5 双层木柱、蛛网室子页 3×6×3 vs 批头 7×7——均按批头执行并写入注释契约登记**；斜坡石质逐层下剖（无楼梯块）；蛛网室「笼邻格满网+短端栅栏」采子页。**piece 表**：pieceStartRoom（10×10 双段拱顶 + Fisher-Yates 洗牌 4 向取 3-4 出口）/ pieceCorridor（3×3 L 折线 + 支撑组每 4 格〔双柱+柱顶 ~55% 火把/木板柱冠，25% 组级缺失，step 0/4 豁免〕+ 残缺轨 70% + 顶角蛛网 12% + 巷壁矿石 15%）/ pieceIntersection（5×5 四角双层双柱+柱冠）/ pieceSlope（每 2 步降 1 格对角下切）/ pieceSpiderRoom（7×7×3 + 2 宽连接廊 + 笼 state=SpawnerStateSpider〔偏差 MobSpider〕+ 笼邻格三层必网 + ~45% 内网只贴实体块 + 两短端栅栏）；箱从 railCells 取锚四向落地 ChestStateMineshaftFlag（偏差 2）。网格 36/40%、Y<48、矿石 15%、hashColumn/hashVoxel 确定性保留。**探针 P-t1001**（矩阵 422→423，矿井首个行为级探针从零新建）：5-seed rig（起点厅 10×10 拱顶 / 巷道 3×3 / 支撑 min gap 4 / 蛛网室+密度 44% 窗+笼 state / 轨残缺 64%∈[50,90] / 箱贴轨 11 / 火把 65 / 10 净 shaft 14 候选）+ piece 表源码钉；**t786 合法演化**：≥8 网的 MobSpider 笼分流入 mineshaftWebCages 不入地牢池份额窗（否则蛛网室笼把地牢蜘蛛份额顶到 77% 假红）。**阴性轮**（合并两刀：kSupportInterval 4→5 + kSpiderRoomPct 60→0）→ P-t1001 恰红两轮稳定（gapMin 5 / spider false / web% -1 / pin false）→ 复绿 **423/0**。零警告；4 轮全量矩阵全 offscreen 前缀（冒烟按批纪律跳过，矿井不触 UI）。

### 🅴 要塞 piece 引擎逐方块重建（t1002）⚠️ 本批最大项
**t1002** 要塞从「45×45 固定布局」重写为 **piece 链式过程生成**（考据口径）：起始螺旋梯 piece → 五向交叉 → 加权房间链 ≤50 间（直走廊 5 长 50% 侧出口 / 转角 / 直梯段 / 螺旋梯 / 五向交叉 0-4 / 箱走廊 / 房间交叉 4 变体 / 监狱厅 / 储藏室 / 填充走廊 16-48 / 图书馆 0-2 且距起点 ≥4 / **传送门房保证生成且距起点 ≥5 间**——生成失败重启换 seed）；**石砖变体逐块随机 45/30/20/5**（普通/苔石砖/裂纹石砖【t998 新方块】/怪物蛋 MonsterEgg【已有】）；**传送门房逐方块**（704 砖+39 铁栏杆格栅入口+15 岩浆+8 石砖楼梯【StoneBrickStairs=110 已有】+12 框架环【EndPortal 已有，每框 10% 预嵌眼 state】+1 银鱼笼【MobSilverfish 已有】）；图书馆大/小（书架行+梯子+箱+蛛网 7% 空气块）；监狱厅（2 牢房铁栏杆+铁门【已有】）；储藏室（双层圆石芯+梯子+箱）；火把按 piece 数量级分布。现有 45×45 常量足迹约束解除（piece 链自然外扩，世界边界钳制+登记）。t759 探针合法演化（传送门房净空腿保留按新引擎重验）； MonsterEgg 5% 与现 ~8% 嵌墙口径的差额属逐块随机化自然结果，t713 既有钉核对。验收：piece 化、传送门房逐方块、变体配比窗、图书馆/监狱/储藏室形制。探针 P-t1002：5-seed rig（piece 清单在场、传送门房 12 框环+格栅+银鱼笼、变体 45/30/20/5 池化窗、图书馆书架量级、≤50 间约束、portal room ≥5 间约束）+ 源码钉 piece 权重表。 ✅✅（be8f9a2）**piece 引擎三段式**：Phase A 实心壳（45×45 地板顶板普通石砖 + dy1..8 墙体逐块变体 hash 45/30/20/5——退役旧 ~8% 蛋 + ~10% 苔圆石嵌墙口径，t713 验收面由 P-t1002 (c) 变体窗接管）；Phase B 派型（17-slot 无重叠格架 ShSlot{x0,z0,x1,z1,depth,fixed}：S0 起始螺旋梯 depth1 → C1 走廊 2 → P2 五向交叉 3 → C3 直梯段 4 → **PR 传送门房 depth5**、P6 大图书馆 depth5、P10 监狱厅/P14 储藏室固定、其余浮动；房间表 Empty6/Fountain5/Pillar5/LibrarySmall4 + 走廊表 Corridor10/ChestCorridor4/FillerCorridor4；static_assert 链深契约〔kPortalChainDepth=5/kLibraryMinChainDepth=4/kStrongholdPieceCap=50〕；派型失败换子 seed +7919 重试且 libs∈[1,2] 验证；kPortalGrateGuaranteed 保证旗）；Phase C 落地（全量掏刻 + 16 门洞表 → 14 个 stamp lambda：StartSpiral / FiveWay 恰 36 台阶 / PortalRoom 固定原点局部坐标〔13×6 高台 + 3 级 9 块石砖楼梯 + 3×3 岩浆盆 + 环沟岩浆河 72+9=81 岩浆 + 12 框架环 10% 预嵌眼 + 银鱼笼 + 39 铁栏杆 = 格栅 7 + 河栏 24 + 台栏 8〕/ LibraryLarge 231 书架+阁楼+13 栅栏+7 梯+2 箱 / LibrarySmall / PrisonHall 恰 20 栏+2 铁门 / StoreRoom 圆石芯+木板阁楼 / 箱走廊 / 填充走廊 rubble + 图书馆蛛网 7%）。**足迹决策**：链半径钳在 ±22 常量内（17 slot 格架紧凑放置）——P-t1000 bounds 零演化保绿（传送门房固定原点局部坐标，B5 读档反推全链同位）。**续棒两修（前任编译死因+真玩法 bug）**：① `slots` 数组名撞 Qt `#define slots` 空宏 → 改名 slotTable（源码钉字面不含该名零波及）；② 传送门房格栅通行口被「门洞表单列掏刻 + 格栅盖回」双重逻辑堵死（玩家不可达）→ g1 显式掏穿双墙带 z=-10/-11（栏数恰 7、总数 39 不变，t759 (d) 腿与可达性同满足）。**探针 P-t1002**（矩阵 423→424）：五层断言（逐方块计数窗〔砖/铁栏杆 39/岩浆 81/框架 12/书架 231/银鱼笼 1〕/ 家具窗 / 变体窗 / 确定性双 generate 同构 / 源码钉权重表+保证旗）；t759 演化（新坐标 + 格栅入口腿 (d)）保绿，P-t1000/t786/P-t998 保绿。**阴性轮**（合并突变：权重表打乱 + kPortalGrateGuaranteed→false）→ 恰 4 红（P-t1002 传送门房整房未落地 diag + 源码钉漂移 + t759 连带〔净空腿直守同房〕+ P-t1000 连带〔B5 反推需 ≥3 框架〕；t960 名单 flaky 同轮）→ 复绿 **424/0**。零警告；全程 offscreen；续棒 150 分钟。

### 🅵 沙漠神殿逐方块（t1003）
**t1003** 沙漠神殿 15×15 → **21×21 逐方块**：四面安卡纹样（**橙/蓝羊毛**，偏差 3 时代口径）+ 正面主入口 + 后侧两副入口 + 顶窗；地面风玫瑰棋盘纹 + 中心蓝羊毛块；蓝块正下石压板接 **3×3 TNT×9**（现已有 TNT 陷阱雏形，按考据重摆）；地下暗渠**藏 4 箱**（现 4 箱保位按考据坐标）；切制砂岩家族替代刻纹（偏差 6）。频率/群系门保留。探针 P-t1003：5-seed rig（21×21 足迹、安卡纹样对称面、风玫瑰+中心蓝块、压板→9TNT 链、4 箱暗渠、副入口/顶窗在场）+ 源码钉。沙漠神殿首个行为级探针从零新建。 ✅✅（3d239bc）**placeDesertTemple 重写**：11 层阶梯金字塔（半宽 10..5，21×21 底）+ 切制砂岩顶冠过梁；四面**对称橙羊毛安卡纹样**（每面 21 格，行掩码表）；地面风玫瑰菱形棋盘（24 橙 + 中心蓝，偏差 3 pre-1.8 羊毛时代口径）；三入口（正面 3×3 阶梯门 + 后侧 u=±5 两副）+ 四顶窗通 L8/L9 空腔；密室深挖 floorY=S-12（wiki Layer -11 箱/压板口径）+ 4 面墙箱朝室心 + 石压板覆 3×3 TNT 9 连锁 + 正面斜向暗道下探密室顶。**探针 P-t1003**（矩阵 424→425，第 425 条，从零新建）：rig 升 **160×160×128**（t307 地表基线 64 下 64 高 rig 恒拒——**历史「沙漠神殿全零生成」的根因在此一并修复**）；28 seed 池 + skip-miss + ravine-damage 净样口径；断言足迹/层半宽、安卡逐格对称、风玫瑰计数、开口、暗道、密室+朝向箱+压板覆 TNT、FNV 重生成确定性 + 6 源码钉。**阴性轮**（kChamberFloorDrop 12→7）→ 暗道/密室/源码钉腿恰红 → 复绿 **425/0**。零警告（dxcompiler 豁免）。 **📎 review0905 #5 wiki 考据收口（2026-09-07，结论=门面截断即忠实，几何不改）**：核 minecraft.wiki [Desert_Pyramid](https://minecraft.wiki/w/Desert_pyramid) + [/Structure 蓝图](https://minecraft.wiki/w/Desert_Pyramid/Structure)（Raw wikitext 逐层解析，raw + 分层蓝图原文归档 `docs/review0905_wiki_desert_pyramid/`）——MC 本尊**完整安卡形**（橙陶瓦环+横杠+垂柱、**蓝陶瓦心**）只在**地面层中央**（蓝图 Layer 0）与**密室**（Layer -4）各一次（Materials：Orange Terracotta=66 / Blue Terracotta=2 = 恰两处蓝心）；外立面仅**前侧双角塔**面有「双竖橙条 + 顶横橙排、中央嵌切制/錾制砂岩」条带装饰（非完整安卡），**门面门洞区本就无完整安卡**：门楣上方即切制砂岩（蓝图 Layer 3 前排 `AAA`），且本尊整体埋沙、无地面门洞，故不存在「入口打断安卡」。本项目「四面中央安卡」为登记再创作口径（偏差 3 羊毛时代的延伸诠释，每面 21 格、正面被门洞+门楣截至 13 格）——与 MC「门面门洞区无完整纹样」**忠实同构**，非缺陷；P-t1003 探针 PASS 文案与 (b) 腿注释已按此措辞（「四面等量」不再作为口径）。 **P-t1003 (g) 重扫口径两修（2026-09-07 首跑 478/2 实证，行为面零改、探针侧修正）**：(g) 确定性腿旧重扫用「扫序前 4 箱」朴素质心重定位神殿，与主循环「首个恰 4 箱净簇」选择口径错位——#3 保底上线后多神殿世界常见（首跑 seed 777 实证：主路径座被峡谷削至 3 箱〔残簇静默跳过〕+ 保底新增座 → 重扫质心 = 3+1 跨座错位 → FNV 静默失配假红，且 (g) 原无 diag）。修 = ① 形制采样拉成 `t1003ShapeOk` lambda（主循环 / 重扫同款复用）；② 重扫改 `t1003FirstCleanCentroid`（全簇迭代 + 恰 4 箱 + 形制筛，与主循环同序同构）→ 选择口径永不漂移；③ (g) 失配带现场 diag（seed/质心/双哈希）。

### 🅶 丛林神殿逐方块（t1004）
**t1004** 丛林神殿单层 → **3 层苔石建筑**（考据口径）：底层**三拉杆谜题**（3 输入块上 3 拉杆/按钮【拉杆若无则按钮，实现工核 palette】，正确组合 → 开墙显主层箱——开墙机制走局部方块事件链，**偏差 4：无活塞**）；**2 组发射器陷阱**（走廊中段 + 转角箱前，压力板触发替代绊线【偏差 4】，藤蔓/蛛网遮蔽，箭 2-14 发【Dispenser 已有】）；2 箱（陷阱走廊尽头 + 谜题后）；cobble/苔石混排按考据。探针 P-t1004：5-seed rig（3 层形制、谜题组合唯一正确+开门腿、陷阱位置+发射器箭数、2 箱位）+ 源码钉。首个行为级探针从零新建。 ✅✅（4dfaf81）**实现（两棒接力 + 主控收口）**：placeJungleTemple 重写为 15×15 双楼板三层苔石混排 keep（逐格 hash 40% 苔 = kMossyPct，MC「cobble or mossy」）+ 11×11 顶；西入口带坡切（dx=-7/-8 穿坡防埋门洞）；陷阱走廊（北隔断全高/南短断）+ 宝藏龛（东端 3×3 IronDoor 封）+ 西北侧室；**拉杆谜题实为 3 Lever 红石 AND 门**（拉杆方块存在！超预期）——南墙 3 拉杆（附 +Z 全 OFF）→ 双粉支线（t869 形状语义臂粉指 B 块）→ B 块（拉杆直供/臂粉共挂）→ 3 NOT 火把汇流线（kMergeStates 平衡表）→ 拐角粉 0x2F → 终 NOT 0x09 → 门侧粉（退 1 格防贴门格）→ **IronDoor t722 红石驱动**；唯一正确组合 = 全 ON（MC 组合逐 seed 随机 → 固定全 ON，登记）；2 组压力板→发射器陷阱（箭 2-14 编码 state bit[5:2]，走廊中段 ±Z 对射 + 转角箱前；绊线→压力板偏差 4）；2 朝向丛林箱；蛛网。**探针 P-t1004**（矩阵 425→426）：5-seed × 160×160×128 rig（铁门簇定位神殿；三层形制净样门/入口净样门〔峡谷切塔/坡度/树侵扰弃样〕/楼梯双跑/苔率窗 [25,55]/陷阱 2 组+箭数窗/锁电路逐格静态/**组合锁 8 组合行为腿**〔每组独立重生世界，全 ON 唯一开〕/2 箱朝向/蛛网/确定性重生成/源码钉 7 串）。**主控收口两修（探针侧）**：① S 锚定从 heightAt(cx,cz) 改**铁门簇 y-1**（结构固有锚——生成后植被/坡度改写地表高度图，漂一格全坐标系平移）；② 锁支座家族检查 dy 1..2→1（S+2 层同格是粉尘/火把本体，旧检查把自家电路判缺座 = 全净样神殿系统性假红）；static legs 加逐腿诊断。**阴性轮**（kMossyPct 40→20）→ 恰 P-t1004 红（moss 19% 出 [25,55] 窗，逐腿诊断现场报值）→ 复绿 **426/0**。零警告；全程 offscreen。 **⚠ review0905 #2 耦合告示（2026-09-07 登记）**：锁电路首帧 state = worldgen 直写的**手写红石稳态字节**（`kMergeStates[6]={0x1F,0x3E,0x3E,0x3F,0x3E,0x6E}` + 拐角 `0x2F` + 终 NOT `0x09`；world.cpp placeJungleTemple 定义处已加指向本告示的注释），conn<<4\|power 编码与红石电力语义（火把馈电 15 / 粉逐格衰减 / t869 形状连接 / 脏集回插 + 火把自回插 ~5 tick 收敛）**强耦合——今后改任何红石常量或传播规则，必须同步重推 kMergeStates 并重跑 P-t1004（逐格静态 + 8 组合行为腿）**，否则 worldgen 首帧与真稳态静默失配（P-t1004 state 钉腿是唯一护栏）。可选根治（登记为后续波次候选，本波不做）：worldgen 放完电路后对电路包围盒触发一次与运行期同源的 dust 重算，消灭手写稳态。

### 📎 R19.19 范围与顺序
t998-t1004（7 项）。**建议顺序：t998 方块包（要塞前置）→ t999 地牢（小步快跑）→ t1000 成就（独立小项）→ t1001 矿井 → t1002 要塞（最大项）→ t1003 沙漠 → t1004 丛林**。每项独立 commit + dev-plan ✅✅；全部完成后只读 review + 统计报告。矩阵基线 419，新探针预计 +7 以上。实现工每任务第一步 = 自行精读对应 minecraft.wiki /Structure 子页补全逐层蓝图（主线程抓取超时的补课），考据要点以本批头为准。**本批头含登记偏差清单 6 条，用户核对时可逐条否决/改判。**

### 📎 review0904 补充登记（实现期近似 + 漏登项，供用户核对/否决）
**t1002 实现期近似**（piece 工程化近似，全部已写入 stamp lambda 注释，升格清单如下）：图书馆保证 ≥1 座（MC 0-2）；走廊 50% 侧出口退化为壁龛；转角 piece 未单列（由走廊+门洞拓扑承担）；螺旋梯/直梯段单层平化；平滑石台阶→石砖台阶；大图书馆 231 书架/104 木板/13 栅栏（wiki 233/166/46）；小图书馆 ~86 书架（wiki 161）；石砖楼梯 9（wiki 8）；岩浆 81（wiki 15，块数级 ≥）；南房双银鱼笼/双箱退役（内容并入监狱厅/储藏室/箱走廊）。**丛林神殿补登**：藤蔓遮蔽未实现（工程无 Vine 方块——客观约束，遮蔽面由蛛网承担）；陷阱箭 2-14 为 dispenser state 考据读数、运行期不消耗弹匣（沿用既登记陷阱机制口径）。**t1000 措辞勘误**：阴性轮红轮实为 FAIL 3（t891/t960 名单 flaky 同轮），「恰 P-t1000 红」指排除名单成员后恰一行为红。**t1001/t1002 阴性轮日志未留存**（红/绿值已录入提交信息与账本：t1001 gapMin 5/spider false/pin false 两轮稳定、t1002 整房未落地 diag+4 红；复盘判据自洽，落盘纪律缺口登记，后续批强制归档）。

## R19.20 实测回归清算批 + 野外内容批（t1005-t1023，19 项；2026-09-04 立项，用户已批准开工；末地维度按用户指示排除；蜘蛛爬墙默认保留）

**立项背景**：用户 9-04 实测（exe 戳 f6e9a51，17:35）。三条 **R19.18 声称已修但野外未愈的回归重新立案**（证伪口径：R19.18 的修复不作对错预设，先在 rig 复现用户症状再修，验收以「rig 症状复现消失 + 用户新 exe 口头确认」双门槛）：① 大量 TNT 后 7FPS/142ms 帧/**sim 仅 0.14ms**、重进同一存档立即恢复 99FPS = **进程级运行时状态泄漏**（非世界数据）；② 生物复制体仍在（进场即有静止贴图生物、不可交互、**一段时间后无限复制**）；③ TNT 链式引爆后持续闪烁不清除（**精确定位：红石块引燃、多 TNT 同时连锁时出现，单发正常**）。用户 F3 快照存档（两份，同存档进出对比）：
- 卡局：`FPS:7 142ms/frame 0.14ms sim`；`mobs 17/64 items 93/93`（item 槽 93 高水位！）；`bump 220 fan 399 emit 21`（t935 指纹机按比例工作，非主因）；`draws:42`；`verts 1532424`（与复绿局同量级）；夜间沙漠
- 复绿局（重进同存档）：`FPS:99 10.1ms 1.65ms sim`；`mobs 17/19 items 4/4`（**89 个 item 实体消失** = 不在世界数据里，是运行时堆积）
- 用户指路：看 F3 render-side 行（gpu/prep/present——t934 交付的仪器正好用上）

**新内容反馈**：小僵尸蛋贴图未统一、小僵尸追击跳不过 1 格；蜘蛛爬墙（口径核对）；**沙漠/丛林神殿新世界未见到**（用户所在地即 Desert 群系）；矿井火把缺失但见光源（F3 `bl:0 ol:15` = 豁口天光，非火把）；矿井地板应「嵌岩段直接石头、洞穴段才铺木板」；蛛网室应是**走廊**不是房间（用户裁 R19.19 批头分歧）+ 毒蜘蛛应真变种；水桶破坏蛛网、水冲火把/红石火把成掉落物；矿井箱应为**箱子矿车**（转正偏差 2）；**矿石按 MC 矿脉模式生成**（用户重点：煤=2×2×5 长条+贴连散块、铁=2×2×2+角贴块等）；骑船攻击目标甄别；存退时间重置；仙人掌不可放火把；铁轨延伸不要求同朝向（t983 过紧回炉）；矿车 V 谷仍打转不上坡（t981 口径再翻案：能量判据）。**参谋清单全做（除末地维度）**：偏差转正（并入 t1012/t1013）、成就树、性能批三、音效层、键位重映射+设置页、光照/绘制优化。

### 🅰 回归清算（t1005-t1007）⚠️ 最高优先
**t1005** TNT 链式引燃闪烁未清除：红石块旁多 TNT 同时连锁引爆 → 部分 TNT 持续闪烁不清除；单发正常。疑面（先复现再修）：t997 引爆预算（4/tick + 1/60s 重挂）与引燃链交互——预算重挂期间红石块每 tick 重供电重燃 / 链式转化路径漏置 Air / 重挂引信的 primed 实体 fuse 被 redstone 邻居反复重置。验收：红石块旁 N×N TNT 阵（含 2×2/3×3/长链）全部终态清零、无永续闪烁实体；单发口径不回归；**阴性轮 + rig 长链复现**。
  → **🔍 调查收口（2026-09-04，三棒 agent + 主控复核）：实体级复现失败，0 行 src 改动，不盲修。** 交付 P-t1005 探针（eda017a：四布局 2×2/3×3/1×4/同帧 5 直供超预算 4 + rig 网格 128→144 +4 足印守卫）与加严面（0c830e3：59 羊填池至 64 cap 满压链 + fuse 冻结僵尸槽检测器双侧自检），矩阵 427/0。证伪清单：僵尸槽结构性不可能（fuse 递减先于一切 continue，tick 单表全槽遍历）；池满=静默跳过（TNT 消失，终态干净）；"slots 7" 是高水位非上限（cap=64）；渲染活读（Main.qml mobHost t935 指纹差分 + t978 count 自愈）无实例缓存；存档不持久化实体（worldstore 仅方块迁移）；fuse 全工程仅 3 写入点（spawn/递减/t997 重挂），红石碰不到。**关单需用户供给**：症状现场完整 `vo.entity`/`vo.red` 日志段 + 实际布线形态（区分「实体残留」vs「QML delegate 冻结」vs 描述外布线如粉馈线重供）。新 exe 不改变行为（无 src 修复）。
**t1006** 生物复制体未愈：进场即有静止贴图生物 + 随时间**无限复制**。R19.18 t978 三层防御（幂等守卫/纠正 bump/count 自愈网）未对症——本单**先复现后修**：rig 重建用户路径（新建世界进场扫描「delegate 可见数 vs 活体数」+ 长 tick 监视幽灵 delegate 增长曲线 + 槽位复用/生成压力测试），把「无限复制」的产出源找出来（疑面：某生成路径产出实体但 AI/tick 未驱动 → 静止；或 delegate 层复用错位 → 每次生成多挂一个僵死 delegate）。验收：rig 复现曲线归零 + 用户症状消除；t978 三层防御保绿不回退。
  → **🔍 引擎侧调查收口（2026-09-04）：五假说行为级全证伪，复现不出 0 行 src 改动。** P-t1006 探针（cbf9a5f，矩阵 428/0）：真 World+EntityManager+QQmlEngine 重建——①进场复刻+480s 夜压长 tick：活体曲线精确停在 27=15 进场+12 区域 cap（上限 47），aliveSlots==liveCount 不变量每秒绿，90s 静止体检测 0 只；②刷怪笼显式石壳 rig：敌对完美平台 12→12 区域 cap、被动恰 4=kSpawnerLocalCap，gate 双侧不破；③真 QQmlEngine mobHost 同构 Repeater 六相轮换（spawn/长tick/kill/复用/clearAll/respawn）：可见 delegate 数==引擎活体数全相守恒，t978 三层真引擎成立；④全生成路径单口 spawnMobCore 完整初始化（AI 不可能未驱动）。登记项：被动笼开敞场 240s 积 21 猪→慢积累至 cap=64 面存在但需创造模式蛋改笼（生存不可达）。**关单需用户数据**：症状现场 F3 mobs N/64 与槽高水位（区分引擎增长 vs 呈现层克隆）、克隆型（进场猪牛羊/敌对/笼迷你）、退菜单重进是否消失、可复现种子存档。剩余测不到面=真 Main.qml Quick3D 场景（Node reparent+Loader 子树+MobModel 重建链）。
**t1007** 进程级卡顿泄漏（重开 R19.18 t997 遗留 + 升级为实机判读落地）：复现 rig = 大量 TNT 引爆 + item 高水位（用户实测 93/93）+ mobs 高水位，采 F3 render-side 行（gpu/prep/present/vmem/pipelineCount + `act ct`）+ 帧时间曲线，定位「重进存档即恢复」的运行时状态面（疑：渲染线程 prep/上传累积、geometry/材质实例泄漏、item 93 槽的渲染成本、跨世界渲染状态残留）。产出根因 + 修或登记治理路径；**用户实测读数与本 rig 曲线对表**。
  → **🔍 落地（2026-09-04）：引擎侧插桩 + 三高水位 rig 全绿 + 根因指认渲染侧，治理路径登记（不盲修）。** 交付两件：**① F3 增补读数**（t1005/t1006/t1007 三单共用采集面）——entities 行改 `mobs L/64 hw H  items L/S hw H2  orbs C/D  primed P  del V/T`：mobs 分母从槽数（delegate 永不销毁单调不降，读 12/47 会被误读成「距满 35」）改钉引擎 kCap=64；hw=槽池高水位（会话历史峰值，clearAll **不重置**——重载后 live 归零而 hw 不变 = 上一世界真到过的峰值，判「引擎增长 vs 呈现层克隆」的基线）；primed=存活 primed TNT（t1005 关单判据：现场永续闪烁 primed>0=引擎实体残留、==0 仍闪=delegate 冻结实锤）；del V/T=mobHost 可见 delegate/总 delegate（t1006 关单判据：delVis>mobLive=呈现层克隆实锤；delTot 恒等槽数）。读数链=新 Q_INVOKABLE（entitymanager.h `primedCount`/`slotHighWater`、itementitymanager.h `liveHighWater`）+ buildF3Text 10Hz 普通 JS（mobHost.children 视觉树扫 delegate 根，自有 mon 属性识别防子节点误计；零 NOTIFY 绑定成本，t976/t977 口径不动）。**② P-t1007 rig**（矩阵 428→429）：真路径 36 primed TNT（t997 布局）+ explosionDroppedItem→spawnItem 转发同构 + 250 named 掉落（跳过合并=计数确定性）+ 64 mob 压满 + 重载对比 + 满池/空池 tick 链墙钟。曲线结论（offscreen 引擎侧面；gpu/prep/present/vmem 无 QRhi 只能实机采，不编数）：(a) 36/36 全实爆零 primed 残留，掉落洪水下 item live ≤200 LRU 钳制，双池 aliveSlots==liveCount 逐样本绿；(b) 250 named → 恰 live=200/slots=200/hw=200（LRU 精确钉）；(c) mobs 恰 64=cap；(d) **重载状态面指认**：clearAll 后 live 全归零、有界残留保留（slots 200/64、hw 200/64）——「重进存档即恢复」重置的运行时状态面=**活体集（+世界内容）**，保留面全部有界 ≤cap（t437 slot-reuse 设计残留），**引擎侧无无界泄漏**；(e) 满池 sim tick 链 avg ~0.021ms/f vs 空池 ~0.0036ms/f（×6），个位 ms 量级与用户 F3 sim 4.63ms 口径一致。**根因**：三角对表（用户 F3 sim 占 7% + render-side 行 gpu/prep+present 重 + 本 rig 满池 sim 个位 ms）→ 7FPS 的 ~14× 恶化在**渲染侧**：活体 delegate 渲染成本随活体数线性放大——items 93/93 语义=live 93/slots 93（全活体高水位；cap=200 LRU + 5min despawn 双钳，非设计上限非泄漏），每 item delegate 内联 ~12 个 geometry/material/Texture 实例互不共享、mob delegate MobModel+多子 Model+armor/eyes Loader 子树，draw-call 数 ∝ 活体数（用户机器上 93 item + 47 mob ≈ 数百 draw 叠加 202 地形段全幅绘制 → 撞 GPU/present 预算）；重载隐藏全部活体 delegate → 回到 ~202 段 → 99 FPS。**治理路径（需实机曲线对表后定；offscreen 无渲染线程/视觉面，不盲修）**：a) 掉落物按 t858 经验球先例压 per-family 单 Model instancing（xporbinstancing.h 头注释已登记的 per-item 贴图分桶复杂度面）；b) 隐藏 delegate 的 `Animation on rotY/bobY` 永不停表（QML visible:false 不暂停动画，t561 火焰同款教训）→ `running: visible` 门控（微收益，恒定 ~2×槽数动画烧帧）；c) mob 族同向治理。关单需用户在新 exe 复跑原场景读 F3 增补行对表。

### 🅱 生物（t1008-t1009）
**t1008** 小僵尸两修：① 生物蛋图标风格统一（小僵尸蛋 vs 其它蛋观感一致——蛋图标渲染路径核一遍）；② 小僵尸追击跳不过 1 格障碍（t988 是驯服狼链；敌对链 baby 的越障跳缺失/失效——疑 baby halfH 缩放影响 t988 的 tdy 跳门或 hostile 跳门门槛）。验收：小僵尸蛋图标统一；小僵尸追玩家越过 1 格台阶（rig 行为腿）。**蜘蛛爬墙口径核对**（随本单登记）：MC 忠实行为 = 蜘蛛可爬墙，默认**保留**；用户若要禁爬/限爬在核对时改判。
  → **✅✅ 落地（2026-09-06）：矩阵 450→451（P-t1008 三腿净增）。** ① 根源 = `itemFilenameMap` 缺 0x25D 行（t952 引蛋时 15 蛋唯一漏行）：pack 开时其余 14 蛋走 spawn_egg.png 两层染色生成管线、小僵尸蛋恒落 MaterialIcon 自绘 Canvas =「风格不统一」观感分叉（t989 查看器侧同诉当时以蛋分区整段移除回避，创造背包蛋区仍在故本单根治）。修 = 补 `{0x25D → zombie_spawn_egg.png}` 映射行（机制等价 MC 幼体僵尸 ≡ 僵尸家族蛋名；demo 包缺独立蛋图 → `spawnEggTint(0x25D)` 生成式回退：**同管线蛋形 + 亮一档黄绿幼体染色**可辨成体暗绿），MaterialIcon 自绘 case 保留为 pack 关回退。② halfH 缩放疑点**排查证伪**：plain baby 追玩家越 1 格台在夜/昼（避光机）/咬带压身三情景 rig 全数越台（aiHostile t670 跳门类型无关、无 #14 压跳门）——真断点在**挂载链**：小鸡骑士（t952 骑手 AI 驱动载具）钉位恒 resting → aiHostile 越障跳门照样点火，但 tickMobMounts 钉位段把骑手 vy/滑流一并清零 = 组合结构上跳不过 1 格台阶（diag 实证台面卡死 30s+：骑手 AABB 压台面格逐轴撤回原地踏步）。修 = **跳意图转移载具**（骑手 vy==kJumpSpeed 且载具贴地 → 载具同款 kJumpSpeed 起跳；载具空中不重点火防逐 AI tick 火箭升空；骑手 vy/滑流照旧清零——#19 陈旧滑流防线不动）：载具主循环重力/落地物理照跑 → 整体升过台顶、骑手 AABB 随钉位抬出后 AI 推进恢复、XZ 钉位把载具拖上台 =「小僵尸的追击移动驱动小鸡」延伸到跳跃轴；t988 狼链（aiWolf tdy 门）与 plain baby 链零改动。验收 P-t1008 三腿：(a) 蛋管线三面钉（映射行滤注释源钉 + `spawnEggTint(0x25D)` 直调非空且幼体亮绿 base 0x5a7a42 + MaterialIcon case 在位）(b) 骑士组合追玩家越 1 格全深石台（骑手 x≥24.5 且 y≥86.3 合取 = 真上台〔P-t988(b) 同款判据〕+ 抵 1.6 咬击带，30s 帽）(c) plain baby 对照腿（同台面独立越障照绿 = 病灶仅在挂载链口径钉）。**阴性轮：摘除 tickMobMounts 跳意图转移块 → 恰 P-t1008 红且唯一红**（diag `b final=(23.51,86.25,22.50)` = 组合回退台面卡死原诉；(a)(c) 与 review0830 batch C 保绿），restore → 451/0。连带 P-review0830C(f)（骑士陈旧滑流腿）**lawful 演化**：t1008② 起组合遇 1 格墙「翻墙」而非「卡死」，西撤相改距离闸（西行至 x≤19 再杀鸡，容一整次再翻墙；杀鸡点离玩家 ≥6 格 → 解除挂载后独立追击恒向西，>0.02 东向帧只剩滑流签名），陈旧滑流回归核（钉位段清零行源钉 + 杀鸡后零东向漂移帧）原样保留；同轮 t882/t960 各一次偶红（登记 flaky 清单）原样复跑即绿。**蜘蛛爬墙口径核对（随本单登记）**：MC 忠实行为 = 蜘蛛可爬墙；本项目蜘蛛与其它敌对同走 aiHostile 1 格越障跳（无独立爬墙路径）——口径确认**保留现状**，本单零蜘蛛代码改动；用户后续若要禁爬/限爬再改判。**待实机确认（观感/行为类）：① 资源包开/关两态下小僵尸蛋与其它蛋观感同族可辨；② 小鸡骑士追玩家能翻上 1 格台阶。**
**t1009** 敌对生物再生清理联动（随 t1006 复盘产出：若复制体源 = 敌对再生路径，本单承接其修复的生成侧清理；若 t1006 已覆盖则本单并档）。
  → **📋 并档（2026-09-05，按本单自身条件）：t1006 行为级证伪敌对再生缺陷**——进场+夜压 480s 曲线精确停 27=15 进场+12 区域 cap（上限 47）、刷怪笼 gate 双侧不破、aliveSlots==liveCount 不变量逐秒绿——敌对再生路径无「无限复制」可承接的修复面。t1006 登记的唯一真实积累面（被动笼开敞场 240s 积 21 猪→慢至 cap 64）需创造模式蛋改笼、生存不可达，已量化入 P-t1006 diag 持续监测。克隆症状剩余嫌疑在呈现层（del V/T F3 读数，t1007 插桩已就位），关单待用户实机数据，无独立代码动作。

### 🅲 结构批二（t1010-t1014）
**t1010** 沙漠/丛林神殿野外生成验证（用户新世界未见到，所在地即 Desert 群系）：N-seed 全量扫 placeDesertTemple/placeJungleTemple 真实落位数 + 生成条件逐条核对（群系门×网格候选×地表判定的联合概率）；若频率过低 → 保底机制（有该群系的世界必落 ≥1 座，对标要塞口径）；若放置静默失败 → 修。验收：rig 多 seed 落位率 + 有群系世界必见。
  → **✅✅ 落地（2026-09-04）：根因=联合概率过低（非放置静默失败），双神殿保底落地（对标要塞口径）。** 数据面：160² 世界沙漠神殿仅 3×3=9 网格候选 × 45% 命中 × Desert 占比 0.8%~18%（成片，seed 间方差大）→ 期望 ~0.7 座/世界；32-seed 全量扫（P-t1010 diag，同源复刻主路径网格）实证 hash 命中候选 0~6/9、其中过群系门 0~2 —— **默认 seed 1337 主路径 3 候选全被群系门拒 = 用户「所在地即 Desert 却从未见神殿」实锤；seed 555555 连 hash 命中都为 0**；旧历史矩阵 944/955 次 worldgen 神殿数=0。地表判定无错位：放置 surfaceY=heightAt 与 generate 地形填充同源（R19.19 的「64 高 vs 地表基线」错位仅存在于 rig 探针侧，P-t1003 已修，本单排除静默失败）。修=**两神殿各加保底**（world.cpp：siteOk 五守卫〔margin/群系/海域/越界/贴基岩〕+ tryPlace 落位器单源收口重构，几何 A..J 原样包作用域字节零漂移，概率主路径逐字保留）：主路径 0 落位且世界含该群系 → 全图合格列选距世界中心最近补座 ≥1（确定性纯函数同 seed 同点位；极小世界边长<2×margin 可无合格列如实 0，登记）。P-t1010 探针（矩阵 429→430）：32 seed × 160×160×128（Main.qml 本体同尺寸）全量 worldgen，qInstallMessageHandler 捕 worldgen 自报（biomes 列数 + temples 座数=引擎单次 generate 真实输出，其余消息链式转发）——**必落腿：desert seeds 32/32、jungle seeds 32/32 全 ≥1（合计 33 沙漠 + 42 丛林，有群系 0 神殿 seed=0）**；实块核对腿：y 带扫 PyramidFlag 箱簇（Chebyshev≤6）/ IronDoor(state 0,y≥50) 计数==自报座数 + 簇心群系核（钉「日志=真实栅格」）；确定性腿：同 seed 二次构 World 四元组全等；源码钉：双保底旗/入口/siteOk 行。**阴性轮：双保底旗置 false → P-t1010 恰红**（32 条 VIOLATION 含 seed 1337「desert cols 1295 but 0 temples」，total FAIL=1 且仅此一探针红）；restore → 绿。全矩阵 offscreen 430 PASS / 0 FAIL（EXIT=0）。 **review0905 #3 口径升级（2026-09-07，出生区可见）**：保底触发门从「全世界 0 座（placed==0）」改「**主路径最近座距世界中心 > kTempleSpawnGuaranteeRadius(56，world.h 单一权威) 且世界含该群系**」——旧门在世界角落 1 座时不触发，出生区仍 0 神殿（用户主诉只解一半）；补座选列逻辑不变（距中心最近合格列，R 内有合格列则补座天然落 R 内）；确定性纯函数不变；小世界 R 超半边 → 门恒开退化为旧口径（登记）。探针 P-t1010b 加严腿：32 seed 逐世界块级扫——群系**合格列**（World::desertTempleSiteOk/jungleTempleSiteOk 公开口同源直读）在 R 内 ⇒ R 内神殿证据 ≥1（必触发且落座 ≤R）+ R 内簇数 ⊆ 日志总数（不重复不膨胀）；R 直读 World::kTempleSpawnGuaranteeRadius。**P-t1010b 探针三修（2026-09-07 首跑 478/2 起三轮实测，行为面零改、探针侧修正）**：① 前提「群系列在 R」→「**合格列**在 R」——实证 R 内有群系 ≠ R 内有合格列（海域/贴基岩/margin 守卫拒），此时保底补座落全图最近合格列、R 外如实落位 = world.h 登记的契约内退化；② 沙漠箱改**全高扫**（y 带 40..64 在 S≤50 低位地形世界漏计 R 内神殿）；③ 判据从「恰 4 箱完整簇」改「**独占块证据在场**」（跑2 实证完整簇判据把峡谷削残的三箱座藏掉；跑3 实证箱组/贴地表门可被整组削光）——沙漠证据 = PyramidFlag 箱 ∨ CutSandstone（worldgen 仅沙漠神殿顶冠+门楣使用，~126 块/座）、丛林证据 = IronDoor 下格（y≥40 隔离要塞监狱门 + 门心群系核）∨ Lever ∨ Dispenser（worldgen 独占丛林神殿），证据独占性 = worldgen 全树 grep 核实。
**t1011** 矿井火把与光源归因：用户见光无火把（F3 bl:0 = 天光穿豁口）；核 ~55% 火把是否真落块（t1001 支撑组实现逐段核）+ 豁口天光机制如实登记；调整火把密度/覆盖（含壁挂位）到「矿井段宏观可见火把照明」口径。验收：rig 火把密度窗 + 光源归因结论写进账本。
  → **✅✅ 落地（2026-09-04）：归因=「天光豁口（固有通道，非火把缺失）+ 火把覆盖稀疏」双面；跨中壁挂 + 起点厅角壁补光，矩阵 430→431。** 逐段核实：R19.19 火把**真落块**——支撑柱顶火把在 pieceCorridor 支撑组内直写（kTorchPct=55 每侧独立 hash、仅空气格、组级 25% 缺失、斜坡段整段豁免零火把），P-t1001 ⑦ 池化 ≥15 早已证在场；覆盖面排查：蛛网 / 矿石 / 轨 / 事件件均在火把之后落或仅置换 Stone/Dirt，**唯一真缺口 = 覆盖稀疏**（柱顶 55%×2 侧 + 组级缺失 25% + 斜坡段零火把 → 阴性轮实测 ~2% 巷腿前 5 步全暗、0-3 步距桶仅 9%，恰为用户「见光无火把」主诉面）。**光源归因登记**：F3 bl:0 的「见光」必为**天光通道**（bl 非零必须 14 级火把近场——lightEmission(Torch)=14、generate 末尾 recomputeLightField 种子全场传播）——carveCaves 洞穴网络与矿井壁交叠出豁口、洞穴连通地表 → 天光 BFS 经豁口渗入巷道（sl>0 / bl=0），与火把覆盖正交、固有存在（登记不复零；本 seed 池修后巷内 sl>0/bl=0 = 0..3/495 采样格）。修=**①跨中壁挂火把**（pieceCorridor：step ≡ 2 mod kSupportInterval 两侧成对、顶层墙插 TorchAttach 1..4 state〔mesher 倾柄位姿同运行期墙火把〕；侧壁被洞穴掏空 → 贴面格地板火把回退；斜坡段同布、curY 随地板降 → 斜坡不再全黑）+ **②起点厅 4 角壁火把**（入口厅照明收口；角格远离四轴出口嘴 → P-t1001 出口 / 拱带探针零扰）。P-t1011 探针（矩阵 430→431）：8 seed × 128×128×64 候选复刻（hashColumn FNV 同源）+ 拱带 36 Planks 净样 20 矿井 67 巷走——**密度窗：火把步距直方图 0-3/4-6/7-10/11+ = 125/16/0/0（0-3 桶 88%，7-10 / 11+ 恒零 = 任何巷走位距最近火把 ≤ 6 步 → 块光 ≥ 8 光斑在位；4-6 桶 = 交叉口 evStep=4/6 5×5 刻清邻位壁挂，由邻火把兜住，如实入窗）**；真可走段全暗前缀 1（≤1 窗；阴性轮 7）；壁挂墙插 87 只 + 起点厅角壁 73/80；块光归因 207/207 火把格 blockLightAt ≥ 12；同 seed 跨轮四元组全等（确定性复跑验证）。**阴性轮：git stash 摘 world.cpp 全部火把面 → P-t1011 恰红且仅此一红**（wallTorches=0、roomTorches=0、暗前缀 7>1、0-3 桶 9%<40%、源码钉失，total FAIL=1）；restore → 绿（t1011 数字逐位复现）。全矩阵 offscreen 431 PASS / 0 FAIL（EXIT=0，2026-09-05 复跑同 431/0）。（**移交登记（非 t1011 范围）**：pieceCorridor 段 B 推进在 t1001 重写中回归——`leg==0 && step>0` 应为 t565 原 `leg==1 || step>0`，现段 B 不推进只重复刻同截面（L 折线第二腿缺失、斜坡段变同柱逐层坑）；走廊几何属 t1012 ②；另 P-t960 弓击退位移腿（dBMax<2.2 紧界 + 猪游走噪声）跨轮抖动 ~40% 偶红，与本批无关，建议后续批次放宽噪声窗。）
**t1012** 矿井地板政策 + 蛛网室走廊化 + 洞穴蜘蛛真变种 + 水破坏附着块：① 地板 = 嵌岩段（下方已是实地）直接石头不铺木板、洞穴空腔段铺木板保可走（用户口径）；② 蛛网室改**走廊形**（用户裁 R19.19 批头分歧：1-2 宽 × 2-3 高 × 3-6 长窄廊 + 满网 + 笼，弃 7×7 房间形）；③ **洞穴蜘蛛真变种**（转正偏差 1：新 MobCaveSpider——蜘蛛模型 0.7× 蓝染 + 命中中毒 DoT〔沿用现有减损/效果链〕+ 蛋图标）；④ **水破坏附着块**：水桶倒水/水流冲刷破坏蛛网、火把、红石火把 → 掉落物（附着块族单一权威 + 流体更新钩）。验收：rig 逐项 + 行为腿。
  → **◐ 第一棒落地（2026-09-05）：①② + 段 B 推进回归（t1011 移交项）完成，③④ 移交下一棒；矩阵 431→432。** ① 地板政策（t565 ⑤ 全矿井 hash 二选一废除）：carveCell 逐列下方承载切换——下方 Air = 洞穴空腔 → Planks 桥面、下方实地 = 嵌岩 → Stone 石底；② 蛛网室走廊形（弃 t1001 7×7×3 房间）：巷侧垂直支廊 1-2 宽 × 2-3 高 × 3-6 长 hash 选型（盐 0x5C1A）+ 廊体满网 + 笼居中（cageOff=2+snl/2）+ 笼位 ±1 轴邻格恒 3 层夹网巢 → 笼周 ≥8 网（t786「≥8 网=矿井蛛笼」分流口径任意选型不破）；旧 7×7 双短端栅栏随房间形废除（口径登记）；③ 段 B 推进回归修复：pieceCorridor 恢复 t565 `leg==1 || step>0`（t1001 误改 `leg==0 && step>0` + 入腿预推进 → 段 B 不推进、L 第二腿缺失 / 斜坡同柱逐层坑）。验收 P-t1012 探针（矩阵 431→432）：8 seed 阴影模型重放（hashVoxel 同源盐值）+ 世界侧核——段 B 463 点 0 天然实体、地板政策两分支 4872:89 零违例、走廊 42/42 选型窗 + 满网 82% + 满网净笼 31/47 + 四源码钉；连带 P-t1001 ② 全步池化口径修正（w=±1 内容物白名单 Air/Cobweb/WoodFence/Torch——支撑双柱 t1001 设计本占 w=±1 sy+1..sy+2，旧 s==6 单步窗 6%4=2 恰躲开支撑步，全步池化后合法支撑拉低伪「在位率」66%<70%，非生成回归）；P-t1011 火把密度窗保绿（129/16/0/0、暗前缀 1≤1、blockLight 211/211）。**阴性轮：段 B revert → P-t1012 恰红（legB 实体 293/463 + 源码钉失，唯一红）；地板政策 revert（恒 Stone）→ P-t1012 恰红（planks 0 / 违例 89 + 源码钉失，唯一红）；restore → 432/0。** P-t960 本轮一次偶红复跑绿（登记口径内 flaky，弓击退紧界 + 猪游走噪声）。**移交下一棒：③ 洞穴蜘蛛真变种 ④ 水破坏附着块。**
  → **✅✅ 落地（2026-09-05 第二棒：③④ 补齐，t1012 整单完成）；矩阵 432→433。** ③ **洞穴蜘蛛真变种**（转正偏差 1）：`MobCaveSpider=20` **枚举尾追加**（存档兼容契约勿插中间）；碰撞盒 0.32×0.21（蜘蛛 0.45×0.30 的 **0.7×**，halfH 恰=腿底 0.30×0.7 → 图层零偏移贴地）；渲染 = mobmodel.cpp `mobType 7/20 共享蜘蛛几何分支`（kValidMobTypeCount 20→21 三级互钉）+ Main.qml delegate **0.7× scale + 蓝染 tint**（pack 命中 cave_spider.png 蓝 tint × 贴图 / miss 暗蓝纯色 #1c3a52，4 红眼随父缩放）+ 刷怪笼迷你表 3 处（scale 0.35 / yOff 0.030 / 眼表）；AI 走 aiHostile（仇恨/越障/铁傀儡转火全继承，`kCaveSpiderAttackDamage=2` MC 简单/普通档）；**中毒 DoT 沿用现有链零新系统**：onMobAttackedPlayer 据型 `applyStatusEffect(EffectPoison, 7s)` → t669/t715 既有 m_poisonTimer（每 1.25s `poisonDamageTaken(1)` 独立链绕护甲 + 剩 1 血不致死 + 效果栏 icon_effect_poison，同 t728 燃烬者挂 EffectFire 先例）；**蛋图标** 0x25E：RecipeRegistry 蛋表双向 + hotbar 调色板/名 + MaterialIcon `drawSpawnEgg("cavespider")`（暗蓝壳红眼簇蓝腿线）+ spawnEggTint 生成式染色 + pack 映射 cave_spider_spawn_egg.png / mobEntityMap cave_spider.png（miss 安全回退自绘）；图鉴第 18 条（预览 scale 1.05/centY/fallback 蓝三表）、音 20→7 同族蜘蛛嘶别名、monster_hunter 计入、`/kill @e[type=cavespider]`；**矿井笼转正**：pieceSpiderRoom 笼 state 0x0E→`SpawnerStateCaveSpider=(20<<1)=0x28`（偏差登记退役；旧存档 0x0E 仍解码 MobSpider 不断档）；中键 pick 蛋 `mobTypeEggId(20)=0x25E`。④ **水破坏附着块**：附着块族单一权威 `BlockRegistry::isAttachableBlock`（Torch 各 TorchAttach state / RedstoneTorch 含熄灭位 / Cobweb）；`tickWaterFlow` 扩散落点 + 下落落点挂 `tryWashCell`（washKeys 多源去重）→ 应用段 `setWaterSilent(Air)` + `blockDroppedAsItem(dropId)`（**掉落链对齐玩家挖除**：火把/红石火把掉自身、蛛网掉线 0x219）+ 同 tick tryAdd 灌水（「冲毁+入水」一步）；**桶倒水（setBlock 源）与已放置源流动共用本路径**；红石火把被毁经 setWaterSilent 内 notePowerWrite 走既有红石重算链（t976/t977 口径：QML 读格绑定经 worldChanged 信号 handler 读取）；石头等非附着块不毁（对照腿）。验收 **P-t1012③④ 新探针**：③(a) 契约腿（枚举 20 / state 0x28 编解码 round-trip / kValidMobTypeCount==21 / 蛋 0x25E 双向 / 旧 0x0E 解码保 MobSpider）+ ③(b) 0.7× 实体腿（spawnMobTyped(20)→0.32×0.21 hostile 恰成体蜘蛛 0.7 比例）+ ③(c) 中毒 DoT 行为腿（Survival 真时钟泵 4.6s → **3 段 poisonDamageTaken 各 1HP**）+ ③(d) Creative 对照（1.5s 泵窗 0 毒伤）+ ④ rig（悬空石平台线 水源|石头|火把|蛛网|红石火把：三附着块→Water ∨ Air + 掉落恰 3 件 id {13,129,0x219}、石头对照完好、水源 state=0 保持）。**家族探针合法演化**：P-t785 14→15 蛋连续同列 + 0x25E id 锁、P-t786 常量锁 +0x28 / round-trip 扩表 / 非法样本 0x29→0x2A / 矿井蛛笼分流 +CaveSpider、P-t787 蛋改型 round-trip 扩表 / 越界样本 20→21、P-t989 图鉴 17→18 / 蛋键绝迹表 +0x25E、P-t1001/P-t1012 世界侧笼扫描 Spider→CaveSpider + 源码钉 SpawnerStateCaveSpider。**阴性轮：④水毁路径中性化（tryWashCell 早退）→ 恰 P-t1012③④ 红（格保持 13/102/129 + 掉落 0，其余 432 绿，唯一红）→ restore → 433/0。** P-t882 本批两红一绿（登记口径内既有偶发 flake，t960 同族猪游走噪声签名 movedF=0.027；复跑清，非本批回归面）。**待实机确认（视觉/动画类）：蓝染 0.7× 洞穴蜘蛛观感、洞穴蜘蛛蛋图标、水流冲刷附着块（毁块同帧入水 + 掉落物弹出）、矿井蛛笼内迷你洞穴蜘蛛自旋。**
**t1013** 箱子矿车（转正偏差 2）：矿井箱改**箱子矿车实体**（矿车+箱容；交互开箱取放；默认静止；可被推动/激活沿轨移动；挖除掉车+箱+内容物）。验收：rig 交互/移动/掉落链。
  → **✅✅ 落地（2026-09-05，三棒接力收口）；矩阵 433→439（P-t1013 六腿净增）。** **实体面（minecartmanager）**：`spawnChestCart(key格, world, 内容键)` —— 落格三态（键格轨上 / 四邻首个轨格 / 键格地面静止 t734），与 spawnCart 共用 `spawnCartImpl` 全量物理（贴轨面 kCartRideH / 朝向 / 坡面俯仰 / 耐久 / 推动动力轨零分叉），变体仅 `chest` 标志 + 内容键三元组（**键 = worldgen 标记箱原格，永不随车移动**——车驶离后 UI / 掉落 / 回生仍按生成格寻址）；`tryMount` 命中箱车拒载（内容容器语义，兜底「箱车挡在普通车前」）；`chestAt`（QML delegate 车斗内箱体两件套）/ `chestKeyAt`（C++ 直调）读口；`destroyCartTail` 加 `voidLoss` 形参分流——箱车非虚空摧毁（生存末击 / 创造瞬破 / 仙人掌岩浆）全模式 `emit chestCartBroken(散布格, 内容键)`（**不发 cartBroken 防车物品双掉**；**对 t767 有意偏差登记**：纯车创造瞬破无掉落，箱车若同口径 = 内容键条目被静默清 = 用户物品数据丢失），虚空卷没（tickDerailedCart / checkCartEnvironment y<0）无掉落移除且**内容键条目保留** → 重进世界回生。**转正链（playercontroller）**：`convertMineshaftChests()` Q_INVOKABLE，Main.qml enterWorld 在 `chestStore.loadAll + carts.clearAll` 之后调——路 (a) `collectBlocksOfId(Chest)` 全图扫 → `isMineshaftChest` 标记位唯一授权 → `clearMineshaftChest`（**静默摘块**：world.cpp 新增，clearBlockSilent 路径不发 blockBroken，防 onBlockBroken(22) 当场掉内容/清刚登记的键/破块音）→ `registerCart` → `spawnChestCart`；路 (b) 存档回生：`cartCells()`（"cart":true 键）中本轮未转正键 → 按键落车（挖毁已清键自然不回生；虚空保留键回生）。**存储面（cheststore）**：`m_cartKeys` 并行标记面——`registerCart`（幂等；无条目建 27 全空条目 = 未开箱回生标记，R19.19 老档已开条目 27 槽原样升级）、`isCartCell`、`cartCells`、clearChest/clearAll/loadAll 同步维护；`allChests` **全空豁免**（cart 键 27 槽全空也落盘，"cart":true 标记向后兼容）；`populateMineshaftLoot` gate 更新（键+全空条目 = 首开 roll 一次；已填 / 非矿车键既有条目 no-op；填充后键升级矿车键）。**呈现层（Main.qml）**：enterWorld 接线、openChest 矿车键跳过「箱上压盖」门控 + 盖子动画（键格 overlay = 幽灵盖）+ 按键分流首开 roll、`onChestCartBroken` 掉落链（车 0x23E + 箱 22 + 逐非空槽 spawnItem 含附魔/改名/耐久 → clearChest 清键）、矿车 delegate 箱体、chestStore 注入。验收 **P-t1013 六腿**：(a) 转正（摘块+静默 blockBroken 计数不增+键登记+邻轨落车全姿态+键=生成格）(b) 交互（findCartHit+chestKeyAt 契约 + 箱车拒载/普通车可骑对照）(c) 掉落链（生存 3 击末击恰一次携键 + cartBroken 零次 + 信号时点槽元数据可读后 clearChest + 创造瞬破同发 + 虚空零发键保留）(d) 移动（默认静止→推 ≥1 格贴轨）(e) 存档回生（空条目 allChests 豁免 → loadAll round-trip → 路 (b) 回生 → populate gate 三态）(f) 源码钉全链。**两红腿收口归因（第三棒）＝探针腿假红，实现零改动**：交接时 (c)(e) 双红的 18:10 版探针用**全局 liveCount 算术**断言，而 rig 所在共享世界的 worldgen 矿井标记箱在同轮 (a) 一并转正（日志 `converted/respawned = 5` = rig 1 + worldgen 4，18:16 diag `live= 5 / 3` 即「实 5 望 3」签名）→ 计数随 seed 漂移必假红；(e) 的 `chest0=false` 是 LIFO 槽复用伪签名（回生车落最高空槽非 0 槽）。修法＝探针改 **rig 作用域槽级断言**（aliveAt/findByKey 按 alive+键寻址，不用全局计数）+ (e) 回放过滤 rig 键 + 每段判定冻结进 diag（survival/creative/void、eIdx/eChest/pop1-3 分列归因）；实现侧 minecartmanager/cheststore/playercontroller/world 三棒间零改动仍全绿 = 探针自阴性论证。**阴性轮：Main.qml 摘 `if (isCartCell) populateMineshaftLoot` 一行 → 恰 t1013 源码钉腿红（唯一红）；restore → 439/0。** t882 登记口径内 mob 漂移 flake 一红复跑清（非本批回归面）。**待实机确认（视觉/交互类）：矿车斗内箱体观感、右键箱车开 ChestUI（含驶离后按键寻址）、挖毁掉车+箱+内容物弹出、未开箱矿车重进世界回生。**
**t1014** 矿脉生成模式（用户重点）：矿石 worldgen 从散点改 **MC 式矿脉**——每矿种定义脉形 profile：煤=2×2×5 长条 + 周围 hash 贴连散块；铁=2×2×2 + 角落贴块；金/红石/钻石/青金石按 MC 1.0 尺寸/高度带 profile；脉内块间允许断连（「基本都连接起来」）。验收：多 seed rig 逐矿种（脉形尺寸窗/连接率窗/高度带/频率经济不涨总矿量）。
  → **✅✅ 落地（2026-09-05）：scatterOres 散点→MC 1.0 式矿脉；矩阵 439→445（P-t1014 六腿净增）。** **脉形 profile 表**（kVeinProfile，每 16×16 cell 每矿种脉数 = base + hash 抖动一位；落点 cell 内 hash 抖动；印章序同旧判定序 钻>金>青>红>铁>铜>煤，仅置换 Stone 先到先得）：**煤** = 2×2×5 长条（横轴 hash 二选一）+ 周围 6 邻壳 hash 贴连散块（25%）→ 均值 ~28 块/脉；**铁** = 2×2×2 立方 + 8 角斜对贴块（35%）→ 裸立方 bbox 恰 2×2×2；**金/钻/红石** = 8 步漂移游走 blob（60% 主轴漂移 + 失位转全向 6 邻，MC 1.0 size 8 口径）、**青金** 7 步（size 7）、**铜**（t308 项目矿种，MC 无 1.0 对应）8 步 blob。**高度带逐字沿用旧散点常量**（「以项目现有高度带为准对齐」：钻[5,40]/金[5,25]/青[5,31]/红[5,16]/铁[5,30]/铜[5,45]/煤[8,列上界随列高自适应]）；blob 竖向越带硬夹（深带零溢出）、铁角块夹带、铁立方抖动上界收一格。**频率经济不涨总矿量**：逐 seed 落旧散点 carve 后存活基线 ±13% 窗（P-t1014 基线 @5 seed×128×128×64：coal 7868/copper 5084/iron 2467/gold 1526/diamond 2106/lapis 2530/redstone 28393），矿脉化实测偏差 **-0.2%..-7.8% 全窗内**。**连接率**（「基本都连接起来」）：26 邻贴连口径 conn26 = 煤 98.9..99.5%/铁 95.3..96.5%/金 91.9..94.4%/钻 92.7..94.8%/青 91.3..94.1%/铜 91.4..92.6%/红石 98.4..98.7%（铁角斜对块按角触并入主域；6 邻直连口径铁 ~73% 为角块天然形态，blob 长条自身全 6 连通）；断连来源 = carve 切穿 + 重叠带先到先得（窗 ≥85% 不卡 100%）。**登记（既有口径如实沿用，非本单引入）**：旧红石散点 `(r2>>24)%10000` 为 8 位截断 → 有效密度 40/256≈15.6%（≠当年代注释 0.4%）→ 红石散点总量被推高 ~40×；本单按「与现散点口径持平」字面沿用该有效密度校准脉频（深层红石近镶嵌毯状成片，hist41≤160 窗钉住），密度再平衡登记为后续玩法平衡任务。验收 **P-t1014 六腿**：(a) 煤脉形（域数 300..470【散点基线 ~7500 孤块】/中带域 bbox 中位长轴 [5,8] 短轴 [3,4]/conn26≥85/带≥99% 严带+绝对[7,60] 矿井巷壁豁免）(b) 铁脉形（中带域 bbox 恰 2×2×2/裸立方 150..215/conn26≥90/带≥99%）(c) 金/红石（尺寸窗+严带+红石合板尾 hist41≤160）(d) 钻/青/铜（尺寸窗+严带+细长 bbox 中位长 [4,8] 短 ≤2）(e) 经济 ±13% 窗逐 seed + 同 seed 复跑 7 矿总量全等（纯 hashVoxel(seed⊕矿盐)，无运行期随机源）(f) 源码钉（kVeinProfile 表/煤横轴选型行/铁立方角分支/blob 失位重定向/逐块石门——阴性敏感）。**阴性轮：git stash revert world.cpp → 恰 (a)(b)(c)(d)(f) 五腿红**（散点签名实锤：coal comps 7520..7743 vs 脉化 356..399、conn26 1..2% vs 98..99%、mid[-1,-1] 无中带域；**经济腿 (e) 按设计保持绿 = 总量判据与形态判据分立**），其余 440 全绿（t882/t927 钓竿族另一次偶红复跑清，登记口径内 mob 游走 flake 非本批回归面）；restore → 445/0（复跑同 445/0）。**待实机确认（视觉类）：洞穴/峡谷壁裸露矿脉观感、深层红石毯观感（登记待平衡）。**

### 🅳 玩法修缮（t1015-t1019）
**t1015** 载具攻击目标甄别：生物坐船时点生物打到生物、点船打到船，单次攻击单目标不双计（命中射线最近 AABB 甄别 + 骑乘组合碰撞盒拆分）。
  → **✅✅ 落地（2026-09-05）：骑乘组合 AABB 拆分甄别；矩阵 445→446（P-t1015 五腿净增）。** 根因 = t866① 改判「乘员命中即重路由 hit*FromRay 最近**任意**车/船」，别的载具挡在乘员身后沿同一上身射线会被误拆（点生物打到别的船）。修法 = Boat/MinecartManager 增 `rayHitDistAt`(指定槽单盒求交) + `hitBoatAt/hitCartAt`(指定结算，寻的/结算分离、结算链逐行同源)；beginMining 骑乘分支把组合拆成「乘员盒 vs 本乘员所乘那一台载具盒」双盒求交取**射线最近**者定唯一目标：载具更近→拆载具（乘员不掉血，末击毁→对账链自动放乘员），乘员更近/载具未中→打乘员本体（review26 #4 保持）；两分支互斥 return = 不双计；甄别射线长 m_hitDist。验收 P-t1015 五腿：(a)瞄乘员上半→乘员掉血船无恙 (b)双盒同射线→船拆乘员满血 (c)**decoy 阴性腿**：另一船挡乘员身后→只伤乘员两船全活（旧改判必拆 decoy）(d1/d2)矿车同族两面 (e)源码钉 rayHitDistAt/hit*At。**待实机确认（观感类）：骑乘矿车/船时点乘员上身与点车身/船身的甄别手感。**
**t1016** 世界时间持久化：存退重进保留退出时刻（WorldStore 增存 day/time 字段，load 恢复；连带 weather/天数）。
  → **✅✅ 落地（2026-09-05）：world_meta 增 clock_phase/clock_day/weather 三键同事务原子落盘；矩阵 446→447（P-t1016 四腿净增）。** saveAll 增第 5 参 `worldTime{phase,day,weather}`（'g'9 float 短往返 / qint64 / 枚举 int；空 map 不写不删=老 caller 兼容）；`loadWorldTime` **逐键缺省**（旧档缺字段 → phase 0/day 0/weather Clear = 新世界首帧，不炸不跳）；`WorldClock.restoreTime` = applyTime 直调（**无** setPhase day+1 副作用，月相=day%8 随 day 复原）；`World.setWeatherState`（设态+该态时长窗重抽剩余时长；非法静默拒；同态零 emit）；Main.qml 退出链传快照 + enterWorld 恢复（QML 编排裸原语，World 不向上依赖时钟）。验收 P-t1016 四腿：(a)真 SQLite 存→关→开→读三键逐值相等 (b)旧四参档缺键→缺省 (c)restoreTime 精确复原+weather 设态/零噪声/非法拒 (d)源码钉 QML 两接线+两 C++ 入口。**阴性轮：saveAll 跳写三键 → 恰存取回路腿红（diag a=0），其余 447 全绿；restore→448/0。待实机确认（观感类）：存退重进的昼夜观感连续性。**
**t1017** 仙人掌不可附着火把：放置预检拒绝仙人掌作火把支撑（连带核对红石火把/按钮等附着族同口径）。
  → **✅✅ 落地（2026-09-05）：torchSupportBlock 单一权威一处排除 Cactus（ShapeFull+isCollidable 双真 = 旧公式恒放行缺口根因）；矩阵 447→448（P-t1017 四腿净增）。** 火把/红石火把放置预检、破块保留复检、World 清格复检三消费端同源收紧（旧档残留按既有失撑掉落链脱落）；木梯/机关预检显式排 Cactus 同口径（附着族支撑黑名单一致）；石头等常规支撑零影响。验收 P-t1017 四腿：(a)谓词 Cactus 假/Stone·Sand 真 (b)真瞄准链 2 高柱**五可瞄面（顶+四侧）×3 kind 全 15 组合拒**（底面目标格被沙/下柱占位拒+天花板规则由 (c) 腿钉 = 六面口径全覆盖）+ 石方块同 15 组合全准 (c)悬空石底面拒=既有共享规则 (d)源码钉三处排除行。**阴性轮：摘除 torchSupportBlock 排除行 → diag a=0 b=0 c=1 d=0（火把真被放上仙人掌顶），其余 447 全绿；restore→448/0。待实机确认（观感类）：仙人掌旁无任何可附着观感自然。**
**t1018** 铁轨延伸松弛（t983 回炉）：沿轨线端点延伸铺设时，新轨自动接续既有轨（连接由邻轨端点拓扑决定，放置朝向无关）；平行侧邻拒连禁令**不回退**（t983 主口径保住）。验收：rig——轨线端点任意朝向放置→接续成线；平行侧邻→仍拒连。
  → **✅✅ 落地（2026-09-06）：端点拓扑松弛 = `railProbeEndpointAligned` 单表判据 + railConnections 规则②③同口径松弛；矩阵 448→450（P-t1018/P-t1019 各一腿净增）。** 修 = **仅 c==0（bit5 纯放置面向）轨**：偏好轴零臂时垂直轴「端点相对」邻（邻轨轴含连接方向：连接位优先 · bit5 兜底 · 坡臂恒真）照连——连接由邻轨端点拓扑决定、放置朝向无关（用户「沿轨线端点延伸铺设接不上线」根因 = 面向轴与线轴相反时偏好轴零臂落 0 连接，轨线断在延伸点）；平行侧邻（邻轴垂直连接方向）仍拒连 = t983 主口径不回退；c!=0 轨既有连接即实拓扑永不松。连带 World 写回单轴镜像扩展（单臂连接 bit5 随实连轴翻转，孤轨形态 = 最后实连轴）。验收 P-t1018 四腿：(a) NS 线端点双向延伸（EW 面向〔错误轴〕放置→接续 + bit5 镜像清零 + 线端轨 Pz|Nz 闭合；NS 面向→照常接续）= 朝向无关 (b) 1 格断桥补接（普通轨 + 金轨三轨族同布局）EW 面向补接→桥 Pz|Nz 贯穿 + 断端单臂闭合（金轨断端 = 规则② fresh 级联单臂，run1 首跑修正探针预期后钉住）(c) 平行侧邻拒连不回退（t983 腿 a 同构复钉：双 0 连接 + bit5 守恒）(d) 源码钉四处。**阴性轮：`railProbeEndpointAligned` 摘除（恒 false）→ 恰 t1018 红且唯一红**（diag：EW 面向延伸轨 conN1=0 + ewKept=true = 断在延伸点原诉复现、普通/金轨断桥双 0 连接；腿 (c)(d) 保绿 = 拒连与钉不受扰），restore → 450/0。P-t960 本批一次偶红（dBMax 2.24>2.2 紧界 + 猪游走噪声，登记口径 flaky）复跑清。**待实机确认（交互类）：沿轨线端点任意朝向铺设当下接续成线；平行侧放两轨仍互不吸附。**
**t1019** V 谷通过物理（t981 口径再翻案）：谷底捕获改**能量判据**——进谷速度足以爬升对面坡（v² ≥ 2g·h 对面坡升）则通过；不足才谷心制动停驻；消灭「打转」极限环（rig 采位置/速度曲线断言通过或单调减速二选一，无往复）。
  → **✅✅ 落地（2026-09-06）：谷底捕获闸改能量判据（t981 速度阈再翻案；矩阵数随 t1018 同批落账 448→450）。** 修 = 逃逸判据 `|speed|≤kCartValleyEscape(10)` → **v² ≥ 2·kCartSlopeGravity·(h_对面坡升 + 0.05 余量) → 放行通过**（g 用项目坡道运动学常量 28×sin45°，与谷内加速/上坡减速同一常量 = 能量账自洽；h_对面坡升 = 沿行进向逐格 +1 连续爬升段总高、按行进向扫描、阶梯壁逐格足额〔扫描上限 16 格〕）；不足 → t981 谷心制动停驻（速度律不变）；0.05 余量盖静置闸 kick（+1.0）贴阈再供能（crest-stall 回谷循环带收口）。速度阈缺陷登记：谷物理近保守，等高壁顶滑落谷底到达能量恰 = 爬出对面壁所需，速度阈只能一刀切（≤10 全捕 = 高能重力车被谷吸住；>10 全放 = 贴阈不足车爬壁失败打转），能量判据逐例二选一。验收 P-t1019 三腿（位置/速度曲线、通过或单调减速两速档无往复）：(a) 低速档 3 格 V（入谷速 ~4.6、v²~21 < 阈 ~41.6）→ **入谷后 |v| 单调不增 + 到谷心距离单调不增（从未越心）+ x 向反转 == 0 + 终位谷心 ±0.06 + 60 tick 静止守卫** (b) 高速档 8 格通电动力平台直通 V 谷（boost ~11.6 入谷、v²~135 > 阈）→ **x 单调不减 + 反转 0 + 越谷心东 ≥0.9**（谷没留住）(c) 源码钉四处（能量放行行/通过阈/对面坡升扫描/扫描上限常量）。**阴性轮：能量判据行换回 t981 逃逸阈 → 恰 t1019 红且唯一红**（两行为腿在两闸下恰同判 = 探针档位闸不变，红由能量判据源码钉承载，diag `a true b true | c false true true true`），restore → 450/0。**待实机确认（物理类）：中速矿车入 V 谷不再「打转」（谷心一次停驻或直接通过对面坡）。**

### 🅴 参谋清单落实（t1020-t1023，末地维度已排除）
**t1020** 成就树扩展：采矿/战斗/探索/生活分支（首杀各生物、首附魔、骑矿车、进入五结构〔要塞已有〕、钓鱼首获、矿脉首矿等），挂 achievementDefs 表+钩子，含图标与 toast。
  → **✅✅ 落地（2026-09-06）：成就 17→31 条（矩阵 451→453，+P-t1020 两腿：结构区域重推导 + 成就树扩展行为/钩子/源码钉）。** 实现面：①achievementDefs 四分支 14 新条——采矿支 3（「煤炭!」挂挖矿时间到 /「铁矿!」挂获得升级 /「红石!」挂钻石!，同镐位阶父链）+ 战斗支 4（首杀蜘蛛/骸骨/潜行者/银鱼挂怪物猎人下，洞穴蜘蛛计入蜘蛛系不设独立条；首杀同事件父先行同 tick 双解锁）+ 探索支 4（「地牢探秘/废矿来客/沙漠寻踪/丛林秘境」各**独立根**，同「隔墙有眼」t1000 先例不挂线避免父前置吞沿）+ 生活支 3（「轨道骑士」骑矿车 /「愿者上钩」钓鱼首获 /「移动金库」开箱车，各独立根）。②World::StructureKind 枚举（0-3+Count，尾追加契约）+ StructureSite 候选上收（placeDungeons/placeMineshaft/placeDesertTemple/placeJungleTemple 选择段**逐字迁移** sites() 单源：概率/抖动/margin/海列/群系/高度守卫同值同序；place* 几何与区域重推导共同消费 → 同 seed 同候选结构性保证）+ rebuildStructureRegions（generate/finishLoad 末从 seed 纯算术重推导落表，**零新增序列化、零体素扫描**，旧存档加载即判，B5 重推导口径）+ insideStructureRegion(kind) 区域表线性扫描谓词（cell 闭区间含墙环，O(区域数) 纯算术）+ structureRegionCount/structureRegion getter（探针/F3 口径）。③PlayerController tick 四结构 false→true 上升沿守卫 m_insideStructure[]（setWorld/finishWorldLoad 全清，同 m_insideStronghold 口径——旧世界陈旧 true 不吞新世界首沿）→ structureEntered(kind) 一次性事件信号（禁每帧直发）。④PlayerProgress onStructureEntered(kind 数值契约，越界防御)/onRodeMinecart/onFishCaught/onChestCartOpened 幂等埋点 + onMobKilled 首杀四生物 + onItemPicked 首煤铁红石（父前置门控 = 同镐位阶自然序列）。⑤Main.qml 路由：Connections onStructureEntered/onFishCaught 直调 Q_INVOKABLE（t976/t977 AOT 教训）+ onRidingCartChanged false→true 边沿 + openChest isCartCell 分支。**旧 exe 三红腿根因（均前两棒 WIP 中途态，终态已修）**：48 高 rig 世界地表带 ≥38 → 神殿 siteOk 塔顶越界守卫恒拒（seeds des/jun = -1）→ rig 升 96 高统一四世界；沙漠锚点误取金字塔中心格（风玫瑰 WoolBlue 覆盖，t1003 钉）→ 改层 0 实心盘边缘 dx=+7 菱域外；探针祖先链未铺 → unlock 父前置吞首杀/首矿（toastB 卡 6）→ 先补 open_inventory→get_wood→crafting_table→sword_time 四祖先同真实生存序。**验收 P-t1020**：(A) 96×96×48 逐种子 1..80 扫四结构独立记首中（地牢加验刷怪笼在位/沙漠金字塔盘缘砂岩/丛林苔石混排地板 = 足迹-体素同源；bounds 闭区间采样含角格与 XZ/y 界外）+ 真 WorldStore 存→beginLoad/loadChunks/finishLoad 重推导区域表四 kind 逐项相等 + insideDungeon 判读一致（种子实锤 1/1/1/1）；(B) 真 PlayerController+真地牢世界行为沿：足迹外 5 tick 零信号→走进恰 1 沿 1 toast→界内 9 tick 不重发→重复进出再沿但 toast 仍 1→finishWorldLoad 重产沿无新 toast；钩子腿：首杀四生物 toast 台账 6/7/8/9、首煤铁红石父门控（未铺镐线先吞后放 9→11→13→15）、骑矿车/钓鱼/箱车/kind 映射 1..3 → 21、幂等+越界 kind 防御不重 toast、achievements() 恰 31 条+形状校验（parentId/名/图标）、loadVariant 回放重进沿重发而 toast 0；(c) 源码钉 15 处（沿守卫/读档重置/信号声明/QML 三路由/定义行/枚举/足迹常量/rebuild/kDesertBiomeGuarantee 存活）。**阴性轮×2（各恰一红 = P-t1020 行为腿，其余 452 全绿）**：A 注释 emit structureEntered(k)（沿守卫行保留 → 源码钉保持绿）→ diag `entered 0 toast 0`；restore → `entered 3 toast 1` 全沿序恢复。B 注释 unlock("kill_spider") → diag `toastB 20 shape false`（21−1 恰差蜘蛛条；同轮 `entered 3 toast 1` 兼证 A 恢复→绿）；restore → 终局轮全绿。**flaky 登记**：t960（已登记偶红族）当日 5 轮 3 红频率偏高，同签名 `arrow false`（dBMax 2.254/2.279/2.257、dSMin 9.80/9.84/9.08 浮点裕量微漂），弓箭时序裕量贴边与本批零交集（t1020 双腿 5 轮全绿），留档观察不追改。**待实机确认清单：①新世界进入地牢/矿井/沙漠神殿/丛林神殿各弹对应探索 toast（老存档读入后同样可判）；②骑矿车/钓起首鱼/开箱子矿车各弹生活 toast；③首次击杀蜘蛛/骸骨/潜行者/银鱼各弹首杀 toast（怪物猎人 toast 同帧先弹、被后者覆盖为正常单槽观感）；④首次拾取煤/铁矿/红石各弹首矿 toast；⑤成就页树形 31 条、新条目图标与父链正确。**
**t1021** 音效层：结构环境音（要塞低鸣/矿井滴水/丛林虫鸣/沙漠夜风）+ 事件音补缺，走既有 Audio 链（Audio/ 层）。
  → **✅✅ 落地（2026-09-06）：四结构环境音 + 事件音补缺三音走既有 Audio 链（矩阵 453→455，+P-t1021 两腿：region 门控行为 + 资产格式/全链源码钉）。** 实现面：①资产（tools/build_sounds.py 程序合成七音，纯 stdlib 确定性 + 既有 finalize DC 阻隔/峰值归一管线；build_sounds.py 另起 `--only` 子集重生成 + soundfile/scipy 懒加载进 load_cc0——无 heavy deps 机器可重生成全部合成音，全量重生成路径行为不变）——stronghold_hum/desert_night_wind 各 8s 无缝循环（首末 80ms 淡化；低鸣 = 52/52.7Hz 双失谐拍频 + 低五度 78.2Hz + 次声 36.5Hz + 两级低通噪声空气床 + 双慢 LFO；夜风 = 三级低通中低频「呜咽」风体 + 深慢双 LFO 谷值钳近静；同 t366 教训循环长音禁宽带高通层）、mineshaft_drip 0.55s 单滴（1900→850Hz 指数下扫 plink + 两级巷道回声）、jungle_chirps 1.1s 蟋蟀式颤音簇（4300-5300Hz 载波 × 60-90Hz AM 不规则簇）、achievement 0.85s 三音上行钟琴（E5-G#5-B5）、chest_open 0.34s 闩扣+木轴吱呀、chest_close 0.24s 合盖闷响+闩扣。②AudioManager：两循环音按 startWaterFlow/startAmbient 先例（looping + 幂等 start/stop + setVolume 即时同步 + maxFrames 16s 长 clip 教训）；两单发音（滴水/虫鸣）登记轻量内部 **QTimer 单发重臂调度**（首滴 0.6-2.1s → 后续 1.4-4.8s；虫鸣昼 5.5-13.0s 稀 / 夜 1.6-5.0s 密由 startJungleChirps(dense) 分流 + 每次播音量随机抖动；同 t250 mob ambientTimer 随机周期先例，stop 后 timeout 早退不重臂）；事件音三枚 replay 单件（playAchievement 0.9 / playChestOpen、playChestClose 0.85）。③PlayerController::structureAmbientZone（Q_PROPERTY int，public 枚举 StructureAmbientZone：0=无/1=要塞低鸣/2=矿井滴水/3=沙漠夜风/4=虫鸣昼稀/5=虫鸣夜密）：tickImpl env 桶**复用 t1020 同一 loop 的四结构在区布尔（零额外谓词调用）**折叠，优先级先折叠先得（要塞 > 矿井 > 沙漠 > 丛林；地牢恒 0 = 四音不覆盖地牢登记口径）+ 复合门（沙漠夜风 = 夜 worldClock.isNight + 露天 skyLightAt≥15 见天口径——白天/密室无风；丛林昼夜皆响切 4/5 值、密度由播放端间隔承担）；值真变才 emit（界内连 tick 零重发禁每帧抖 QML）；setWorld/finishWorldLoad **静默清 0**（不清则存档停区内重进 zone 不变不发 → 环境音永不恢复；QML 退出显式全停兜底、重进首 tick 值变即重发恢复）。④Main.qml：zone 分流 Connections（先全停再启对应音 = 单音床互斥；AudioManager 幂等兜底）+ 退出世界/回主菜单两路径显式全停四音 + onAchievementUnlocked → playAchievement（结构进入等成就 toast 同源共享本音 = 「结构进入提示音」随之覆盖，toast 单一通道不另造重复音）+ openChest/closeChest → playChestOpen/playChestClose（方块箱与箱子矿车同源单一通道，箱车开箱同响 = 机制等价 MC storage minecart）。**验收 P-t1021**：(A) 门控行为（96³ 单世界逐种子 1..80 扫四 region 表〔一份 worldgen 同填四表，t1020 四世界分扫省时变体，种子实锤 1/1/1/1〕+ 96×96×48 要塞世界 t1000 同款扫 hasStronghold；真 PlayerController Spectator noclip 定点 + 真 WorldClock setPhase 钉昼/夜）：足迹外→0、要塞内→1（界内 6 tick 零重发）离开→0、矿井 bbox 中心→2 昼夜无关→足迹外→0、沙漠 bbox 角列自下而上找首个见天格（确定性）：昼 0 / 夜 3、地下角格（skyLight<15 先钉）夜仍 0（夜探密室无风）、丛林内昼 4 / 夜 5、仅地牢格恒 0；足迹外采样点五谓词全外（防重叠结构串扰）。(B) 资产+全链钉：七 wav chunk 遍历解析（不假设 44 字节头）RIFF/WAVE + PCM s16 mono 44100 + data 非空 + 时长带（循环 ≥6s / 单发 ≤2.5s）+ qrc 七条目 + 源码钉 34 处（zone 属性/值域枚举/折叠 emit 门/复合门/双置零、QML 八路由、AudioManager 七 Q_INVOKABLE、CMake 七登记、build_sounds.py 七生成器）。**阴性轮（双病灶同轮、各腿各由其专属病灶染红 = 恰 2 红其余 453 全绿）**：A structureAmbientZone() 读路径短路恒 0（源码钉零扰）→ 门控腿红；B stronghold_hum.wav 运行时移除 → 资产腿红 diag `open-fail`；双 restore → 终局轮 455/0（零警告编译）。**待实机确认（听感类）：①要塞内持续低鸣循环、离开即停；②矿井内随机间隔单滴（首滴快、后续 1.4-4.8s 不规则）；③丛林神殿内虫鸣昼夜都响、夜晚明显更密；④沙漠神殿顶冠/坡面夜间起夜风（白天与地下密室无风）；⑤成就解锁 toast 上行钟琴（含进结构 toast 同音）；⑥开/关箱子与开箱子矿车有开合声；⑦四环境音均为背景级不压前景音效；⑧master 音量滑条对四环境音即时生效。**
**t1022** 键位重映射 + 设置页扩展：settings.json 持久化链上加键位映射表（UI 设置页编辑）；手柄支持调研（Qt6 无 QGamepad，可行性结论或登记）。
  → **✅✅ 落地（2026-09-06）：13 动作键位映射表上 settings.json 持久化链（矩阵 455→457，+P-t1022 两腿：映射表/持久化/冲突面板 + 引擎生效链面板）。** 实现面：①**KeybindManager**（src/Core/keybindmanager.{h,cpp}，Core 叶子，QML_NAMED_ELEMENT 门面同 ResourcePackManager 模式）：静态 actionTable **单一权威** 13 动作（forward=W back=S left=A right=D jump=Space sneak=Shift inventory=E drop=Q chat=T camera=F5 debugTime=F6 modeCycle=G debugOverlay=F3；canonical=引擎消费键=默认键；id=持久化键+QML 设置页 model+探针断言面三方同源）+ 正向 keyFor（QML keyInput 七 UI 动作分支比较 + 设置页行显示）+ 反向 canonicalKey（物理键→归属动作 canonical；无归属原样透传 = 未重绑旧键语义保留，如 forward 改 ↑ 后 W 仍前进）+ actionOfKey（冲突占用方查询）+ applyBinding（一键多动作冲突拒收 ApplyConflict / 未知动作与非法键（<0x20、Key_unknown）拒收 / 同值幂等零副作用不抖 revision）+ resetDefaults（全回默认+落盘）+ keyDisplayName（QKeySequence NativeText）+ revision 版本号（QML 绑定可见依赖，t976 AOT 教训）。固定不可映射键登记口径：Esc（指针逃生口，重绑软锁风险）/数字 1-9（hotbar）/Enter 别名/F3 组合弦 B、G/Ctrl 整栈修饰/鼠标键（攻击放置非键盘域）。②**持久化**：settings.json "keyBindings" 节（{动作: Qt::Key 整数}）；读候选链与 resourcepackmanager resolveSettingsPath 同序（exe/../ → AppLocalData）+ **缺节/缺行/坏值逐行落默认**（旧档向后兼容不炸）+ 未知动作名忽略（前向兼容）；写=读现有文件保留其它字段（playerSkin 等）仅覆盖本节（writeSettings 管线同款），写失败降级内存态告警不崩；setStorePath 显式路径测试密闭（生产不调用）。③**引擎侧**：PlayerController::setKey 入口单 choke 点 `key = m_keybinds->canonicalKey(key)`——全部 m_keys 消费点（step 移动 / 跳跃 / 蹲疾跑状态机 / 双击空格切飞）零改动即受映射（重映射只在入口翻译成 canonical），release 侧对称翻译进出同键不残留；null 注入不翻译=旧行为（探针旧装配安全降级）；Q_PROPERTY keybinds peer 注入（同 worldClock 模式，Game→Core 向下依赖合规）。④**Main.qml**：keyInput 七 UI 动作分支（chat/inventory/drop/camera/debugTime/modeCycle/debugOverlay）改 keybindsMgr.keyFor(action) 查询（死亡态 chat 放行键同步走映射表；运动六键照旧透传物理键经引擎 choke 点翻译）；设置页键位组：13 行 Grid + 点击录制捕获（keyRecorderT22 独占键盘，Esc 取消；完成/取消/面板关闭三路径归还 keyInput 焦点防游戏键失聪）+ 冲突拒收提示（占用方中文名）+ 恢复默认键位按钮 + 行显示 revision 触碰绑定；设置面板内容列改 Flickable+DarkScrollBar（键位组加入后内容超高，820 硬塞到极限）。**验收 P-t1022**：(A) 映射表面板：13 动作默认表逐项钉死（id/canonical/两两不冲突不变式，重排漏行改键即暴露）+ 旧档缺 keyBindings 节全默认 + 重映射 forward→Up 真落盘（playerSkin 保留）+ **新实例重载映射保持**（canonicalKey ↑→W 反向规范化、canonical 自身恒等）+ back 认领冲突键拒收映射不变 + W 让出后可被认领（canonicalKey W→S）+ 同值幂等 + 未知动作/键值 0 拒收 + resetDefaults 落盘改写（Key_Up 十进制值从盘消失）+ 无归属键透传复位 + 部分节/坏值行落默认（jump=Key_J 生效、sneak=5 非法落 Shift、其余不串）+ keyDisplayName "W"。(B) 引擎生效链面板（真 PlayerController×KeybindManager 注入 + 模拟按键事件）：默认 Space 生存跳离地又落地 + 重映射 jump→C 按 C 真跳 + sneak→X 进 Crouch 松开回 Walk + forward→Up 按 ↑ 前进且让出的 W 透传仍前进 + null 注入对照原始键直入（旧行为面）+ 源码钉 17 处（QML 七动作 keyFor 路由/录制器/冲突文案/恢复默认/revision 触碰守卫/焦点归还/Q_PROPERTY 注入/setKey choke 行/CMake app+test 双目标）+ **密闭哨兵**（工程根真实 settings.json 面板前后逐字节不变——本单实现头注释明令禁止写穿真实档）。**探针两处首跑假红（均探针侧，实现零改）**：①无参 pc.tick() 连调 dt≈0（tickImpl dt=m_clock.restart() 墙钟差）→ 物理/重力零推进全腿假红——tick 前 busy-wait ≥17ms 喂墙钟（t889 pumpFor 同款）即愈；②yaw -90 前向 = (-sin,-cos) = **+X 非 +Z**，(iv)(v) z 位移断言恒假——yaw 改 180 对齐断言轴（wishHoriz 朝向公式锚）。**阴性轮：摘 PlayerController::setKey 的 canonicalKey choke 行（注释保留源码钉子串=档位闸不变）→ t1022 面板 B 红且为唯一非 flaky 红**（重映射腿全灭：ii C 不跳 / iii X 不蹲 / iv ↑ 不走；i 默认 Space 恒等、iv-legacy W 透传、v null 对照全保绿 = 旧路径不受扰；同轮 t960 偶红属登记 flaky 族），restore → 复跑 **457/0**（同轮 t882 偶红复跑清，均登记 flaky 族与本批零交集）。**手柄调研结论（登记不引入，零新依赖红线）**：Qt 6 全系无 QGamepad（QtGamepad 从未移植进 Qt 6；2025-05 qt-project 开发邮件列表仍在讨论是否复活、无官方时间表）。候选路径盘点：①SDL2/SDL3 gamepad API——行业标配但引入新第三方依赖，踩本批红线；②社区移植 qtgamepadlegacy / Arno Rehn port——非官方、维护与 Qt 版本跟进风险高；③Windows 原生 XInput / Raw Input 平台层——零依赖但需自写平台层与「按钮/轴→动作」映射。**登记**：未来若做手柄，KeybindManager actionTable 的「动作→输入」单一权威抽象天然是落点——加一张「设备轴/按钮→动作」第二映射表即可，引擎侧（canonical 消费）仍零改动；当前批不引入任何依赖。**待实机确认（交互类）：①设置页点击任一键位框→按新键→行显示更新且立即生效（前进改 ↑ 后 ↑ 与 W 都能前进）；②冲突绑定（把后退绑到已被占用的键）弹占用方中文名提示且映射不变；③「恢复默认键位」一键全回 WASD 默认并落盘；④录制中关面板（Esc/返回/回主菜单）后游戏键不聋（焦点归还）；⑤改键后重启 exe 映射保持；⑥旧存档/无 keyBindings 节的 settings.json 读入全默认不炸。**
**t1023** 性能批三（弹性大项，调研+首批）：meshing 线程池核实（t906 登记：`threads: 0/0 sync meshing` 是否仍同步）、draw call 折叠、平滑光照/光照增量调研；产出调研报告 + 首批可落地项。
  → **✅✅ 落地（2026-09-06）：三调研收口（docs/perf-batch3-research-2026-09.md）+ 首批两落地（动画门控 4 条 + AO 单项默认关；矩阵 457→460，+P-t1023 三腿）。** 调研结论（如实登记不硬做项与理由同报告）：①**meshing 线程核实（t906 回标）= 仍全程 GUI 线程同步**——src/ 全树 7 种线程原语 0 hit（本批复核），F3 行 `threads: 0/0 (sync meshing)` 是 t906 钉死的事实字符串非活测量（从未有过 worker，非「退化/未启用」）；残余成本面 = 编辑/载入当帧 buildMesh 串行（survey 高水位 mesh 桶 116–427ms），已缓解面（t472 视距门控/t188/t489/sun 量化门/t860/t972）为真实边界；改造面五硬前置（World 零锁数据竞争 / 「破块 <1 帧」编辑语义重定义 / QQuick3DGeometry 线程规则无明文 / 换世界 vs in-flight job / 量级 4–10 周）→ 登记独立任务路线（C3 halo 快照底座 → C1 worker 化 → SIMD①），本批不硬做。②**draw call 折叠（t1007 治理路径）**：a 掉落物 instancing = 大工程（三族异构分桶 / slot-reuse 契约链 / billboard alpha 红线 / F3+B 调试面数据源），登记周级分步（方块段→工具几何→billboard）+ 低风险过渡微步（per-delegate 资源共享），不硬做；b 隐藏 delegate 动画 `running: visible` 门控 + c mob 族同向 = **首批落地**。③**平滑光照调研**：全量版（角点 4 格光场平均）改造面大（光采样 ~4×走逐格 ChunkManager 路由 / greedy 合并键不含角点光→合并率大跌 / 视觉全场景回归同 t183 敏感度）→ 登记与纹理数组（自研 RHI）同批评估（共享顶点格式扩字段改造点，survey B1 已预留 ao[4] 位）；**AO 接触阴影子集 = 首批落地**（不动光场语义的最小单项）。**首批落地**：①QML 永停表门控 4 条——item delegate rotY/bobY `running: entRoot.visible`、endereye/enderpearl spin `running: <node>.visible`（QML `Animation on` visible:false 不停表，t561 同源；200 槽×2 + 47 槽×2 = 最坏 ~494 条无限动画烧 GUI 帧 → 只剩可见活体；t696 established 模式，槽复用重显 from 重启 = 新实体语义）。②AO 单项（`VoxelLight::kAoFactor {1.0,0.8,0.6,0.5}` + `aoCornerFactor` 双侧同遮钳 3 曲线单点定义；`ChunkGeometry.aoEnabled` **默认 false 出厂关** + setAoEnabled Dirty 重建 + t472 视距门控，同 setGreedyMeshing 形；地形段逐格 culled 路径角点三探针——遮挡判据 = `occludesNeighborFace` 与邻面剔除同谓词单一权威，探针走 blockAtWorld 跨 chunk 路由；AO 乘在光场钳制**后**允许低过 kVcMin 地板；aoEnabled=false 因子恒 1.0 零开销旁路顶点色与 t1023 前逐字节一致；范围钉死 greedy 合并键 / 流体水面 / 异形 cross 段均不采样）。③Main.qml `window.aoEnabled` + terrainGeo 绑定 + ESC 视频设置开关行（同 t166b 阴影开关 Row 自绘模式；无 settings 持久化，同 shadowsEnabled 口径）。**验收 P-t1023 三腿**：(a) 门控源码钉（Main.qml 注释滤除后 4 条门控文本全体在场，任何一处删/改绑即红，t860 先例）；(b) AO 行为级（ChunkGeometry 直驱 t860 先例 + vertexData 直读 t965 先例；rig 清场 5×5×7 + 3×3 石台 @42 + L 形双墙 @43：AO 关 = 中心顶面四角同色平坦基线，开 = 四角精确 {0.5, 0.8, 0.8, 1.0}×flat（乘在光场钳制后），关回 = 顶点缓冲逐字节复原；顶点按「**4 连续顶点四边形**」隔离——culled 每面 4 角点连续 append 且单位方格四角仅中心面唯一覆盖，与邻块/树冠共角顶面天然隔离）；(c) meshing 事实钉（src/ 递归扫 QThreadPool/QThread/QtConcurrent/QFuture/moveToThread/std::thread/std::async 7 原语 = 0 且 F3 事实行在场——两事实互锁，线程化立项必须同改防 F3 谎报）。**探针首跑假红（探针侧，实现零改，t1022 同型教训）**：角点色断言误取 Vtx float[5]（nz）当 ny → 滤出的是 +Z 侧面顶点（(1,1) 角 n=0 假红 + 三角假值 0.5/0.6/0.66）；且「按坐标收集全部共角顶点断言同值」在树冠遮天天光梯度下结构性不成立（共角邻面块光场不同）→ 四边形隔离修复后 460/0（诊断走临时独立工具直驱 World+ChunkGeometry 三探针打印，已移除不入库）。**阴性轮（双病灶同轮，t1021/t1022 先例）**：A AO 乘法旁路强制 1.0（`false && m_aoEnabled`，lambda 保引用零警告）→ t1023b 红；B rotY 行 `running: entRoot.visible` 摘除 → t1023a 红；同轮恰此 2 红其余全绿（t1023c 事实钉不受扰；t882 偶红属登记 flaky 族——探针修复轮同代码全绿，与本批零交集）；双 restore → 复跑 **460/0**。**应用构建**：qmlcachegen Main_qml.cpp 重生成零错 + offscreen 冒烟 `root objects after load: 1` 不秒退（进程 30s+ 存活零 QML 错误）。**待实机确认清单（用户在新 exe 上）：①动画门控观感回归：掉落物落地自转/浮动如常、拾取瞬间无残留转动、末影之眼/珍珠投掷自旋如常；②AO 目视：ESC 视频设置开「环境光遮蔽」→ 墙根/拐角/树干底部柔和暗角、关 → 与旧观感逐像素一致，同场景 F3 mesh 耗时开/关对比，确认后另单翻出厂默认；③draw call 实测：F3 render-side 行 + drawCalls 在 items 93/93 高水位场景前后对比（本批省 GUI 动画 tick 不改 draw 结构，instancing 收益量化留待 a 立项时采）；④meshing `threads: 0/0 (sync meshing)` 行为与探针共存说明（线程化立项须同步改 F3 行 + P-t1023c）。**

### 📎 R19.20 范围与顺序
t1005-t1023（19 项）。**建议顺序：回归清算 t1005→t1006→t1007（用户最痛三件，复现优先）→ t1010 神殿验证 → t1011-t1014 矿井+矿脉 → t1015-t1019 玩法 → t1008-t1009 生物 → t1020-t1023 参谋落实**。每项独立 commit + dev-plan ✅✅；全部完成后只读 review + 统计报告。矩阵基线 426。**测试跑法硬性口径沿用：一切测试/冒烟命令 offscreen 前缀（QT_QPA_PLATFORM=offscreen + 插件路径），动真实鼠标键盘先问用户。** t1005/t1006/t1007 属「实机复现类」，须用户在新 exe 口头确认后真正关单；t1006/t1007 的 rig 复现是本批最高纪律（R19.18 的教训：没复现过的修复不算修复）。

## review0907 只读 review 修复（A 组；2026-09-07 收口，第三棒接续前两棒 WIP）
矩阵 **463/1（中途态，exe 陈旧假红）→ 464 PASS / 0 FAIL**（全部三轮：正轮 / 阴轮 / restore 终轮，offscreen 全程）。逐项处置：
- **A-P1-1 箱子矿车战利品无限再生** → ✅ `fix(review0907): chest-cart loot one-shot looted flag`（298b55b，cheststore.h/.cpp + 探针腿 a56b11d）。根因：t1013 矿车键「全空条目豁免落盘」让「取空后条目」与「未开箱条目」落盘面不可区分 → 重开按同 seed 重 roll 同 6 件（会话内+跨存档）。修 = 独立持久 `m_lootedKeys` 并行标记面：roll 发生即置位、allChests `"looted":true` 落盘 / loadAll 读回 / clearChest·clearAll·loadAll 同清，gate 加 `&&!looted`（空条目回生语义不变，老档缺键 = 从未 roll）。
- **A-P1-2 煤脉 128 高世界 60 硬帽封顶** → ✅ 并入 `fix(review0907): coal vein column-adaptive ceiling + bit5 corner guard`（c4c99b4，world.cpp）。修 = kProfiles 煤 yMax=0 哨兵 → 上界纯列自适应 hc-5（对齐旧散点 stoneTop=h-3 口径；64 高 rig hc-5 ≤ 58 本就不触发帽 → (a)-(f) 零变）。**阴性轮回灌修探针**：阴轮一「还原硬帽 → t1014(g) 假绿」——帽顶 60 长条姿态上探恰 +1 越界 y=61（三 seed 实证 maxY=61），腿阈值 y>60 不敏感；收紧 y≥62（列自适应 s777 实测到 63）后阴轮二恰红（5 seed 全 0）→ restore 终轮恰绿（s777:3）。
- **A-P2-1 结构沿检测被要塞臂整 tick 吞** → ✅ `fix(review0907): unconditional structure edge detection`（e179e44，playercontroller.cpp）。根因：tickImpl 结构 region loop（t1020 沿 + t1021 音区折叠）整个挂在 `m_insideStronghold` else 臂上——要塞足迹与矿井/神殿 region 重叠处 structureEntered 边沿永不触发。修 = loop 无条件执行，要塞低鸣折叠移到 loop 后作纯最高优先级覆盖（只定音区值，不再吞沿；复合门/值变才 emit 均不变）。
- **A-P2-2 键位别名透传 + 黑名单缺席** → ✅ `fix(review0907): keybind alias drop + forbidden-key blacklist`（33cdf6c，keybindmanager.h/.cpp）。4a：canonicalKey 无归属由「原样透传」改返 `Key_unknown` 丢弃（改绑 forward W→C 后物理 W 经透传恒等继续驱动的隐藏别名），PlayerController::setKey 对称早退（press/release 同门不残留）；4b：isValidKeyValue 落实固定不可映射黑名单全列（Esc/Enter/数字 0-9/B/G/修饰键），applyBinding 幂等判定后置 `ApplyForbiddenKey` 拒收（modeCycle→G 同值重绑幂等不回归），load() 坏值落默认。
- **A-P3-1 bit5 单轴镜像拐角误翻** → ✅ 并入 c4c99b4（world.cpp recomputeRailConnections）。单臂镜像两分支加交叉轴零位守卫（X 臂要求无 Z 位 / Z 臂要求无 X 位）：拐角（X+Z 各一位）不再被误判单轴翻转偏好位（回归「拐角不算轴」注释口径），纯单轴照旧镜像。t1018 行为腿 (a)(b)(c) 实证零变；(d) okD4 钉同步新镜像行全文——**中途 463/1 唯一红即此钉文本失配 + exe 陈旧（钉修复 07:08 晚于 exe 06:56，第二棒死于重建前），非守卫改坏行为**。
- **登记项 A-P2-3 / A-P3-2** → **登记遗留不处置**（review0907 报告原文为准；B 组滤注释钉帮手 / t960 放宽 / t1023 弃钉改红归第二波）。
**阴性轮（四病灶同轮，t1021/t1022/t1023 多病灶同轮先例）**：①摘 `m_lootedKeys.insert` → 恰 P-t1013 红（diag `regenRejected=false lootedSaved=false`）；②kProfiles 煤 yMax 0→60 → 恰 P-t1014(g) 红（收紧后 5 seed y≥62 全 0）；③canonicalKey 复透传 + ④isValidKeyValue 复仅值域 → 恰 P-t1022 面板 A/B/C 三红（diag `aliasKilled=0`、`fbEsc=0 fb3=0 fb0=0`）；恰 5 红其余 459 全绿（t960 阴轮一偶红属登记 flaky 族，本批零交集）。restore（快照字节级一致 cmp 验证）→ 终轮 **464/0**。**应用构建 + offscreen 冒烟**：voxelsandbox 全量增量零错，`root objects after load: 1`（logs/voxelsandbox.log）35s 不秒退零 QML 错。**待实机确认清单（用户新 exe）：①矿车箱取空重开不再出同批战利品；②128 高世界高山地表附近可见裸露煤脉（>y60）；③要塞与矿井重叠区进出仍有进入提示音/成就；④改绑前进后旧键不再移动、Esc/数字键录不进键位框。**

## review0907 第二波：探针质量组（B 组；2026-09-07，只动 tools/redstone_matrix_test.cpp）
矩阵 **465 PASS / 0 FAIL**（464 基线 + t1019(b2) 临界腿独立 PASS 行；offscreen；另 464/0×5 连跑验证 t960 零偶红）。逐项处置：
- **B-P1-1 套件级滤注释钉帮手 + 9 组回灌迁移** → ✅ `test(review0907): suite-level comment-filtering pin helper + 8-group migration`。新增匿名空间帮手 `pinSet(path, {{id, needle},...})`（`SrcPin` 三参 ctor，minCount 缺省 1）：读文件 → **字符串感知剥 `//` 行注释与 `/* */` 块注释**（t989 stripQmlComments989 先例套件级抽版；`.txt/.py/.cmake` 走 `#` 行注释——CMake/Python 注释里的引号会把 C++ 式状态机带偏，首跑实测）→ needle 只在非注释文本 contains/count → 返回失配 id 列表落 diag。迁移组：t1013(f) 11 针 / t1014(f) 8 针 / t1015(e) 8 针 / t1016(d) 5 针 / t1017(d) 3 针 / t1020(c) 15 针 / t1021(B) 34 针 / t1022(B) 21 针（接线 18 + copy 3 分列）/ t1018(d) 6 针（B-P2-2 同族核查顺带）。**迁移前核对表**：同算法 Python 剥离器对全部 111 针逐条验证「剥注释后 count≥minCount」——111/111 全绿，即审查员担心的「needle 只匹配注释文本」的假绿在本批 8 组中 0 例（4 处注释文本钉是显式点名项，单独改锚见下）。**敏感度证明（代替阴性轮）**：Main.qml:2285 `audio.startMineshaftDrips()` 整行注释 → 跑矩阵恰 **FAIL:1**＝t1021 音效链组红、diag 精确指认 `qml-route-drips(x0<1)`，其余 464 组全绿（含全部迁移组）→ Edit 还原 `git show HEAD | cmp` 字节级一致 → 复跑绿。即「滤注释钉真能抓注释化回归」留档成立。
- **B-P1-2 t960 箭腿贴边阈值** → ✅ `dBMax < 2.2f` → 实测包络 `< 3.0f`。依据：套件历史 **13 次偶红 diag 全部 dBMax ∈ [2.202, 2.279]** 擦线超限（0.8s 量测窗叠猪游走随机位移，与击退位移同量级），dSMin 同期 9.06..9.84 从未威胁 5.2 下限；审查建议的相对判据 `dSMin > 4*dBMax` 被**同批数据否决**（最差组合 9.06 < 4×2.28=9.12 仍红）。3.0 = 观测最大 +31% 余量；判别力承载在间隔断言 `(dSMin−dBMax)>3.0`（实测最差 6.77）与 ×6 倍率腿。改后 t960 **连续 5 轮全矩阵零偶红零 diag**（464/0×5）。
- **B-P2-1 t1023 两处静默弃钉改响红** → ✅ Main.qml 找不到（原 note 弃钉）与 src/ 找不到（同）两分支改 `++totalFail` + FAIL 行（文件找不到 = 布局破坏应响红）；t1023(a) 本体滤注释逻辑保持不变。
- **B-P2-2 注释文本钉改锚真实语句** → ✅ t1020 `读档重进同清四结构进入沿守卫`（playercontroller.cpp:611 注释）→ `m_insideStructure[k] = false;` **minCount 3**（setWorld:135 / finishWorldLoad:614 / leaveWorld:962 三处生命周期边界同清契约）；t1015 `t1015 甄别`（:1397 注释）→ `rideCartAt`/`rideBoatAt` 甄别入口两语句；t1017 尾注释 `// t1017` 从钉串剥除只钉语句；t1018(d) okD4 尾注释钉（「// t1018 单 Z 臂…」措辞改动即误红的隐患钉）→ 语句本体 minCount 2（贯穿 Z + 单 Z 臂两镜像行）+ 单 Z 臂守卫行 + 交叉轴零位守卫。
- **B-P2-5 显示名/文案钉分类** → ✅ t1020 `轨道骑士` 显示名钉 → def id 行 `{ "ride_minecart",`（文案可改版，id 行才是契约）；t1022(B) 3 条 UI 文案钉（恢复默认键位/按任意键…/冲突：「）保留但 **copy 钉独立记账**（`okPinW22` 接线 18 / `okPinC22` copy 3 分列 diag `pinsW[]`/`pinsC[copy]`），PASS 行文案明示 "SEPARATELY COUNTED from 3 copy pins (presence pinning, not wiring)"。
- **B-P2-3 t1019 边界能量腿** → ✅ (b2) 临界档独立 PASS 行：通电动力轨 boost 是逐 tick 朝 12.8 档 lerp 无解析式 → **运行时自搜索**（平台长 L∈1..4 × 平段衰减格 D∈0..1 共 8 档逐试，入谷帧差分测 v²），任一档落通过阈 ±10% 窗（镜像 2·19.799·1.05=41.58；kCartSlopeGravity 私有不可直引，同值硬镜像 + 生产常量另由 (c) 钉互锁）即断言「通过（越心东 0.9）或停驻（心 ±0.15）二选一、无极限环（rev ≤ 1：单次爬壁回溜是文档化的贴阈捕获形态，反复往复才是被打掉的打转环）」；判据漂移（阈式/余量改版）时阈边行为翻转最先在本腿暴露。
- **登记项（本波不处置，后续批次）**：
  - **项目根解析帮手**：`QCoreApplication::applicationDirPath() + "/.."`（或 `/../..`）在套件出现十余处（本波 pinSet 调用点又沿用同式）——建议后续抽 suite 级 `projectRoot()` 帮手统一（顺带收口 `/../..` 双层候选差异，t1023a 现有两候选路径形态可并入）。
  - **nextSlot 行距 3 < P-t1005 足印 9 的顺序耦合**：t1005 四布局 rig 清场/弹坑足印 ±4（跨 9 行）大于 nextSlot 行距 3——同列相邻 slot 足印重叠，靠 probe **声明顺序**保证不互踩；重排 probe 顺序或改行距前必须先核对 t1005 足印（建议在 nextSlot 注释处补登记）。
  - **t1016 浮点逐位钉**：phase 的 float `'g'9` 短格式 round-trip 逐位钉对浮点实现/编译器优化敏感（-ffast-math 类开关、不同 MinGW 版本可能翻红）；如未来跨工具链构建，该钉先降级为容差断言。
  - **t882/t927 根因=RNG 游走、修法=EntityManager 测试缝**：两探针的偶红根因是实体 AI 随机游走叠加断言窗，彻底修法是给 EntityManager 开可注入 RNG 的测试缝（构造注入 QRandomGenerator 派生）；涉及生产头文件改动，登记后续批次。
  - **keyRecorderT22 / settingsPanelT22 探针味命名**：Main.qml 内 t1022 探针专用 id 带 T22 后缀（探针依赖其存在性钉 `settingsPanelT22.recordingAction`/`keyRecorderT22.forceActiveFocus()`）——后续 QML 清理勿当「调试残留」删除，连带假红。
  - **t1022/t1023 独立留档阴性轮缺口**：本波敏感度证明覆盖 t1021 类（注释化 → 恰红 → 还原 cmp 留档）；t1022（3 copy 钉注释化）与 t1023（(a) 四门控语句注释化 / (c) 事实行删除）的逐组阴性轮未单独跑，登记后续批次补（迁移后滤注释钉的失败面已由 pinSet diag id 精确化，补跑成本低）。

## review0907 第三波：跨批高危 + 波A回归收口（C 组；2026-09-07，fix×5 + 探针×4）
矩阵 **469 PASS / 0 FAIL**（465 基线 + 4 新 PASS 行；offscreen）。**阴性轮（合并单轮）**：显示拆分（keyDisplayName 复用绑定黑名单）/ 蛛网豁免（摘 Cobweb 豁免）/ 键格守卫（摘 placeBlock 预检）三点同步复原 → **466 PASS / 3 FAIL** 恰红预测三腿（t1022 显示层 / t1012 笼刷 / t1013(h) 守卫+拒载），restore 后复跑 469/0。应用本体 offscreen 冒烟 rootObjects=1。逐项处置（Review_2026-09-07.md 中 #1 / 低 #3 + 09-06 review 跨批跟踪高危 #1/#3、中 #4；探针腿 `test(review0907): wave-C legs` bb2894a）：
- **C-P1-1 keyDisplayName 黑名单回归（review #1 中）+ 默认键单向门（#2 低）+ 拒收码顺序（#5 Info）** → ✅ `fix(review0907): keybind display/bind split + canonical rebind`（4c5e534，keybindmanager.h/.cpp + Main.qml）。判定拆分：`isKeyLikeValue`（非键值域守卫，keyDisplayName 唯一门——Shift/G/Esc 等黑名单键显示名照常给出）∥ `isBindableKey(key, canonical)`（绑定目标黑名单，applyBinding / load 专用）——设置页 sneak/modeCycle 两行键名恢复、冲突提示不再空键名；own-canonical 例外一行（改绑走开后可单键绑回默认，无须全量 reset；#2 单向门闭合）；黑名单判定先于 conflict（被占用的固定键报「固定键」非「冲突+空键名」二义，#5）；QML 录制器 ApplyForbiddenKey 单出「该键为固定功能键，不可绑定」专文案（不再落「无法识别」else 臂）。探针 P-t1022 面板 D：keyDisplayName(Shift/G/Esc) 非空正确名 + 0/Key_unknown 仍空 + own-canonical 绑回 + 分流顺序（占用黑名单键 ForbiddenKey / 占用普通键 Conflict 对照 / 拒收映射不变）+ Main.qml 接线钉 1 与 copy 钉 1 分列记账。
- **C-P1-2 满网蛛网走廊堵死洞穴蜘蛛刷怪笼（09-06 三高危 #1，跨批）** → ✅ `fix(review0907): spawner spawn predicate exempts cobweb cells`（0968545，entitymanager.cpp tickSpawners）。陆生谓词 here/above 收 Cobweb 豁免（机制等价 MC：cave spider 在网窝里照常刷；蛛网 ShapeNone 无碰撞不阻生成）；取「豁免」弃「刷出后清蛛网」——清网破 t786「≥8 网=矿井蛛笼」分流口径，豁免零 worldgen 面；其余非实体格（火把/草/轨）维持 Air-only 口径不变（review26 #19 支撑收口不受扰）。探针 P-t1012⑤：worldgen 真满网笼 rig（0x28 笼 + 8 邻×2 层全非 Air 严格签名 = 摘豁免必红的阴性轮面）玩家激活圈内 tickSpawners 长 tick（80×0.1s > kSpawnerInterval 6s）→ 笼邻 Cobweb 格刷出 MobCaveSpider。
- **C-P1-3 标记格双容器别名（09-06 三高危 #3，跨批）** → ✅ `fix(review0907): reject chest placement on cart key cells`（0968545，playercontroller.cpp placeBlock 通用预检层）。Chest + `isCartCell(目标格)` → 拒（不挥不消耗，t1017 附着拒绝族 no-op 口径；与 t973 生物格门同层——只答「格子被不被保留」，族专项预检前）；仅拦 Chest（熔炉/工作台无 ChestStore 寻址面）；旧档已错误放置的登记不迁移（历史不可辨）。探针 P-t1013(h)：键格放箱被拒（P-t945 真瞄准链）+ 摘键后同瞄点放置成功对照 + 键格 Air 承载回生键不被破箱抹除的守卫语义。
- **C-P1-4 矿井包络 kEnvHalf=26 推导算术错（review #3 低）** → ✅ `fix(review0907): mineshaft envelope half 26->16`（0968545，world.cpp mineshaftSites）。L 形垂直两腿重推导：负向腿房缘 6 + lenA-1 9 = 15、腿 B 垂直向 ≤10、蛛网支廊 ≤7 → 轴向最远 15 + 1 墙环 = **16**（旧注释按两腿共线 5+10+10 误推 26）；「废矿来客」触发 bbox 与矿井氛围音 zone 门控同源收窄 = 预期效果；区域表纯重推导旧档无迁移面。探针 P-t1020(a'') 包络行为腿：全图 Rail 格（矿井 worldgen 唯一放置块；Cobweb 有丛林神殿/要塞噪声不可用作签名）全部落 region bbox（巷道不出包络）+ region-0 水平半边 = 32 = 2×16 值钉（演化点：piece 几何演化时随 world.cpp 推导注释同步改）。
- **C-P1-5 生物可登乘箱子矿车（09-06 中 #4，跨批遗留）** → ✅ `fix(review0907): mobs skip chest carts in board scan`（0968545，entitymanager.cpp tickVehicleRiding Pass C）。扫描循环 `chestAt(i) continue` 一行（读口已有），对齐玩家侧 tryMount 拒箱车语义（内容容器不可骑；蜘蛛钉坐车斗视觉穿插）。探针 P-t1013(h) 拒载腿：箱车同格蜘蛛不登乘 + 普通车同格猪恰登乘对照（证伪「登乘链整体失效」假绿）。
- **登记项（本波不处置，review Info 面）**：
  - **review #4 Info「移动金库」埋点口径**：openChest isCartCell 分支开箱即解锁（空车/未取物也弹 toast）——口径可接受（MC 开箱型进度同款），登记。
  - **review #6 Info pre-t1013 掏空旧标记箱跨档一次重 roll**：转换路径识别「曾有条目但已空」历史不可辨，成本高于收益，登记（与「跨档重 roll 既有口径」同族，一次性仅旧档）。
  - **review #7 Info 神殿保底全图扫描 generate 期两遍**：生成/加载期一次性成本量级可忽略，登记。

## review0906 中优先清偿（D 组；2026-09-07，#5-#8 + 低 #16 全清）

矩阵 **473 PASS / 0 FAIL**（469 基线 + 4 新 PASS 行：t1018(e) / t1019(d) / t1019(e-f) / t1017(e)；offscreen）。逐项处置（Review_2026-09-06.md 尾「清偿状态（2026-09-07）」段同步标注）：

- **D-P1-1 坡臂恒真 → 上层定向线幽灵连接臂（09-06 中 #5）** → ✅ `fix(review0906): slope-arm layer-state endpoint check`。RailProbe 增补 `upState`/`downState`（blockregistry.h；与 sameState 同一填充纪律：仅运行期 World::recomputeRailConnections 填，worldgen placeMineshaft / 转辙器聚合构造点缺省 0 = fresh 保守读 NS——stateless 探针下坡臂分支行为零变，唯一行为面就是运行期填充后的判读 = 修复本体）；`railProbeEndpointAligned` 坡臂分支改读对应层 state 的连接位/bit5 轴判「端点相对」（含本轴向连接位 → 真；c!=0 不含 → 假；sc==0 → bit5 保守读），且**两处消费点同批收口**：规则②③延伸松弛表 + **规则① t982 坡优先路径**（坡臂存在即胜的旧口径对「线端 + 幽灵坡臂」拐角布局放行 = 线端既有单连 Nx 被改写成指向下方 stub 的单向坡臂——线被斩断 + 单向驶入，t1018(e) 首跑实证 end 2→4；现坡臂非端点相对 → 弃坡臂保合格臂单连，不落弯道分支；坡邻真 fresh state 全零保守接受 = worldgen 统一重算 / 邻置收敛窗零变）——上层垂直定向线（c!=0 无本方向连接位）不再从**任一侧**放行单向幽灵臂。探针 P-t1018(e)：上层 EW 线下方 EW 面向 fresh 轨 → 0 连接 + bit5 守恒 + 线端连接守恒（Nx 不被劫持）+ 上层 fresh stub 坡臂照连阳性对照（拒绝非一刀切；断言极性按实际 +Z 下轨 Pz / −Z stub Nz 对齐）+ 规则① 守卫双源码钉。
- **D-P1-2 能量判据下行半幅早评漏算剩余下行收益（09-06 中 #6）** → ✅ 并入 `fix(review0906): valley-floor energy account + golden-rail powered-only exemption`（minecartmanager.cpp）。判据改谷底账 `v2 + 2·kCartSlopeGravity·d ≥ pass2`（d = 沿运动向到谷心剩余水平距离）——旧式每 tick 采当前 v² 直比阈，捕获是接管性（跳过坡物理）→ 车永无「滑到谷心能量变大再放行」机会，hOpp=1 时入格缘 v²∈[~21.8, 41.6) 错杀带闭合。既有腿 (a)（21+19.8=40.8<41.6 仍捕）(b)（更过）(b2)（自搜索自适应，窗口内档在新公式下恰为通过档）全保持绿。探针 P-t1019(d)：带内判别档（入谷 v²∈[25,39] = 错杀带内可达域；单格 boost 档实测 37.8-38.5，更低档不可达——平段衰减 ~0.2 v²/格而坡下降增益落在采样点之后；运行时自搜索 L1..4×D0..4）→ 应通过（谷底账 38.5+19.8=58.3 ≥ 41.58，maxX > center+0.6；旧式 38.5<41.58 恒捕 = 判别裕度双向 3.1/16.7）+ rev ≤ 1。
- **D-P1-3 金轨谷格豁免不看通电态（09-06 中 #7）** → ✅ 并入同提交。豁免收窄为「通电金轨」（`GoldenRailStateOnFlag` 置位才 return false，与 boost 路径同判定位）；断电金轨谷格照捕——t939③ 刹车只在静置分支管不住滑行车（旧无条件豁免 = 滑行车交还纯保守谷物理 + 两壁 kick 再供能 = t981 往复复发）；捕获停驻谷心 |speed|→0 后恰落入 t939③ 静置刹车 = 稳定不动点，两语义无缝衔接。minecartmanager.h 契约注释与实现注释同步修正口径劈叉。探针 P-t1019(e/f)：断电金轨谷 + 静置闸起步车（入谷 v²≈21，谷底账 40.8<41.58）→ 捕获停驻谷心 rev==0 + 60 tick 静止守卫；通电对照（红石块直供谷格）→ boost 接管通过（收窄不误伤通电轨型）。
- **D-P1-4 失撑复检未同步排除仙人掌（09-06 中 #8）** → ✅ `fix(review0906): shared mech/ladder attach-support authority`。新增 BlockRegistry::mechLadderSupportBlock（= isFullCube && != Cactus）单一权威，四处同源：木梯/机关放置预检（原裸 isFullCube||Cactus 显式对收口）+ 木梯/机关失撑复检（原裸 isFullCube → Cactus ShapeFull 恒真 = 「放置拒、残留不掉」劈叉根因）。探针 P-t1017(e)：旧档残影注入（侧贴仙人掌梯子——梯非整立方不触 checkCactusOnEdit ④ 柱存活；仙人掌顶贴地拉杆——顶面附着不触 ④ 水平邻接口径）→ 挖**另一侧邻格**（支撑柱全程存活）→ 梯/拉杆经 6 邻复检掉落 + 柱无恙断言（防「柱先塌带走附着块」假绿）+ 石墙梯挖支撑对照（挖掘链 sanity）。
- **D-P1-5 对壁扫描逐格 +1 即 break（09-06 低 #16）** → ✅ 并入 D-P1-2 提交（minecartmanager.cpp hOpp 扫描）。break 条件放宽为该列**三高探针**（railProbeDelta 同口径 k-1/k/k+1 三层）皆无轨才 break，层差计入爬升账 hOpp = k+δ（δ∈{-1,0,+1}：楼梯壁照旧 / +2 跳升 δ=+1 足额计入 / 壁顶平台 δ=-1 不虚增）——+2 跳升 / L 形爬升壁不再低估 hOpp（阈值低估 → 能量边界车被放行去爬爬不出的壁）。病态几何触发面窄但修法与 #6 同文件同函数、成本一行，选择修复而非登记。探针随 t1019(d) 腿间接覆盖（扫描口径钉 okC3 同步更新）。
- **阴性轮（合并单轮，四病灶同步摘除；已实测 matrix_d1_lesion.log = 469 PASS / 4 FAIL）**：#5 摘坡臂新判据（坡臂分支恒真 return true）/ #6 d 采样归零（还原 v2 直比阈）/ #7 还原无条件金轨豁免 / #8 复检前置裸 isFullCube → 四新腿**恰红实测**（t1018(e) 幽灵臂绿返红 / t1019(d) 带内通过变被捕 / t1019(e/f) 断电金轨往复复发 / t1017(e) 残留不掉），其余 469 组全绿（四病灶以「行为摘除、钉文本保留」的最小手术实施 = 源码钉零误红，恰红面严格收敛在四条行为腿）；restore（diff 与绿轮逐字节一致核验）后复跑 **473/0**（matrix_d1_final2.log）。

## review0906 低七项清偿（D2 波；2026-09-08，#9-#15 + Info #18 全清）

矩阵 **479 PASS / 0 FAIL**（473 基线 + 6 新 PASS 行：t1012c / t1013b / t1014(h) / t1015(f) / t1016b / t1017(f)；offscreen；test rig 世界 96×144→96×180 = slot 232 位，D2 +3 腿预算打穿 144 深度的 qFatal 实锤）。逐项处置（Review_2026-09-06.md 尾「清偿状态」段同步标注）：

- **D-P2-1 贴地/贴面支撑黑名单同族漏网（09-06 低 #9）** → ✅ `fix(review0906): #9+#10`。新增 BlockRegistry::solidSupportBlock 统一权威（= isFullCube && != Cactus 的包装层，**不动 isFullCube 本体**——渲染/剔除/出生列/重力坍落非附着消费端零回归），一处收紧全族同口径：铁轨放置预检②（原裸 isFullCube）、积雪层 belowSupport 分支、trapdoorSupportBlock 排 Cactus、isTopFlushSupport 完整立方分支、mechLadderSupportBlock（D1 权威）委托并入。探针 P-t1017(f) 贴面/站顶全家族扫：活板门瞄仙人掌侧面（唯一可依附面）拒 + 粉/雪/轨/门站顶面全拒 + 柱无恙断言 + 石头顶对照照常放置（防过杀）+ 谓词钉；P-t945(h) 源码钉同步换 solidSupportBlock 口径（旧 isFullCube 钉对 WIP 必红）。
- **D-P2-2 水毁附着族漏 Ladder（09-06 低 #10）** → ✅ 同提交。isAttachableBlock 并入 Ladder（dropId(Ladder)=自身 → 掉落链与玩家挖除同源免费成立）；**Rail 刻意排除登记**（注释即契约：worldgen 矿井轨网为不可再生场景资产，洞口洪流不可成片掏轨；梯毁后可随手重放无此顾虑）。探针 P-t1012c：谓词腿（Ladder ∈ 族 / Rail ∉ 族 / Stone 阴性 / dropId 自证）+ 行为腿（水源 +X 扩散入贴墙梯格 → 冲毁置 Air/Water + blockDroppedAsItem 掉自身 id；同源反向扩散轨格完好对照 = 刻意排除的行为面钉）。
- **D-P2-3 海列 cell 整格跳过 ≠ 旧逐列口径（09-06 低 #11）** → ✅ `fix(review0906): #11+#18`（world.cpp scatterOres）。cell 级海列一票否决移除，海列拒绝兜底回归 tryOre 逐列海检（h ≤ wl+1；纯海 cell 的脉落点全数被拒 = 零成本余量），海岸过渡带恢复旧散点分布。探针 P-t1014(h)：**专测海 seed 路线**——diag_t1014h 实测 t1014 五固定 seed 的 128×128×64/128 高窗内**零海中心 cell**（旧口径的 cell 跳过在这些世界从未触发 = 原候选腿恒 vacuous），40-seed 扫描命中 4 个、取前 2（150461/174218，各恰 1 个滩心 cell，中心列高 58/59）：每 seed 陆地列（h≥60）7 矿种计数 ≥200（实测 747/782，正裕量 ~3.5×，阴性口径只剩邻 cell 越界蹭入）+ censusSeaMeta 结构钉确认海 cell 存在（防 seed 漂移 vacuous 绿）+ 计数走同一 censusT1014（口径零分叉）。连带：t1014(c) 红石脉数窗 1390→1300 + 金脉 conn26 阈 95→85%（#18 重挑限水平 4 向后 blob 行必走一步、walk 更远 → 大脉占比升、连通率实测 92-94%；金色窗与红石窗均按 D2 首跑 5-seed 实测校准）。
- **D-P2-4 回生落车守卫 + 开箱遮挡（09-06 低 #12）** → ✅ `fix(review0906): #12+#13`。(a) spawnCartImpl 落格 isCollidable 时向上扫首个可落格（车贴轨/贴地基准按新格重算，内容键与落格解耦；顶到世界顶仍无空格 → 拒生成、箱车键留 ChestStore 下次进世界重试；玩家放置路径不受扰——placeBlock 已验落格 Air）。(b) placeBlock 箱车分支补遮挡守卫 chestDist ≤ m_hitDist（主选体未命中 m_hasHit=false 恒过；与攻击/骑乘「实体须不晚于命中方块」同口径）。探针 P-t1013b：(a) 键格填 Stone → 车落上一格地面姿态（0.3875）+ 键格保持 Stone + chestKeyAt 原址；(b) 墙格挡主选体（hit≈1.6 < chest≈3.2）→ opens=0，拆墙同几何 → 恰一次携键（守卫不过杀贴脸开箱）。（D2 首跑归因注：phase-1 隔墙点击在箱车分支被守卫拒后落到通用放置分支，把默认手持 Stone 误放到墙击面邻格，拆墙后幽灵石头距眼 ~0.54 抢占主选 = opens 0 假红；探针已改空手分流。）
- **D-P2-5 攻击者眼位落入载具盒恒判载具胜（09-06 低 #13）** → ✅ 同提交。BoatManager/MinecartManager::rayHitDistAt（t1015 攻击甄别专用单盒射线）起点在盒内（slab tmin==0）改返 **-1 未中**——旧「0 ≤ 任何 mobDist」恒真 = 眼位落盒时点乘员身体恒判载具胜；findBoatHit/findCartHit（登乘/开箱寻的）**不改**——「挨着车即可上」依赖盒内命中返 0，两射线口径分叉以登记注释钉死。探针 P-t1015(f)：车心柱眼位落车厢盒内点乘员身体 → 乘员掉血、车耐久不动（实测 lesion：mob 0 hp 10→10 / cartHp 2 = 旧判载具胜复发面）。
- **D-P2-6 进世界无条件恢复天气抽掉首场短窗（09-06 低 #14）** → ✅ `fix(review0906): #14+#15`。loadWorldTime 增 **hasWeather** 键（存档真带 weather 才 true——缺键默认 0 与「真存过 Clear」自此可区分）；Main.qml enterWorld 仅 hasWeather 才 setWeatherState，缺键走 resetWeather 原生首场晴偏短窗（20/45s 不再被重抽为常规 45/120s）。探针 P-t1016(a)(b) 带键 round-trip hasWeather=true / 旧档四参形态 false + P-t1016b(a) 来源腿 + QML 恢复守卫 `if (wt.hasWeather)` 源码钉（滤注释钉 pinSet）。
- **D-P2-7 applyTime float 折算大 day 吞相位（09-06 低 #15）** → ✅ 同提交。worldclock.cpp applyTime 改**整数拆分**：`day * qint64(periodMs) + qint64(double(phase) * periodMs)`——day 段整型精确、phase 段 double 折算（截断 ≤1ms ≪ 100ms tick），day≳2²² 后 float ULP（该量级 ≈0.4 天）不再吞相位小数，restore→onTick 重派生 round-trip 相位保真。探针 P-t1016b(b) 大 day 相位保真腿。
- **Info #18 blob 竖向重挑零位移卡步** → ✅ 并入 D-P2-3 提交（world.cpp scatterOres）：带缘重挑限**水平 4 向** kH4（旧 6 向可抽中竖向 = dy 已抹平 + dx=dz=0 零位移步同格重试到步数耗尽，脉提前夭折）。探针面 = t1014(f) 源码钉 kH4 判定行（阴性轮敏感）+ (c)/(d) 脉形窗按 D2 实测校准。
- **rig 健壮性连带修（D2 首跑实锤 4 红腿归因；diag_t772 + 三轮全矩阵定位）**：depth 144→180 令 worldgen 洞穴/地表分布整体迁移（worm 起点按深度取模重排），暴露三处**探针地形依赖**：(a) t772 order-A torch/lever→IronTrapdoor 假红——槽 (26,41,85) 一带 y39..43 现天然空腔（diag 实测），活板门浮空放置后「源放置」这一邻格编辑触发 t851 失撑复检（hasAttach 无任何实体面：火把/拉杆非 trapdoorSupportBlock；RedstoneBlock case 因红石块自身算实体面而幸存——判别面实证）→ 板当即脱落，设备格 id 归 0；修 = 每槽显式铺接收器正下方支撑石（探针语义「相邻供电」与地形无关）。(b) t919/t826/t827 运行期扫空区（z<125 域）不再保有一体 16×7/18×5 全空盒 =「no clear rig area」假红；修 = 扫空失败 fallback 远端定址（z=169 行 = slot 220+，本轮尾部 ≈ slot 184 永不触及）+ 自净空工作盒后照常铺设。**教训（t814 同源深化）：探针选址/器件落位必须自带保障，禁依赖 worldgen 在目标层恰好空/实心**；三教训排查记录：①exe/源 mtime 对齐核验（02:29 exe vs 01:12 源修正）②纸面推演出「红石块 case 幸存 vs 火把 case 脱落」的判别面后 diag_t772 单因子复现（分 slot 复刻 21 case 序列 + 逐 tick 器件 id/state）③行为红非钉红（pinSet 零涉）。
- **阴性轮（合并单轮，四病灶同步摘除；已实测 matrix_d2_lesion.log = 474 PASS / 5 FAIL）**：#10 摘 isAttachableBlock(Ladder) / #12b 摘 chestDist≤m_hitDist 守卫 / #13 rayHitDistAt 盒内还原返 0 / #14 摘 hasWeather 键 → 五腿**恰红实测**（t1012c 谓词+行为双红：梯存水未毁 drops 0 / t1013b(b) 隔墙 opens=2 复发 / t1015(f) 点身体判载具胜：mob hp 10→10、cartHp 2 / t1016(a)+t1016b(a) hasWeather 键面红），其余 474 组全绿（行为摘除、钉文本保留 = 恰红面严格收敛在五条腿）；restore（四文件字节级回拷 cmp 核验）后重建复跑 **479/0**（matrix_d2_final.log）。

## review0905 五项清偿（2026-09-07，#2/#3/#4/#5/#6 全清）

矩阵 **480 PASS / 0 FAIL**（479 基线 + P-t1010b 加严腿 1 行；offscreen）。逐项处置（Review_2026-09-05.md 尾「清偿状态（2026-09-07）」段同步标注）：

- **R-P1-1 神殿保底「出生区可见」口径（中 #3，用户主诉直击）** → ✅ `fix(review0905): temple guarantee proximity`。触发门 `placed == 0` → `nearestCenterSq > guaranteeRSq`（主路径最近座距世界中心 > kTempleSpawnGuaranteeRadius=56，world.h 单一权威、探针同源直读；R 覆盖出生游走圈，小世界超半边退化为旧口径登记），desert/jungle 两神殿同构；补座选列不变（距中心最近合格列）；确定性纯函数不变；siteOk 判定公开为只读查询（探针同源直读零复刻漂移）。探针 P-t1010b：见 t1010 回标（含首跑 478/2 两修：合格列前提 + 全高扫/丛林门群系核）；**阴性轮**：触发门还原「sites 空才触发（nearestCenterSq > 1e17，语义同 placed==0）」→ **恰红 478 PASS / 2 FAIL**（matrix_r0905_lesion.log）：P-t1010 源码钉腿（diag「source pins drifted (guarantee flags / R gate / siteOk)」）+ P-t1010b（9 条 VIOLATION：角落神殿世界出生圈 0 证据）；restore（字节级 cmp 核验）→ 复跑 **480/0**（matrix_r0905_final.log，EXIT=0）。
- **R-P1-2 t1011 壁挂火把裸 blockAt/setBlock 卫生收口（低 #4）** → ✅ `fix(review0905): torch wall-mount guard hygiene + F3 cap()`（world.cpp pieceCorridor 跨中壁挂块）。贴面格/支撑壁显式界内 continue（越界整对跳过 = 不落火把不回退地板，对齐 putStruct 越界静默口径）+ 两处写入收口 putStruct 同口；正常域（margin 守卫在）零行为差 → P-t1011 密度窗/光斑数字逐位不变，免阴性轮。
- **R-P1-3 t1007 F3 mobs 分母硬编码 /64（低 #6）** → ✅ 同提交。EntityManager 增 `Q_INVOKABLE int cap() const { return kCap; }`（kCap 单一权威暴露），Main.qml entities 行改读 `entityManager.cap()`；连带 P-t1007 (f) 源码钉腿随行迁移（`"/64 hw "` 针 → cap() 读口针 + 头文件 cap() 针），PASS 文案 mobs-N/64-hw → mobs-N/cap()-hw。
- **R-P1-4 t1003 正面安卡 v4 横杠被门楣覆写（低 #5，先考据）** → ✅ `docs(review0905): wiki 考据结论 + kMergeStates 告示`。考据结论=**MC 门面本就无完整安卡，现状即忠实，几何不改**：完整安卡形（橙陶瓦、蓝陶瓦心）只在地面层中央（Layer 0）与密室（Layer -4）各一次，外立面仅前侧双角塔面条带装饰（初稿「四角塔各面」系蓝图误读，复核原档已更正：塔只有前侧两座），门面门楣上即切制砂岩（Structure 蓝图 Layer 3 前排 AAA），本尊全埋无地面门洞——出处 minecraft.wiki/w/Desert_Pyramid + /w/Desert_Pyramid/Structure（raw wikitext 逐层解析，2026-09-07；raw + 蓝图归档 docs/review0905_wiki_desert_pyramid/）。本项目四面中央安卡 = 登记再创作口径，正面被门洞+门楣截至 13 格与 MC「门面门洞区无完整纹样」忠实同构。落地=P-t1003 (b) 腿注释 + PASS 文案改措辞（「四面等量」→「三面完整安卡 + 门面截断 = MC 忠实」）+ world.cpp D 步注释 + dev-plan t1003 回标；探针断言数字零变化（免阴性轮，文案/注释面）。
- **R-P1-5 kMergeStates 红石耦合告示登记（中 #2）** → ✅ 同 docs 提交。dev-plan t1004 回标加**耦合告示**（改红石常量/传播规则必须同步重推 kMergeStates 并重跑 P-t1004；P-t1004 state 钉腿 = 唯一护栏）+ 可选根治（worldgen 放完电路后对包围盒触发运行期同源 dust 重算）登记为后续候选不在本波做；world.cpp kMergeStates 定义处加指向告示的 ⚠ 注释行。纯注释/文案，P-t1004 源码钉（pin 语句本体 `static const quint8 kMergeStates[6] = ...`）不受扰。

## R19.21 MC 对标玩法批（t1024-t1029，6 项；2026-09-08 立项，用户授权自动出需求自动执行）

用户指令：三份 review 清偿后继续自动参谋+实施，目标是「和我世界差不多的产品」；用户保留随时叫停权。本批主攻**生存循环核心缺口**（MC 最核心的"床-重生-繁殖-农业"三件套项目还没有）+ 一项性能治理 + 一项工程债。

**t1024 床的完整语义** → ✅✅ `fix(t1024): bed complete semantics - respawn anchor + thunder gate + refusal copy`（bac476b）+ `test(t1024): matrix 480->483`（8182e11）。**基线盘点**：t388/t457/t898 已交付睡觉状态机（trySleepAt 夜门/怪物门 + sleepFade 三阶段 + 跳清晨 + respawn 读 m_spawnPos），本任务增量=①雷暴拒睡门（kWeatherThunder=3 int 编码，`World::Weather` 刻意私有）+ 敌对文案对齐规格「你不能休息，附近有怪物」；②**床锚生命周期**：成功入睡瞬间即设锚（MC「睡上即设」口径，按钮醒/受惊醒均保锚）——`bedSpawnValid` Q_PROPERTY（QMetaObject 契约面探针覆盖）+ `setBedSpawn/clearBedSpawn` 单点收口 + `bedSpawnLost` 信号；③**挖床失效链**：finishMiningAt 床分支判锚格（本格或配对格==锚格同层）→ clearBedSpawn 回 kSpawn pristine + bedSpawnLost（仅玩家直挖口径，爆炸/岩浆不触发已登记）；世界换代（seedChanged）同步清锚；④**存档持久化**：WorldStore.saveAll 第 6 参 `{valid,x,y,z}` → bed_x('g'9)/bed_y/bed_z/bed_valid 四键与 chunks/meta 同事务原子，loadBedSpawn() 逐键缺省（旧档 hasBed=false=世界出生点重生），bed_valid=0 门压残留坐标键（挖床后退出不回填）；⑤Main.qml 编排：enterWorld 恢复床位锚 / 退出第 6 参 / 三条用户面播报（睡眠设锚=dev-plan 文案「重生点已设置」、死亡床锚重生=「你已回到床边重生」、床锚丢失=「床被破坏，重生点已失效」——deathScreen 提示拆为设锚+重生两沿登记）。**探针** P-t1024a（regenerate 地形 rig、床贴地表使 snapSpawnToGround 恰落床顶）：入睡即设锚 + 跳清晨 phase=0.75 时间面 + respawn 落床位 + 敌对拒睡（零位移保锚）+ 雷暴拒睡 + 双门清空正控制；P-t1024b 真实注视链（captured+垂直射线+创造瞬破）挖锚床→双半清+锚失效+bedSpawnLost 恰一次+重生点回 (80,80,80)；P-t1024c 真 SQLite round-trip + 旧档缺键 + 源码钉（QML 编排/文案/四 C++ 契约面，阴性轮敏感）。**矩阵 480→483 PASS / 0 FAIL**（matrix_run2.log）。**阴性轮**：摘敌对拒睡门+摘挖床清锚（单构建双摘）→ **恰红 480 PASS / 3 FAIL**＝t1024a/b/c 三腿（matrix_neg.log，其余零回归）；restore 后复跑 t1024 三腿恒绿。**flaky 台账**：restore 复跑两轮 t882 偶红（matrix_run3/4.log，diag okFar-false 签名稳定；t882 在矩阵序先于 t1024 块执行且源码与 483/0 轮逐字节相同→纯墙钟/负载噪声非本批回归；t882/t927/t960 同源 flaky 治理=t1029 特许）。

**t1025 动物繁殖** → ✅✅ `fix(t1025): breeding MC-parity + baby-follow/food-lure + love gates`（11a1a2f）+ `test(t1025): matrix 483->485 breeding probes`（9bcaff0）。**基线盘点**：t400 求偶链（喂食→love mode→寻偶 AI 相遇配对产崽 + inLoveAt→QML 心形 delegate）+ t479 喂幼加速成长 + t952 幼体物理盒基建已有，本批增量=①**数值回标 MC 口径**（现实秒折算：恋爱窗 30s / 繁殖冷却 5min=300s / 幼崽成长 20min=1200s / 喂幼减 10%=120s——kBreedCooldown 60→300、kBabyGrowTime 120→1200、kBabyFeedGrow 12→120 随基准回标；探针缝 `setBreedTimings` 运行期秒改写，产品路径零调用恒 MC 值，不持久化）；②**幼崽物理盒同倍缩**（bred 幼崽 halfW/halfH ×kBabyScale=0.5 + 血量减半 maxHealth 5 + 出生挂满 1200s 成长；长大 `applyMobCollisionBox` 单一权威还原成体盒/血量×2——t952 小蹒跚者同款机制，非 t400 旧版仅视觉缩）；③**双阴门**（enterLoveMode：`if (e.baby) return false` 幼崽不可求偶 + `if (e.breedCooldown > 0.0f) return false` 冷却门——均钉钉 P-t1025a，阴性轮敏感）；④**幼崽跟随**（孤儿/非孤儿幼崽认 kBabyFollowRange=16 内最近**成年同种**为亲：yaw 钉向 + 强制行走 + 贴身 1.5 停步；`findNearestAdultSameType` 门=alive && !dead && 同种 && !baby；AI 优先级 求偶寻偶 > 引诱 > 幼崽跟随，位移交 aiWander 复用碰撞撤回）；⑤**食物引诱落地**（非登记不做——Entities 层 `setFoodLure` bool 门控表收 bool 不沾物品 id（PLAN §2 分层，t960 caller 算好写值先例），Game 层 `breedFoodMatches` **单一权威**：牛/羊=小麦、猪=胡萝卜·马铃薯、鸡=种子，喂食分流与引诱同源消费防口径漂移；`PlayerController::updateFoodLure` 挂真 Hotbar selectedSlotChanged/slotsChanged 链（UniqueConnection）+ 注入即刷，空手全清；kFoodLureRange=10 / 停步 1.2；持食物聚拢同种 < kBreedRange 即配对，同 MC 手法）；⑥**粒子方案**：爱心反馈复用 t400 inLoveAt→QML delegate 心形 Model，零新增粒子系统。**探针** P-t1025a（喂食恋爱→双满产崽→幼崽字段 0.5/盒 0.225/HP5/1200s + 冷却门/幼崽不可繁殖门两阴腿 + 1s 缝冷却关→开双向 + 0.8s 缝成长还原盒/1.0/HP10 精确逆）；P-t1025b（双亲击杀孤儿→锁定最近同种成体 yaw 精确钉向 + 净位移朝之 + 异种 cow 对照不认；引诱半径内猪钉玩家 + 位移朝玩家、半径外猪游走有界 yaw 不钉、关门解钉；真 Hotbar 链四物种门控表 + 空手全清；源码钉锁缝/引诱/常量面——两道求偶门钉随 P-t1025a 同行）。**矩阵 483→485 PASS / 0 FAIL**（matrix_run.log）。**阴性轮**：摘 enterLoveMode 双求偶门（单构建双摘）→ **恰红 484 PASS / 1 FAIL**＝P-t1025a（cooldownGate/babyGate/gateClosed/gateReopened 四腿齐红 + 双钉 miss，P-t1025b 保绿证其余系统零回归，matrix_neg_t1025.log）；restore 后复跑恒绿 485/0（matrix_run2.log）。**探针教训**：①首跑 follow/lureIn 双腿红为**单位面假红**——yawAt 返回**度**（t239 QML eulerRotation.y 面约定），与 atan2 推导的弧度方位直比恒差 57.3 倍（diagF yaw=-87.68 vs toSheep=-1.5236 rad 实为钉向成立）；比较位统一转弧度后恰红面收敛为零，引出本批真结论=实现本身正确（yaw 钉向/位移/dot 全过）；②未 regenerate 的 World 带 fBm 程序地形掩埋探针选址（嵌入→stuck-escape→位移腿全红）→ 工作体积显式净空 y=11..16（t1024a 同款纪律）。

**t1026 小麦农业闭环**：锄头右键泥土/草方块→**耕地**（新方块，kCap 内追加；被踩/水流冲回泥土登记）；种子右键耕地→**小麦作物**（4 生长阶段，随机 tick 推进+光照门槛）；成熟挖→1 小麦+1-3 种子；未熟挖→种子。种子来源：锄草方块概率掉。面包配方（3 小麦）接 RecipeRegistry。探针：锄地/播种/阶段推进/收获掉落表/面包合成。
  → **✅✅ 落地（2026-09-09，五棒接力+主控亲自收口）：t236/t237/t246 既有耕作链上的窄增量，矩阵 485→487。** 盘点实情：耕地（Farmland=24 含湿润 state/cross 渲染）、小麦作物（WheatCrop=25 八阶段 state）、随机 tick 生长（tickCropGrowth 确定性哈希骰子+光照门 kCropMinLight）、锄草掉种（t246）、收割（t237）均已在库——本单窄增量=①成熟收获种子 1-2→**1-3**（bounded(1,4) 对齐本批口径，净增益恒正）；②TallGrass 掉种/作物收获/dropUnsupportedCropsAround 三路 spawnItem 计算**收敛单一权威**（失撑掉落与玩家破块产出同源，零分支漂移；t619 成就埋点按 review-L2 区分计数）；③blockregistry.h 补水蚀/踩踏简化登记（水毁清单不含耕地=非附着块）。**面包=3 小麦配方 t236 已在库**，P-t1026b 直接覆盖（3×3 三行合成 round-trip 绿）。**探针 P-t1026a/b**（+330）：真玩家路径全链（锄地转换含石拒/8 格播种耗种判据/确定性骰子复现生长——模拟窗==实际窗逐位相等+1 单调/光照门三阴性株/成熟 1 麦+1-3 种未熟 1 种）+ 面包三行合成 + 源码钉。**收口根因（主控亲自）**：t1026a 连续三棒红于收获段 = **t1022 同款 dt 坑**——updateMining 的 dt=m_clock.restart() 墙钟差（:927），探针连 tick 时 dt≈0 → progress += 0×speed 恒 0 → 6000 tick 挖不满瞬破门槛（TDIAG 三联签名 postBlock 25/progTicks 6002/prog 0）；修=mineBlockF 每 tick busy-wait ≥17ms（t889/t1022 先例）→ 5 tick 破格转绿。**阴性轮：摘 tickCropGrowth 光照门 → 恰 t1026a+b 红**（其余 485 全绿，matrix_neg.log）→ restore（cmp）→ **487/0**（matrix_final.log；首轮 t882 登记族偶红复跑清）。**待实机确认（观感类）：耕地湿润贴图变暗、小麦八阶段生长观感、面包合成 UI、锄耐久消耗。**

**t1027 掉落物 instancing 治理首批**（t1007 治理路径 a 落地，用户 7FPS 主诉治理）：93 items 场景 draw call ∝ 活体数——按 t858 经验球 instancing 先例把**掉落物压 per-family 单 Model**（同 family 同贴图合批；旋转/浮动动画进 shader/统一时间驱动）。这是周级大项，本批只落**首批**（族划分+最大族 item 合批+渲染回归验证），全量分步登记。探针：instancing 路径单元腿（族分桶正确性/实例数=活体数）+ 渲染观感实机确认。
  → **✅✅ 落地（2026-09-09，首批切片=族划分+最大族合批+回归验证，余面分步登记）：矩阵 503→505 PASS / 0 FAIL。** **族盘点**（itemHost delegate 全分支贴图/几何引用实查）：①方块整立方族（BlockCube+共享 voxelAtlas；泥土/石头/圆石/木板/砂/羊毛16色…挖掘产出主面，活体数期望最大→本批合批面）；②3D 形状族（isItem3DFamily 24 id，ItemShapeGeometry+voxelAtlas）；③异形 billboard 族（partial/cross/bed 非 3D，per-id 图标 Texture）；④工具 3D 族（五几何+弓，tier 色）；⑤工具/材料 billboard 族（ToolIcon/MaterialIcon sourceItem per-id）；⑥光晕壳族（UnitCube 半透，全族横切）——②~⑥登记分步（见末）。**实现面**：家族谓词收编 **C++ 单一权威**（ItemEntityManager::isItem3DFamily 24 id 自 Main.qml 字面量表原样搬运 + isPlainCubeDrop 整立方判 = 合法方块段 ∧ 非异形/cross/床/3D 族；static_assert 钉 Count≤0x100 前提——工具/材料段经上界排除；Main.qml 函数退化薄委托）；**BlockDropInstancing** feeder（Game 层，t858 XpOrbInstancing 同款公开 API 模式：16ms QTimer markDirty + getInstanceBuffer、零稳态分配；动画下沉解析式 rotY=ph/3s×360 + bob=0.075·(1−cos πs) 2s 三角波（InOutSine out-and-back 推导同 t858），slot×0.37s 错峰=旧「相位=创建时刻」等价，公式逐字对齐旧 delegate）；Main.qml **blockDropInstHost 固定 8 桶池** per-id instanced Model（族内 per-id 差异只在 BlockCube 几何 UV→桶粒度=itemId，同 id 全实例共享同一几何+材质=1 draw/桶；材质与旧分支逐字同参 NoLighting+voxelAtlas+天光 baseColor）；桶指派走 entitiesChanged **信号 handler 普通 JS**（t976/t977 AOT 教训；稳定指派：在用桶保持/活体数降序补位/id 升序 tiebreak 防抖），桶满溢出 id 保底走 delegate 旧路径（visible 链追加 hasBucket 排除=两侧同源谓词）。数据链零改动：拾取/despawn/合并/物理全在 C++ 管理器（feeder 只读）。**工程坑登记（moc）**：Q_PROPERTY(ItemEntityManager\*) 用前向声明触发 Qt 6.11 moc TypeMustBeComplete static_assert——xporbinstancing.h 的前向声明模式靠 mocs_compilation include 顺序侥幸存活（moc_xporbmanager 恰排 moc_xporbinstancing 前），本头按字母序排 moc_itementitymanager 之前即炸；修=直接 include 全量头（blockdropinstancing.h 头注释登记，后续 feeder 类照此）。**探针 P-t1027a/b（+2）**：feeder 内容级（族分桶正确性=只收同 id 整立方活体、torch/材料段永不进桶、实例数=活体数、XZ 精确+bob 带 Y∈[pos.y,pos.y+0.15]+scale0.3+纯 Y 旋转、拾取 setCountAt(0)/removeAt/clearAll 反射锁步、同 id 就近合并**不加实例**=1 实例 count3=1 条目）+ 族谓词行为级全枚举（整立方收录含 16 色羊毛段/3D 24 id 全排除/楼梯 16 partial 不入/工具·材料·越界不入）+ 源码钉（hasBucket 排除链/BlockDropInstancing manager+familyId 接线/reassignDropBuckets handler/薄委托/feeder 过滤=阴性轮敏感面）。连带 **t925 源码钉锚串合法演化**：掉落侧家族表锚自 Main.qml 字面量迁移 itementitymanager.cpp case 表（t965 锚串演化先例），查看器侧不动。首轮 2 红根因：探针格距 2.0 恰落 kMergeRadius 塌缩活体数（t490fix 合并是正确行为，rig 绕开=格距≥3）+ t925 锚串迁移——均修（matrix_run_t1027.log）。**阴性轮**：单构建双摘（feeder 族过滤/桶匹配两行 + delegate hasBucket 排除子句=回退全体 delegate 渲染）→ **恰红 P-t1027a+b 两腿**（其余 503 全绿；diag：feeder 无过滤 cntDirt=cntCobble=cntTorch=7 全收、pins deleg/feeder miss，matrix_run_t1027_neg.log）；restore 后复跑恒绿 **505/0**（matrix_run_t1027b.log）。app 构建 EXIT=0 + offscreen 冒烟存活无崩溃（GUI 子系统无 stderr，既往空日志口径）。**分步治理路线（登记）**：二批=3D 形状族 per-id 桶（ItemShapeGeometry 几何缓存共享）；三批=光晕壳族全族合批（per-instance color 承载附魔紫/灰+呼吸 opacity 进实例表）；四批=工具 3D 族（五几何+per-instance color tier 色）+工具/材料 billboard 族+异形 billboard 族（per-id 贴图桶）。**待实机确认（渲染观感类）：F3 draw call 前后对比（93 items 场景 item 面 2×活体→桶数+壳数，全族批次后趋近 O(桶)）、instanced 旋转/浮动观感与 delegate 版一致性、桶满 8 降级路径观感、日夜天光调制一致性。**

**t1028 音符盒**：新方块；右键循环调音（0-24 半音，音高提示）；攻击/红石信号触发发声（运行期正弦合成 piano-ish 音色复用 t1021 程序化管线，不出新 wav）；下方方块材质改变音色族（登记 3 族：木/石/沙）。探针：调音 round-trip/红石触发/音高参数断言（听感实机确认）。
  → **✅✅ 落地（2026-09-09）：NoteBlock=143 枚举尾追加（存档契约），矩阵 487→502（+15 腿）。** 实现面（816cb9a）：state bit[4:0]=音高 0-24（noteBlockTunedState 单一权威 +1 回绕）+ bit5 通电记忆位（RailSwitchPoweredFlag 先例）；右键调音+攻击按下沿发声（MC「攻击奏响照常挖」口径）；红石通电上升沿发 noteBlockPlayed 语义事件（refactor-plan 29.4：音频走 Event 不走 World）；音色=build_sounds.py gen_note_piano 25 个程序合成钢琴 wav（离线管线=dev-plan 兜底 B，运行期合成登记不做）+ 播放速率倍移近似三族（木 ×0.5 低拨/石 ×1.0 低鼓/沙 ×2.0 脆响——独立乐器采样登记留后续）；配方 8 板环+红石粉心（MC 1.0 口径）；creative 调色板红石页。探针（b91cb87）=12 红石源全覆盖 + 调音回绕 + 攻击沿。**阴性轮**：摘调音 mod 回绕 + 摘沿判定 → 恰 t1028 全家 14 红（其余 488 全绿）→ restore（checkout）→ **502/0**（matrix_t1028_final.log）。**连带修复（d27d980）**：816cb9a 的 audiomanager.cpp 裸名 `Clip`（Data 嵌套类型）致 **app 目标编译失败**——矩阵测试目标不含 audiomanager.cpp（app 独有编译单元）所以矩阵绿不代表音频面可编；修=auto 引用 + **套件盲区登记：音频面编译校验必须走 app 目标**。app 构建 EXIT=0 + offscreen 冒烟 rootObjects=1。**待实机确认（听感类）：25 半音音高爬升、三族音色差异（倍移近似的观感）、红石连奏、调音提示文案。**

**t1029 t882/t927 RNG 测试缝**（工程债清偿）：EntityManager 加 wander 冻结/RNG 注入测试缝（`setWanderFrozen(bool)` 或 RNG 种子注入——以最小侵入为准），P-t882/P-t927 显式冻结游走→三族钓鱼 flaky（t882/t927/t960 同源噪声）确定性化；flaky 台账收敛。探针：缝存在性+冻结行为腿；阴轮=关缝复跑历史偶红签名复现一次登记。
...
**t1030 骨粉与作物催熟**：骨头→骨粉（3 骨粉/骨，合成或直接掉落物形态）；骨粉右键小麦/耕地作物推进 2-3 生长阶段（消耗 1 骨粉，未熟不可再催到熟的边界口径按 MC）；骷髅掉骨链盘点（若骷髅无掉骨则补掉落表）。探针：合成/催熟推进/耗尽/边界。**先盘点：骷髅掉落表、WheatCrop state 推进口的随机 tick 依赖面。**
  → **✅✅ 落地（2026-09-10，续接收口棒）：功能本体零改动收口 + 行为探针补链，矩阵 505→507 PASS / 0 FAIL。**（`fix(t1030)` d8e5756 + `test(t1030)` eaef577）**盘点回标（五面全在库 → 零 src 功能改动）**：①骨头 `BoneId=0x217`（recipe.h；获取链=骷髅死亡恒掉骨头 ×1-2（Main.qml MobBones 分支 spawnItem 0x217）+ 地牢/要塞/钓鱼战利品池（loottable.cpp）→ **掉骨链已通，零改动**）；②配方 1 骨头→3 骨粉 shapeless 2×2（recipe.cpp kRecipes 单表一行，2×2/3×3 通吃）；③WheatCrop=25 八阶段（`WheatCropStageMax=7`），`World::applyBonemeal` 统一入口确定性哈希骰子（hashVoxel(seed⊕使用序号×φ)→advance=2+(roll>>8)%2 ∈ +2..3 钳顶 7；成熟/非目标返 false 不耗不挥）——**生长推进零 Math.random，无随机 tick 依赖面**；④右键分流 playercontroller placeBlock 骨粉分支（材料段非方块在 `m_selectedBlock==Air` 守卫之前分流；生存 takeStack 1/创造豁免；200ms 冷却；命中即 return 不走放置）；⑤物品面：creativeMaterials 材料页（hotbar.cpp）+ MaterialIcon `drawBonemeal` + 名称表全在库。**本单增量**：①成熟施用口径登记钉死（world.cpp 早退语句旁 comment-only：二选一取「无效应不消耗」，非 MC 观感「消耗无生长」——d8e5756）；②探针 P-t1030a/b（+2，真玩家路径）：(a) 行为链（1 骨头 2×2+3×3 同命中恰 3 骨粉/2 骨头阴性；生存 stage2 连施恰 +2..3 带内含钳顶步、恰耗 1、id 不漂移、2-3 次封顶——数学保证与骰子分布无关；成熟再施=口径钉死；创造同链不消耗）；(b) 边界（右键石头不耗不响，瞄 +x 侧面入——瞄 y14.9 会先撞 y15 预置石侧面）+ 十针 pinSet 源码钉（分流判据/applyBonemeal 入口调用/成熟早退/推进语句本体/推进带常量/双 id 存档契约/配方 pattern+产物行）。**探针 rig 教训（阴性轮暴露，已入 lessons）**：真消费端 rig 必须显式建模 QML selectedBlock 绑定（Main.qml `selectedBlock: hotbarVM.selectedBlockId`，Hotbar::selectedBlockId 材料段→Air）——探针无 QML 接线时 `m_selectedBlock` 保持构造默认 Stone（playercontroller.h），阴性轮摘分支后 fall-through 走通用放置路径（放默认方块+扣选中栈）→ t1030b edge 腿假红（cnt 4→3、方块不动）；修=rig 构造后 `setSelectedBlock(Air)`。**阴性轮**：摘骨粉分流判据（false && 前缀）→ **恰红 506 PASS / 1 FAIL = P-t1030a**（grow/mature/creative 三腿红因正确：阶段冻结 + badCnt 0 = 零消耗；配方腿与 P-t1030b 保绿——world.cpp/recipe 层未动；matrix_t1030_neg.log）→ restore（Edit 反向）→ 终跑恒绿 **507/0**（matrix_t1030_pos/final.log 均 507/0，pos 首跑一次过——几何修正后）。**验证**：binary mtime > 全部改动源 mtime；app 目标 voxelsandbox 重建 EXIT=0 + offscreen 冒烟 EXIT=124 存活 12s + logs/voxelsandbox.log 尾部 20 行留存（smoke_t1030_voxelsandbox_tail20.log，review0910 #2 证据链新纪律首例执行）。**待实机确认（观感类）：骨粉图标观感、催熟阶段跳变观感、创造背包材料页骨粉呈现。**
...
**t1031 狼驯服**：骨头右键野狼概率驯服（MC 口径 1/3 尝试，烟雾/爱心粒子反馈）；驯服态=项圈红领（渲染面）+ 坐下/跟随切换（右键）+ 跟随传送边界；驯服狼参战（玩家攻击目标联动，复用 attack-target 体系）；野狼主动敌对口径（MC 狼中立方，不主动攻击）。探针：驯服概率缝（RNG 注入或统计窗）/坐下跟随/项圈渲染钉/参战。**分层注意：驯服态在 Entities，项圈渲染在 QML，骨道物品在 Game——沿 t1025 引诱表先例。**
  → **✅✅ 落地（2026-09-10，续接收口棒）：Entities 层增量 + 遗产收口验证，矩阵 507→509 PASS / 0 FAIL。**（`fix(t1031)` 9bc2c73 + `test(t1031)` 31bc798）**遗产盘点（五面全在库 → Game/QML 零功能改动）**：①骨头分流 playercontroller.cpp 骨头分支（`heldItemId == BoneId` 判据在 `m_selectedBlock==Air` 守卫之前；findMobHit 独立 mob 射线；命中未驯服狼 → tameWolf + **无论成败耗 1 骨头**（创造豁免）+ 挥手；已驯服 → no-op（t878①）；非狼/未命中 → return 不放置）；②空手坐站切换（同文件 `heldItemId==0` 分支 → toggleWolfSit，t831 补齐）；③参战接线（attackMob → `setWolfTarget(entityIndex)`，t480；+ t952 被咬仇恨双向注册）；④项圈（MobModel `collarVisible` 几何环带 subset + materials[1] 红领 + Main.qml `wolfTamedAt(index)` 绑定，t986 几何化收口）；⑤超距传送跟随（`kWolfTeleportDist=12`（MC 1.0 ≈32 格 × 小世界口径缩比，**登记所选值**）+ 近主人 isCollidable 安全位选点，t878 阈值收口）。**本单 src 增量（Entities 层）**：①驯服概率测试缝 `setTameRollOverride(int)`（Q_INVOKABLE；缺省 -1 = 生产零改动；≥0 接管 tameWolf 样本千分比 `(roll%1000)/1000.0 < 0.33` 驯中——同 setWanderFrozen/setBreedTimings/m_chickenJockeyChance 缝先例，生产零调用）；②野狼**中立方收口**（aiWolf 未驯服分支 → 纯 aiWander + `Q_UNUSED(playerTargetable)` 签名兼容；旧 t480 kWolfDetectRange 侦测追咬玩家分支整体退役、常量随之删除；**激怒反击面登记不做**）。**登记简化**：成功爱心沿在库（tameHeartTimer kTameHeartDuration=4s + inLoveAt）；失败烟雾粒子不做（日志即可，探针只钉状态面）；存档实体不序列化（t1013 口径；slotFingerprint 属 QML 差分指纹非持久化）→ 驯服态不持久化=登记，revision 不 bump。**探针 P-t1031a/b（+2，真输入链 + pinSet 剥注释）**：(a1) 缝 999 必败=骨头恰耗 1+不驯+无爱心沿；(a2) 缝 0 必成=驯服态位+爱心沿+5.5s 收心；(a3) 空手坐/站两向切换零消耗；(a4) 参战=beginMining 真攻击链 → 接线 → 狼咬僵尸掉血越过玩家空手一击+咬带内（minDz ≤ kAttackRange+0.1）；(a5) 野狼中立=targetable 玩家 3 格 6s 零 mobAttackedPlayer+XZ 逐位钉位（wander 冻结只关游荡、旧 chase 不受冻结影响=敏感性保留）；(b1) 骨头右键非狼不耗不放置；(b2) 空手右键野狼不驯不坐；(b3) 缝缺省零调用（setter/接管语句/声明/缺省 -1 四钉 + playercontroller 全文无 setTameRollOverride）；(b4) 接线语句本体四钉（骨头分支判据/空手坐站切换/attackMob 参战接线/Q_UNUSED 中立门）。**#14 rig 迁移**：r0830C(d)「攻击距离内不起跳」旧以野狼咬玩家供流，收口后改**驯服狼防御分支供流**（同 chase lambda 同咬带门；目标=静止猪 setWanderFrozen 冻结；咬击落地=healthAt 掉血计数）。**阴性轮**：摘驯服分流（`false &&` 前缀，t1030 先例；判据行保留 → b4 恒绿）→ **恰红 508/1 = P-t1031a**（diag tamed=false/sitOn=false=摘分流直接后果；b 腿与其余 507 全绿；matrix_t1031_neg.log）→ restore（Edit 反向）→ 终跑恒绿 **509/0**（matrix_t1031_pos/final.log 均 509/0，pos 首跑一次过）。**验证**：binary mtime > 全部改动源；app 目标 voxelsandbox 重建 EXIT=0 + offscreen 冒烟 EXIT=124 存活 12s + logs/voxelsandbox_t1031_tail20.log 留存（review0910 #2 纪律沿用）。**待实机确认（观感类）：项圈红领观感、驯服爱心沿观感、坐姿几何（MobModel sitPose）、超距传送观感、野狼中立观感。**
...
**t1032 掉落物 instancing 批 2 + review 登记收口**：3D 形状族 per-id 桶（ItemShapeGeometry 几何缓存共享）；**补桶满 8 降级路径行为腿**（≥9 种同屏整立方 id，第 9 种起 delegate 渲染不断链不双渲——P3 登记①）；**8×16ms 定时器收口评估**（共享单 ticker 或全桶空 `running:false` 门控——P3 登记②，若做须阴性轮定时器行为）。探针：族 2 分桶/降级腿/定时器门控腿。
  → **✅✅ 落地（2026-09-11）：批 2 形状桶 + review0910 #2/#3/#4 收口，矩阵 509→513 PASS / 0 FAIL。**（482b9fa / 1af5316 / 15fe901）**主面（3D 形状族 per-id 桶）**：feeder 扩维——BlockDropInstancing 增 `shapeFamily`（bool，缺省 false = 批 1 行为逐位不变；true = 收 isItem3DFamily 25 id）；两侧谓词同源三面钉（feeder 收纳侧过滤 / QML 重算侧 reassignShapeBuckets 薄委托 / delegate 排除侧 hasShapeBucket visible 链——全部回源 ItemEntityManager::isItem3DFamily 单一权威，批 1 hasBucket 纪律沿用）；Main.qml 第二桶池 host `blockShapeInstHost`（8 桶 Repeater，批 1 同构：在用桶保持 / 活体数降序 + id 升序 tiebreak / 整体数组替换保 notify / Component.onCompleted 兜底）；桶 Model 几何 = `ItemShapeGeometry { blockId: blockShapeInstHost.buckets[index] }` per-id 单几何单材质（桶内全实例共享 = 同 id 同 geometry，材质与旧 ItemShapeGeometry 分支逐字同参：NoLighting + Mask + alphaCutoff 0.5 + opacity 0.99 + voxelAtlas + 天光乘子 baseColor）；桶满 8 溢出 → delegate 旧路径保底（visible 链追加 hasShapeBucket 排除，不丢渲不双渲）。**附魔台 94 台顶书保留**：dropBookNode 上移 entRoot 直属（instancing 无法承载 per-instance 子树——94 入桶后 shape Model visible=false，书保留 delegate 逐实体渲染，桶内/桶外两路径观感一致），position 换算 `entRoot.bobY + 0.46 * 0.3` + 显式 scale 0.3（世界变换逐位不变，review27 #9 不预缩放口径沿）；review27-9 / t924 源码钉锚串合法演化（t965 先例：锚随实现迁移、查看器侧 etBookNode 字面量不动、断言不放宽仍逐字钉新公式 + 页坐标 count）。**review0910 #2（桶满降级）**：headless 可达面做腿 P-t1032b（9 种互异整立方 id 同屏——第 9 种 familyId=0 feeder 恒空表不双渲；桶释放→重指派→实例表恢复=活体数；再释放→复空；形状桶同语义含 -1 哨兵）；**QML 编排面盲区如实 scoped 不虚报**（reassign*/has*Bucket 纯 QML JS handler 不可行为级断言——维持源码钉 + 实机确认项 ≥10 种 id 定向冒烟）。**review0910 #3（定时器收口，做）**：空转门两池同款——16ms QTimer **构造不启钟**，refreshTicker 按「familyId>0 且桶内（按收纳谓词）有活体」启/停；活跃沿 = setManager / setFamilyId / setShapeFamily / manager entitiesChanged（活体集变更完备沿：spawn/removeAt/setCountAt/clearAll/despawn/物理位移全经 notifyChanged 单点 emit；setManager 换源先 disconnect 旧）start + markDirty 兜底防丢帧；空转沿 stop（~500 次/秒空唤醒 + 每帧空表重取收口，16 feeder 全量）。t858 XpOrbInstancing 恒走钟**不在本单范围**（登记后续顺手）。**review0910 #4（不让位）**：维持现状登记——kept 逻辑两池逐字同款，`if (id > 0 && counts[id] > 0) { next[k] = id; kept.push(id) }` 钉 minCount=2（future 让位逻辑加入即红）。**探针 P-t1032a/b/c/d（+4）**：(a) 25 id 逐 id 入形状桶条数=该 id 活体数（顺带压 setFamilyId 换桶沿）+ 楼梯 16（即便被误指派）/ 整立方 / 工具 0x100 / 材料 0x200 不进形状桶 + 火把不进整立方桶（跨族互斥双模式）+ 实例内容批 1 同参（XZ 精确 / bob 带 [pos.y, +0.15] / 0.3 均匀 / 纯 Y 旋转）；(b) 降级可达面（见 #2）；(c) 空转门状态机（构造 / 接线 / 指派无活体三停 + 首活体 entitiesChanged 沿启动 + 新鲜表即得 + 泵 50ms 活跃保持走 + 拾走停 + 泵不复活 + 形状模式谓词感知三停〔dirt 切形状模式 / 门在位 id 不匹配 / 本族入桶启动〕+ 构造体无 `m_ticker.start()` 结构钉）；(d) 同源钉 pinSet 剥注释 13 枚（host id / shapeFamily / reassign fn+call / hasShapeBucket 排除 / geometry per-id / familyId 绑定 / kept 不让位 ×2 / hdr Q_PROPERTY + probeTickerActive / feeder 过滤 + refreshTicker + start/stop + entitiesChanged 沿）+ 结构面（重算侧薄委托无字面量表、材质五参逐字同参窗）+ 几何共享等价断言（同 id 两实例 vertexData 全等、异 id 不等——「同 id 同 geometry 指针」的 headless 等价面；指针本体由「每桶单几何」结构钉承载，QML 场景树 headless 不可达如实登记）。**阴性轮两轮**：①摘形状过滤（`false &&` 前缀，判据行保留 → 钉腿恒绿）→ **恰红 512/1 = P-t1032a**（diag stairs=1 = 楼梯混入形状桶；matrix_t1032_neg.log）→ restore（Edit 反向）；②摘空转门（构造 start + refreshTicker 早退）→ **恰红 512/1 = P-t1032c**（其余腿与其余 511 全绿；matrix_t1032_neg2.log）→ restore → 终跑恒绿 **513/0**（matrix_t1032_pos/final.log 均 513/0）。**验证**：binary mtime > 全部改动源；app 目标 voxelsandbox 重建 EXIT=0（qmldir/qmlcachegen 过 = QML 语法构建期收口）+ offscreen 冒烟 EXIT=124 存活 12s + logs/voxelsandbox_t1032_tail20.log 留存（review0910 #2 证据链纪律沿用，运行日志零 QML 错误）。**rig 教训（首跑暴露，已修）**：空转门活跃沿腿 plain 模式 feeder 不可配 3D 族 id——torch 13 非 isPlainCubeDrop，谓词侧永不活跃恰是门的正确行为（首跑 activeEdge 假红暴露），改配 Dirt 桶后两模式各得活跃沿。**待实机确认（观感类）**：≥10 种 3D 族 id 定向冒烟（每掉落恰渲一次 + 94 书在位）、形状桶旋转/浮动动画一致性、94 桶内/桶外书观感一致、F3 draw call 前后对比。
...
**t1033 床锚自然破坏联动**：爆炸/岩浆等非玩家路径销毁锚床 → clearBedSpawn（review0909 #4；经信号/回调转发守分层——Entities 清块路径不可反向依赖 Game，沿 seedChanged 收口模式经 QML 编排或信号）。探针：TNT 炸锚床 → 重生点失效 + 播报恰一次；水冲不触（床非附着块不在 wash 清单，口径登记）。
  → **✅✅ 落地（2026-09-11，review0909 #4 清偿）：World 语义信号 + PlayerController 直连收口，矩阵 513→516 PASS / 0 FAIL。**（`fix(t1033)` 12f4558 + `test(t1033)` 14a789e + 本 docs）**盘点回标（契约先行盘点五项）**：①爆炸清块单点 = **World::destroySphereSilent**（Entities 侧 detonateTntSphere / detonateStalker 两爆炸入口共用 = 一处覆盖；球内逐格直写 + 末尾批量收口）；②t1024 全链现状：finishMiningAt 床分支（挖任一半 → 配对格联动清 + 锚判等 → clearBedSpawn + emit bedSpawnLost；Main.qml onBedSpawnLost 播「床被破坏，重生点已失效」）+ setBedSpawn/clearBedSpawn 幂等单点 + bedSpawnAnnounce（入睡设锚沿专发，勿混淆）；③seedChanged 收口实态 = PlayerController::setWorld 内 `connect(w, &World::seedChanged, …, Qt::UniqueConnection)` **C++ 直连**；④TNT 探针先例 t996：PrimedTnt 实体 + 固定 dt 细步驱动引信（无墙钟依赖，td trap 免疫）；⑤床两半判等先例 t1024（`x==anchor ∨ x+pd==anchor` 同层谓词 + bedPartnerOffset 单一权威）。**主修（双文件小增量）**：①World::destroySphereSilent 毁前 capture 床格 state（setBlock(Air) 前——id 变更重置 state=0 同门 t134）→ 世界写收口后逐格 emit **blockDestroyedBed(x,y,z,state)** 语义信号（消费者只见终态零重入面）；②PlayerController::onWorldBedBlockDestroyed（seedChanged 同款 setWorld 直连）：谓词与 finishMiningAt 逐字同构（y 同层 ∧ 本格 ∨ 配对格==锚格，配对偏移解自毁前 state）→ 命中 clearBedSpawn + emit bedSpawnLost（t1024 既有播报面自动生效，**零新增播报面**）；两半同爆双信号被 m_bedSpawnValid 门幂等塌缩成一次播报（挖掘链+爆炸链双路径同汇 clearBedSpawn 单点不双播报）；未设锚/非锚床早退零 emit。**选型登记**：契约给「Main.qml 编排转发」与「沿 seedChanged 收口模式」两变体——取后者字面（C++ 直连）：bedSpawnLost 须由 PlayerController 发（纯转发 clearBedSpawn 无播报面，违「自然破坏走同一收口即自动播报」），且矩阵 harness 为 C++ 直编不可达 QML 面；信号方向 World→PlayerController 与 seedChanged 同向，分层铁律不破（World 只发语义事件零反向依赖）。**自然路径口径登记（按「爆炸主面 + 其余登记」scope 不扩面）**：岩浆/火不毁床（flammable() 单一权威表不含床，焚毁路径不存在）；水冲清单 isAttachableBlock（火把/红石火把/蛛网/木梯）不含床 → 水冲不触（探针钉）；世界换代清锚走 seedChanged 既有链（刻意静默不播报，t1024 口径）；其余 setBlock(Air) 面（门/火蔓延/结构重生成等）皆特定 id 定向写，无通用毁床面（登记）。**探针 P-t1033a/b/c（+3）**：(a) 真引信链爆炸失效主腿（clearBlockSilent + spawnPrimedTnt 引燃统一尾 → ents.tick 0.015625×400=6.25s > kPrimedTntFuseSec 5s 固定 dt → detonatePrimedTnt）→ 床两半消失 + valid 置假 + lost 恰 1（双信号塌缩）+ spawnPoint 回 pristine (80,80,80)；(b) 幂等面两腿：未设锚炸床零 emit（lost/validChanged 全零）+ 同层 10 格外非锚床被炸锚纹丝不动（Y 门 + x/z 判等判别力）；(c) 口径钉：流水真流到床旁（邻格成流工作面）但床格仍 Bed + 锚保留零 lost，随后真注视挖掘链挖同一锚床 → lost 先水后挖累计恰 1（不双播报回归；t1024b 存量腿独立钉挖掘链本体）。**阴性轮**：摘 setWorld 内 blockDestroyedBed connect（注释掩蔽，restore 反向）→ **恰红 515 PASS / 1 FAIL = P-t1033a**（diag `broken true invalidated false lost 0 valid true` = 「床被炸了重生点还在」病灶本体重现；其余含 t1033b/c 与 t1024a/b/c 全绿；matrix_t1033_neg.log）→ restore（Edit 反向）→ 终跑恒绿 **516/0**（matrix_t1033_pos/final.log 均 516/0，pos 首跑一次过）。**验证**：binary mtime > 全部改动源；app 目标 voxelsandbox 重建 EXIT=0 + offscreen 冒烟 EXIT=124 存活 12s + logs/voxelsandbox_t1033_tail20.log 留存（app 自写日志尾部 20 行，零 QML/C++ 错误）。**待实机确认（观感类）：TNT 炸已睡锚床后死亡重生落世界出生点 + 播报文案观感（引擎侧已钉，实机一键确认）。**
...
**t1034 门/床/活板门 sneakPlace 存量缺门收口**：useBlock 三分支补 `!sneakPlace` 门（review0909 #2 存量登记项；对齐机关件模式；防「潜行持方块无法对门/床面放置」）。探针：三件潜行右键放置/非潜行正常使用两向。
  → **✅✅ 落地（2026-09-11，review0909 #2 存量登记项清偿）：placeBlock 右键链三分支补门，矩阵 516→519 PASS / 0 FAIL。**（`fix(t1034)` 181ad6d + `test(t1034)` 13347f9 + 本 docs）**盘点回标**：sneakPlace 定义 playercontroller.cpp :3511（`m_keys.value(Qt::Key_Shift)`，t523 口径）；缺门三分支 = 床 :3569（→trySleepAt）/ 门 :3590（isDoor ∧ ≠IronDoor）/ 活板门 :3611（WoodTrapdoor）；先例 = 同函数机关件八分支（工作台/熔炉/箱子/附魔台/铁砧/发射器/投掷器/音符盒）全部 `!sneakPlace` 前置；三分支**无**潜行空手特殊语义需保留（空手潜行 → 下方 `m_selectedBlock==Air` 守卫拦，t1028 音符盒同口径）。**主修（单文件 3 处条件前置 + 注释收口）**：三分支补 `!sneakPlace &&`；**床门选型登记**：门加 placeBlock 分支头而非 trySleepAt 内入睡窗口判定之后——潜行放置语义整链优先（夜门/雷暴门/怪物门一概不触达），且 trySleepAt 直调面（QML/探针）语义不变（MC：潜行右键床=放置，不睡）；铁门不受影响（t722 本就不吃右键、fall-through 放置）；t523 注释块「门/活版门/床不绕过」旧口径与 review0909 #2 存量登记注同步改写（旁路口径统一登记：机关/浆果丛/末地门/传送门仍不绕过）。**探针 P-t1034a/b/c（+3，真 placeBlock 链 + pinSet 源码钉，t1028b rig 惯例一行一探针）**：(a) 门——潜行持方块右键门板 +X 面 → 木板落邻格 + 两半合态 bit2 不动 + doorToggled 零发；非潜行对照 → 两半同翻 + doorToggled 恰 1 + 无放置（use 吞右键）；(b) 床——非潜行空手白天右键 → 拒睡文案「只能在夜晚或雷暴中睡觉」恰 1（契约口径：白天非雷暴拒绝睡=使用照常，不算放置失败对照）；潜行持木板 → 放置落侧邻格 + refused 计数冻结 + 不入睡 + 床两半原样；(c) 活板门——潜行持方块 → 放置 + 合态 bit0 不动 + 零 doorToggled；非潜行对照 → 翻板开 + 恰 1 doorToggled + 无放置。三探针各携本门行 pinSet 针（cpp-door/bed/trapdoor-sneak-gate）。**rig 教训（首跑阳性暴露）**：床碰撞盒 = 床垫低盒 y[0,~0.31]（ShapeBed t457/t496），aim y 15.5 掠过低盒侧沿命中顶面 +Y → 放置目标格变床顶格 (12,16,16) 潜行腿假红；修 = 床腿 aim 改侧脸带内 y 15.15。**阴性轮（单构建三摘一轮，t1028 先例）**：摘三门（恢复 pre-fix 条件行）→ 潜行右键重 routed 进使用——门开/板翻/床二次拒睡（refused=2=潜行点击进睡链病灶签名）+ 零放置 → **恰红 P-t1034a/b/c 516 PASS / 3 FAIL**（对照腿不受影响保绿；matrix_t1034_neg.log）→ restore（git checkout 反向）→ 终跑恒绿 **519/0**（matrix_t1034_pos/final.log 均 519/0）。**验证**：binary mtime > 全部改动源 mtime；app 目标 voxelsandbox 重建 EXIT=0 + offscreen 冒烟 EXIT=124 存活 12s + logs/voxelsandbox_t1034_tail20.log 留存（零 QML/C++ 错误；仅既登记的 Particles3D offscreen 降级 WRN）。**待实机确认（观感类）：潜行右键门/床/活板门放置落点观感、非潜行使用面回归观感（引擎侧已钉，实机一键确认）。**

**t1035 豹猫驯服**：生鱼喂食驯服（MC 口径走近缓慢喂食，驯服后信任态不跟随不项圈——与狼模型刻意分化）；生鱼来源盘点（若钓鱼无鱼则登记简化或补钓鱼产出）。探针：驯服/信任态/与狼的分化口径。
  → **✅✅ 落地（2026-09-11，R19.22 批次末项）：盘点零缺口 + 分化口径现状钉，矩阵 519→520 PASS / 0 FAIL。**（**零 src 功能改动**；`test(t1035)` 4459d33 + 本 docs）**盘点三问回标（先盘点后落锤，强预警命中——豹猫面大量在库）**：①驯服链现状 = **完整可达**：t481 生鱼概率驯服（kOcelotTameChance=0.33、失败仍耗鱼、毛色变体 0..2）+ t949 实机右键输入缝（eventFilter 生鱼/狼肉 mob-feed 分流 → placeBlock 生鱼分支自分流驯服/回血/求偶；P-t949(a) 真输入腿 + (b) 熟鱼阴性在库）+ t963 变体资产收口（全黑档退役 → 三色家猫花纹）；②**驯服后行为口径 = 现代 MC（生产现状 > dev-plan 设想）**：驯服猫**跟随**主人（kOcelotFollowSpeed=4.0 走近 / kFollowMinDist=2.5 停步 / >kOcelotTeleportDist=12 瞬移，t878⑤）+ 坐/站命令（t481 空手分支）+ **红项圈**（t963 用户返修「没看到项圈」双落点镜像补齐）+ 不防御不反击（t923c2）+ 驱赶 Stalker（t481）——本条目「信任态不跟随不项圈与狼分化」设想已被 t878（跟随+瞬移）/ t963（项圈）先行演化掉，**登记过时、行为零改动、探针按现状钉**（与狼真分化面 = 无防御不反击 / 生鱼驯服 vs 骨驯 / 三色家猫变体 vs 灰狼+项圈战斗伙伴）；③生鱼来源 = **钓鱼池在库**：LootTable::fishingPool() RawFishId 权重 60/108 最高（~55% 常见获物，t401 单一权威；钓起行为面 t836 系 poolIds 探针在库 + 熔炼生→熟/+2 饥饿 t836 口径钉在库）——**无需补产出**。**探针 P-t1035（+1，Entities 层行为腿 + 直调钉，平石台 rig）**：(a) 驯服猫坐态留守（toggleOcelotSit → 玩家 3 格外 1s 零位移，镜像 t831(c) 狼腿——猫坐态此前无行为钉）；(b) 站态跟随走近（玩家 7.5 格外 2s 位移 ≥2）；(c) 过远瞬移（>12 格 1s 内跳至 ≤10 环）+ 近距 6 走跟不跳变（8 帧 ≤1.0）；(d) 跟随态字段分化（wolfTamedAt/wolfSittingAt 对猫槽、ocelotTamedAt/ocelotSittingAt 对狼槽跨型互查恒 false——同槽真值只在本型 accessor，两链独立成证）；(e) 生鱼来源直调钉（fishingPool 含 RawFishId 且最高权重，随机池无关）。**阴性轮**：摘 aiOcelot 站态跟随 chase（`false &&` 前缀，t1030/t1031 先例）→ **518 PASS / 2 FAIL 恰红同缝两腿**：P-t1035(b)（猫不再走向主人）+ review0830 batch C (a) 生存对照腿（该探针同缝钉「creative/survival control closes to stop band」——盘点未检出的一处猫跟随既有钉，阴性轮实证其与 P-t1035(b) 共缝敏感）——同因两红非环境杂音，其余 518 全绿（matrix_t1035_neg.log）→ restore（**Edit 反向**，git diff 零残留）→ 终跑恒绿 **520/0**（matrix_t1035_pos/final.log 均 520/0，pos 首跑一次过）。**验证**：binary mtime > 全部改动源；新增警告 0（build log 3 条 QFile::open nodiscard 为 HEAD 既有探针代码，本单 diff +114 行不含）；app 目标 voxelsandbox 重建 EXIT=0 + offscreen 冒烟 EXIT=124 存活 12s + logs/voxelsandbox_t1035_tail20.log 留存（60fps profiler 稳态，零 QML/C++ 错误）。**R19.22 批次六单 + t1036 插单至此全闭环。**待实机确认（观感类）：驯服猫跟随/瞬移观感、坐姿与项圈观感（t963 已登记项顺验）。
...
- **#1（待定夺 → 定夺为修）→ ✅✅ 落地（2026-09-10，t1036）**：`bedSpawnValidChanged(bool restored)` 参数语义与 Qt NOTIFY 属性约定相悖（参数=来源沿≠属性新值；当前 QML 全命令式现读不咬人）。**立项 t1036**（t1030 后顺手小单）：信号改回无参 `bedSpawnValidChanged()` + 播报源另立 `bedSpawnAnnounce(bool restored)`（只在入睡设锚沿 emit）；改动面 ≈6 处（信号声明+4 emit+QML handler:3145+矩阵钉 hdr-bedSpawnValid-signal-src+P-t1024a (e) restored 断言改挂新信号）；阴性轮=摘播报信号腿恰红。
  → **✅✅ 落地（2026-09-10，t1036：信号职责分离，矩阵恒 507 PASS / 0 FAIL）**。①声明（playercontroller.h:712/:717）：`bedSpawnValidChanged()` 无参化（纯属性 NOTIFY，Qt 6.11「NOTIFY 参数=属性新值」约定，声明式绑定安全）+ 新增 `bedSpawnAnnounce(bool restored)` 播报源；②四 emit 点：trySleepAt 入睡设锚沿（:3212-3213）双发 `validChanged()` + `announce(false)`（恒发口径不变）；setBedSpawn 读档回填沿（:247）/ clearBedSpawn 置假沿（:259）/ sleepAdvanceToDawn 幂等守卫沿（:3134）仅无参 validChanged、零 announce；③QML（Main.qml:3147）：handler 改挂 `onBedSpawnAnnounce`（播报门 `player.bedSpawnValid && !restored` body 不变）；runExitSave :1031 / respawn :1327 两处命令式现读不动；④矩阵（`test(t1036)` d0cd4b3）：P-t1024a 探针双信号各自记录（validChanged 记发射时属性值——valid 沿时序不变 false/true/true；announce 单独记 restored），(e) 腿随迁（入睡沿 announce 恰一次 restored=false + 回填/置假沿零 announce），钉三处（hdr-bedSpawnValid-signal-src 新签名 + hdr-bedSpawnAnnounce-signal + qml-bedspawn-announce-handler）。**验证**：binary mtime > 全部改动源；阳性 **507/0**（matrix_t1036_pos.log 一次过）；阴性摘 `emit bedSpawnAnnounce(false)` → **恰红 506 PASS / 1 FAIL = P-t1024a**（diag `sleepAnnounce=false`，restoreQuiet/其余腿全绿；matrix_t1036_neg.log）→ restore（Edit 反向）→ 终跑 **507/0**（matrix_t1036_final.log）；voxelsandbox 重建 EXIT=0 + offscreen 冒烟 EXIT=124 存活 + logs/voxelsandbox.log 尾部 20 行留存（voxelsandbox_t1036_tail20.log）。非目标守约：bedSpawnValid 属性 / 存档四键 / respawn / 锚失效逻辑零改动，未加腿（507 基线保持）。
...
## R19.22 批次插单 t1037（批次 review A P3-2 定夺修；2026-09-11，MC 口径=床毁即醒）

**t1037 睡眠中锚床被毁即醒**：批次 review A 实读证据（HEAD=e673860）：玩家入睡期间锚床被爆炸摧毁（t1033 新路径 blockDestroyedBed → clearBedSpawn）时，clearBedSpawn 已清锚播「重生点已失效」，但随后的 sleepAdvanceToDawn（:3153-3155）**无条件重写 m_spawnPos / m_bedAnchor 回已毁床位并把 m_bedSpawnValid 翻回 true**（守卫沿静默）→ 重生点在已不存在床位上重新武装。禁改 sleepAdvanceToDawn 本体逻辑、禁动天气清态、禁动 bedSpawnAnnounce。
  → **✅✅ 落地（2026-09-11）：clearBedSpawn 单点睡眠中断收口，矩阵 520→521 PASS / 0 FAIL。**（`fix(t1037)` 1e7eca3 + `test(t1037)` 521ab79 + 本 docs）**盘点先行（clearBedSpawn 全调用面 ×「睡眠中」组合语义，统一收口定夺并登记）**：①`onWorldBedBlockDestroyed`（t1033 爆炸槽，:255）= **病灶本体**：睡眠 Settled 计时中锚床被炸 → 清锚后 m_sleeping 仍真 → Settled 计时满落 sleepAdvanceToDawn 重写已毁床位；②`onWorldSeedChanged`（换代，:233）：实际流程换代前必经 release()（:744 已 cancelSleep，「release 已清」既有口径）→ 睡眠中换代不可达；即便防御性可达，旧码同病灶（跳晨把重生点写回旧世界床坐标）；③`finishMiningAt` 床分支（:1739）：睡眠中输入域冻结（tickImpl :1247 m_sleeping 分支 updateSleep 后即 return，不进 pollMouse/mining）→ 挖床与睡眠互斥，零行为面。**定夺 = 统一收口**：clearBedSpawn 头部 `if (m_sleeping) cancelSleep();`（先走既有受惊醒路径瞬断睡觉序列再清锚；MC「床毁即醒」对一切锚失效路径成立，三调用方语义一致无特判，最小面一处 2 行）——中断后 m_sleeping 翻假 → updateSleep 早退 → sleepAdvanceToDawn 永不可达（绝不跳晨，醒时相位保持）；出床瞬移由 cancelSleep→leaveBedTeleport 承担（床已毁，扫床周首个可站位格，MC 下床语义）；非睡眠态 no-op（cancelSleep 自守），t1024 幂等面零变化；②③两调用方纯防御兜底。契约红线守约：sleepAdvanceToDawn 本体 / 天气清态 / bedSpawnAnnounce 三面零触碰。**探针 P-t1037（+1，行为级，t1033a 正交先例）**：锚床 setBedSpawn（读档恢复入口）→ 子夜 setPhase(0.5) + **setRunning(false) 冻结时钟**（「醒，非跳晨」相位判别器：skipToDawn 直写 elapsed 不经 QTimer，暂停态照样跳——冻结下相位从 0.5 挪走唯一来源就是跳晨）→ trySleepAt 真实入睡 → 17ms 泵至 Settled（2s 窗内，captured 路径 updateSleep 才跑）→ TNT 引燃（引燃链统一尾）→ ents 固定 dt 400×0.015625=6.25s > 5s 引信（**只泵实体不泵 PC**：Settled 窗 2s < 引信 5s，泵 PC 会让跳晨抢在爆炸前）→ 断言①床两半消失 + 醒（sleeping 翻假）+ 相位仍 0.5（非跳晨）+ valid 假 + lost 恰 1 + spawn 回 pristine (80,80,80) + 置假沿零 announce（t1036 口径）→ 再泵 PC ≈5s 冲过原 Settling 窗 → 断言②不重新武装（valid 仍假 / spawn 仍 pristine / lost 仍 1 / 相位仍 0.5）。**阴性轮**：摘 clearBedSpawn 睡眠中断（`false &&` 前缀，t1030 先例）→ **恰红 520 PASS / 1 FAIL = P-t1037**（diag `woke=false noRearm=false lost=1 valid=true spawn=20.5 41 20.5 phase=0.75` = 病灶全签名重现：睡未醒 + 跳晨 + 重生点重新武装回已毁床位；其余 520 全绿；matrix_t1037_neg.log）→ restore（**Edit 反向**，git diff 零残留）→ 终跑恒绿 **521/0**（matrix_t1037_pos/final.log 均 521/0，pos 首跑一次过）。**验证**：binary mtime > 全部改动源；app 目标 voxelsandbox 重建 EXIT=0 + offscreen 冒烟 EXIT=124 存活 12s + logs/voxelsandbox_t1037_tail20.log 留存（60fps profiler 稳态，零 QML/C++ 错误；唯一 WRN=既登记的 offscreen 关停 QEventDispatcherWin32 噪音）。**待实机确认（观感类）：入睡中锚床被 TNT 炸毁 → 立即醒在床边（黑叠层瞬隐）+「重生点已失效」播报 + 天色仍夜（不跳晨）——引擎侧已钉，实机一键确认。**

### R19.22 批次 review 收口（2026-09-11 14:30，双只读 reviewer + t1037 定夺修）

**reviewer 结论（窗口 2a12fc8..e673860）**：A 代码正确性 **pass**——P0/P1/P2 全 0，存档零触碰、t1036 四 emit 沿语义精确（QML handler 唯一换挂无残留）、t1031 缝缺省零调用+状态互斥、t1032 连接沿完备无丢帧面/三面谓词同源、t1033 判锚谓词与挖床链逐字同构+幂等塌缩+无重入面、t1034 三门在位（铁门豁免保留）；**预存在论断与 t1033 选型偏离独立坐实**（pickaxe 抽验 t480 ce279a3 / t481 5e98481 / t831 5f9a40c / t878 d9849e6 / t949 ab48c69 / t963 bf65c29 / 项圈 645d118 与 git 史全吻合；World→Game 信号方向与 seedChanged 同向不破分层）。3 P3。B 探针+台账 **pass**——0 P1/P2，22 份日志逐个实测零偏差，算术链 505→520 ✓，dt 坑全窗口零踩（0 处 mineBlockF，引信链全固定 dt 细步），QML 盲区如实未虚报。3 P3 钉法列明。

**发现处置**：
- **P3-2（A）定夺为修 → t1037**（1e7eca3/521ab79/c6672f5）：睡眠中锚床被炸 → clearBedSpawn 统一睡眠中断收口（`if (m_sleeping) cancelSleep()` 2 行，MC「床毁即醒」对一切锚失效路径成立），矩阵 520→**521**，阴性轮恰红含病灶全签名。
- **P3-1（A）→ 用户 Review_2026-09-11.md #1 定夺为修**：94 附魔台书桶内/桶外动画相位失锁（台体入桶走 feeder 独立时钟、书仍 delegate QML 动画相位=创建时刻；用户审查推荐修法=**94 整体排除出形状桶**——reassignShapeBuckets skip 94，hasShapeBucket 对 94 恒 false → 台+书恒走 delegate 同链同相，旧观感逐位恢复；94 低频掉落合批损失可忽略）→ **t1038 按此修法执行**。
- **用户 Review_2026-09-11.md 其余项处置**：#2（两池实机定向冒烟）维持实机待办（批 2 后盲区 ×2 已注）；#3（bedSpawnAnnounce dead 参数）登记维持；#4（爆炸半床残留）登记=本段 P3-3 同项。清偿确认对齐：review0910 #1/#3 已清偿、#2 部分、#4 维持、review0909 #2 已清偿——与 audit #4 一致。
- **P3-3（A）登记**：destroySphereSilent 球面切半床无 t428 式配对联动（球破坏既有行为，因 t1033 信号面变得可观察；锚语义仍正确——任一半毁即清锚）。
- **P3（B）三条登记**：t1031b 缺席断言裸 contains（pinSet 正向语义表达不了「全文不含」，方向安全）；t924 锚串迁移后新锚钉裸 contains（合法演化注释在）；三处 indexOf+mid 范围序断言结构性豁免列明。
- **台账备注**：t1032 条目 hash 已回填（482b9fa/1af5316/15fe901）；t1034 阴轮 restore 用 git checkout 违禁令字面（无损失）——已入 agent-state 纪律行，后续棒 restore 必须 Edit 反向。

**批次统计（2a12fc8..c6672f5）**：25 提交（fix 7 / test 8 / docs 10），矩阵 **505→521 PASS / 0 FAIL**（+16 腿：t1030 +2 / t1036 +0 / t1031 +2 / t1032 +4 / t1033 +3 / t1034 +3 / t1035 +1 / t1037 +1）；全程 offscreen、阴性轮齐（t1032/t1033/t1037 多轮）、app 构建+冒烟+logs/voxelsandbox_*_tail20.log 留证齐。**实机确认总表 16 项**（reviewer B ③节：t1030×3 观感 / t1031×5 观感 / t1032×4 含 review0910 #2 定向冒烟 / t1033×1 / t1034×1 / t1035×1）。

---

## R19.23 收官与稳健批（t1038-t1042，5 项；2026-09-11 立项，用户常设授权自动执行）

主线：instancing 治理收官（批 3/4 全族 O(桶)）+ review 遗留定夺 + 探针稳健性补丁。

**t1038 附魔台 94 排除出形状桶**（用户 Review_2026-09-11.md #1 定夺修，修法从其推荐：非锁相）：reassignShapeBuckets 计数循环 skip id 94（hasShapeBucket 对 94 恒 false → 台+书恒走 delegate 同链同相，旧观感逐位恢复；instancing 无法承载 per-instance 子树的正确推论=94 整体不桶化，94 低频掉落合批损失可忽略）。改动面：Main.qml reassignShapeBuckets 1 处 + P-t1032a「25 id 逐 id 入桶」腿同步改（94 改断言不桶化）+ dev-plan 登记。探针：94 有活体时 hasShapeBucket 恒假 + 书台同相位（delegate 同链）；阴性轮=摘 skip 恰红（94 入桶腿红）。矩阵基线 521。
  → **✅✅ 落地（2026-09-11）：94 整体不桶化（QML 桶池 skip + C++ 收纳侧同参排除两道），矩阵 521→522 PASS / 0 FAIL。**（`fix(t1038)` a1bc01c + `test(t1038)` f7bfe2b + 本 docs）**改动面**：①Main.qml reassignShapeBuckets 计数循环 `if (id === 94) continue`（登记注释「Review_2026-09-11 #1：instancing 无法承载 per-instance 子树，94 整体不桶化保书台同相」）——hasShapeBucket 对 94 随 skip 恒 false（无第二处特判，P-t1038a 源钉 hasShapeBucket 切片零 94 字面钉住该不变量）；台+书恒走 delegate 同链同相（entRoot 同一动画钟），旧观感逐位恢复；②blockdropinstancing.cpp getInstanceBuffer + hasLiveMember shapeFamily 模式同参排除 94（第二道防线：即便桶池误指派 94 也恒空表 + 空转门不走钟——review0910 #3 谓词同参纪律；生产路径 QML 桶池永无 94 桶，本层纯防御不可达）+ blockdropinstancing.h 批 2 头注释登记；③三处旧注释改写（shape host 大注释「两路径观感一致」→「整体不桶化」、delegate 侧排除注释、h 头注释）。**探针（+1 腿，521→522）**：P-t1032a「25 id 全员入桶」腿改写——94 改断言**不桶化**（isItem3DFamily(94) 仍真 = delegate 首闸保持 + 形状 feeder 对 94 恒空表期望 0；其余 24 id 不变）；新增 **P-t1038a 行为腿**——94 活体在场：feeder 查询面空表（「形状桶无 94 桶」的 headless 可达投影）+ 空转门同参停钟（无僵尸钟）+ 火把同场对照照常入桶走钟 + QML 编排面盲区源码钉（reassignShapeBuckets 切片含 skip + 登记注释；hasShapeBucket 切片零 94 字面）；P-t1032d 结构钉同步修订（94 skip = reassignShapeBuckets 函数体内**唯一豁免**的字面 id 比对且登记注释紧邻前置，其余字面 `id === ` 仍禁——防旁路谓词权威）。**阴性轮两轮（false && 前缀，Edit 反向 restore，全程未用 git restore）**：①摘 C++ 收纳侧 skip ×2 → **恰红 520 PASS / 2 FAIL = P-t1032a（diag famMiss 94）+ P-t1038a（noTable/noClock false）**，其余 520 全绿（matrix_t1038_neg.log）→ restore；②摘 Main.qml skip → **恰红 520 PASS / 2 FAIL = P-t1038a（qmlPin false）+ P-t1032d（结构钉 reassign false）**，其余 520 全绿（matrix_t1038_neg2.log；首轮误在未重建的病灶 binary 上跑出 3 红混轮，重建后单变量干净轮覆盖为准——process 教训：摘 C++ 病灶后的 restore 必须先重建再跑下一轮）→ restore。**验证**：binary mtime > 全部改动源（test exe 17:27 > blockdropinstancing.cpp 17:17；voxelsandbox.exe 17:46 > Main.qml 17:44）；阳性首跑一次过 **522/0**（matrix_t1038_pos.log）= 终跑恒绿 **522/0**（matrix_t1038_final.log）；voxelsandbox 重建 EXIT=0（build_t1038_app.log 零 error）+ offscreen 冒烟 EXIT=124 存活 12s + logs/voxelsandbox_t1038_tail20.log 留存（60fps 稳态，唯一 WRN=既登记 offscreen 关停 QEventDispatcherWin32 噪音）。非目标守约：批 1 hasBucket 桶池 / 94 之外任何族分桶 / dropBookNode 结构 / 锁相方案四面零触碰。**待实机确认（观感类）：丢附魔台 → 台体与小书同相旋转/浮动（书不嵌入台体）；94 不再占形状桶槽位——review0911 #2 的 ≥10 种 3D 族 id 定向冒烟项维持。**
**t1039 instancing 批 3：光晕壳族全族合批**（UnitCube 半透；per-instance color 承载附魔紫/灰 + 呼吸 opacity 进实例表；空转门同款）。探针：壳族分桶/颜色实例表/呼吸相位/跨族互斥。
  → **✅✅ 落地（2026-09-11）：光晕壳族全族单 instanced Model（GlowShellInstancing，1 draw/全族壳），矩阵 522→526 PASS / 0 FAIL。**（`fix(t1039)` 9dfd7d0 + `test(t1039)` 1fccb3b + 本 docs）**盘点结论（先行）**：旧 delegate entShell（Main.qml）**无 visible 条件** = 壳全族横切（每个活体掉落实体恒带一壳，与本体族无关）；紫 (140,64,230)/灰 (176,176,176) × tintBySkyLight（minLight floor 0.4）、灰静态 opacity 0.35、附魔呼吸 0.28↔0.45 双腿 800ms InOutSine（t696）；壳与本体同挂 entRoot（bobY/rotY 同锁）。**改动面**：①新增 `src/Game/glowshellinstancing.{h,cpp}`（Game 层呈现适配器，只读 ItemEntityManager pos/enchants/alive——附魔判定经既有 enchantsAt 与 QML entHasEnch 同源；全族单 Model 单实例表，壳无 per-id 几何/材质故**无需桶池**——「第三池」设计自由度，壳独立成池与本体族解耦）；②Main.qml entShell 追加排除链 `visible: !glowShellInstHost.hasShellAt(index)`（薄委托 feeder Q_INVOKABLE——两侧谓词同源最强形式，同 id 壳不双渲）+ 呼吸动画 running 追加 `entShell.visible &&` 门（t1023 隐藏停表先例；降级路径行为不变）+ 新增 glowShellInstHost 单 Model（UnitCube + 白基色 NoLighting；skyLight/minLight 绑 worldClock/window）；③CMakeLists 两目标列源。**机制**：per-instance color 承载 rgb×天光 k=minLight+(1−minLight)×skyLight + alpha（0.35 静态/呼吸解析式 0.28+0.17·½·(1−cos(π·s))，s=fmod(ph/0.8,2)，ph=t+slot×0.37 逐字对齐批 1/2 口径）；Qt 官方 **hasTransparency(true)** 使表 alpha 在不透明材质上生效；收纳=槽序前 **kShellCap=128** 活体（manager 上限 200，溢出真实可达 → 尾槽 delegate 壳保底不丢渲不双渲；壳排除链只挂 entShell 节点与本体 hasBucket/hasShapeBucket 链正交）；空转门 t1032 同款（无 familyId，任一活体=活跃沿）。**登记（口径变化，先断言后实现）**：壳-体相位脱锁**接受**——壳是独立半透 halo 非 t1038 不透明嵌入件；批 1/2 本体 feeder 同 slot×0.37 口径同实例化期构造（偏移 ms 级），delegate 保底本体最坏恒定 0.15 bob 差。**探针（+4 腿 522→526）**：P-t1039a 全族收纳+实例表内容（pos/scale 0.45/纯 Y 旋转/颜色实例表——**色彩断言按 Quick3D calculateTableEntry 线性落表口径镜像 sRGBToLinear 多项式**，= 渲染管线权威存储值、视觉与 delegate 材质路径同色；alpha 透传不进伽马）+天光两沿+拾取塌表；P-t1039b 谓词镜像（双 feeder 确定性同表）+越界/空槽/无 manager 恒 false+跨池正交（壳收火把 13 而本体整立方桶 13 恒空、泥土两池并存——壳体异件不双渲≠互斥）+死槽翻转/槽复用回升；P-t1039c 空转门三停一启+事件泵保持+色调沿不启钟+130 活体溢出面（恰 128 尾 2 槽 false）+拾头槽回补恢复+构造体无 start 源钉；P-t1039d 接线源钉 pinSet（QML 薄委托 9 针 + C++ 口径 12 针）。**阴性轮两轮（Edit 摘病灶/Edit 反向 restore/轮间先重建，纪律⑥）**：①摘空转门（构造 start+refreshTicker 早退）→ **恰红 525/1 = P-t1039c**（matrix_t1039_neg1.log）→ restore；②摘壳收纳 alive 过滤（false && 前缀）→ **恰红 524/2 = P-t1039a+P-t1039b**（matrix_t1039_neg2.log，P-t1039c/d 与 522 旧腿全绿）→ restore。**验证**：阳性首轮一次过 **526/0**（matrix_t1039_pos.log；首轮曾 525/1——颜色实例表按 sRGB 裸值断言被 calculateTableEntry 的 sRGB→linear 落表转换打红，查明 Qt 源码后改为线性镜像口径，属探针口径修正非放宽）= 终跑恒绿 **526/0**（matrix_t1039_final.log）；binary mtime > 全部改动源；voxelsandbox 重建 EXIT=0（build_t1039_app.log 零 error）+ offscreen 冒烟 EXIT=124 存活 12s + logs/voxelsandbox_t1039_tail20.log 留存（60fps 稳态；WRN 全部既登记/既存在：offscreen 字体目录 + QEventDispatcherWin32 + 2 条 Repeater3D Node-delegate 型警告（8 月 .bak 同型，批 1/2 桶 Repeater 既有）+ AmbientParticles size 警告（.bak 同在））。非目标守约：工具/billboard 族（t1041）、t858 XpOrb、数据链、F3 埋点零触碰。**待实机确认（观感类）：任意掉落物壳显/夜间变暗；附魔丢弃紫晕呼吸；壳-体 bob 相位脱锁观感（登记的接受口径）；>128 掉落物溢出壳保底。**
**t1040 rig 稳健性补丁**：t1026a/t945 等真消费端探针的 m_selectedBlock fall-through 盲区（t1030 登记：无 QML 接线时停在构造默认 Stone，阴性轮可能走通用放置扣栈假红）——受影响腿显式 setSelectedBlock/或断言消费面收窄；盘点全窗 fall-through 面，逐腿加固；不加行为。
  → **✅✅ 落地（2026-09-11，主控亲自收尾——实现棒在 docs 前撞 5h 限额，fix/test 合并提交 e9eec34 已落、验证已齐）：8 处探针显式持物加固，src 零改动零断言改动，矩阵恒 522 PASS / 0 FAIL。**（e9eec34 单提交 + 本 docs）**加固面（8 站点，全部 `setSelectedBlock(Air)` 建模 QML 材料/工具段→Air 接线 + 逐站点因果注释）**：t1005 烈焰弹双 PC（发射分支按 heldItemId 无条件 return 不读 selectedBlock，fall-through 不可达——兜底建模）；t949 喂食三分支生鱼/熟鱼/狼肉（eventFilter 喂食分流 + 食物分支 beginEating 不进 placeBlock）；t996 打火石（引燃分支按 heldItemId）；t1017 粉尘腿（清前腿置入的 WoodTrapdoor 残留 selectedBlock）；t1026a 锄/种子（含阴性腿「石头不耕/泥土不种」——heldItemId 分流且无条件 return，m_selectedBlock 从未被读，堵未来腿误放误耗）；t1028 空手调音（空手 fall-through 到 m_selectedBlock==Air 守卫，防 aim 落空踩构造默认 Stone）。**盘点结论**：真消费端右键链腿的 fall-through 风险面=材料/工具段手持物品（QML 绑定归 Air 而探针默认 Stone），方块段/空手腿无风险；阳性轮依赖意外扣栈的腿=零（加固后 522/0 与加固前逐位一致）。**验证**：矩阵阳性 **522 PASS / 0 FAIL**（matrix_t1040_pos.log，与加固前完全一致=纯 rig 前置证据）；binary mtime > 源（exe 19:17 > tools 18:13；voxelsandbox 19:33）；app 重建 EXIT=0 + 冒烟 EXIT=124 + logs/voxelsandbox_t1040_tail20.log 留存。**阴性轮豁免**（登记理由）：纯 rig 前置加固无新断言面，以「加固前后阳性全绿 + 盘点清单」为证据（dev-plan R19.23 段预登记的豁免条款）。**待实机确认**：无新增（rig 面）。
**t1041 instancing 批 4 收官：工具 3D 族（五几何+tier 色 per-instance color）+ 工具/材料 billboard 族（per-id 贴图桶）+ 异形 billboard 族**——全族批次后 item 面趋近 O(桶)，t1007 治理路径 a 终点；F3 draw call 对比列实机。
  → **✅✅ 落地（2026-09-12）：剩余三族全合批（工具 3D + 工具/材料 billboard + 异形 billboard），t1007 治理路径 a 终点——全族批次后 item 渲染面=feeder/池查询腿（桶数上界+溢出保底），矩阵 530→534 PASS / 0 FAIL。**（`fix(t1041)` + `test(t1041)` + 本 docs）**盘点结论（先行）**：②~⑤族 delegate 现状——工具 3D=六分支（五几何 type1-5 + 弓 type7，scale 0.45，tier 色映射 t472/t557/t589 × tintBySkyLight(k=minLight+(1−minLight)×skyLight)，不透明，继承 entRoot 自转；弓带子节点白弦 t330 蜘蛛丝白 245 不随 tier）；剪刀 type6/打火石 type9/材料段（isMaterial 单边 ≥0x200 含护甲）=BillboardQuad 0.3 + 朝相机 euler(camPitch, camYaw−rotY, 0)（子节点合成世界=Ry(camYaw)·Rx(camPitch)）+ alphaCutoff0.5+opacity0.99+flipV:false+ToolIcon/MaterialIcon sourceItem 64×64；异形（partial/cross/bed 非 3D）同 billboard 但贴图=iconSourceForBlock PNG（无 flipV 覆写）+generateMipmaps:false；钓鱼竿 type8 掉落 delegate **无分支**（t803 只补打火石）——如实维持不入族（登记，非本单范围）。批 1/2/3 三池结构盘点→**不复用：新增三池**（同构 per-id 桶池，moc 全量 include 教训照搬——两新 feeder 头直接 include itementitymanager.h）；工具 3D 五几何=独立几何类（Pickaxe/Hoe/Axe/Shovel/Sword/BowGeometry，非 ItemShapeGeometry）→ 桶 Model 按桶 id 的 toolType 切七单例几何（含弦）；billboard per-id 贴图两路盘点：atlas+UV 进实例表需自定义 shader=PLAN §2-A 禁区 → **选型=per-id host 桶池**（与批 1/2 同构，登记）。**改动面**：①ItemEntityManager 三谓词单一权威（isTool3DDrop=isTool∧type∈{1,2,3,4,5,7}／isIconBillboardDrop=(isTool∧type∈{6,9})∨≥MaterialIdBase／isBlockIconBillboardDrop=(partial∨cross∨bed)∧!isItem3DFamily，QML 薄委托同源）；②`src/Game/tooldropinstancing.{h,cpp}`（工具 3D 桶 feeder：tier 色×k 进 **per-instance color**〔t1039 color 实例表先例，材质白基色=旧「材质 baseColor=tier×tintBySkyLight」等效面〕+ **stringPass 第二实例表**承载弓弦——instancing 不承载 per-instance 子树〔t1038 先例〕，弦独立 Model 同变换口径仅色异，两 feeder 同帧构造 ms 级相位脱锁=批 1/2/3 已登记接受口径）；③`src/Game/billboarddropinstancing.{h,cpp}`（billboard 双池单 feeder 类：**itemIconFamily** 模式开关〔批 2 shapeFamily 先例〕+ **camPitch/camYaw Q_PROPERTY 进实例表 rotation=(camPitch,camYaw,0)**——朝相机旋转的逐位等效面，相机沿 setter markDirty；billboard 不自转〔旧分支显式抵消 rotY〕，bob 同批 1/2 公式）；④Main.qml 十分支排除链（工具六分支 !hasToolBucket×6／剪刀/打火石/材料 !hasItemBucket×3／异形 !hasIconBucket×1）+ 三新 host（toolDropInstHost/itemIconInstHost/blockIconInstHost 各 8 桶，reassign 薄委托谓词；图标池 wrapper Item 双图标显隐切 ToolIcon/MaterialIcon）。**空转门**：两新 feeder t1032 同款（构造不启钟/谓词感知/活跃沿 start+markDirty 兜底/probeTickerActive 钉/构造体无 start 源钉）；既有三池零触碰。**探针（+4 腿 530→534）**：P-t1041a 工具桶收纳+互斥（剪刀/材料/火把/钓鱼竿误指派恒空）+实例数=活体数+拾取塌表+变换逐字（XZ 精确/bob 带/0.45/纯 Y 自转）+tier 色线性落表镜像（木 138/90/46、铁 216/216/230、弦白 245+alpha1.0，P-t1039a sRGBToLinear 口径）+天光两沿（0.5→r=97 还原逐位）；P-t1041b 双池互斥（火把不进异形桶/异形 id 不进图标桶/工具 3D+整立方不进图标桶——双向）+换桶沿+朝相机 rotation=(25,40,0)+camYaw 沿跟随（40→90 markDirty 路径）+scale0.3+拾取塌表；P-t1041c 三池空转门三停一启+事件泵+谓词感知（剪刀/竿桶恒停恒空、弦表只服务弓桶、itemIconFamily 切换停/复钟）+构造体无 start 源钉；P-t1041d 接线源钉 pinSet（QML 27 针：三 host id/reassign 薄委托/排除链 6+3+1/familyId 绑定/几何 per-id/stringPass/itemIconFamily/camPitch·camYaw/图标 wrapper+头三谓词声明+C++ 口径 23 针：收纳谓词/tier 色映射逐字/弦白/公式/scale/天光 k/空转门/entitiesChanged 沿/billboard rotation）+异形材质同参 window 切片。**rig 教训（登记）**：t256 slot-reuse——rig 首实体被拾后槽 0 被后续 spawn 复用（LIFO），按「第 N 个 spawn=槽 N」断言拾取沿会错拾（首轮 533/1 = P-t1041c bIdleEdge 恰红根因）——修正槽号注释非放宽。**阴性轮两轮（Edit 摘/Edit 反向 restore/轮间先重建，纪律⑥）**：①摘空转门（构造 start+refreshTicker 早退）→ **恰红 533/1 = P-t1041c**（matrix_t1041_neg1.log）→ restore；②摘双 feeder 收纳谓词（false && 前缀）→ **恰红 531/3 = P-t1041a+b+c**（c 的三处谓词感知空表面 shearsPred/rodPred/bModeIdle 恰红——收纳谓词正是其保证面；纯钟态面与 P-t1041d 全绿；matrix_t1041_neg2.log）→ restore。**验证**：阳性 534/0（matrix_t1041_pos.log）= 终跑恒绿 **534/0**（matrix_t1041_final.log）；binary mtime > 全部改动源；voxelsandbox 重建 EXIT=0（build_t1041_app.log 零 error）+ offscreen 冒烟 EXIT=124 存活 12s + logs/voxelsandbox_t1041_tail20.log 留存（60fps 稳态；WRN 仅既登记 QEventDispatcherWin32 offscreen 关停噪音）。**QML 编排面盲区如实登记**：reassignToolBuckets/reassignItemBuckets/reassignBlockIconBuckets/has*Bucket 纯 JS 编排 headless 不可行为级断言（review0910 #2 同型）——源码钉（P-t1041d）+ 实机确认覆盖。非目标守约：xporb、数据链（拾取/合并/物理）、F3 埋点结构、壳族零触碰；桶满 8 溢出 delegate 保底不丢渲不双渲。**待实机确认（观感/性能类）：F3 draw call 前后对比（93 items 场景 item 面趋近 O(桶+壳)，9 桶+壳 1 draw 上界）；工具 tier 色与弓弦观感；弓弦-弓身相位脱锁观感（登记接受口径）；billboard 朝相机与旧版一致性；图标池切桶重绘观感；桶满 8 降级路径观感。**
**t1042 中立生物受击口径修正**（2026-09-11 用户叫停纠正原口径：「牛羊被打反击绝对不是我的世界原版」——原条目凭印象把被动型写成反击型，作废重写）：**MC 原版口径 = 牛/羊/猪/鸡纯被动，受击只惊逃（panic flee，加速游离玩家），永不反击；会反击的只有野狼**（中立，被打→敌对反击玩家）；**豹猫被打只逃不反击**；驯服狼被打→不反击主人。**实现面**：①被动型惊逃——受击沿设惊逃态（方向=离玩家，加速/时长登记，MC panic ~8s 量级），复用游走/移动基建；②野狼反击——受击沿 chasing=玩家（复用 t1015/t1031 体系），超时/超距清除同 hostile 收口；③豹猫受击惊逃（与①同机制）；④驯服狼被打→零反击（t1031 豁免维持）；⑤幼崽不反击只惊逃（登记）。探针：打牛→惊逃（远离玩家位移+零反击）、打野狼→敌对反击、驯服狼被打→零反击、豹猫→逃逸；阴性轮=摘惊逃/反击分支恰红。基线 526。
  → **✅✅ 落地（2026-09-12）：被动型受击惊逃（panic flee）+ 野狼唯一反击（MC 原版口径），矩阵 526→530 PASS / 0 FAIL。**（`fix(t1042)` + `test(t1042)` + 本 docs）**盘点回标（先行五面）**：①受击沿=PlayerController::attackMob（唯一生产入口，t242/t866）——既有 setWolfTarget（t480）/setGolemRetaliate（t635）写点先例，本单追加第 3/4 写点；②被动型现状=受击零反应（仅 t249 通用击退）；③豹猫现状=aiOcelot 未驯服分支纯 aiWander（t1031 后无任何受击反应）；④惊逃最小侵入点=t1025 引诱/寻偶同款「钉 yaw+wanderSpeed+wanderTimer → 位移交 aiWander」；⑤驯服狼被打现状=对攻击者零反击已成立（aiWolf 无此分支）——登记：主人打自家驯服狼时 setWolfTarget 共享目标语义会让**其它**驯服狼追咬该狼（t480 既有口径，非本单范围不动）。**本单增量（Entities 层为主）**：①`Entity::panicTimer`（struct 末尾 DMI 纪律）+ 常量 `kPanicDuration=8.0f`（MC panic ~8s 量级）/`kPanicSpeed=2.0f`（2×kWalkSpeed 疾走，慢于 kChaseSpeed 2.8 可甩脱）；②`setPanicFlee(i)`（类型门收口：pig/cow/sheep/chicken + **未驯服豹猫** + 狼幼崽；敌对/造物/驯服狼/野狼成体/驯服猫 no-op）+ `aiPanicFlee` 共用移动器（每 AI tick 衰减 + 钉 yaw=atan2(pdx,pdz) 背离玩家 + 疾走 + wanderTimer=0.4 防重掷，位移交 aiWander 复用碰撞/边界/冻结缝）；③三消费点置顶（通用被动链 if/else 重排、aiOcelot 未驯服分支、aiWolf baby 前置子句——**狼幼崽恒驯服**（t480 父代继承）故前置子句惊逃窗尽后落回跟随语义）+ 羊吃草门读 panicTimer（惊逃打断吃草，eatTimer 冻结保留）；④`setWolfProvoked(i)`（驯服/幼崽豁免门）+ aiWolf 未驯服分支反击面：chasing **只**由受击沿置位（旧 t480「见人就咬」不复活——侦测带内仅**续期**已有记忆、带外 kChaseMemory 衰减、归零弃追同 hostile 收口）→ 追击玩家（复用 t923 chase lambda：越障跳/泳跃同款）+ 近距咬击 `mobAttackedPlayer(kWolfAttackDamage=4)`（wolfAttackCooldown + t321 全局节流同 hostile）+ `!playerTargetable` 清反击态（aiIronGolem `golemAngry && playerTargetable` 同款门）。**Q_UNUSED(playerTargetable) 随 t1042 退役**——P-t1031b `cpp-wolf-neutral-gate` 针**重钉**为 `if (!e.wolfTamed) {`（反击只走受击沿挑逗不见人就咬的中立门语义不变；t1031「激怒反击面登记不做」由本单按用户纠正口径清偿）。**探针 P-t1042a/b/c/d（+4，真 attackMob 链 + pinSet 剥注释）**：(a) 牛——空手命中恰扣 1HP + panicTimerAt==8.0（时长登记面）+ 3s 逐 0.5s 窗增益 ≥0.4 且总增益 ≥3.5（panic 2 b/s vs wander 上限 1 b/s → wander 噪声免疫判据）+ moveSpeed ∈[1.4,2.6] 疾走带 + 零 mobAttackedPlayer + 8.5s timer 恰归零 + 0.7s 后 moveSpeed ≤1.05（重掷回落）；(b) 豹猫——同链逃逸三面 + 零反击；驯服狼对照腿——被主人命中恰扣 1HP、零惊逃态、6s 零 bites（t1031 豁免维持）；(c) 野狼——命中存活后 6s 内 ≥1 次 mobAttackedPlayer（≥1HP、type==MobWolf）+ 狼-玩家最小 XZ 距 ≤1.7 咬击带（t1031a 参战腿判据）；幼狼——t400 真链繁殖（enterLoveMode×2）产崽 + 摘除双亲（防命中幼崽的 setWolfTarget 泼到驯服双亲污染惊逃位移）+ 命中后 panicTimer==8 + 2s 增益 ≥2.5 + 零 bites；(d) 接线源钉 12 枚（两 wire 语句/三定义/狼豁免门/惊逃三消费谓词 minCount=3——`false&&` 前缀不摘语句=阴性轮恒绿恰红律）。**新增读口**：`Q_INVOKABLE float panicTimerAt(int)`（同 healthAt 读口家族，矩阵钉时长登记面+超时回落腿）。**阴性轮（单构建三摘一轮：aiPanicFlee 三消费点 `false &&` 前缀，Edit 反向 restore，轮间先重建——纪律⑥）**：→ **恰红 527 PASS / 3 FAIL = P-t1042a/b/c 惊逃腿**（panicTimer 不衰减恒 8.0 + 零惊逃位移；t1042d 针腿与其余 526 全绿；matrix_t1042_neg.log）→ restore → 终跑恒绿。**验证**：阳性首跑一次过 **530/0**（matrix_t1042_pos.log）= 终跑恒绿 **530/0**（matrix_t1042_final.log）；binary mtime > 全部改动源；voxelsandbox 重建 EXIT=0（build_t1042_app.log 零 error）+ offscreen 冒烟 EXIT=124 存活 12s + logs/voxelsandbox_t1042_tail20.log 留存（60fps 稳态；WRN 仅既登记 offscreen 降级：Particles3D 两处 + QEventDispatcherWin32 + delegate 提示）。非目标守约：群体仇恨/掉落表/敌对 AI/豹猫繁衍零触碰；tamed 成年猫不惊逃（跟随语义不变，MC 1.0 猫被打不逃口径登记）。**待实机确认（观感类）：打牛/羊/猪/鸡 → 疾逃 ~8s 回落；打野狼 → 被追咬；驯服狼/猫被打观感不变；幼崽受击惊逃观感。**

执行顺序：t1038 → t1040（稳健插段）→ t1039 → t1042 → t1041（性能大项殿后）。矩阵基线 **521**。纪律沿用：offscreen + 阴性轮 + Edit 反向 restore + 验收日志带任务号 + logs tail 留证 + 每单闭环同步 agent-state。盘点先行（t1039/t1041 光晕壳与工具族现状、t1042 中立反击面、t1038 书节点结构）。

### MC parity 专审（2026-09-12，用户要求排查同类偏离；窗口 657bba1^..39afab6 120 笔）

**结论：verdict pass，0 高 / 3 中 / 7 低**——「被动反击」同类方向性错误无第二例；窗口内 9 个行为提交是朝 MC 修的（t1017 仙人掌 / t1034 潜行旁路 / t1037 床毁即醒 / review0909 雷暴翻案 / t1031 野狼改中立），新写机制常量普遍对 MC 数值钉。**系统性风险=「内部登记≠用户知情」**（3 项中级全部是「代码注释登记了但用户不知情」——牛羊反击事故的同构隐患）。

**整改（立即生效）**：①建 **docs/parity-ledger.md 用户可见偏离台账**（单源：待用户裁决 3 项 / 刻意偏离 10 项 / 待考据 2 项）；②**流程硬门固化**：任何 parity 影响面定案必须写三元组（MC 版本基准 + wiki 出处 + 翻案/维持结论）；凡简化/豁免/kept-as-is 统一入台账，不入台账的偏离按 MC 行为实现；批次收口前跑台账差集核对。③**三项中级不自动实现**（产品裁决权在用户，裁决菜单在 parity-ledger「待用户裁决」节）：
- **裁-1 铁轨水蚀豁免**（中）：MC 流水冲毁铁轨；工程为保护矿井 worldgen 轨刻意豁免——裁决「入水毁族+矿井防水」or「保留豁免」。
- **裁-2 蜘蛛爬墙**（中）：MC 蜘蛛垂直爬墙；工程只有越障跳，且与 dev-plan t285「可爬墙 ✅」矛盾——裁决「实现爬墙」or「改 t285 表述定不爬」。
- **裁-3 耕地踩踏/水蚀退化缺失**（中）：MC 耕地被踩/被水冲回泥（可毁苗）；工程整段缺失——裁决「立项补链」or「登记缺失」。
7 项低级发现（幼崽血量减半 / 音符盒缺 hat 族玻璃播鼓 / 拉杆按钮不旁路 / 种子 1-3 基准待定 / 天气剩余时长不持久 / 成就发明超集+ride_minecart 偏松 / 结构门控环境音）已入台账「刻意偏离-登记」或「待考据」节，其中低-3（拉杆/按钮补 !sneakPlace）与低-2（glass→hat 族）为小修，可随后续小单顺手清。

### 用户裁决落地（2026-09-12）：「一切按照我的世界原版的准则来开发」——偏离即 bug，队列开发完后自动修

裁-1/2/3 全部定案为**按原版实现**，与 7 项低级一并列为 **parity bug 队列**（t1043-t1046），在 t1041 收官后、R19.23 批次 review 前插入执行；台账（docs/parity-ledger.md）状态同步更新。

**t1043 铁轨水蚀 + 矿井防水**（裁-1）：Rail 入水毁族（isAttachableBlock 单一权威）+ placeMineshaft 生成期防水（水下段不放轨/轨下垫不刷层——盘点后选最小面，保 worldgen 轨存活=防水是伴随义务非豁免）。探针：流水毁轨腿 + 矿井轨 worldgen 防水腿（海列/水种子专测）；阴性轮=摘生成期防水恰红矿井腿。
  → **✅✅ 落地（2026-09-12）：铁轨三族入水毁附着族 + placeMineshaft 生成期干燥门防水（伴随义务），矩阵 534→536 PASS / 0 FAIL。**（`fix(t1043)` + `test(t1043)` + 本 docs）**改动面**：①blockregistry.h isAttachableBlock 追加 `|| isRail(id)`（普通轨 103 / 动力轨 127 / 探测轨 128 三族单一权威一并纳入；review0906 #10 的「矿井轨水蚀豁免」废除——parity-ledger 裁-1 用户定案「一切按 MC 原版」，流水冲毁全部三种铁轨 wiki 已引证；轨族 dropId=自身 → 掉落链免费成立）；②world.cpp placeMineshaft 轨铺设重构为「候选收集 → walk 完成后统一干燥门落块」：hash 保留率判定改登记 railCandidates（不落块），pieceCorridor / 起点厅 / 途中事件全部 carve 完后对每个候选核查「本格仍 Air ∧ 切比雪夫距 2 的 5×5×5 邻域无水」才 setBlock（半径 2 = 全部直接 breach 几何覆盖；登记残余：距水 >2 的轨不在门内，全量防水=运行期流场模拟超出最小面定案）；连接位由下方 t565④ 统一重算覆盖不受影响。**探针（+2 腿 534→536）**：P-t1043a 流水冲毁铁轨腿（三族 dropId=自身，流水进入轨格 → setWaterSilent + blockDroppedAsItem 掉落链对齐玩家挖除）；P-t1043b 矿井 worldgen 防水腿 + 双机制源钉（12 个全新生成 128×128×64 世界全轨干燥扫描〔数千轨防 vacuous〕+ blockregistry 家族接入行 / world.cpp 候选行·干燥扫描·跳过·落块语句 pinSet，comment-immune）。**阴性轮两轮**：轮1 摘 blockregistry `|| isRail(id)` 家族接入行 → 恰红 5（t1043a/b + t1012c + 病灶自身 pin 面——3 腿 5 子计：t1043a 谓词+wash 双子计、t1043b 扫描+源钉〔病灶行 pin hdr-rail-family-join 在 miss 集〕、t1012c 谓词+wash 双子计，matrix_t1043_neg1.log 在案；枚举补全 t1012c——review0913-B 勘误）；轮2 摘 world.cpp 干燥门（`false && wet`）→ **恰红 535/1 = t1043b 唯一红**（t1043a 与 534 基线全绿，matrix_t1043_neg2.log）。matrix_t1043_neg2rc.log = 恢复会话首次 neg2 崩溃残段（520 PASS 无 total 行），留存作证据不删（review0913-B P3-4 登记）。**断链与恢复（如实登记，用户 0912 评审指认）**：断链会话阴性轮2 构建在 [35/37] 中断（遗留僵尸 cmake/ninja 进程锁 build 目录未灭），干燥门变异残留工作区且 agent-state 误报 READY/工作区干净 → 恢复会话按用户评审流程执行：agent-state 置 RECOVERY_REQUIRED（防再断链）→ Edit 反向恢复变异（纪律③，禁 git checkout）→ 重建（build_t1043_recovery.log，僵尸进程清理后）→ 阳性轮 pos2 **536/0**（matrix_t1043_pos2.log）→ 重注入干燥门病灶 → 重建（build_t1043_neg2.log）→ 阴性轮2 恰红 535/1 → Edit 反向恢复 → 重建（build_t1043_final.log）→ **final 536/0**（matrix_t1043_final.log，binary 12:29 > 全部源）。**环境坑（登记）**：恢复会话沙箱对后台任务内 stdout/stderr 文件重定向全面失效（shell 直接重定向 / cmd //c / tee 三式全部 0 字节蒸发），改用 **PowerShell Start-Process detached 跑法**（进程脱离会话树，stderr 实时落盘 logs/neg2_err.log → 归档 matrix_t1043_neg2.log）——后续会话长任务跑矩阵沿用此法或 nohup。**遗留进程清剿**：断链会话多个挂起脚本层（cmake/ninja 僵尸 ×2、遗留 bash 后续步骤自发启动矩阵 ×2）逐一定位 taskkill（wmic 按 CreationDate/ParentProcessId 甄别）。**非目标守约**：玩家后放轨 / 玩家引水冲轨 = MC 原版照冲（生成期防水非运行期豁免）；轨下垫层方案未采（最小面=干燥格放置）。**待实机确认（观感类）：worldgen 矿井轨在含水洞穴带出现率下降观感；流水就近冲毁玩家轨行为**。
**t1044 蜘蛛爬墙**（裁-2）：MobSpider 贴实体方块面垂直攀爬（MC 速度量级登记）；dev-plan t285「可爬墙 ✅」表述勘误同步。探针：蜘蛛撞墙贴面净爬升腿；阴性轮=摘爬墙分支恰红。
  → **✅✅ 落地（2026-09-14）：aiHostile 三条追击分支（仇恨狼/铁傀儡/玩家）水平位移双轴皆撤回（撞墙挡死）时调 aiSpiderWallClimb 贴面攀爬脉冲，矩阵 536→538 PASS / 0 FAIL。**（`fix(t1044)` + `test(t1044)` + 本 docs）**机制**：被阻轴贴面前探列（halfW+0.25，越过 AABB 前沿一个 AI 步长——固定 0.6 偏移对宽体 mob 停位缝隙临界漏探，阳轮首跑实证后修正）在脚位层或身体层有可碰撞方块（isCollidable 与撤回判定同源）→ vy=kSpiderClimbSpeed（名义 2.4 b/s 登记，≈kChaseSpeed 0.86×「≈平地或稍慢」口径）+ 解除 resting；AI tick 4 帧窗内重力衰减后 vy 恒正（0.61>0）不触发落地扫描 vy<0 snap 回弹 → 净爬升 ≈1.2 b/s；爬到墙顶水平试探自然解锁 → 越檐走既有追击水平移动（登记简化：无独立檐机制）。**边界登记**：能力门仅 Spider 家族（MobSpider+MobCaveSpider，MC cave spider 同样爬墙，同批纳入）；其余敌对不入口；aiWander 游荡不调（MC 蜘蛛游走不爬墙）。**探针（+2 腿 536→538）**：P-t1044a 蜘蛛 2 高 1 宽墙阻路净爬升（maxY≥87.0 墙顶带 + 爬升先于越檐时序 + 越檐后终点距玩家 ≤1.2；固定 dt=0.016 细步泵）；P-t1044b 洞穴蜘蛛同场景对照 + 僵尸位（Shambler）不爬升对照（maxY≤86.0 恒墙根）+ pinSet 源钉（爬墙调用 ×3 三分支全接线 minCount=3 + isClimber 门 + 脉冲语句 + header 声明；仇恨狼/铁傀儡两分支 headless 无对照腿 → 结构钉补位，t1047 O-4 先例）。**阴性轮**：摘 isClimber 门（false && 前缀）→ **恰红 536/2 = t1044a/b 唯二红**（536 基线全绿，matrix_t1044_neg.log 在案）→ Edit 反向恢复（纪律③）→ 重建 → 终跑 **538/0**（matrix_t1044_final.log，binary > 全部改动源）。**t285 勘误**：本单落地起「可爬墙 ✅」表述成立（t285 当时仅落地快速移动，见任务表行勘误注记）。**待实机确认（观感类）**：蜘蛛追击爬墙越檐的动画观感（攀爬帧腿摆走追击速 moveSpeed 语义）与洞穴蜘蛛同能力实机表现。
**t1045 耕地踩踏/水蚀退化**（裁-3）：实体踩踏概率回泥（带苗毁）+ 流水冲耕回泥；探针：踩踏概率缝 + 水冲回泥腿。
  → **✅✅ 落地（2026-09-14）：耕地踩踏概率回泥（玩家+mob 同公式同缝）+ 流水冲耕转换回 Dirt（转换非掉落），矩阵 538→541 PASS / 0 FAIL。**（`fix(t1045)` + `test(t1045)` + 本 docs）**三元组（踩踏概率）**：基准 MC Java（1.x 起公式稳定）/ minecraft.wiki w/Farmland Trampling 节「jumps/falls on the block (with chance equal to distance fallen - 0.5)」/ 结论=P=clamp(fall−0.5,0,1) 落地；**维基缺口注记（流水冲耕）**：Java/Bedrock wiki 均无「流水毁耕」条目，本腿按裁-3 定案口径实现（台账登记，非维基引证）。**改动面**：①World 层单一掷骰权威 `farmlandTrampleRoll(fall)`（kFarmlandTrampleFallMin=0.5 概率地板=走路并入恒不踩；缝 `setTrampleRollOverride` 千分比，t1031 setTameRollOverride 同式，生产零调用走全局 RNG=实体物理交互随机性范畴）+ 玩家踩踏分支概率化（playercontroller t639④「fall>1.0 恒踩坏」确定性门退役——100% 踩坏偏离 MC 概率口径，本单清偿）；②mob 落地沿踩踏（entitymanager t970 落地沿锚点：支撑格捕获 restCellY + 同公式掷骰 + setBlockSilent 回 Dirt + 新信号 `farmlandTrampledByMob`→PlayerController::onMobTrampledFarmland 直连清苗 + dropCropDrops 弹落——掉落表不出 Game 层的分层口径；mobGriefing 门与 Java modern 0.512 尺寸豁免门不入：工程无 gamerule 系统 / Beta1.0 基准无尺寸门，均登记）；③流水冲耕（tickWaterFlow `tryWashFarmland` **转换变体**：扩散落点耕地 → setWaterSilent(Dirt,0) 湿润 state 随 id 消失、**不灌水**（Dirt 实体挡水）+ 全程零 blockDroppedAsItem——与 t1012/t1043 附着块「冲毁变掉落物」分型；**静水源（level 0）邻接不冲**选型=保 hydration 农田依水而建基建面，仅流水（state>0）接触冲耕；上方作物保留=同 MC 干化回土口径，毁苗只属踩踏链）；踩踏毁苗走既有 dropCropDrops（t1026 单一权威，成熟 1 麦+1-3 种 / 未熟 1 种）。**探针（+3 腿 538→541）**：P-t1045a 玩家真物理下落（跳高带 fall≈1.26 → P≈0.76 <1 缝两端可判）缝0必踩（湿耕地回 Dirt state0+苗清+恰 1×SeedId）+ 缝999必不踩；P-t1045b 蜘蛛落地踩耕（缝0 必踩 + farmlandTrampledByMob 直连弹苗恰 1×SeedId）+ 0.06 微落差概率地板对照（地板内缝0也不踩）+ pinSet 源钉 14 针（掷骰本体/缝消费/玩家分支/mob 分支/回土/发信/信号槽声明/水冲两调用点/转换应用/静水源门/直连×2）；P-t1045c 流水冲湿耕地→Dirt+零掉落信号+水不入格+干耕地同道退化+静源豁免对照。**阴性轮（一构建双摘：掷骰本体 false&& + 水冲两调用点 false&&）**→ 恰红 **538/3 = t1045a/b/c 唯三红**（538 基线全绿，matrix_t1045_neg.log）→ Edit 反向恢复（纪律③）→ 重建 → 终跑 **541/0**（matrix_t1045_final.log，binary 07:07:56 > 全部改动源 07:07:34）。**连带修复（review26-1 rig 适配，断言未放宽）**：耕地走廊腿僵尸出生高 kRigY+2→kRigY+1——概率化后 2 格高落 P≈0.56 概率踩坏脚下耕地（feetOff 断言被新 MC 行为打破）；该腿验证面是「行走贴真顶」非「坠落」，出生降入概率地板（落差 0.0625 恒不踩）恢复确定。**过程注记**：阳性首跑 539/2 暴露两探针面缺陷——t1045c rig 水源放地板层（y=84 面上）下坠排空不蔓延（rig 几何 bug，改地板 y=83 / 水耕地同道 y=84 grounded）+ review26-1 行为性打破（上注），非机制缺陷；修后 pos 541/0 一次过。**验证**：voxelsandbox 重建 EXIT=0 零 error（build_t1045_app.log）+ offscreen 冒烟存活 13s 杀灭（60fps 稳态 prof 多窗；WRN 仅既登记 offscreen 降级：Particles3D 两处 + delegate 提示；smoke 重定向 0 字节为 t1044 既录环境坑，tail20 从 app 日志 logs/voxelsandbox.log 取）+ logs/voxelsandbox_t1045_tail20.log 留存。**非目标守约**：干/湿退化概率差异、村民踩踏特判、与 t1042 惊逃联动（惊逃乱跑踩耕地=自然发生）零触碰；作物踩踏以弹落为准。**待实机确认（观感类）**：跳跃踩农田概率回泥观感（跳一次 75%，不再必坏）；mob 路过农田偶发踩坏；瀑布浇耕地 / 流水漫田渐次回泥观感。
**t1046 parity 小修合集**：①拉杆/按钮补 `!sneakPlace` 旁路（t1034 同式）；②音符盒 glass→hat 第四族（倍移近似；Glass 脱 GroupStone 前先盘点 materialGroup 影响）；③幼崽血量回成体上限（繁殖+蛋孵两沿）；④天气剩余时长持久化（world_meta 追加键，存档契约尾追加）；⑤小麦种子基准钉死（wiki 三元组引证：Java 现代 1-4 / 旧版 0-3，按项目基准版本取值，探针随钉）；⑥ride_minecart 对齐 MC 1km 累计 + 发明成就 UI 标注「原创」（既往批准功能不删除）——**t1048 勘误：⑥口径两轴皆偏**（MC On A Rail 真口径 = 距乘车起点单方向 ≥500 米，见下 t1048 段）；⑦**音符盒 noteClips 析构补齐**（用户 0912 评审 #4：AudioManager::Data 新增 25 个 `Clip noteClips[kNotePitchCount]`，析构只释放旧音效池后直接 ma_engine uninit——补 `for n in kNotePitchCount: ma_sound_uninit(&d->noteClips[n].sound)` 对齐旧池释放模式，资源生命周期面）。

### 用户评审落地（2026-09-12 上午，流程裁决 + 新发现登记）

用户对 09-08～09-12 窗口（67 提交）做了独立评审：提交流程 8/10、测试与证据链 7/10、当前代码可验收性 5/10、长期重构方向 4/10。落地项：

- **流程裁决（已执行）**：t1043 断链恢复 = 严格证据链重走（变异恢复 → 重建 → 阳性 → 重注入 → 阴性恰红 → 恢复 → 重建 → final，日志/binary/源时间戳三方对齐后才提交）；agent-state 一致性硬门入 Workspace Guard 纪律⑦。
- **规格变更门（新增，纪律⑧）**：实现偏离原计划文字时，必须先写 SPEC_CHANGE（dev-plan 任务行改写 + 偏差点明示）再实现，禁止在收口文档中单方面回溯改口径。案例：t1028 计划原文「运行期正弦合成 piano-ish 音色，不出新 wav」，实际 816cb9a 落地为 25 个离线生成 WAV + 文档事后解释为「离线管线兜底 B」——机制本身已验收不回滚，但流程缺口由此门补上。无人值守期间凡遇「实现必须偏离计划文字」情形：低风险（纯实现细节、无行为/口径变化）可明示登记后继续；涉及行为口径 / 资产形态 / 用户可见面的偏离一律挂起待用户裁决。
- **实机确认项追加（用户评审 #3，t1041 相关）**：批 1/2/4 六个桶池 Repeater 的 3D delegate 挂载面需实机渲染验证——rig 冒烟 tail20 历来有「Repeater3D Node-delegate 型警告 ×2」（8 月 .bak 同型），且新桶 Model 无旧 delegate 的 `parent === null → itemHost` 兜底（t53/t64 教训）；若确认孤儿 → 掉落物合批面整体不可见。验证法：F3 draw call 前后对比 + 丢落物看合批 Model 是否渲 + 控制台孤儿告警清零。同轮验证 Review_2026-09-12 #1（t1039 壳排除链非反应绑定 >128 溢出双壳/丢壳）与 #2（stringPass 废钟）。
...
**t1047 GOV-20260912-1 纠偏闭环**：
  → **✅✅ 落地（2026-09-14 主控亲自收尾——实现棒限额死亡后接手）：四修全落地，矩阵恒 536 PASS / 0 FAIL，四项敏感性各自实证。**（ef7dd0b fix + 71140d7 test + 本 docs）O-1 壳排除链 revision 触碰（review0912 #1）+ P-t1039d 针改写；O-2 弦 feeder 弓门 + P-t1041c 新增 strBowGate 行为面；O-3 狼退出惊逃集（SPEC_CHANGE@t1047，常设裁决解）+ P-t1042c 改写零 panic；O-4 headPitch 门 + cpp-headpitch-panic-gate 结构钉（headless 无吃草态缝如实 scoped）。**阴性证据链（三轮）**：neg1 四摘 533/3（t1039d/t1042c/t1042d 红，O-2 缺席）→ **O-2 面空转实锤**：剪刀 toolType 6 不在 isTool3DDrop（{1-5,7}），弦 feeder 对剪刀桶族谓词先拦——切剪刀 id 的断言无论门在与否恒绿，证明力零 → 探针面改承重设计（切桶目标 PickaxeWood + spawn 活体）+ **槽位号修正**（额外 spawn 推偏后续 hardcoded 槽，t256 教训复训；**neg2 归因勘误（review0913-B P2-1）**：矩阵 diag 实测 neg2 时 strBowGate=false **且** bIdleEdge=false **双因在场**——弓门真信号 + 槽偏假信号叠加，非旧文「neg2 的 t1041c 红即槽偏所致假信号」的单因归因；neg3 前修复槽偏后单摘单红归因才纯净）+ **去 count 过约束**（缺陷面=废钟非表条目，表由 QML visible 兜底）→ neg3 仅摘 O-2 **535/1 恰红 t1041c** → restore → 终跑 **536/0**（matrix_t1047_pos/neg1/neg2/neg3/final.log）。**验证**：binary mtime > 全部改动源；voxelsandbox 重建 EXIT=0 + offscreen 冒烟 EXIT=124 存活 12s + logs/voxelsandbox_t1047_tail20.log 留存。**过程注记（主控接手实录）**：实现棒 09-12 下午限额死亡，WIP=四修复本体（已核无变异残留）；09-13 上午蓝屏一次（cc1plus 峰值 11GB，历史 4.5GB——编译内存压力诱因，再发改 -j 1）；蓝屏后重建意外已过（EXIT=0）直接进验证阶梯。**待实机确认**：>128 掉落物壳显/丢壳不再永续（review0912 #1 同项）、非弓工具桶弦钟归零、驯服幼狼被打零反应、惊逃羊头回正。**GOV-20260912-1 闭环确认：audit #5 YELLOW 纠偏完成，回原队列。**

**t1046 parity 小修合集（七项收官）**：
  → **✅✅ 落地（2026-09-14 主控亲自收尾——实现棒于重启+限额双重事故死亡，WIP 七项面完整存活）：矩阵 541→545 PASS / 0 FAIL。**（51cc43c 六项 gameplay + 309a69d noteClips + f9effa4 test + 本 docs）①幼崽血量回成体满血（繁殖+蛋孵两沿，长大还原链退役）；②音符盒 glass→hat 第四族（id 直判不动 GroupStone，倍移近似登记）；③拉杆/按钮 `!sneakPlace` 旁路（isManualIgniter 单一权威三类机关，t1034 同式）；④天气剩余时长持久化（weather_timer_ms 键尾追加，恢复精确续跑=MC RainTime 口径）；⑤小麦种子基准钉死 0-3（~Beta/1.0 基准，wiki 三元组引证：Java 1.0+ 现代=1-4、Beta/基岩=0-3；0=颗粒无收合法，t1026 旧 1-3 两版皆非退役；**review0913-A/B 勘误：「现代=1-4」旧读存疑撤回——复核现代 Java 亦 0-3，与钉死值一致**）；⑥ride_minecart 对齐 MC「On A Rail」1km 累计 + 发明成就描述标注（原创）（不删除既往批准功能；**t1048 勘误：1km 累计口径两轴皆偏，改距乘车起点单方向 ≥500 米径向**，见下 t1048 段）；⑦noteClips 析构补齐（app 独有编译单元，app 重建 EXIT=0 即编译验证）。**阴性轮**：六摘一构建 → **恰红 538/7**（t1025a=①回插 / t1026b=⑤ pin / t1028a+t1028c=②hat 翻红两腿 / t1046a=③ / t1046c=④ / t1046e=⑥；⑦ 契约豁免无探针面）→ Edit 反向 restore → 终跑 **545/0**（matrix_t1046_pos/neg/final.log）。**t1046e 原创标注断言抽查面（review0913-B P3-2 登记）**：3/9 项抽查（spider/dungeon/cart 三正例 + wood/fish 两反例），余 6 项由 achievements() 描述同源生成面覆盖（UI 单一来源，description 即 toast/面板文案权威），如实 scoped。**验证**：双目标串行重建 EXIT=0（-j 20 上次 OOM 失败，-j 1 稳）；binary 14:31 > 全部改动源；冒烟 EXIT=124 + tail20 留存。**待实机确认（观感类）**：跳跃踩田 75% 回泥手感、玻璃下 hat 脆响、潜行右键机关放置、天气续跑、骑矿车 1km 成就节奏、发明成就「（原创）」标注。**parity 波（t1043-t1046）至此全清：裁-1/2/3 + 台账 7 低全部闭环。**

**顺序（终）**：t1046✅ → **R19.23 批次 review（双只读）+ 修复 + 统计 + audit #6** → R20 重构主线（R20.03 测试分层起）。矩阵基线 545。

**t1048 批次 review 收口（review0913-A P2-1 主体 + P3/台账清偿全量）**：
  → **✅✅ 落地（2026-09-14）：On A Rail 口径勘误（500m 单方向径向制）+ 批次 review P3 清偿，矩阵恒 545 PASS / 0 FAIL（改写不加腿）。**（183782b fix + a72ca74 test + 本 docs）**主体（A P2-1）**：MC 1.0 On A Rail 真口径 = 「乘矿车到达距**乘车起点**单方向 ≥500 米的一点」（wiki 引证 reviewer A），t1046 首版「累计 1km」阈值与判据两轴皆偏。修法：①阈值 kMinecartRideKm=1000 → **kMinecartRideGoal=500**（改清晰名）；②判定改**径向位移制**：新埋点 onMinecartRideStarted(x,z)（Main.qml ridingCart 上升沿携当帧玩家水平坐标记起点——分层选型登记：QML 同持边沿与坐标两源，PlayerProgress 零向上依赖；新成员 m_minecartRideOrigin + active 判定窗）+ onMinecartMoved 签名扩 (delta, x, z) 每帧携位置采样，水平距离**平方** ≥ 500² 解锁（平方比较免开方，恰阈缝 FP 精确；「单方向 = 径向距离」口径选型登记）；③minecartTravelBlocks 降级 display-only 统计（负增量仍不计里程、径向采样照走——回折不缩径向）；④loadVariant 清判定窗（径向状态不入档）。**探针（改写不加腿，545 恒）**：P-t1046e 改写——非累计制钉死（折返 600 累计 / 径向 0 仍锁）+ 恰阈 500 ≥ 缝（半精度可表示值 300/0/99.5/400.5/500 校准，加速缝全精去噪）+ 起点未记录防伪解锁（判定窗门）+ 统计 1600 round-trip + 描述钉 500m 口径；t1020 腿随签名联动（径向 600 越阈避开校准缝）+ moved 路由 pin 随签名。**阴性轮**：摘恰阈缝判定（径向比较 ≥→>）→ **恰红 544/1 = t1046e 唯一红**（diag: unlock=false@恰500、travel/load/desc/pins 全绿=病灶仅触缝；matrix_t1048_neg3.log）→ Edit 反向 restore → 重建 → 终跑 **545/0**（matrix_t1048_final.log，binary 17:33:31 > 全部改动源）。**阳性轮（如实登记）**：matrix_t1048_pos.log 首跑 544/1——**review26-1 农田走廊腿翻红（feetOff 1.19347 越障跳瞬态）**，同 binary 重跑 matrix_t1048_pos2.log **545/0 全绿**；该腿为**批次 review 新发现（登记待办）**：预先存在的环境敏感面（新 TU 布局下 3/4 翻红、失败轨迹逐位一致、同 binary 有全绿反例、与 RNG 状态不相关）——疑似 walk/support 路径潜伏 UB（未初始化或布局敏感 FP）被 t1048 重编译布局暴露，**非 t1048 行为面**（t1048 改动=注释+矿车成就逻辑，与该腿零语义交集），建议单开小单 UBSAN/布局排查。**其余清偿（本 docs）**：A-P3-1 entitymanager 算术注释勘误（0.61→0.53，入 fix commit）；A-P3-2 onMoved 同帧双喂登记不修（台账新行）；A-P3-3+B-台账3 low-4 引证行勘误；B-台账2 low-1..6 全翻牌带 commit 号；B-P3-1 t1043 neg1 红集枚举补 t1012c；B-P2-1 t1047 neg2 双因归因勘误；B-P3-4 matrix_t1043_neg2rc.log 残段登记；B-P3-2 t1046e 抽查面 scoped 登记。**验证**：voxelsandbox 重建 EXIT=0 零 error（build_t1048_app.log）+ offscreen 冒烟 timeout 12 EXIT=124 存活 + logs/voxelsandbox_t1048_tail20.log 留存（60fps 稳态；smoke 重定向 0 字节为 t1044/t1045 既录环境坑，tail20 从 logs/voxelsandbox.log 取）。**待实机确认（观感类）**：骑矿车 500m 单方向解锁节奏（原 1km 节奏减半，径向制下绕圈不再是有效进度）。**人工待办移交主控**：批次统计 + audit #6；review26-1 潜伏 UB 排查立项。

### R19.23 批次 review 收口（2026-09-14 18:00，双只读 reviewer + t1048 勘误单）

**reviewer 结论（窗口 a833004..8f7d9df，实为 17 笔 6 fix/5 test/6 docs）**：A 代码正确性 **pass**——存档/状态机/水系耦合/渲染音频生命周期/事故面五维全净，预存在论断与恢复链自洽；1 P2 + 3 P3。B 探针+台账 **fail→P1 当场闭合**——抓到 **t1046 收口漏提交 Main.qml**（C++ 删 onRodeMinecart 槽、HEAD QML 仍调用，真机 TypeError；agent-state 误报干净违反纪律⑦）→ **补提交 a121581 闭合**（探针运行时读工作区文件，545/0 证据链对提交后状态成立）；2 P2（neg2 双因归因失实 / ledger low 行未翻牌）+ 4 P3 全部并入 t1048 清偿。

**批次统计（a833004..4d584ee）**：21 提交（6 fix / 5 test / 10 docs），22 文件 +1875/−262（src+tools 17 文件 +1746/−231）；矩阵 **534→545 PASS / 0 FAIL**（+11 腿：t1043 +2 / t1044 +2 / t1045 +3 / t1046 +4；t1047/t1048 改写不加腿）；全程 offscreen、阴性轮齐（t1047 三轮链为最曲折也最扎实段——B 专项复核结论「自洽且最终证明成立」）、app 构建+冒烟+tail20 留证齐。**实机确认总表 16 项**在 reviewer B 报告（agent-state 摘录）。

**遗留移交**：**review26-1 农田走廊腿环境敏感翻红**（新 TU 布局 3/4 红、失败轨迹逐位一致、同 binary 全绿反例、RNG 无关——疑似 walk/support 潜伏 UB 被重编译布局暴露，非 t1048 引入）→ **GOV-20260914-1 = t1049 UBSAN/布局排查单**（见 audit #6）。

**t1049 GOV-20260914-1 纠偏闭环（review26-1 农田走廊腿排查）**：
  → **✅✅ 落地（2026-09-14 续棒——实现棒 1 限额死亡后接手）：根因定位＝非 UB（「疑似潜伏 UB」假设证伪），矩阵恒 545 PASS / 0 FAIL（修复不加腿、断言未放宽）。**（48c2f09 fix + 3351a91 test + 本 docs）**根因链（代码级 + 数值 + 取证三重验证）**：review26-1 腿 (a) 下半砖僵尸（zA）跨段泄漏进腿 (b)——slab→farmland 换地后 zA 脚位（砖顶 kRigY+0.5）留在耕地格内部（新支撑真顶 +0.9375 高于脚位）→ aiHostile **玩家路径**越障跳探（entitymanager.cpp :4156，无攻击距离门——与仇恨/golem 分支不同、贴身照探）把**自身所在耕地格**判墙（isJumpObstacle :40-52 真顶 > 脚位+1e-3）→ 虚假起跳 + t670 滑流（:8254 应用，贴身越顶后回漂）落回走廊中段 → 落地 fallDist≈0.75 触发 t1045 踩踏掷骰（world.cpp :2458，QRandomGenerator 全局 RNG，P=fall−0.5≈0.25）→ 掷中则走廊耕地变 Dirt 满格台阶（+0.0625）→ 腿 (b) zB 走到台阶**合规越障跳** → feetOff = 离散弧顶采样（kJumpSpeed 8.4 / kGravity 28 / dt 0.016 半隐式欧拉 19 tick ≈1.19347）>0.02 翻红 = t1048 pos.log 的 544/1。**对 t1048 登记的两点归因勘误**：①「与 RNG 无关」不成立——同 binary 红绿翻转就是踩踏掷骰 RNG（取证：pos.log 与 pos2.log 构建戳同为 15:43/git a121581 = 同一 binary 一红一绿）；②「跨 binary 失败轨迹逐位一致」恰证**非 UB**——1.19347 是跳跃物理常数决定的离散弧顶，binary 无关；「环境/布局敏感」实为腿逻辑自身（泄漏+掷骰），与 TU 布局无关（t1048 布局 3/4 红 = 级联重掷提高命中率，非布局暴露）。**UBSAN 登记：不可行**——Qt mingw1310_64 与系统 mingw64（15.1.0）均**不附带 libubsan/libasan 运行库**（实测 ld: cannot find -lubsan；两工具链全盘无 ubsan/asan），契约替代探针组因根因闭环无需实施。**修复（48c2f09）**：腿 (a) 断言后、换地前 `ents.removeEntityAt(zA);` 遣散泄漏 mob——腿 (b) 从净实体态起：zB 出生落差 0.0625 → 踩踏掷骰 p≤0 概率地板提前返回（:2461）→ **全腿零 RNG 消费确定化**。**结构钉（3351a91）**：pinSet 自文件自钉 "review26-1-legb-despawn" → "ents.removeEntityAt(zA);"（105 个 pinSet 首例钉 tools/ 自身，路径 exeDir/../ 约定；理由：泄漏在「掷骰未中」run 下行为断言无法区分已修/未修，只有源钉能把回归变红）；提交树与 3× 验证树逐字节一致（md5 d25cdc1a）。**引擎侧真 quirk 登记（R20 walk 重构输入，本单不动行为面）**：mob 被支撑抬高型方块放置嵌入时，自格被跳探判墙 → 原地跳 + 反向滑流 + 落地触发踩踏级联（生产面 = 玩家在 mob 脚下换方块可诱发跳+践踏作物；修复归 R20 统一处理）。**复现刻画**：修复前当前 binary 连跑 5/5 全绿（matrix_t1049_pos_r1-r5.log，r1=上一棒遗产；P(红)≈0.25 首掷 + 掷中后级联重掷 P≈0.69 与 5 绿/t1048 布局 3/4 红两观察自洽）。**验证**：-j 1 重建（binary 22:34 > 全部改动源）→ **3× 全矩阵全绿 545/0**（matrix_t1049_final1/2/3.log）→ voxelsandbox 重建 EXIT=0 + offscreen 冒烟 EXIT=124 存活 + logs/voxelsandbox_t1049_tail20.log 留存。**GOV-20260914-1 闭环确认：audit #6 YELLOW 纠偏完成，R20 开工前置门解除。**

**顺序（终）**：t1048✅ → t1049✅（GOV-20260914-1 闭环：非 UB，review26-1 腿确定性修复 + 结构钉）→ R20 重构主线（R20.03 测试分层起——测试大 TU 拆分 + 矩阵 --filter 两项提速投资首落地）。矩阵基线 **545**。

### R20.03 测试分层（2026-09-15，测试大 TU 拆分 + 矩阵 --filter 两项提速投资）

  → **✅✅ 落地（2026-09-15）：tools/redstone_matrix_test.cpp 单 TU（3.4MB / 47,999 行 / -j 1 编译 ~85min）拆为 tools/matrix/ 分层结构 + 矩阵 --filter 子串选腿，矩阵恒 545 PASS / 0 FAIL（腿文本/断言/执行序零改动）。**（09b8cb7 refactor + 3daab66 feat + 本 docs）

**目标 A：拆分（09b8cb7）**。结构 = `matrix_main.cpp`（app + 返回码）+ `matrix_helpers.{h,cpp}`（include 枢纽 + 原匿名命名空间共享件 tickN/kRigY/SourceDef/RecvDef/fishCatchSpeedMirror/SrcPin/pinSet + 原 main 局部共享 rig 状态成员化 `MatrixRun`：w/slotIdx/信号计数器/totalFail/nextSlot/placeRigBlock）+ **8 段 TU**（原 main 体 L296-47995 连续行区间：section01_redstone_core 409KB / 02_early_probes 427KB / 03_mid_probes 437KB / 04_instancing 476KB / 05_render_ui 437KB / 06_worldgen_drag 468KB / 07_chests_mobs 444KB / 08_recent 304KB，全部 ≤500KB 目标）。段函数按原执行序 runAll() 依序调度——**腿间状态依赖与执行序零改动**（review26-1 跨段泄漏教训）。

**拆分不变量证据链**：①切段前 md5 存证（build/r2003_proof/seg01-08.raw），8 段拼接 == 原 main 体（md5 **82ac70c7ede1a2ed647fea738734be6b**）；②搬移后逐段 diff 终检：除 **18 行登记随迁**外逐字节恒等——14 处 `__FILE__` 相对路径 `../src/ui` → `../../src/ui`（tools/matrix/ 深一层；**首跑 530/15 实锤即此**：QML real-chain harness 的 temp 转存全断 = t873/t874/t875/t953/t956/t962/review0830-23/t966-t978 族全红，属自读面随迁豁免项非腿文本改动）+ review26-1-legb-despawn 结构钉自读路径随迁（`tools/redstone_matrix_test.cpp` → `tools/matrix/section02_early_probes.cpp`，t1049 钉随迁契约）；③**全矩阵 2× 545/0**（matrix_r2003_split_run2/run3.log），PASS 行与 t1049 权威基线恒等——唯一漂移 = t813 构建戳时间/哈希 + t997 腿名内嵌实测毫秒数，与 t1049_final1 vs final2 自身漂移**同类**（预存在运行期变量，非拆分引入）。

**目标 B：--filter（3daab66）**。main 解析 `--filter <substring>`：358 个顶层腿块包 `runLegMulti({全部静态腿名}, [&](){腿体原文})`——任一腿名含子串即整组执行（多腿共享 rig 作用域按块级门控）；未命中组逐名输出 `SKIP | <name>` 并计 skipCount；t740 矩阵 96 腿逐迭代运行期名 `runLeg("%1->%2".arg(src.name, rc.name))` SKIP 逐腿精确；新增汇总行 `=== total SKIP: n ===` **仅 filter 模式打印**（不加参数输出与原先逐位一致）。记账口径：SKIP 按静态打印语句计（循环腿多次执行按语句计 1）；账面闭合 = 全跑 406 语句 + 95 矩阵名 = 501 门控单元，545 行 = 501 + 44 循环腿额外执行。**验证**：`--filter t1046` → EXIT=0，PASS=7（全含 t1046，零非命中腿）+ SKIP=494 + `=== total SKIP: 494 ===`（matrix_r2003_filter_t1046.log）；`--filter review26-1` → EXIT=0，7 PASS 含 despawn 钉腿（随迁自钉实测绿）**1.6s 跑完**（matrix_r2003_filter_review26-1.log）；无 filter 全跑回归 → 545/0、SKIP=0、汇总行同形，PASS 行与拆分版二进制恒等（matrix_r2003_filter_none2.log）。

**提速实测（本单验收核心）**：拆前单 TU -j 1 编译 **~85 分钟**（t1049 实测）→ 拆后：单段 TU 增量「改一段→重编+链接」**12-19 秒**（实测 13.4/18.8/14.4/19.5/12.0s 五段）；全量冷编（helpers.h 触发 10 TU + 链接）**2m4s**；全矩阵运行 8m19s（语义同前不变）；**任务级 filter 面验证 1.6s**。周期收益：加探针改一段的验证循环从 ~85min 编译 + 全跑 8min → 秒级编译 + filter 面秒级验证；「预期把增量编译压到分钟级」达成且超出（秒级）。

**过程注记（如实登记）**：段边界首版依据裸花括号配对表在 t1020 面板 A/B 共享作用域（L37141-37539 跨界块）处切穿——S6 编译期 expected '}' 实锤，字符串感知深度扫描器重定界后重做两段 md5 存证（其余 6 段边界经全量深度扫描证明完好）；App 目标 voxelsandbox 重建 EXIT=0（src/ 零触碰，40s）+ offscreen 冒烟 EXIT=124 存活 + logs/voxelsandbox_r2003_tail20.log 留存（60fps 稳态；smoke 重定向 0 字节为 t1044 起既录环境坑，tail20 从 logs/voxelsandbox.log 取）。**登记语义变化（非行为变化）**：src/Renderer/mobmodel.h 与 src/Core/resourcepackmanager.cpp 注释仍指旧文件名（src/ 冻结不碰，文档级漂移如实登记，R20.04+ 顺带修）；README 探针路径/计数刷新（397→545 + tools/matrix/）。
...
### R20.08 WorldFacade（2026-09-15，World 查询/写入接口收窄：窄视图包装 + GameSession/ChunkGeometry 双示范迁移 + 旧 World 仍为 Implementation）

  → **✅✅ 落地（2026-09-15）：src/World/worldfacade.h（header-only 收窄视图，非 QObject 纯值语义包装）+ GameSession/ChunkGeometry 双示范迁移 + tools/matrix/section12_worldfacade.cpp 新段置尾 4 腿，矩阵 556→560（band 558±2 内），World 类零改动、QML 零迁移。**（fix + test + 本 docs 三段提交；实现由后台 agent 起头、网络断链后主控现场盘点续完成——WIP 编译已过、死在验证前，验证链全数主控亲跑）

**四验收（plan §29.3 R20.08 原文）**：①「新代码不再直接获取 Chunk 内部指针」——WorldFacade public 面**零 Chunk\*/ChunkManager 外泄**（无转发无返回，结构性不可能经本面拿 chunk 指针）；**双示范迁移**：GameSession 命令写 `m_facade.setBlock` ×2 + 编辑回读 `m_facade.blockAt`（R20.07 新代码收口续行），ChunkGeometry 退役 myChunk() 直取——chunk 存在/脏/流体专用脏三门改经 Facade 查询，**mesher 自此零 Chunk\*（渲染侧唯一 Chunk 内部指针消费点消除）**；r2008c 正面钉（chunkDirty/chunkFluidOnlyDirty/chunkExists/WorldFacade(*m_world) 计数）+ 禁出反探常驻守「不回流」。其余现存调用点全量盘点（关单迁移清单，本单不全量迁移）：**worldstore.h/cpp** = 存档域直读 chunk blob（豁免口径同 worldgen；Chunk 生命周期 R20.10 收口）、**world.h** = ChunkManager 定义权威本体、**Main.qml** 的 chunks() 仅存注释（代码零调用）。②「World 规则使用统一查询」——高频读（block/state/光照/heightmap/地表/群系/天气/碰撞与完整立方谓词）Facade 单一入口收拢，**逐行委托不复制**（防两套规则漂移）；r2008a 全网格等价腿：stride-1 全 48×48 列 ×6 深度 ≈13.8k 格 block/state/谓词/光照 + 列级 heightmap/height/biome/weather 与 World 权威逐位一致 + OOB 语义逐位一致（y 超顶天光 15 / 网格外 Air 双面同值）。③「Renderer 不需要持有可变 World」——渲染面访问形态盘点：ChunkGeometry/BlockCube 对 World 全部消费均为**只读**（blockAt/stateAt/光照/heightmap + worldChanged 信号，零写调用；BlockCube::setBlockId/setBlockState 系其自身 QML 属性访问器非 World 写）；mesher 脏门经 Facade 后连 Chunk\* 都不再持有；**类型级收口（Q_PROPERTY World\* → const World\*/只读视图）留后续**（会动 QML 绑定面，本批渲染行为零变化，登记）。行为实证：r2008c 端到端——chunk(1,1) 内编辑脏门即时重建（TallGrass 落地顶点增长）、chunk(0,0) 编辑顶点不变（脏门 chunk 域拒重建）。④「旧 World 暂时仍可作为 Implementation」——Facade 纯转发零逻辑复制：r2008b 写入面等价（双生世界同一编辑序列 A=直调 World::setBlock vs B=经 Facade：全栅格 id+state 逐位一致 + 返回值逐笔一致[含 no-op 重写拒 / OOB 拒] + blockBroken/blockPlaced/worldChanged 三信号计数一致）；r2008d 对照柱旧直调路径仍可用且同面；World 类零改动（r2007 全腿继续全绿 = 未被替换的常驻证据）。

**写入面口径**：Facade 只暴露命令语义写（setBlock id / setBlockWithState id+state，四转发到 World::setBlock「写栅格的唯一入口」）；**静默写族（setBlockSilent/setWaterSilent/setBlockFromEntity/clearBlockSilent…）不转发**——收窄 = 不新增旁路，静默族全量收口归 R20.09 EditBuffer 正席（登记）。tick 模拟泵家族仍由 GameSession 直调 m_world（模拟泵非查询/写面，Facade 不收拢——选型登记）。

**验证**：分段增量构建（section12 单段秒级）→ filter r2008 **4P/0F** + filter r2007 **4P/0F**（section11 委托钉随迁移同步 `m_facade.setBlock` ×2，authority 仍是 World::setBlock——worldfacade.h 委托钉在 section12 r2008c）→ 全矩阵 **560 PASS / 0 FAIL ×2**（matrix_r2008_pos/final.log，EXIT=0；556 权威 PASS 行 diff = 4 新增（r2008a-d）+ 5 变更全落已登记漂移类：t813 戳时间/哈希、t830 采样计数、**t979 涉水出口计数 16→15（包络 ≥6 的实测计数，同 RNG 漂移类新增登记）**、t997 计时、t1023c src 文件计数 133→134=恰为本单 1 新头**线程原语仍 0**）→ app voxelsandbox 重建 EXIT=0 + offscreen 冒烟 EXIT=124 存活 + logs/voxelsandbox_r2008_tail20.log 零错误（唯一 WRN = 既登记项）。

**阴性轮（实跑存证 matrix_r2008_neg.log）**：变异 = gamesession.h executeCommand 旁路 Facade（m_facade.setBlock → m_world.setBlock ×2）→ rebuild → filter **恰红 1 腿 r2008c**（红因双确认：正面钉 `m_facade.setBlock(` x0<2 + 禁出反探 `[command-bypass]`）→ **Edit 反向还原** → rebuild → filter 回绿。变异构建式豁免登记：facade 转发短路（blockAt 改返常量类）属纯编译期行为，常驻等价腿 r2008a 必红，不另起构建轮。**过程坑登记（两项均为腿/钉自身首红，非实现缺陷）**：① 首跑 2 红——r2008a 抽样算术错（stride 3 = 16×16 列 ×6 深度 = 1536 格 < 9000 门槛，改 stride 1 全网格 13824 格 + 描述文本同步）；r2008d 选址双坑（破/放同落 chunk(1,1) → affectedCount=1；破目标未验证非空气 → 空操破不发作、事件塌成 1）——修法：破位 breakCol 扫描加 `blockAt(h)!=0` 过滤 + 破/放分驻 cx 1/2 **恒异 chunk** + diag 补全判别信息（pre/post id、事件逐项、changedBlocks、双 chunk 键）。② **pinSet 0 计数钉恒不红**（失败条件 cnt<minCount 对 minCount=0 永假）——首版五个「禁出」SrcPin 全为空转钉，首阴性轮恰红全靠正面钉兜底；修法：禁出钉改 **minCount=1 反探**惯用法（miss 非空 = 零命中 = 合规；复用 pinSet 剥注释器防注释提及误伤——r2007b 手工 contains 同型先例）+ 重组后带变异重跑阴性存证（红因含 [command-bypass] 反探实证非空转）。教训入 agent-state Workspace Guard。

**待实机确认**：无（渲染行为零变化——mesher 三门语义逐位等价 + app 冒烟稳态）。**下一任务：R20.09 EditBuffer**（plan §29.3：一次 Tick 内方块修改合并 WorldDelta——多次相邻编辑不产生重复通知、规则终态与旧实现一致、DirtyChunkSet 可被测试、mesh 调度不再直绑每一次 setBlock；gamesession 静默写收口正席）。

### R20.09 EditBuffer（2026-09-15，一次 Tick 内方块修改合并 WorldDelta：同格合并去重通知 + DirtyChunkSet 可测集合 + GameSession 批量收口示范 + 旧 World 仍为 Implementation）

  → **✅✅ 落地（2026-09-15）：src/Core/editbuffer.h（header-only 值类型组件：DirtyChunkSet 独立可测集合 + EditBuffer 累积器，非 QObject 无 AUTOMOC）+ GameSession 首消费方迁移（noteEdit→record 委托 / runOneTick 收口 takeDelta + 合并面派生事件 / lastDirtyChunks 调度视图）+ Review_2026-09-15 #3① 顺带清偿（Command.blockState 字段 + PlaceBlock 五参权威落地）+ tools/matrix/section13_editbuffer.cpp 新段置尾 4 腿，矩阵 560→564（band 562±2 顶格内），World/QML/PlayerController/ChunkGeometry 零触碰（渲染行为零变化）。**（fix + test + 本 docs 三段提交）

**四验收（plan §29.3 R20.09 原文）**：①「多次相邻编辑不会产生不必要的重复通知」——同格同 Tick 多写合并（EditBuffer::record 后写胜，返回 Merged 抑制重复通知），Tick 内**零通知**、收口按合并编辑面派生 BlockChanged（每格一条、id=终态）；r2009b 行为腿：一 tick 三条 FIFO 命令打同一格（放石→破→放土）= 恰 1 事件（id=终态土，非中间态石/空气）+ delta.changedBlocks==1（旧逐写实现为 3 事件/计数 3——重复通知面就此退役）+ 脏集 1 chunk + 次稳态 tick 空 delta（合并状态不跨 tick 边界泄漏）。②「规则终态与旧实现一致」——EditBuffer **只记录不写入**（写入权威仍是 World::setBlock 家族，GameSession 语义事件回调后登记）；r2009b 双生对照：GameSession 路径 vs 直调 World::setBlock 同三连写 + 同 tick 数旧路径泵（tickRedstone 代表）→ 该格终态逐位一致 + **全栅格 id+state 逐位一致**（裸地形稳态 13-tick 家族全早退，编辑面只由命令写构成）；r2007a/r2008d 存量等价腿继续全绿 = 常驻证据。③「DirtyChunkSet 可被测试」——独立类型纯值面（固定 64 容量数组 + 有效前缀，同 WorldDelta::kMaxAffectedChunks 单一权威引用）：三态 add（Added/AlreadyDirty/Full——event.h 头注登记的「细分语义留 R20.09 再定」由本类型收口，WorldDelta::addAffected 保守面零改动）+ contains/at 插入序/幂等去重不占位/满载拒/clear；r2009a 类型腿全断言 + EditBuffer 面（同格合并后写胜/三态 record/takeDelta const 幂等/负坐标 floorDiv 路由 (-1,-1)/记录面 1024 满载 Overflowed 可见）。④「mesh 调度不再直接绑定每一次 setBlock」——类型层前提：脏信息以**集合**形态 Tick 末一次可查（dirtyChunks()/takeDelta，非逐 setBlock 粒度）；会话面示范：GameSession::lastDirtyChunks() 调度视图 + tickCompleted 每 tick 恰一发；r2009c 批量收口腿：一 tick 跨 3 chunk（破 chunk(0,0)+放 chunk(2,1)+放 chunk(1,2)，cx/cz 结构性分驻）= 恰 1 发 tickCompleted + delta 携全 3 chunk + 脏集与 delta 集合逐键一致（单源）+ 事件 FIFO 序 + 次稳态 tick 双零残留。**ChunkGeometry/QML 渲染路径零触碰**（worldChanged→buildMesh 直绑仍在——渲染行为零变化；全量接线登记非目标，归 R20.10+ Chunk 生命周期 / R20.13 MeshBuilder 消费侧）。

**DirtyChunkSet 与 R20.06 WorldDelta 的选型（头注释登记）**：**分化不复用**——DirtyChunkSet 独立立类型（渲染调度视角：三态可辨、插入序可遍历、将来承载网格状态维度），WorldDelta 保持 R20.06 通知快照形态零改动（r2006-r2008 存量契约零破坏），EditBuffer::takeDelta 是前者到后者的**单点投影**。不复用 WorldDelta 本体的理由：①addAffected 对幂等重复/集满共用 false 返回（调度面必须区分合法幂等与需扩容决策的异常态）；②调度域扩展（NoMesh/MeshQueued 状态族）钉在通知类型上会越摊越大；③存量消费面零迁移。**落位 Core 非 Game**（头注释三依据）：零 World 依赖纯值类型（BlockEdit/WorldDelta/ChunkKey 皆 Core 叶子，chunk 模长 caller 传参同 fromWorld 纪律）；未来消费方 WorldSimulation 系统族在 World 层（PLAN §2 低层永不 include 高层）；event.h/snapshot.h 头注既有 R20.09 登记闭环同域。

**附带清偿（Review_2026-09-15 #3①）**：Command 加 `quint8 blockState = 0`（placeBlock 工厂尾参缺省 0 向后兼容——全部既有提交方/r2006a 腿零改动）+ executeCommand PlaceBlock 转 `m_facade.setBlockWithState` 五参权威（带朝向/半砖/湿度态方块经命令放置不再被 4 参版静默重置 state=0）；r2009d 行为腿实证 Farmland state=3 落地 + 默认 0 兼容柱；section11 r2007b / section12 r2008c 钉同步（`m_facade.setBlock(c.pos` 前缀式×2 → `m_facade.setBlock` ×2 覆盖双入口 + setBlockWithState 单独钉防退化；`ChunkKey::fromWorld` 钉随路由形态迁 editbuffer.h，会话面改钉 `Chunk::kSize` 路由模长单一权威）。

**验证**：分段增量构建（-j 4，section13 单段秒级；GCC13 对 vector 重分配内联链的 -Wstringop-overflow 假阳性警告如实在案，零 error）→ binary mtime（10:20:42）> 全部改动源（最晚 10:19:34）→ filter r2009 **4P/0F** + 前置段 filter r2006 3P / r2007 4P / r2008 4P 全绿（钉迁移零破坏）→ 全矩阵 **564 PASS / 0 FAIL ×2**（matrix_r2009_pos/final.log，EXIT=0；560 权威 PASS 行 diff = 4 新增（r2009a-d）+ 4 变更全落已登记漂移类：t813 戳时间/哈希（09:24→10:26 / cf63c07→7086e61）、t997 计时、t979 涉水计数 16→15、t1023c src 文件数 134→135=恰为本单 1 新头**线程原语仍 0**；t830 本次在容差内未漂）→ app voxelsandbox 重建 EXIT=0 + offscreen 冒烟 EXIT=124 存活 + logs/voxelsandbox_r2009_tail20.log 零错误（尾部仅退出时 QDxgiVSyncService 清理警告既登记项；0 重建 0 跨界写稳态 60fps）。

**阴性轮（实跑存证 matrix_r2009_neg.log + matrix_r2009_restore.log）**：变异 = 注释掉 gamesession.h runOneTick 收口行 `m_lastDelta = m_edits.takeDelta(m_tick);`（模拟收口旁路——delta 恒空、takeDelta 针句入注释）→ rebuild → filter **恰红 3 腿**（r2009b delta 断言 + r2009c 批量收口断言 + r2009d 源码钉 `m_edits.takeDelta(` x0<1——针句入注释即计数归零，兼证剥注释钉「注释提及不算数」的反向面）且 **r2009a 纯类型腿不误伤**（恰红边界正确）→ **Edit 反向还原**（禁 git checkout/reset）→ rebuild → filter 回绿 4P/0F。**过程坑登记（两项均为腿/钉自身首红，非实现缺陷）**：① r2009c 首跑 [chg=4]——破位 breakCol 只过滤了「目标非空气」未过滤「**顶上无附着**」：破表面块后写后钩子级联清除其上 TallGrass（worldgen placeTallGrass 草表层确定性散布）→ 第 4 条编辑混入（诊断 e2.blockId=0/pos 失配 + chg=4 判别信息直接定位）；修法：breakCol 加 `blockAt(h+1)==0` 过滤（破后零级联前提入选址纪律——「破目标验证」需含其附着邻域）。② r2009d 钉 `lastDirtyChunks` minCount=2 误设（声明点仅 1 处）→ x1<2 恰红自证后修正 minCount=1——新钉 minCount 必须对着实际出现次数核，不是拍脑袋翻倍。

**非目标（登记）**：①静默写族（setBlockSilent/setWaterSilent/setBlockFromEntity/clearBlockSilent…）调用点全量接 EditBuffer——类型已正席（record 三态面已备），接线归 R20.10+（每族接线伴随模拟泵返回值/脏标记语义盘点）；②ChunkGeometry worldChanged→buildMesh 直绑与 QML 渲染路径——本批零触碰（渲染行为零变化不变量），按 DirtyChunkSet 调度重建归 R20.13 MeshBuilder 消费侧；③WorldDelta ranges/light regions 明细——合并粒度到「格」已满足本单四验收，ranges 化归 R20.13 消费输入形态；④Review_2026-09-15 #1（stepTick dt 钳制，期限「R20.08 接线前」已过但当前唯一消费者矩阵探针 dt 受控不红）与 #2（空手 sneak 机关交互）维持原登记，随其各自任务面收口。

**待实机确认**：无（渲染行为零变化——渲染路径零触碰 + app 冒烟稳态；GameSession 仍无 QML 消费方）。**下一任务：R20.10 Chunk 生命周期**（plan §29.3 R20.10：Absent/Loading/Generated/Active/Loaded/Evicting 六态——当前固定 10×10 世界仍可运行、Chunk 可卸载重载、无未保存数据静默丢失、QML 不再决定 Chunk 真实生命周期）。

### t1050（2026-09-15，Review_2026-09-15 两项纠偏一单两修：① GameSession::stepTick dt 入口双钳制 + ② 空手潜行右键交互 MC 口径恢复）

  → **✅✅ 落地（2026-09-15）：src/Game/gamesession.h stepTick 入口双钳制（方案①）+ droppedDtCount 计数暴露；src/Game/playercontroller.cpp useBlock 12 门收窄 sneakPlaceBlock（手持方块才旁路放置）；tools/matrix 新腿 5 条（r2007e + t1050a-d）+ 既有 5 钉同步，矩阵 564→569（band 568±2 顶格内）。**（fix + test + 本 docs 三段提交）

**修 1（Review0915 #1，中）根因/修法**：`m_accumMs += qRound(deltaSecs*1000.0)` int 累积器 + while 无上界——超大 dt（真机 Adapter 接墙钟后断点续跑 / 切后台恢复量级）qRound 超 int 域即 UB，或 1e9ms 级累积单泵连跑上万 tick（世界模拟假死）；负 dt 入累积器成负时间债（吃掉后续正常 dt，且无日志）。**选型方案①（入口双钳制）非方案②（保留累积 + 单泵 N tick 上界）**：②留有 qRound 溢出洞（1e6 秒级 dt 在 tick 上界生效前就溢出 int 累积器），①在入口把累积器输入域钳死 [0, kMaxStepSecs=1.0s]（1000ms ≪ INT_MAX，溢出结构性不可能）；「超界整体丢弃」与暂停「dt 丢弃不欠账」同门——异常墙钟差不 catch-up 不补账（单泵至多 10 tick，无万 tick 连跑）；负 dt/NaN → 零累积（`!(deltaSecs >= 0.0)` NaN 比较恒假一并拦），负时间债入口焊死；droppedDtCount 背压可见 + qWarning 携总量。**兼容性**：既有探针 dt 全部 ≤0.7s（r2007a 0.3 / b 0.7 / c 0.6），钳制不触达——N×stepTick(0.1)==stepTick(N×0.1) 与暂停停表逐位兼容（--filter r2007 5P/0F 实证）。

**修 2（Review0915 #2，低=MC 口径偏差）根因/修法**：51cc43c③ 把机关（Lever/WoodButton/StoneButton）并入 `!sneakPlace` 旁路（t523 七 UI 族 + t1034 门/床/活板门同款裸门）——空手 sneak 右键落放置路径被 `m_selectedBlock==Air` 守卫拦成「无效应」（4 交互族 + 7 UI 族 + 音符盒共 12 门同疾）。**MC 口径引证（parity 三元组）**：行为=潜行右键「手持物品的使用优先于目标方块交互」，空手/持非方块物品无「手持物品可用」→ 照常交互（拉杆扳动/按钮按/门开合/活板门翻板/床入睡/箱子等开 UI）；版本基准=Java 1.4.6（12w49a）引入潜行对可交互方块面放置、沿用至今；出处=minecraft.wiki/w/Sneaking Effects 段原文 "Pressing use prioritizes using a held item over interacting with a targeted block"（2026-09-15 实读）。旧注释「机制等价 MC 空手 shift 右键箱子无效应」系登记口径错误（MC 空手 shift 右键箱子=打开），随修改写。**修法**：判据收窄 `sneakPlaceBlock = sneakPlace && m_selectedBlock != Air`（selectedBlock 经 hotbar 对非方块物品槽已归 Air——桶/锄/食物/骨头等分支注释在案，判据恰为「手持可放置方块」），12 门统一改读。**既有腿零行为改写需求（纠偏非放宽的边界）**：t1034a/b/c、t1046a、t1028b(1b) 潜行腿全部是「持方块」语义——合取判据下新旧门对持方块取值同恒，逐位兼容（filter t1034 3P / t1046 面 / t1028 3P 全绿实证）；改写的只有 5 处源码钉针句（cpp-{note,door,bed,trapdoor,mech}-sneak-gate → `!sneakPlaceBlock`）与旧口径注释——被纠偏的是 **src 侧登记错误口径**，探针无「空手 sneak 无效应」行为断言在库（grep 空手/sneakPlace 全量核对）。**残余登记**：箱子矿车裸键门（placeBlock m_hasHit 块外、sneakPlaceBlock 无作用域；空手 sneak 右键箱车不开箱同疾）维持原状，随下一波 parity 单收口。

**矩阵**：r2007e（section11，dt 钳制四柱：3600s 整体丢弃=0 tick + droppedDtCount==1 + tick 不动 / 丢弃后 0.1s 泵恰 1 tick[累积器零残留不受污染] / 负 dt −0.5s 自身零推进且后续 0.1s 泵恰 1 tick[无负时间债] / 恰上界 1.0s 不超界=10 tick[严格大于才丢]）；t1050a-d（section08，四族空手 sneak 交互各一腿 + 每腿配「持方块潜行=放置优先」对照柱：拉杆扳动 bit0=1 + swingArm 恰 1 + 无放置 / 门开合两半 bit2=4 + doorToggled 恰 1 / 床入睡链触达[白天拒睡文案恰 1 次不入睡] / 活板门翻板 bit0=1 + doorToggled 恰 1）；t1050a 另钉判据合取形态（阴性轮摘合取项即红）。qWarning 落矩阵日志（「dt 3600 s exceeds 1 s cap」）= 背压可见实证。

**验证**：分段增量构建（-j 4；gamesession.h 触达 section11/13 + playercontroller/section08 全重编 + 链接）→ binary mtime（12:03:48）> 全部改动源 → filter t1050 **5P/0F** → 前置段回归 filter r2007 **5P/0F**、t1034 **3P/0F**、t1046 面（含 t1046a 机关 + t1050a）7P/0F、t1028 **3P/0F**（t1028b 持方块潜行旁路不回归）全绿 → 全矩阵 **569 PASS / 0 FAIL ×2**（matrix_t1050_pos/final.log，EXIT=0；564 权威 PASS 行 diff = 5 新增（t1050a-d + r2007e）+ 3 变更全落已登记漂移类：t813 戳时间/哈希（10:26→11:44 / 7086e61→c760606）、t997 计时、t979 涉水计数；**t1023c 本单零漂移**[无新增文件，线程原语仍 0]）→ app voxelsandbox 重建 EXIT=0 + offscreen 冒烟 EXIT=124 存活 + logs/voxelsandbox_t1050_tail20.log 零错误（尾部 WRN 与 r2009 基线同为 teardown 既登记项，稳态 60fps）。

**阴性轮（实跑存证 matrix_t1050_neg{1,2}.log + 各 restore）**：neg1 = gamesession.h 摘负 dt 门（`false &&` 前缀变异）→ 全矩阵**恰红 1 腿 r2007e**（−0.5s 负债入累积器，后续 0.1s 泵 0 tick ≠ 1——快红无 tick 风暴；超界门变异同理 (1) 红但会 36000 tick 连跑挂死，登记不取）→ **Edit 反向还原** → rebuild → filter r2007 回绿 5P/0F（matrix_t1050_neg1_restore.log）。neg2 = playercontroller.cpp 摘 sneakPlaceBlock 合取项（退回裸 `sneakPlace`）→ 全矩阵**恰红 4 腿 t1050a-d**（空手 sneak 腿全红 + t1050a 判据钉红），既有持方块腿（t1034a/b/c、t1046a、t1028b）与其余 564 腿零误伤 → **Edit 反向还原** → rebuild → filter t1050 回绿（matrix_t1050_neg2_restore.log）。

**过程坑登记（腿自身首红，非实现缺陷）**：r2007e 首跑 [neg got=0 after=1 tick=1]——负 dt 断言把「负泵后 tick==0」与「后续泵后 tick==1」写在同一 && 链，两个 stepTick 都已执行后才求值 → `tick==0` 恒假；修法：拆两步（负泵后先存 negSelf，后续泵后再合取）。教训：断言取照与被测状态必须同帧，跨泵状态先快照再合取。
...
### R20.10 Chunk lifecycle（2026-09-15，Chunk 六态生命周期状态机：manager 侧表 + 守卫式转移唯一入口 + Facade 三门生命周期接线 + 固定世界默认稳态零变化）

  → **✅✅ 落地（2026-09-15）：src/World/chunklifecycle.h（header-only 六态类型 + 转移表单一权威，非 QObject 无 AUTOMOC）+ ChunkManager 生命周期侧表（m_lifecycle 与 m_chunks 同布局同索引；recreate 全表 Loaded 默认稳态）+ setLifecycle 唯一转移入口（转移守卫在调用点）+ lifecycleAt 查询 + World::setChunkLifecycle C++ 面 forwarder（非 Q_INVOKABLE）+ WorldFacade 三门（chunkExistsAt/chunkDirtyAt/chunkFluidOnlyDirtyAt）叠加 chunkLifecycleQueryable 门（态 ∈ {Loaded, Active}）；tools/matrix/section14_chunklifecycle.cpp 新段置尾 4 腿，矩阵 570→574（band 572±2 顶格内），ChunkGeometry/QML/PlayerController/GameSession 零触碰（ChunkGeometry 门经 Facade 零改动继承生命周期语义）。**（fix + test + 本 docs 三段提交）

**四验收（plan §29.3 R20.10 原文）**：①「当前固定 10×10 世界仍可运行」——recreate() 全表置 **Loaded（默认稳态 = 常驻已加载）**：固定 10×10 / fresh 48×48 世界的门查询、玩家路径、QML、渲染与 R20.10 之前逐位等价（r2010b 行为腿：3×3 全 Loaded + Facade 存在门 3×3 全 true + 脏门与裸脏标记 pass-through 等价 + 编辑破/放往返全栅格 id+state 逐位恒等 + worldChanged 照常 + 态表不动；r2007/r2008/r2009/determin/re-gen/store/save 前置回归全绿 = 常驻证据）。②「Chunk 可以卸载并重新加载」——r2010c 端到端：两步卸载（Loaded→Evicting→Absent）→ Facade 三门恒 false（邻 chunk 不受扰）→ 卸载窗内 mesher 门跳过（被观测 chunk 编辑落地而顶点数不动）→ 三步重载（Absent→Loading→Generated→Loaded）→ 三门恢复 + 全栅格逐位恒等（含窗内编辑）→ 门复 live（再编辑顶点差分）。③「没有未保存数据被静默丢失」——**Edits-on-evict 最小解释选型（plan 未定义处，本单显式登记）**：Evicting/Absent 只表达状态可达（数据保留在内存，路由读写不受生命周期门控——worldgen/store 豁免域直读 chunk blob 的前提），**不实现落盘驱逐**（数据保留策略登记为后续单，避免越权产品决策）；r2010d 存档往返腿：存①→卸载 (2,2)→存②（worldstore 直读 chunk blob 豁免域不受态影响——Absent 态下 saveAll 照常序列化全部 9 chunk）→ 新世界 beginLoad+loadChunks+finishLoad 回读 → 双世界全栅格 id+state 逐位恒等 + store 未动生命周期态。④「QML 不再决定 Chunk 的真实生命周期」——生命周期访问面**结构性不进 QML**：chunklifecycle.h/chunkmanager.h 零 Q_INVOKABLE/Q_PROPERTY（r2010d 反探钉）、World::setChunkLifecycle 非 Q_INVOKABLE（反探钉）、Main.qml 零 ChunkLifecycle 提及（QML 零迁移主线不变量）——QML 根本无生命周期访问面。

**六态转移图与选型（chunklifecycle.h / chunkmanager.h 头注释登记）**：合法边恰 8 条——① Absent→Loading（requestLoad，R20.11 GenerationJob 将来挂点）② Loading→Generated（generateDone）③ Generated→Loaded（promoteToResident）④ Loaded→Active（activate）⑤ Active→Loaded（deactivate）⑥ Loaded→Evicting（requestEvict）⑦ Evicting→Absent（finishEvict）⑧ Evicting→Loaded（cancelEvict——驱逐途中又被需要）；其余 28 组合（含自转移）一律非法。**态存 manager 侧表非 Chunk 内字段**，三理由：① 豁免域隔离——worldstore 直读 Chunk blob 三数组（R20.08 关单登记豁免），态放 Chunk 内会给存档语义引入「该不该序列化」的诱惑面，侧表让 Chunk 保持纯体素容器、store 路径结构上看不见生命周期；② Absent/Evicting 需要「实例缺席仍可表态」——最小解释下驱逐不销毁 Chunk 对象，实例字段无法表达「对象在但内容不可用」；③ 定长稠密 vector 与 m_chunks 同生同灭，recreate() 唯一初始化点，O(1) 索引零分配 churn。**门查询可见面 = {Loaded, Active}**（chunkLifecycleQueryable 单一权威）：Loading/Generated（内容在途未晋升）与 Absent/Evicting（离开中/已离开）对 mesher 三门不可见；默认稳态全 Loaded → 三门与 R20.08 行为逐位一致。**r2008c 结构钉兼容**：worldfacade.h 三门保持 `chunks().chunk(` 逐字 ×3（钉 minCount=3 不破坏），ChunkGeometry 侧零改动（门经 Facade 继承生命周期语义）。

**矩阵（570→574）**：r2010a（纯图腿零世界：枚举域恰 6 态 + 独立编码 8 边表 vs 转移权威 36 对逐对互证 + 门谓词独立核 + 裸 3×3 网格默认稳态/越界 Absent/越界写拒 + 全 8 边合法路径走通访遍 6 态 + 取消边单独走 + 守卫式 setter 六态 ×6 目标逐对核【每次探测前经合法路径重布点】）；r2010b/c/d 见验收①②③。

**验证**：分段增量构建（-j 4；chunkmanager/worldstore/chunkgeometry/world.cpp 随 chunkmanager.h 触达重编 + section14 新 TU + 链接）→ binary mtime（14:30 级）> 全部改动源（最晚 matrix_helpers.cpp）→ filter r2010 **4P/0F**（EXIT=0）→ 前置回归 filter **r2007 5P / r2008 4P / r2009 4P / determin 30P / re-gen 5P / store 42P / save 34P** 全绿 EXIT=0（worldgen 逐位恒等 + worldstore 豁免域读写语义零破坏）→ 全矩阵 **574 PASS / 0 FAIL ×2**（matrix_r2010_pos/final.log，EXIT=0；570 权威 PASS 行 diff = **+4 新增（r2010a-d）**+ 3 变更全落已登记漂移类：t813 戳时间/哈希（13:14→14:17 / f368f6e 基线）、t997 计时、t1023c src 文件数 135→136=恰为本单 1 新头**线程原语仍 0**；t979 本次在容差内未漂；零未归因漂移）→ app voxelsandbox 重建 EXIT=0 + offscreen 冒烟 EXIT=124 存活 + logs/voxelsandbox_r2010_tail20.log 零错误（尾部 WRN 同 r2009/t1050/t1051 基线 teardown 既登记项，稳态 60fps 0 重建）。

**阴性轮（实跑存证 matrix_r2010_neg.log + matrix_r2010_restore.log）**：变异 = chunkmanager.cpp setLifecycle **摘转移守卫调用点**（`if (!chunkLifecycleTransitionLegal(...)) return false;` 两行注释——t1051 教训应用：摘调用点，非 `false &&` 前缀把条件守卫变无条件执行）→ rebuild → filter **恰红 2 腿同源归因**：r2010a 行为半（28 非法转移全部被接受 + 拒绝不改态断言红——每态探测前重布点的腿结构使红因逐对可辨）+ r2010d 结构半（守卫调用点钉 `chunkLifecycleTransitionLegal(` x0<1——针句入注释计数归零，行为/结构两层对同一变异检出）；r2010b/c 不误伤（合法路径在无守卫下照常成立 = 恰红边界正确）→ **Edit 反向还原**（禁 git checkout/reset）→ rebuild → filter 回绿 4P/0F（matrix_r2010_restore.log）。

**过程坑登记（三项均为腿自身首红，非实现缺陷）**：① r2010a 首版守卫扫略在**合法转移被接受后直接续扫**——态已移动，后续「非法」比较用了陈旧前置态（Loaded→Active 接受后再试 Loaded→Loaded 实为 Active→Loaded 合法），假红形态 22 项；修法：每次探测前经合法路径重布点到目标态（resetLoaded 汇点各态出路 + Loaded 出发五条去程，布点自证入断言）。教训：状态机扫略腿的「每对转移探测」必须态独立成立，禁止依赖前序转移后的残留态。② r2010c 首版断言「重载后 chunkDirtyAt==true（窗内编辑留脏）」假红——World::setBlock 的 post-emit clearAllDirty（t155g 既有行为）在 mesher 门跳过重建时**照样清全表脏标记**，窗内编辑的脏被静默消费；修法：弃该断言（既有语义非生命周期面），挂起编辑的可见性改由重载后再编辑的重建差分（v3 != v2）承担——首版 v3<v2 方向断言同样假红（窗内 Stone 一并进重建净量 +8），改判移动不断言方向。③ r2010d 首版 loadChunks 前漏 `store.setWorld(&wS2)`——blob 回灌进 store 仍持有的 wS1 自身、wS2 恒零填充（delta@(0,0,0) 14/0 vs 0/0 判别信息直接定位）；教训：WorldStore 换目标世界必须显式 rebind setWorld，loadChunks 写入面 = m_world。

**非目标（登记）**：①**落盘驱逐/数据保留策略**——Evicting→Absent 只表达状态可达，数据零销毁（Edits-on-evict 最小解释，见验收③），落盘驱逐登记为后续单（届时须与 worldstore saveAll 协调「Absent 态 chunk 是否入 blob」的语义决策）；②玩家距离驱逐器/streaming——R20.11 ChunkScheduler / 后续单，本单零自动转移（全部转移显式事件驱动）；③生命周期不门控路由读写——blockAt/setBlock 对非查询态 chunk 照常读写（豁免域前提 + 默认稳态零变化；查询面收口在 Facade 三门）；④箱车裸键门残余 / review0915 #4（R20 walk 维持）/#6（搭下一笔 entitymanager）/#7（登记）维持既有登记。

**待实机确认**：无（默认稳态行为零变化——渲染/QML/玩家路径零触碰 + app 冒烟稳态）。**下一任务：R20.11 后台 GenerationJob 同步版 ChunkScheduler**（plan §29.3：不引入多线程，统一请求/优先级/取消/结果模型——request ID、重复请求合并、过期请求丢弃、换 worker 不改调用者；**R20.11 收口后治理计数满 5 → 先治理审计 #8 再续**）。

### R20.11 GenerationJob 同步版 ChunkScheduler（2026-09-15，统一请求模型：request ID / 重复合并 / 过期丢弃 / Result 结果模型 / worker 可替换缝 + R20.10 生命周期互锁）

  → **✅✅ 落地（2026-09-15）：src/World/generationjob.h（header-only World 层请求编排组件，非 QObject 无 AUTOMOC）——GenerationJobKind 三 kind（Generate/Load/Mesh）单调 requestId / pending 域 (kind,key) 重复合并（别名制：工作只跑一次、每别名各收自己的 outcome）/ 取消 + 世界 epoch 两级丢弃（都不投递）/ priority 0 最紧急（同优先级按提交序）/ GenerationWorker 缝（Result&lt;void&gt; execute，调用者只见 submit/cancel/pump/takeOutcome 面）/ 可挂 ChunkManager* 驱动生命周期边①②（best-effort 经 setLifecycle 唯一守卫入口）；tools/matrix/section15_generationjob.cpp 新段置尾 4 腿，矩阵 574→578（band 576±2 内），**零调用点迁移**：World/world.cpp/worldstore/GameSession/ChunkGeometry/QML 全零触碰（生产路径零接线 = 固定世界零变化的最强形态）。**（fix + test + 本 docs 三段提交）

**四验收（plan §29.3 R20.11 原文）**：①「生成、加载、mesh 请求都有 request ID」——单一单调计数器跨三 kind 分配（1 起、0 无效哨兵、永不复用），每条请求可独立追踪/取消/收结果（r2011a：三 kind 严格递增唯一 id）；②「重复请求可以合并」——同 (kind,key) 仍 pending 时合并为同一 job 的新别名：工作恰执行一次（同 chunk 二次请求不重跑生成）但两别名各收自己的 Completed outcome（合并不减提交方可见性）；执行完的再请求 = 新 job 重跑（合并仅 pending 域，如 mesh 编辑后需重建）；不同 kind 同 key 不合并（r2011a 行为钉）；③「过期请求可以丢弃」——两级丢弃均「不投递」：取消（pending 专属，被取消别名永无 outcome，全别名死亡 job 整体跳过零执行；幂等拒重复取消）+ 世界 epoch（submit 快照 epoch，pump 时 != 当前 = 过期丢弃不执行不投递——R20.12「world epoch 丢弃旧任务」的同步版前置）（r2011b）；④「后续替换成 worker 不需要改调用者」——执行后端藏 GenerationWorker 接口后，调用者代码字面同一（r2011c 用同一 driver lambda 喂两个**不同实现类** worker 实例：requestId 流 / outcome 流 / 执行轨迹三方恒等 = 可替换性无头实证不加线程；中途 setWorker 换实例调用点零改动）。

**请求模型五要素与生命周期互锁选型（generationjob.h 头注释立此存照）**：①结果模型 = worker Result&lt;void&gt; 失败面 **Error 原样穿透**到 GenerationJobOutcome.error（code/message 逐位，r2011c 失败腿：42/"synthetic worker failure" 落到调用方），成功面 outcome FIFO 序可精确预测（r2011a③ [idG,idG2,idL,idM]）；②容量策略 = job 队列 64 上界 + 满载提交 fail(kErrQueueFull) 可见拒绝（命令/事件队列同门纪律），**outcome 队列刻意不设上界**（同步内联 pump 无生产者/消费者不对称，容量策略是 R20.12 线程化后的真问题，登记不预设计）；③执行序 = (priority 升序, 首活 requestId 升序)；④生命周期互锁 = **只驱动边①②**（执行开始前 Absent→Loading——即 chunklifecycle.h 预留的「R20.11 挂点」；成功后 Loading→Generated），晋升 Generated→Loaded（边③）**不由本层驱动**（驻留晋升是 mesh/激活策略决策面，登记后续），失败无边可走 → 停在 Loading（恢复策略登记 R20.12），取消发生在执行前 = 状态从未离开 Absent（「不推进」而非「回退」——六态表无 Loading→Absent 边，本层绝不非法回退）；驱动是 best-effort 记账：非法转移（默认稳态 Loaded→Loading）被守卫拒绝即静默忽略、不影响工作执行——**这正是固定世界零变化的结构性根据**（r2011d：Loaded chunk submit+pump 后 9 格生命周期表快照逐位不动 + fresh 世界全表 Loaded）；⑤World 挂点 = 可变 ChunkManager*（与 World::setChunkLifecycle forwarder 同一终点 setLifecycle），**刻意不**给 World 加非 const chunks() 访问器（World 文件零改动），真实 World 接线（submit 侧只对 Absent chunk 提交的策略层）登记 R20.12。

**矩阵（574→578）**：r2011a（请求模型核心零世界：三 kind id 唯一递增 / 重复合并「一次执行双 outcome」/ 完成后再请求重跑 / 优先级序 D(0)→B(1)→A(5)→C(5) / 第 65 个不同键满载拒绝 + 满载下合并仍可达 / 空 outcome 取值正常 false）；r2011b（取消 + 过期：未知取消 false / 取消零执行零投递账面清空 / 合并 job 部分取消幸存者照常 / epoch bump 后旧请求静默丢弃新请求照常）；r2011c（失败面穿透 + 可替换性）；r2011d（互锁 + 零变化 + 源码钉：worker 执行时刻实测 Loading（边①先于工作）/ 取消不推进恒 Absent / 失败停 Loading / Loaded chunk 工作执行而表快照恒 / fresh 世界稳态 / 值纪律四钉 + worker 缝签名 + `m_chunks->setLifecycle(` ×2 边调用点 + kErrQueueFull / 反探 generationjob.h 零 Q_INVOKABLE/Q_PROPERTY + world.cpp/worldstore.h/Main.qml 零 scheduler 引用）。

**验证**：分段增量构建（-j 4；仅 section15 新 TU + helpers/CMake + 链接）→ binary mtime > 全部改动源 → filter r2011 **4P/0F**（EXIT=0，matrix_r2011.log）→ 前置回归 **r2007 5P / r2008 4P / r2009 4P / r2010 4P / determin 30P / re-gen 5P / store 43P / save 34P / worldgen 22P** 全绿 EXIT=0（worldgen 逐位恒等 + worldstore 豁免域零变化；store 42→43P = r2011d 腿名含 "worldstore.h" 自匹配词过滤，登记类漂移、矩阵权威计数不受影响）→ 全矩阵 **578 PASS / 0 FAIL ×2**（matrix_r2011_pos/final.log，EXIT=0；574 权威 PASS 行 diff = **+4 恰为本单 r2011a-d** + 1 变更 = t813 构建戳时钟类（14:→15:，已登记漂移类），t997/t979/t1023c 本次零漂，零未归因漂移）→ app voxelsandbox 重建 EXIT=0 + offscreen 冒烟 EXIT=124 存活 + logs/voxelsandbox_r2011_tail20.log 稳态 60fps（尾部 WRN 与 r2010 基线逐字同源 teardown 噪音）。

**阴性轮（实跑存证 matrix_r2011_neg.log + 还原后同文件追加 restore 段）**：变异①摘合并——submit 合并分支条件前置 `false &&`（= 无重复检测，语义恰为「摘合并」）→ rebuild → **恰红 r2011a 单腿**（[merge pj=4][exec n=4][out n=0]——重复提交各成新 job：job/执行/outcome 三计数面同红；r2011b 部分取消场景无合并时终态同形不误伤、r2011c 两侧同变仍恒等、r2011d 零重复提交）→ Edit 反向还原；变异②摘过期丢弃——pump 选活扫描摘 `r.epoch != m_worldEpoch` 子句（取消子句保留）→ rebuild → **恰红 r2011b 单腿**（[stale hit=1]——过期 K3 被执行，diag 形态即「摘丢弃」本体；全矩阵其余腿零 epoch bump 不受扰）→ Edit 反向还原 → rebuild → filter r2011 回绿 4P/0F。两变异均头内单行手术、还原后 grep 零 NEG-R2011 残留。

**过程坑登记（两腿自身修正 + 一处编译错，实现零返工）**：① section15 首版编译错（`const bool reOk` 二次赋值）——构建期即红即修；② r2011c 首版钉了绝对 outcome 序与绝对执行次数（`==4`）——NEG1 首跑误伤本腿（[repl trace=0]：摘合并时两侧同变为 5 执行仍恒等，绝对计数无理红）；修法：可替换性腿只钉 **A≡B 相对恒等**（绝对 FIFO 序归 r2011a③ 被测语义腿专钉），swap 腿旧 worker 计数改快照锚——恰红面收缩到 r2011a 单腿。教训：**阴性恰红面设计先于腿文定稿**——「辅助腿钉绝对值」会把主语义腿的红扩散成假阳性面；③ r2011d④ 首版设计「9 格全 Loaded」断言在②③场景已置非默认态后必然假红（设计期修正为表快照对照 + (1,0) 特判，未进构建）。

**非目标（登记）**：①**真后台线程化**（R20.12 后台 GenerationJob）——线程队列容量策略、退出排干、GUI 不阻塞全归 R20.12；②Loading 失败恢复策略（重试/回退边）——六态表无失败边，登记 R20.12 与真实 worldgen 接线一并收口；③真实生产接线（生成/加载/mesh 的现网调用点迁移 + submit 侧 Absent-only 策略层 + World 挂点访问器）——本单零调用点迁移是固定世界零变化的最强保证，接线即 R20.12 本体；④Edits-on-evict / streaming 玩家距离驱逐器——延续 R20.10 登记；⑤箱车裸键门残余（随下一波 parity）/ review0915 #4（R20 walk 维持）/#6（搭下一笔 entitymanager）/#7（登记）维持既有登记。

**待实机确认**：无（零调用点迁移 = 生产路径行为零变化——app 冒烟稳态）。**下一任务：治理审计 #8（计数满 5，主控亲自执行）**——R20.05-R20.11 七单连闭（其中 R20.09 后 5 单：R20.09/t1050/t1051/R20.10/R20.11）+ 批次结束双触发；审计通过后再续 plan §29.3 后续单（R20.12 后台 GenerationJob / R20.13 MeshBuilder）。

### R20.12 后台 GenerationJob（2026-09-15，真线程阶段第一单：最小地形生成路径后台化——非阻塞 / 纯函数输出 / epoch 丢弃 / 退出安全 + R20.11 缝的第三 worker 实现）

  → **✅✅ 落地（2026-09-15）：src/World/terraingen.h（纯地形生成**单一权威**：Perlin 置换表 / noise2 / noise3 / fbm / heightAt（含 heightWithBiome 防二次群系计算）/ biomeComputeAt / seaCorner / seaColumnHeight / isSeaSandColumn / hashColumn / hashVoxel / fade·lerp·grad2·grad3 / kWaterLevel / generate() 首循环列体 fillTerrainColumn——全部自 world.cpp/world.h **逐行迁入**（值逐位同式），Biome 枚举迁入 + World 侧 using 别名保持调用点零改动）+ src/World/backgroundgeneration.h（BackgroundGenerationWorker = GenerationWorker 缝的**第三个实现**（前两个 = r2011c 双类可替换性实证）：isAsynchronous/submitAsync/takeCompletedAsync 三点接入异步协议，调用者面 submit/cancel/pump/takeOutcome 零改动）+ tools/matrix/section16_backgroundgen.cpp 新段置尾 4 腿，矩阵 578→582（band 580±2 内）；world.cpp 同步路径经 WorldColumnSink 走同一 fillTerrainColumn，后台 worker 经缓冲 sink 走同一函数——**同步与新 worker 共用一份生成逻辑，禁复制第二份**（worldgen 逐位恒等腿族守迁移零漂移）。**（fix + test + 本 docs 三段提交）

**四验收（plan §29.3 R20.12 原文）逐条断言**：①「GUI 不被整 Chunk 生成阻塞」——submit 立即返回（提交断言先于任何 pump 的代码事件序 = 账面 9 pending/0 outcome/0 in-flight）+ 有界轮询 deadline 内收满 9 chunk outcome（deadline 10s ≈ 实测毫秒级负载 3 个数量级余量，超时才红）+ 主线程泵轮计数 ≥1（等待期事件计数对账，不比对墙钟）+ 线程身份 id 三重对账（执行线程 == 工作线程 && != 调用者线程——确定性不靠时序）（r2012a）；②「seed、ChunkKey、generator version 决定输出」——**承重墙三重锚**：(a) 双 worker 实例同 (82,{1,1},v1) 双跑缓冲逐位一致（identical + contentHash），(b) 后台输出 ≡ 主线程直调同一 fillTerrainColumn 的手写 capture sink 缓冲（传输层换零线程零队列，字节不动），(c) fresh 真世界 48×48×96 s82 的中块 256 列逐列 envelope==min(heightAt,H-1) 且表层 id 逐群系一致（Desert→Sand / Snowy→SnowLayer state≤2 / 其余→Grass；中块列距四角 ≥22.6 > 海域最大可达 ~19.9 = 结构性内陆）+ 方向对照 seed 83 输出必不同（恒等断言非空转）（r2012b）；③「world epoch 可以丢弃旧任务」——交接后才 bump epoch 的在途任务**仍执行**（worker 不识 epoch——执行账面 2/2、双缓冲都到）但投递面恰 1 条 outcome（新 epoch K2；K1 的过期产物按完成序先到、按键识别由调用方丢弃——「丢弃」在调度侧 deliver 过滤器，与 R20.11 同门）+ 交接前取消 = 永不执行永不投递（执行/数据计数不动，双账收干）（r2012c）；④「退出时不会被任务卡死」——三个立即析构场景（在途+待队并存 / 仅待队零执行 / 空转）腿体全部走完 = join 有界返回的实证（挂死 = 整体超时红）；析构语义 = stop 置位 + notify_all + join，**队内未开工任务整体丢弃**（与 epoch 丢弃同门：世界已亡旧产物无投递价值），join 上界 = 至多一个在途任务（纯函数单 chunk 毫秒级）（r2012d）。

**迁移路径清单（plan「迁移一个最小地形生成路径，不一次性迁移所有结构」的登记面）**：**已迁**（= terraingen.h，12 件：置换表/noise2/noise3/fbm/heightAt/biomeComputeAt/seaCorner/seaColumnHeight/isSeaSandColumn/hashColumn/hashVoxel/generate() 列体 + 助手 fade/lerp/grad2/grad3 + 常量 kWaterLevel/kChunkSize/kGeneratorVersion）；**未迁**（= 仍 World 同步专有，登记 R20.13+）：*Cells/rebuildStructureRegions/biomeAt 的 memo 壳（透明缓存语义零影响）。迁移验证 = worldgen 22P / determin 30P / re-gen 5P 全绿（**逐位零漂移**，改前改后同跑同果）。

**线程原语选型（backgroundgeneration.h 头注释立此存照）**：std::thread + std::mutex + std::condition_variable，**不用 Qt 线程设施**——依据 = R20.12 红线（audit #8 已立）worker 侧禁 QObject/QML/QQuick3D：worker 全程不持 QObject（输入 GenerationRequest / 输出 GeneratedChunkData 纯值；执行体 TerrainGen 纯函数），Qt 线程设施会把 QObject 语义重新引入 worker 侧；线程安全根据 = 无共享可变态（TerrainGen 构造后只读 + 锁护卫三条 FIFO + stop 标志单锁串行化）。**t1023c 事实钉同变更修订**（腿文自述「线程化须同变更更新本探针」的义务履行）：src/ 树线程原语命中只允许落在唯一受认可落点 World/backgroundgeneration.h 且仅 std::thread；F3 行 "threads: 0/0 (sync meshing)" 照常钉住（mesher 仍同步——后台化的是生成不是网格化）。**线程期容量策略**（延续 R20.06 分化登记口径，第三形态「传递性有界」）：待执行队列 64 上界满载 kErrQueueFull 可见拒绝 → scheduler 交接相按背压留队下轮 pump 重试；完成/数据两队列刻意不设上界（每完成必对应已受理任务，在途 ≤ 交接数 ≤ 上界；主线程延迟收割只是账面积压，resultDataCount 可见，无丢失风险）。**异步泵两相**（pumpAsync）：交接相 = 共用 selectNextLiveJob 选活序逐 job submitAsync（受理即边① handout + 记入 m_inFlight 账本；死 job 只清自己账面、在途别名保留）；收割相 = takeCompletedAsync FIFO 逐完成记录 → 边②（成功面）+ deliver 投递（**取消/epoch 过滤在投递时刻生效**——交接后才过期的在途结果被静默丢弃 = 验收③后台版；Error 载荷经 CompletedGeneration（持 Error 聚合——Result<void> 不可默认构造不宜作队列成员）换算回 Result<void> 走既有 deliver 穿透）。**结果应用主线程**：worker 只算不自写世界，applyGeneratedChunkData 经 ChunkManager::setBlock 5 参守卫（越界拒/标脏/边界邻接标脏）落格；生命周期边①②与 R20.11 同款 best-effort（终态 Generated ⟹ 两边皆合法经历——Absent 上边②必被守卫拒）。**固定世界零变化**：默认稳态 Loaded chunk submit+泵收干 outcome 正常而 9 格生命周期表逐位不动（r2012d⑥ 后台版复证）+ **后台 worker 不接生产路径**（world.cpp/worldstore.h/Main.qml 零引用反探）= app 面零变化。

**矩阵（578→582）**：r2012a（非阻塞 + 真线程身份 + 9 缓冲三要素戳）；r2012b（纯函数性承重墙：双实例 ≡ 同步共享函数 ≡ 真世界 256 列锚 + 方向对照）；r2012c（epoch 丢弃后台版 + 交接前取消）；r2012d（三场景退出安全 + Absent→Loading→Generated 互锁 + 守卫入口落格逐位比对 + 取消不推进 + 固定世界零变化 + 源码钉[QObjectFree 输出钉/线程原语/cv 唤醒双点位/容量上界/异步缝覆盖/背压串/chunk 生成器/守卫应用入口/kChunkSize≡Chunk::kSize 互钉 + terraingen 单一权威五钉 + world.h/cpp 接线四钉] + 反探[backgroundgeneration.h·terraingen.h 零 Q_OBJECT/Q_INVOKABLE/Q_PROPERTY、world.cpp 零 buildPermutation/m_perm/BackgroundGenerationWorker、worldstore.h/Main.qml 零 worker 引用]）。

**验证**：分段增量构建（-j 4；world.cpp + section16 新 TU + helpers/CMake + 链接）→ binary mtime > 全部改动源 → filter r2012 **4P/0F ×3 连跑**（防 flake 三连，全绿才继续；无裸 sleep 断言、deadline 面超时才红、线程身份 id 对账）→ 前置回归 **r2007 5P / r2008 4P / r2009 4P / r2010 4P / r2011 4P / worldgen 22P / determin 30P / re-gen 5P / store 43P / save 34P** 全绿 EXIT=0（迁移零漂移由 worldgen/determin/re-gen 承重）→ **阴性轮**（存证 matrix_r2012_neg.log 含双变异 + r2011 不误伤验证 + restore 段，见下）→ 全矩阵 **582 PASS / 0 FAIL ×2**（matrix_r2012_pos/final.log，EXIT=0；两轮 PASS 腿名 diff = 空；578 权威 diff = **+4 恰为本单 r2012a-d** + 4 登记类 = t813 构建戳时钟（15:→17:）/ t997 计时数字 / t979 涉水数字 / t1023c 腿名同变更修订，零未归因漂移）→ app voxelsandbox 重建 EXIT=0 + offscreen 冒烟 **EXIT=124** 存活 + logs/voxelsandbox_r2012_tail20.log 稳态 60fps（尾部 WRN = timeout kill teardown 噪音与基线同源；后台 worker 不接生产路径 → app 面零变化，如实登记）。

**阴性轮（实跑存证 matrix_r2012_neg.log + 还原段）**：变异①**摘「过期结果不投递」**——deliver() 的 `req.epoch != m_worldEpoch` 子句前置 `false &&`（调度侧丢弃语义本体精确摘除；同步腿不受扰的结构根据 = r2011b 的过期断言钉在**选活扫描** selectNextLiveJob 的同形子句[未动]，且同步 pump 执行+投递同调用内 epoch 不可能中途变化）→ rebuild → **恰红 r2012c 单腿**（[epoch outs=2]——过期 K1 outcome 被投递，diag 形态即被摘语义本体；r2012a/b/d 不受扰 + filter r2011 4P/0F 不误伤验证）→ Edit 反向还原；变异②**摘「结果经守卫入口写入」**——applyGeneratedChunkData 的 `if (id == 0)` 空气跳过改 `if (true)` = 全部跳写（守卫写入调用点摘除）→ rebuild → **恰红 r2012d 单腿**（落格逐位比对全零 vs 缓冲非零；其余三腿不受扰）→ Edit 反向还原 → rebuild → filter r2012 回绿 4P/0F EXIT=0。恰红面设计先于腿文定稿：辅助断言只钉相对恒等（A≡B/方向对照），绝对计数归被测语义腿（r2012c 的 outcome==1 即 epoch 丢弃语义本体）。

**过程坑登记**：①Result&lt;void&gt; 不可默认构造（私有 ctor + 非 aggregate）——CompletedGeneration 首版 `Result<void> result{}` 编译红；改持 Error 聚合（可默认构造、trivially copyable 钉不破），调度侧 pumpAsync 换算回 Result 走既有 deliver 穿透；worker 内部队列直接复用 CompletedGeneration 类型（首版重复定义 Completion 结构体 operator= 不匹配即改）；②r2012d 腿名 "deterministic" 与 determin 过滤词碰撞（determin 30→31P 假漂移）——改词 "ordering inherent, not timing-dependent" 消除，determin 回 30P；③noise3 迁移首版嵌套括号笔误（内层 lerp 尾参 u/v 混淆）——写入时即对照原文修正（逐位同构是承重墙，非可选优化）；④std::thread 成员声明序陷阱——threadLoop 触碰的锁/队列必须先于线程成员构造（成员声明序 = 构造序），已按此排序并注释。

**非目标（登记）**：①真实生产接线（Absent-only 策略层 / World 挂点访问器 / 现网生成调用点迁移）——本单后台 worker 零生产接线 = 固定世界零变化最强形态，接线 = 后续单本体；②Loading 失败恢复策略（六态表无失败边；后台 worker 生成恒成功面，失败路径由缝的 Error 穿透承载）；③其余结构路径后台化（迁移清单「未迁」面，R20.13+ 按需逐个收口——MeshBuilder 先行）；④Edits-on-evict / streaming 驱逐器（延续 R20.10 登记）；⑤箱车裸键门残余（随下一波 parity）/ review0915 #4/#6/#7 维持既有登记。

**待实机确认**：无（后台 worker 不接生产路径 = 生产行为零变化——app 冒烟稳态 60fps）。**下一任务：R20.13 MeshBuilder**（plan §29.3：CPU 网格算法从 ChunkGeometry 抽出——MeshBuilder 不依赖 QQuick3D / 输入 snapshot / 输出 owning ChunkMeshData / 旧 QtQuick3DAdapter 可消费新结果 / 网格视觉与顶点统计与旧路径一致）。

### R20.13 MeshBuilder（2026-09-15，网格算法单一权威抽出：CPU mesher 自 ChunkGeometry 整体迁出——快照输入 / owning 输出 / 适配面消费 / 顶点逐位一致）

**五验收（plan §29.3 R20.13 原文）逐条断言**：①「MeshBuilder 不依赖 QQuick3D」——meshbuilder.{h,cpp} 零 QQuick3D include（r2013d 反探常驻钉死；QQuick3D 灌注只留在 ChunkGeometry 适配面）（r2013d）；②「输入是 snapshot」——唯一输入 = ChunkMeshSnapshot（R20.06 snapshot.h 头注登记的「稠密方块域快照本单消费侧」）：采集函数把原散布在网格循环的现查面（Facade blockAt/stateAt/sky/block 四门 + columnTopSurfaceY 列顶）一次性采成自持稠密域，采集后底层世界改动不影响再构建输出（r2013b：采后破柱顶+放 Stone → 同快照逐位复现 + 拷贝独立 + 重采非空转 + accessor OOB 语义与 World 同面）；③「输出是 owning ChunkMeshData」——顶点/索引/统计全自持（QVector<Vtx>/QVector<quint32> + vertexCount/triangleCount），QObjectFree + copyable 双 static_assert 钉，ChunkGeometry 只镜像统计与灌注（r2013c 计数穿透 + r2013d 结构钉）；④「旧 QtQuick3DAdapter 可以消费新结果」——ChunkGeometry::buildMesh 收敛为「bake 参数定格 → captureChunkMeshSnapshot → MeshBuilder::build → 灌 QQuick3D」，Q_PROPERTY/信号面零变化（r2013c 消费面回归 r2008c 同款：in-chunk 编辑即时重建×2 / out-of-chunk 跳过且零计数）；⑤「网格视觉和顶点统计与旧路径一致」——抽取=搬移不改写，r2013a 承重：9 段路由变体（terrain/water/lava/glass/ice/cutoutOnly/cutout 恢复态/greedy/AO）× 2 chunk（内陆 + 世界边）双路径顶点 48B/Vtx + 索引逐字节一致 + 已知小场景悬浮立方 Δ24v/Δ12t(36 idx) 精确差（两路同值）。

**抽取搬移清单（chunkgeometry.cpp → meshbuilder.cpp，逐段；搬移非复制——r2013d 反探钉「网格本体不回流」）**：FaceDef/kFaces 六面表 / occludesNeighborFace（t746 叶特例）/ tileFor（箱子·熔炉·发射器·投掷器·耕地·末地框·南瓜·红石灯八特例查表）/ farmlandHydrBrightMul / AtlasTileCount UV 常量（tileW/hx/hy/v0/v1）/ PASS1 异形+cross 合批（partial/cross/door/trapdoor 路由 + PartialLightCtx 六向光 + railProbeDelta/dustClimb/LilyPad 上下文）/ PASS2 流体变高面（fluidId/maxLevel/stripFrames 水岩浆参数化 + t892 surfH/renderTop + t893 流向四向旋转 + t391 静态水涟漪）/ greedy mask 合并（MaskEntry tile+sky+block+hydr 合并键）/ 逐格 culled 主路（段分流 + AO 三探针 + dayMul 只乘天光公式）/ FrameProfiler meshN 族事件计数。**伴生机械替换两类**：数据来源（`m_world->skyLightAt(...)` → `snap.skyLightAt(...)` 等同名同签名访问器；`m_lavaOnly` → `snap.lavaOnly` 等段开关）与输出容器（局部 verts/idx → ChunkMeshData 成员引用）；`sunShadowAt` 语义由 VoxelLight::sunShadow **泛化缝**承接（voxellight.h 新增 sunShadowColumnTop 模板——PCF 数学体单点保留，World 版委托之，BlockCube 零改动，禁两份软影逻辑）。

**快照类型选型（meshbuilder.h 头注立此存照）**：放置层 = **src/World**（meshbuilder.{h,cpp}，同 mesher 子系统 partialblockgeometry/voxellight 同层先例）——依据：①消费 Vtx/PartialLightCtx/PartialNeighborCtx（World 层）与 Chunk::kSize；②快照域是 chunk 网格（16×16×H+pad）——放 src/Core 违「Core 叶子不依赖 World」，放 src/Renderer 造成 chunkgeometry(World)→Renderer 向上依赖破 PLAN §2（t41/partialblockgeometry 同族判例）。域设计：pad=2（AO 探针自邻格再 ±1 的最大触达）→ 块/state/光四数组 20×20×H；列顶域 kTopDim=21（PCF kMaxShadow=2 步进 + floor/ceil 半格触达 [origin-2,+18]，与 kMaxShadow 编译期互钉——增大须同步扩域）；元数据 = 采集时定格的 ChunkGeometry 烘焙状态（dayMul/sunDir/shadowsEnabled/aoEnabled/greedyMeshing/六段路由开关）+ 原点/世界高；accessor 与 World 查询同名同 OOB 语义（y≥H 天光 15/域外空气/列顶 -1）。

**采集成本登记（如实）**：每次重建新增一次 16×16×H（pad 域 20×20×H）列扫 = 4 数组 ×38.4k 格查 + 21×21 列顶查 ≈15.4 万次 Facade 查询/次——旧路径同量级查询散布循环内（每格被自身+6 邻重复读），采集前移为一次性顺序扫，总量同阶、位置集中；这是后续 worker meshing 的输入形态预演（快照自持可跨线程、build 纯函数于快照零可变态）。实测编辑重建链仍即时（r2013c + app 冒烟 mesh 0reb 稳态）。

**worldfacade 补口**：height() / columnTopSurfaceY(x,z) 两纯转发（采集统一走收窄面；r2008a 全部 minCount 钉不受扰，r2008 全绿复证）。**t879/t893/review27-15 三源码钉同变更修订**（t1023c 先例义务履行）：三腿源码钉原指 chunkgeometry.cpp 的 mesher 本体文本（isCutoutTrapX 路由 / flowDir 流向判定[含 `m_lavaOnly`→`snap.lavaOnly` 机械替换] / nctx.belowId LilyPad 填充）随迁改指 meshbuilder.cpp——钉语义不变（钉的就是网格本体，本体搬家钉面跟随）。

**验证**：分段增量构建（-j 4；meshbuilder 新 TU + chunkgeometry.cpp 改 + helpers/CMake + 链接）→ binary mtime > 全部改动源 → filter r2013 **4P/0F ×3 连跑**（防 flake）→ 前置回归 **r2007 5P / r2008 5P / r2009 4P / r2010 4P / r2011 4P / r2012 4P / worldgen 23P / determin 30P / re-gen 5P / store 43P / save 34P / t155（含于 r2007-r2009 链）/ t860 2P / t1023 3P / vertex 13P / mesh 30P** 全绿 EXIT=0（渲染路径重构单：矩阵顶点腿族 + app 冒烟双确认）→ **阴性轮**（存证 matrix_r2013_neg.log 含三变异 + restore 段，见下）→ 全矩阵 **586 PASS / 0 FAIL ×2**（matrix_r2013_pos/final.log，EXIT=0；两轮 PASS 腿名 diff = 空；**582 权威 diff = +4 恰为本单 r2013a-d 置尾**，零未归因漂移）→ app voxelsandbox 重建 EXIT=0 + offscreen 冒烟 **EXIT=124** 存活 + logs/voxelsandbox_r2013_tail20.log 稳态 60fps + F3 `mesh 0.00(0reb [0d 0s 0w])` 计数行稳态（尾部 wakeUp WRN = timeout kill teardown 竞态与 r2012 基线同源；7 条 qrc QML Delegate/size WRN = 既有启动噪音，零错误）。

**阴性轮（实跑存证 matrix_r2013_neg.log + 还原段）**：变异①**摘 AO 因子**——meshbuilder.cpp aoCorner 的 `VoxelLight::aoCornerFactor(o1,o2,oc)` → `1.0f` → **恰红 t1023b 单腿**（既有 AO 顶点腿·绝对角点阶梯 {0.5,0.8,0.8,1.0}×flat 断言；r2013a 不误伤——AO 变异在共享新实现内，适配路径委托同一实现双向同错仍互等 = 相对恒等腿的恰红面收缩正确）→ Edit 反向还原；变异②**顶点序翻转**——MeshBuilder::build 返回前 `std::reverse(verts)`（计数不变位序全变）→ **恰红 t1023b 单腿**（四连顶点四边形隔离的角序阶梯为序敏感断言；同时暴露并登记：r2013a 相对恒等腿位序盲——纯位序扰动在新旧共享实现后无相对面可测，此覆盖边界如实入账）→ Edit 反向还原；变异③**摘 bake 镜像**——chunkgeometry.cpp `bake.aoEnabled = m_aoEnabled` → `false`（适配消费缝变异，仅新腿可见）→ **恰红 r2013a 单腿**（geo 侧 AO 被摘 vs 腿侧显式 bake 双向失配 → AO 变体字节比对红；其余变体与 r2013b/c/d 不误伤 = 新腿恰红实证）→ Edit 反向还原 → rebuild → filter r2013 4P/0F + t1023 3P 回绿。

**过程坑登记**：①初版「自由柱」选址判据（h+1 四向空气）在 seed 82 内陆带全灭（林区 TallGrass/树冠占格致 [rig][known-site][edit-sites] 三处选址失败假红）——改「空气袋」扫描（自格+6 邻全空、y∈[40,H-2] 高空带）+ 悬浮立方 6 面全见 Δ24v/12t 精确可算；②triangleCount 口径 = 三角数（idx/3）非索引数——首版断言写 36（索引数）而实测 12（三角数）恰红，diag 双路径 20696→20720 同值即证等价成立、纯断言口径错（教训：顶点统计断言先核 t10 以来「triangle = idx/3」口径）；③三源码钉腿（t879/t893/review27-15）在 mesh filter 回归暴露红——文本钉指向已搬移本体，同变更修订（非削钉）后回绿；④阴性轮设计修正：抽取后「旧路径」= 委托同一实现，共享数学体的变异只能被绝对值断言腿（t1023b）抓、共享数据面的变异才能被相对恒等腿（r2013a）抓——恰红面按「变异落点 ∈ 谁的唯一消费域」设计。

**非目标（登记）**：①worker meshing（快照可跨线程 + build 纯函数已就绪，线程化 + 调度接线 = 后续单本体；t1023c 线程原语白名单不触——本单零线程）；②快照缓存/复用（现采现建即弃，缓存策略归 worker meshing 单）；③greedy 合并键含 AO / 流体水面 AO / 异形段 AO（t1023 报告既有登记不变）；④Loading 失败恢复 / Absent-only 策略层 / Edits-on-evict / 箱车裸键门残余 / review0915 #4/#6/#7 维持既有登记。

**待实机确认（必须列——网格视觉一致性属观感面，矩阵字节比对不能替代目视）**：①地形段整体观感（culled 贴图/光照/软影与重构前无异）；②水面分层与涟漪（t892 液面 7/8 + t391 波带）；③冰/玻璃半透段；④异形方块（门/活板门/栅栏/铁轨坡度/红石粉爬墙/睡莲叶高）；⑤世界边界剔除（边到边无缝）；⑥AO 开关观感（若用户开 aoEnabled）。**下一任务：R20.14 EntityStore**（plan §29.3：先迁移掉落物或一个简单实体族，不同时迁移全部生物——EntityId 稳定 / 模拟实体不依赖 QML / 可见实体由 snapshot 决定 / 旧 EntityManager 可作过渡 Adapter）。

### R20.14 EntityStore（2026-09-15，掉落物模拟单一权威抽出：ItemEntityManager 收敛过渡 Adapter——EntityId 稳定 / 模拟零 QML / 可见实体快照决定 / 孪生等价承重）

**四验收（plan §29.3 R20.14 原文）逐条断言**：①「EntityId 稳定」——store 侧每活体在 acquireSlot 时刻获配**稳定 EntityId**（quint32，自 1 单调递增、永不复用、存活期恒定；后续 Event::entityId（quint32，event.h「0 = 非实体域」哨兵）与存档域的对接口径）；**双轨语义过渡**（entitystore.h 头注立此存照）：槽位下标（t256/t978 LIFO 复用、count 单调不降）= QML Repeater 消费面承重契约**逐位保留**，EntityId 是 plan 级稳定 id 与槽位解耦——同槽先后两任实体 EntityId 必异（r2014a：id 严格递增 + idD≠被移除的 idB + 合并幸存者 idA 跨后续全部操作恒定）；②「模拟实体不依赖 QML」——EntityStore 非 QObject、零 QtQml/QtQuick include、零信号（QObjectFree static_assert 编译期钉 + r2014d「Q_OBJECT/Q_PROPERTY/Q_INVOKABLE/QQuick/qqml 禁出」反探常驻）；世界读经 World* 参数向下只读（blockAt/stateAt/isCollidable/supportTopYAt，同 ItemEntityManager 旧分层纪律），变更通知经 notifySink 回调缝上行（QObject 信号面归 Adapter）；store 直驱无 QObject 即全模拟可达（r2014a 全序列裸 store 跑通 = 行为级佐证）；③「可见实体由 snapshot 决定」——EntityStoreSnapshot/EntityStoreEntry 值类型（QObjectFree + copyable 双钉，全值成员自持/无指针/只读观察面）在**每处 notifyChanged 沿 + clearAll 重建**（m_visible 缓存 ≤kCap 条目）；r2014b：沿后立即可见（远格 spawn 沿即新条目+新 revision）/旧快照值隔离/深拷贝独立（拷贝可变不回灌）/空槽不入集（可见集恰=活体集，逐槽对账 aliveAt）/元数据可见面齐备（附魔/名 trimmed/耐久/EntityId）/Adapter 面快照与其逐槽读口对账；**本单边界（诚实划线）**：QML/instancing feeder（BlockDropInstancing 等）仍经 Adapter 逐槽读——feeder 改读快照属渲染管线大迁移=后续单（登记非目标），消费面语义零变化是本单承重墙；④「旧 EntityManager 可以作为过渡 Adapter」——ItemEntityManager 全部 Q_INVOKABLE/Q_PROPERTY(count/revision)/entitiesChanged 信号面零变化，方法体**一行委托 m_store.\*（不复制逻辑）**；r2014a 承重：同一 11 步操作序列（合并三连/带名不合并/定向弹出/部分拾取/removeAt/投掷 LIFO 槽复用/40 帧重力落定/岩浆焚毁/批内合并/寿命注入 despawn/空场 clearAll）驱使裸 EntityStore 与 Adapter **每步后全状态逐位一致**（count/revision/liveCount/highWater + 逐槽 alive/resting/pos/itemId/count/entityId/enchants/name/durability）。

**迁移族选型依据（头注立此存照）**：掉落物族（ItemEntityManager 所辖）= plan 原文首选项「先迁移掉落物或一个简单实体族」——最独立（不依赖 EntityManager 总管、自含模拟闭环、spawn 物理/合并/拾取/despawn/焚毁全链自持）、headless 探针先例最厚（t1027/t867/t804/t889 直编先例）；生物族（EntityManager 总管）零触碰（plan「不要同时迁移全部生物」红线）。**放置层 = src/Entities**（entitystore.{h,cpp}）：PLAN §2 分层「Entities: 玩家/生物/掉落物」本就该容纳掉落物模拟权威（ItemEntityManager 头注既有自述「本层属 Entities」——它只是暂居 src/Game 的呈现 ViewModel）；store 放 Entities、Adapter 留 Game = Game→Entities 向下依赖合规（PLAN §2「依赖只向下」），store 向下只依赖 Core（blockregistry/result）+ World 前向。

**Adapter 收敛面**：①Q_INVOKABLE/Q_PROPERTY/C++ 直调面（PlayerController pickupScan 的 aliveAt/countAt/enchantsAt/nameAt/durabilityAt/setCountAt/isPickupReady/tick/spawnItemThrown/deferWallClocks + QML Repeater 的 count/revision/aliveAt/posAt/itemIdAt/clearAll + feeder 的 entitiesChanged 连接与家族谓词）**签名与语义零变化**；②notify 三沿语义逐位保留：非批 notifyChanged 即 emit / t354 批内标脏 endBatch 末尾 1 次收口 / clearAll 无条件直发（**不 bump revision**——旧 clearAll 只 emit 不 ++revision，r2014 首跑误设 +1 恰红已修正断言并登记为既有语义）；③渲染家族谓词五件套（isItem3DFamily/isPlainCubeDrop/isTool3DDrop/isIconBillboardDrop/isBlockIconBillboardDrop）留 Adapter：渲染域单一权威（QML delegate 排除侧与 C++ feeder 收纳侧同源）非模拟域，t1027b/t1041d/t925 源钉同址保留（搬它们=纯钉面扰动零验收价值）；④新增测试缝 ageLifetimeClock(qint64)（deferWallClocks 对偶——+ms 只能顺延寿命不能老化；生产路径零调用，t1029 缝纪律；Adapter 同名镜像供孪生腿同步驱动）。**「模拟本体禁回流」反探**（r2013d 同款）：itementitymanager.{h,cpp} 禁再现 kMergeRadius/kGravity/acquireSlot/m_freeSlots/supportTopYAt/std::vector<ItemEntity（r2014d 常驻）。

**EntityId 语义选型（store 侧 id 口径）**：本单选择「**槽位 + EntityId 双轨**」而非「id 全量替换槽位」——①QML Repeater 消费面（count 槽表长模型 + 槽下标寻址 + LIFO 复用 + count 单调不降防 delegate 泄漏）是 t256/t978 三层修法的承重结构，id 替换槽位=重写呈现契约（违反现行玩法路径零变化）；②plan「EntityId 稳定」的语义核心 = 实体的身份标识跨其生命周期恒定且不因其它实体生灭而复用——EntityId 在 store 层满足该语义（r2014a 断言），并预留 Event::entityId（quint32 域对齐）/存档序列化/未来跨族统一（EntityManager/XpOrb 等后续迁移单沿用同口径）的对接口径；③快照条目同时携带 slot 与 entityId 两键（findSlot 槽寻址 + entityId 稳定身份），消费方按需取用。

**验证**：分段增量构建（-j 4；entitystore 新 TU + itementitymanager 收敛 + helpers/CMake 接线 + moc 波及面）→ binary mtime > 全部改动源 → filter r2014 **4P/0F**（首跑后修两处断言口径，见坑登记；终态 4P ×6 连跑含 redesign 复核）→ 前置回归 **r2007 5P / r2008 5P / r2009 4P / r2010 4P / r2011 4P / r2012 4P / r2013 4P + 掉落物腿族 t1027 2P / t1032 6P / t1039 5P / t1041 4P / t882 4P / t927 5P + 邻近面 t867 1P / t889 2P / t804 1P / t814 2P / t856 1P / t868 1P** 全绿 EXIT=0（迁移单：Adapter 消费面全族回归 + app 冒烟双确认）→ **阴性轮**（存证 matrix_r2014_neg.log 含 definitive 变异 + restore 段，见下）→ 全矩阵 **590 PASS / 0 FAIL ×2**（matrix_r2014_pos/final.log，EXIT=0；586 权威 diff = **+4 恰为本单 r2014a-d 置尾**；两轮 PASS 腿名 diff 仅 t979 涉水 exits 15/16 + t997 tick 计时 0.1752/0.1791 两处**登记漂移类**，零未归因漂移）→ app voxelsandbox 重建 EXIT=0（binary 23:10 > src 22:42）+ offscreen 冒烟 **EXIT=124** 存活 + logs/voxelsandbox_r2014_tail20.log 稳态 60fps + item tick 行 0.00 稳态（Adapter→store 链常开正常）。

**阴性轮（实跑存证 matrix_r2014_neg.log + 还原段）**：变异①**NEG1 摘 spawnItem 合并**——entitystore.cpp spawnItem 合并守卫 `false &&` 前缀（r2011b「使分支永不可达」惯用法）→ **恰红 r2014a + t1027 恰两组**（两组均含合并语义绝对断言：r2014a 合并三连 liveCount==1/count==3/批内合并 count==2；t1027 批 1 腿 okMerge liveCount==1/mergedCount==3）——r2014b/c/d 全绿 + 跨单 r2011 4P 无误伤（**恰红面设计先于腿文修正**：首版 r2014b/c 场景夹具含合并步被 NEG1 误伤成 4 腿红 → 按 R20.11「辅助腿只钉相对恒等」纪律重设计为无合并场景[格距 >kMergeRadius]，红面收缩到语义腿恰两组）→ Edit 反向还原；变异②**NEG2 摘 notify 沿快照重建**——notifyChanged 内 rebuildVisibleSnapshot() → `if (false)` → **恰红 r2014b 单腿**（快照陈旧 vs 活体读失配；r2014a 不读快照故绿 = 变异落点 ∈ 快照腿唯一消费域）→ Edit 反向还原 → rebuild → filter r2014 4P/0F 回绿。

**过程坑登记**：①store 声明缺缺省参（Adapter Q_INVOKABLE 有缺省而 store 直驱调用编译错）——补齐与 Adapter 一一对应；②beginBatch 头内 inline + cpp 定义重复（->error）——收敛头内 inline；③**clearAll revision 语义误判**：旧实现只 emit 不 ++revision（我按「变更必 bump」预设断言 rev+1 恰红）——核旧码后确认 store 逐位保留是对的、断言改回 rev 不变（教训：迁移单的绝对断言先核旧实现逐位行为再落笔）；④重力落柱选址漏验 h+2（林冠占格致落点 y 出窗假红）——补 h..h+3 全验 + settle/burn/despawn diag 补齐（选址结构性保证纪律的又一实例）；⑤NEG1 首轮红面 4 腿超设计声明——按恰红面纪律重设计腿场景（见上）而非放宽变异（教训：**腿文定稿前先写阴性轮期望红面**，R20.11 教训在共享实现+场景夹具组合下的再现）。

**非目标（登记）**：①feeder 改读快照（BlockDropInstancing/GlowShell/ToolDrop/BillboardDrop 改为快照驱动 = 渲染管线大迁移，plan「可见实体由 snapshot 决定」的完全体；本单只落快照权威与沿重建，消费面零变化承重）；②生物族迁移（EntityManager 总管/XpOrb/Boat/Minecart 家族沿用本单 EntityId+Adapter 口径逐族后续单）；③Event::entityId 接线（EventQueue 实体事件填充 entityId = 后续事件面单）；④掉落物存档持久化（现 clearAll 即弃语义不变；持久化归 SaveCoordinator R20.15+ 域）；⑤Loading 失败恢复/Absent-only 策略层/Edits-on-evict/箱车裸键门/review0915 #4#6#7 维持既有登记。

**待实机确认（必须列——掉落物行为属可无头断言面但观感仍须目视）**：①合并动画观感（两实体合并不加 delegate、count 数字跳变是否自然）；②拾取吸附节奏（免拾窗 0.5s + 磁吸观感与迁移前无异）；③爆炸批量掉落（t354 收口的 delegate 一次性补齐无卡顿）；④岩浆/火/仙人掌焚毁瞬间消失（无动画语义确认）；⑤冰面长滑/薄支撑贴面（t468/t867 物理手感）。**下一任务：R20.15 SaveCoordinator**（plan §29.3 原文：先保持 SQLiteAdapter，再统一 SaveGeneration 和恢复状态——保存不直接读取正在变化的 WorldState / 玩家世界进度统一 generation / 保存失败不报告成功 / 旧档仍可读 / 故障注入可模拟锁·磁盘错误·损坏 Chunk）。

### R20.15 SaveCoordinator（2026-09-15，存档协调层：SQLiteAdapter 零改动之上统一 SaveGeneration 与恢复状态——冻结-持久化分离 / marker-first 部分写防护 / 失败绝不报成功 / 旧档双向兼容 / 故障注入三缝）

**五验收（plan §29.3 R20.15 原文）逐条断言**：①「保存不直接读取正在变化的 WorldState」——**冻结快照协议**：`SaveCoordinator::saveAll` 对活体 World 的唯一读点 = `captureSnapshot`（步②），此后持久化只消费 `WorldSaveSnapshot` 值（seed/dims + 逐 chunk 三段深拷贝 blob）；下游 WorldStore::saveAll（现有接口零改动）经**内部冻结缓冲世界**读到的恒是冻结点——r2015b 承重墙：`SaveFaultHook` 的 World 段窗（冻结后、持久化前）内改动活体（拆 A + 放 B），重读档 = 冻结点逐字节（voxels/states/light 三段 memcmp 全等，A 在 B 无）≠ 活体（A 去 B 在，两栅格指纹必异）——单线程下「保存期间世界在变」的无头行为级实证；②「玩家、世界和进度有统一 generation」——一次 saveAll 调用 = 一个代次 newGen 覆盖三部分（world 走下游事务性 saveAll、player 走 savePlayerData、progress 走 saveProgress），三部分全部成功才在 save_coord 台账盖 `complete_generation` 戳——三部分共享同一 complete 代次（r2015a：receipt 一张、saveOkCount 恰 +3、Clean 1/1→2/2 单调）；③「保存失败不会报告成功」——receipt.ok()==!isError(error)；**complete 戳是「成功」的唯一权威**：任一请求部分失败/故障注入/戳写失败 → receipt 一律失败；r2015c 注入 Finalize 段（各部分已写、崩溃在收尾前）receipt 仍失败（rok=0）——数据完好也不得报成功；真锁（第二连接 BEGIN EXCLUSIVE，t974 先例）下 marker-first 第一闸使**零部分尝试**（saveOkCount 恰 +0）+ receipt 失败；④「旧档仍可读取」——兼容性论证见下节；r2015a（无台账旧库 = Fresh，legacy 裸写面照常）+ r2015c（真锁失败后/部分写后/崩溃收尾后/台账被清后四种形态重读档全部照常：栅格指纹逐位等 + player/progress 旧值正确）；⑤「故障注入测试可以模拟锁、磁盘错误和损坏 Chunk」——锁 = 真实第二连接 BEGIN EXCLUSIVE（r2015c a 段）；磁盘错误/崩溃 = `SaveFaultHook(SaveFaultStage)` 按段注入（BeginMark/World/Player/Progress/Finalize 五段；生产零挂载恒不触发）；损坏 Chunk = 外部 SQL 直改 blob（r2015b：voxels 截 7 字节 → worldstore 既有尺寸守卫跳过坏块，9 存 8 载入、好块内容无损、返回数即损坏可见面，台账域不受扰）。

**兼容性论证（数据安全硬门逐项，双向读写）**：本单**零不可逆存档格式迁移**——①落盘字节面：chunk blob 三段布局/world_meta 既有键/各业务表（chunks/player_state/chests/furnaces/dispensers/progress）**零变化**（worldstore.{h,cpp} 一行未动——r2015d 反探钉 save_coord/SaveCoordinator 禁入 worldstore + `kSchemaVersion = 2`/saveAll 签名/saveOkCount 属性三正面钉）；②协调层新增面仅一表两键：`save_coord(key TEXT PRIMARY KEY, value TEXT NOT NULL)` 存 'generation' 与 'complete_generation' 两行十进制文本，CREATE TABLE IF NOT EXISTS 幂等补建、不 bump user_version 不 bump world_version——**旧代码读新档**：worldstore 全部读路径对未知表全盲（loadMeta 只读 world_meta，其余读路径只碰自己的表）→ 零影响；**新代码读旧档**：save_coord 不存在 → recover() SELECT 失败 → Fresh（0/0，视为无台账合法态）→ 零影响；删台账键（r2015c d 段）同型回 Fresh 且读路径不受影响；③协调层台账用**独立命名连接**（"voxelsandbox_savecoordinator"，仿 worldstore scan/rename 独立连接先例），与 WorldStore 主连接互不占用，两连接并发同文件由 SQLite 锁协议仲裁（t974 已实证该形态）。

**SaveGeneration 与恢复语义（最小落地，头注释立此存照）**：代次 = 保存批次号（qint64），newGen = max(最后尝试, 最后完整)+1——**中断/失败消耗号段但不重置不复用**（r2015c：1→[锁失败不记号]→2(注入 Player)→3(注入 Finalize)→4(收敛 Clean)，跨中断单调）；恢复状态机三态：Fresh（无台账 = 协调层之前旧档/新库/台账被清——合法干净态）/ Clean（generation == completeGeneration = 最后一次保存完整收尾）/ Interrupted（generation > completeGeneration = 最后一次保存未收尾，部分写可能存在）。**marker-first 部分写防护协议**（崩溃窗口逐段枚举于 savecoordinator.h 头注）：步①先落在途标记（台账写不进 = 外部锁/磁盘错误 → 立即放弃**零部分尝试**）；步②③冻结 + World 段注入窗（未触库）；步④-⑦ world→player→progress 逐部分写且**短路**（前序失败后序不试——杜绝新 player 压旧 world 的混合写）；步⑨全部成功才盖 complete 戳。崩在中途：已写部分新版/未写部分旧版，gen>complete 如实标记，旧完整代次数据照常可读（SQLite 事务保证每部分自身原子）；**协调层不回滚不改写存档数据**（只增不改 = 兼容性底线），Interrupted 的最小恢复动作 = 上报调用方按需重存收敛（r2015c e 段实证收敛到 Clean 4/4）。**冻结缓冲**：WorldStore::saveAll 从 m_world 读 blob（现状接口）→ 协调层把冻结快照回放进内部缓冲世界（dims 惰性重建 + beginLoad 零填充无 worldgen，跨保存复用）+ 保存期临时改绑下游、收尾恢复原绑；m_store/m_buffer 两裸指针成员（QObjectFree 边界登记：指针不构成子对象），m_buffer 拥有故本类不可拷贝。

**接线与零变化面**：新件 src/World/savecoordinator.{h,cpp}（World 层存档域正席，与 worldstore 同域；非 QObject 无 AUTOMOC；QObjectFree 五连钉 + 快照 copyable 钉在头）。**零生产接线**（R20.11 GenerationJob 先例）：world.{h,cpp}/worldstore.{h,cpp}/Main.qml 零触碰（r2015d 反探钉三面常驻）→ QML 零迁移、现行保存/读档行为零变化是结构性事实；WorldStore 只经现有 Q_INVOKABLE 面被驱动（saveAll/savePlayerData/saveProgress/isOpen/world/setWorld）。错误码 2xx 段 = 存档协调域（result.h 百位段约定延伸：201 NotBound/202 StoreNotOpen/203 CoordSql/204 SnapshotApply/205 StoreRejected/206 FaultInjected）。

**section19 四腿（filter r2015；矩阵 590→594）**：r2015a 统一代次语义[一代次三部分+saveOkCount 恰 +3/+7、Fresh→Clean 1/1→2/2、legacy 裸写两域正交（store 不产代次）+player/progress 逐代次往返]；r2015b 冻结-持久化分离承重墙[World 段窗内漂移活体→重读=冻结点三段逐字节≠活体栅格 + 损坏 chunk 降级一致 9 存 8 载好块无损]；r2015c 故障注入+恢复三态[真锁零尝试零计数+旧档照读、注入 Player=world 新/玩家旧如实 Interrupted 2>1、注入 Finalize=全写仍不报成功 Interrupted 3>1、收敛单调 4、删台账→Fresh 读路径无损]；r2015d 结构钉[QObjectFree×5 编译期复述+快照值隔离探针+协议关键步唯一落位正面钉+Adapter 全盲/QML 零面/Main.qml 零提及/world 零接线反探+kSchemaVersion=2 正面钉]。

**验证**：分段增量构建（-j 4；新 TU savecoordinator + section19 + helpers/CMake 接线；首编一处 const 深拷贝写面编译错即修）→ binary mtime(23:53) > 全部改动源(≤23:53) → filter r2015 **4P/0F ×3 连跑**（EXIT=0）→ 前置回归 **r2007 5P / r2008 5P / r2009 4P / r2010 4P / r2011 4P / r2012 4P / r2013 4P / r2014 4P + store 48P + save 38P + worldgen 23P + determin 30P** 全绿 EXIT=0（store 48=43 台账+2[r2014a/d 名含 store]+3[本单 a/b/d]、save 38=34 台账+4[本单四腿]——差值全对账零未归因）→ **阴性轮**（存证 matrix_r2015_neg.log 含 restore 段：NEG1 摘 finalize 守卫[complete 戳条件 true || 前缀]→恰红 r2015c 单腿[红形态=失败保存被盖戳 st=Clean g=2/2,3/3+receipt 翻 ok=验收③与恢复分类同时失守的本体]；NEG2 摘冻结改绑调用点[`if (false)` 前缀]→恰红 r2015b 单腿[红形态=sameAsLive=1+voxels mismatch@chunk(0,0)=保存了漂移活体的本体]；两处 Edit 反向还原→rebuild→filter r2015 4P/0F 回绿）→ 全矩阵 **594 PASS / 0 FAIL ×2**（matrix_r2015_pos/final.log，EXIT=0；594 权威 diff = **+4 恰为本单 r2015a-d 置尾**；两轮 PASS 腿名 diff 仅 t979 涉水计数 + t997 tick 计时两处**登记漂移类**，零未归因漂移）→ app voxelsandbox 重建 EXIT=0 + offscreen 冒烟 **EXIT=124** 存活 + logs/voxelsandbox_r2015_tail20.log 稳态 60fps（prof[1s] 60fr / frame 16.0ms）。

**非目标（登记）**：①生产接线（Main.qml 保存流迁移到 SaveCoordinator = 后续单；现保存/读档行为零变化）；②掉落物/实体存档持久化（R20.14 登记，沿袭本单 generation 对接口径）；③后台/异步保存线程化（冻结快照值形态已是跨线程就绪前置，线程面归后续单+受 t1023c 白名单钉管辖）；④Loading 失败恢复/Absent-only 策略层/Edits-on-evict/箱车裸键门/review0915 #4#6#7 维持既有登记。
...
### §29.4-P1 GenerationPolicy 策略层骨架（2026-09-16，filter 词 r2016；矩阵 598→602）

  → **✅✅ 落地（2026-09-16）：src/World/generationpolicy.h 新建（header-only 纯决策组件）+ tools/matrix/section20_generationpolicy.cpp 置尾（CMakeLists + helpers.h 声明 + runAll 尾调用三点互钉）；矩阵 598→602。**（fix + test + 本 docs 三段提交；设计单无根因节——写设计选型依据）

**设计选型依据（头注释立此存照，§29.4 D1/D4/D5 建议列为语义依据）**：①**组件形态** = header-only 非 QObject 纯值组件（chunklifecycle/generationjob 同形态，World 层无 AUTOMOC），decide 为 const 纯函数（同输入逐位同输出，零隐状态——r2016d 双实例/双调用恒等实证）。②**decide 纯缝** = `std::function<ChunkLifecycle(int,int)>`：零 World*/Chunk*/QObject* 指针（R20.08 facade 纪律）、零线程原语（t1023c 白名单纪律，纯同步无锁）、null 缝 = fail-safe 恒空、缝返回六态域外值按「不请求不驱逐」忽略。③**距离度量 = 切比雪夫** max(|dcx|,|dcz|)（头注释声明选型：chunk 流式请求域天然方窗 (2r+1)²、纯整数全序零浮点；拒欧氏 sqrt/浮点比较面）。④**streamingEnabled 默认 false = §29.4「固定世界默认关」**：decide 入口短路恒空（任何输入两列表恒空）+ P1 零调用点迁移（World/GameSession/QML 零触碰）双保险 = 固定世界零变化最强形态（r2011 先例）。⑤**D4 双半径 + 归一化**：generationRadius ≥ renderRadius 构造处强制——负值取 0、全部半径类量统一 0..255 域（gen 钳 255 与 GenerationRequest::priority quint8 对齐）、render>gen 收敛到 gen、scan∈[gen,255]；静默规整不抛错的选型理由 = 参数来自配置面（P2 起接线），越界属配置病非运行时错，规整 + params() 读回自检好于抛错打断流式主循环。⑥**scanExtentChunks 第四参数（设计强制增补，相对任务单三参数的登记偏差）**：缝只能点查不能枚举，toEvict 候选域（距离 > gen）必须显式有界——scanExtent 只影响候选枚举边界、不改变任何阈值语义，D1 无限世界下 P3 接线期由驱动方定实际窗宽；同受 [gen,255] 归一化管辖。⑦**toRequest = 闭域 Absent-only**（距离 ≤ gen 含边界）：priority = 距离秩（0 最紧急，GenerationRequest 同门）升序、同距 (cx,cz) 字典序——输出全确定性；非 Absent 五态（Generated/Loaded/Active/Loading/Evicting）一律不请求——r2011 头注释登记的「真实流式世界里 submit 侧必须只对 Absent chunk 提交」策略面本单收口。⑧**toEvict = 半径外驻留三态**（Generated/Loaded/Active 且距离 > gen）：距离降序（最远先卸）同距字典序；**[render, gen] 环带永不驱逐（D4 承重语义——渲染余量）**；Absent/Loading/Evicting 恒不入；驱逐只看 gen 半径（render 单独调不改 toEvict，r2016c 实证）。⑨**值纪律** = QObjectFree 四型编译期钉（Params/Request/Evict/Decision）+ 条目 trivially copyable 钉 + Decision 持 QVector owning 非平凡可拷贝分化登记（ChunkMeshData 先例同款）；条目 operator== 供向量恒等断言。**接线登记非目标**：P2 移动驱动（位置沿 → toRequest 逐项 submit GenerationJob）/ P3 卸载+Edits-on-evict（toEvict 逐项驱动 ⑥⑦ + SaveCoordinator 落盘回灌）/ worker meshing（D6 同批）/ 生产参数定值（P5 调参）/ QML 动态化（P4，全 R20 唯一动 QML 相位）。

**腿面（section20 置尾 4 腿，全段零世界——缝 = QMap 合成生命周期表[缺省 Absent = ChunkManager::lifecycleAt 越界语义同门]，rig 世界零接触，无 RNG/tick/时间源；恰红面设计先于腿文定稿[R20.11 纪律]）**：**r2016a 默认关零变化承重墙**——默认参数本身即「关」形态字段钉（streamingEnabled=false + gen/render/scan 默认值）+ 3 策略实例（默认构造/显式默认参数/显式关异参数）× 3 缝（全 Absent/全 Loaded/手工混合）× 3 玩家位（(0,0)/(5,-7)/(-100,42)）27 组合两列表恒空 + null 缝 fail-safe（enabled 实例同门验证）。**r2016b 激活请求面**——gen=2 render=1 scan=4，玩家原点 5×5 窗 25 格全手工布（五态齐备：Absent 布点含 d0 玩家格/d1 三格/d2 五边界格；Generated/Loaded/Active/Loading/Evicting 各有驻留格）：toRequest 恰 = 9 格 Absent 集（1+3+5）精确序匹配 + priority 非降/同距字典序递增谓词复断言 + 边界格 (2,0)/(-2,2) 点名含入 + 驻留格零请求点名 + (3,0) 半径外 Absent 不请求；场景 2 玩家重位 (3,-2) gen=1（3×3 窗九格全布）钉坐标偏移语义 = 请求相对玩家非相对原点；**本腿零 toEvict 断言**（R20.11 交叉感染纪律——NEG2 时不受扰）。**r2016c 驱逐面**——gen=3 render=1 scan=5：半径外驻留格 5 个（d4×2 同距对/d5×3 同距对）逐个恰入一次（QSet 无重复 + count==5）、期望序 = d5 三格字典序在前 d4 两格殿后（最远先卸）；环带驻留格 4 个 + gen 内 1 个**零驱逐点名（D4 承重）**；半径外非驻留态（Absent/Loading/Evicting）零驱逐；render 1→0 与 1→3（=gen）两变体 toEvict 逐位恒等 = **驱逐只看 gen 半径**。**r2016d 确定性+规整一致性+结构钉（相对恒等面——不钉 disabled 绝对空面，NEG1 不误伤）**——混合缝双列表非空实证（恒等非空转）下同实例双 decide + 跨实例恒等逐位一致；越界入参（render>gen/scan<gen/全负/gen>255）规整后字段与显式合规双生逐位一致（含全负→零窗单格决策 + 上界钳 255 字段钉）；腿内 QObjectFree/trivially copyable 复钉（头钉被删即双红，r2015d 先例）；pinSet 剥注释正面钉（policy 类/纯缝类型/decide 签名/默认关字段/Decision QObjectFree 钉）；反探 minCount=1 forbiddenAbsent 惯用法（r2007b/r2011d 先例，防注释提及误伤）——generationpolicy.h 禁 `: public QObject`/Q_OBJECT/Q_INVOKABLE/Q_PROPERTY/QThread/std::thread/QMutex/`World *`/`ChunkManager *`；零接线反探 world.{h,cpp}/gamesession.h/Main.qml 零 GenerationPolicy 引用。

**验证链（全绿）**：分段增量构建（-j 4，新段 TU 小；首编一处 residentCells 数组 bool 误声明编译期响亮揪出即修）→ binary mtime（09:43:40）> 全部改动源（≤09:43:25）→ filter **r2016 4P** 首跑绿（首跑 3 处腿文坑即修见过程坑）→ 前置回归 **r2007 5P / r2008 5P / r2009 4P / r2010 6P / r2010b 3P / r2011 4P / r2012 4P / r2013 4P / r2014 4P / r2015 4P** 全绿（与任务单/台账逐一吻合零缺腿）→ 阴性轮（见下）→ 全矩阵 **602 PASS / 0 FAIL ×2**（matrix_r2016_pos/final.log，EXIT=0；**598 权威 PASS 行 diff = +4 恰 r2016a-d** + 漂移仅登记类：t813 构建戳 03:49→09:45 与哈希 60ffafd→b4963f0、t997 墙钟计时 0.176→0.1784 级、**t830 自然刷新采样计数 40↔41**[同 binary pos/final 两轮 41/40 互证 = 进程 RNG 逐轮波动，断言面仅下界 samples≥40 与本单无关——t830 在 section03 先于本段执行]、**t1023c src 文件计数 145→146**[新增 generationpolicy.h 机制 +1：r2011=137/r2012=139[+2]/r2013=141[+2]/r2014=143[+2]/r2015=145[+2] 同款 +N 先例；白名单语义钉面零漂移——std::thread 仍仅 backgroundgeneration.h 单落点]；pos/final 腿名 diff 仅 t830 计数 + t997 墙钟数字，腿集合恒等 602）→ app voxelsandbox 重建 EXIT=0 + offscreen 冒烟 **EXIT=124** 存活 + logs/voxelsandbox_r2016_tail20.log 稳态 60fps（prof[1s] 60fr / frame main*16.0(60) / act ct 全零稳态）。

**阴性轮（实跑存证 matrix_r2016_neg.log 含双还原段；恰红面声明先于腿文、实测红面 == 声明面）**：NEG1 = 摘**默认关短路**（decide 入口 `if (!m_params.streamingEnabled)` 前置 `false &&` = disabled 路径也产出决策）→ rebuild → filter r2016 → **恰红 {r2016a} 单腿**（红形态 = 被摘语义本体：27 组合全量请求/驱逐单现身——全 Absent 缝下 81 格 toRequest、全 Loaded 缝下 48 格 toEvict）→ Edit 反向还原 → rebuild → 4P 回绿；NEG2 = 摘**驱逐半径守卫**（收集环 `if (cheb(dx,dz) <= gen) continue;` 前置 `false &&` = 扫描窗内全部驻留格入 toEvict）→ rebuild → **恰红 {r2016c} 单腿**（红形态 = 被摘语义本体：环带/gen 内驻留格 (3,0)(-2,1)(2,2)(1,0)(0,0) 现身 toEvict，want 5 格 got 10 格；r2016a 短路在恒空不红、r2016b 零 toEvict 断言不红、r2016d 相对恒等两侧同变不红——辅助腿零交叉感染达标）→ Edit 反向还原（严禁 git checkout/restore）→ rebuild → 4P 回绿。
...
### r2017 §29.4-P2 前置清偿单（2026-09-16，filter 词 r2017；矩阵 602→606）

  → **✅✅ 落地（2026-09-16）：src/Game/gamesession.h（fix A+C）+ src/World/generationjob.h（fix B）+ src/World/generationpolicy.h（fix D）三文件四修 + tools/matrix/section11（r2017a/c 新腿）/section14（r2017b 新腿）/section20（r2017d 新腿）/section13（r2009b/c 同变更修订）；矩阵 602→606。**（fix + test + 本 docs 三段提交；输入 = docs/agent-review-2026-09-16.md #1/#2/#3 三条「中」+ Info「GenerationPolicy setParams 不再规整」，主控逐条读源实锤分诊——四修全部位于 P2 玩家移动驱动接线面的潜伏债）

**fix A（#1 PlaceBlock state=0 同 id 异 state 强刷回 no-op）**：executeCommand 的 PlaceBlock 分支按 blockState 双路由——`state == 0`（默认/兼容面）走 4 参 `m_facade.setBlock(c.pos, c.blockId)`：World 权威对同 id 早退 no-op（state 保留、零信号零写后钩子，world.cpp 4 参版实读核验：`oldId == id → return false`）；`state != 0`（显式面）维持 5 参 setBlockWithState 全写（R20.09 权威落地不变）。语义依据（引证三元组，2026-09-16 真实 web 实读）：minecraft.wiki/w/Block_properties「replaceable」属性——blocks placed "in the same location as the replaceable block replace it" + "Most blocks are not replaceable"（Java 现行机制）→ 放置只进 Air/可替换格，目标格已被同款不可替换方块占用 = 放置无效、原方块无变化。4/5 参在「Air 格首放」「异 id 格替换」「同 id 同 state(0)」上行为逐位一致（都写 id,state=0，world.cpp 双版本逐行核验），**唯一语义变化 = 同 id 异 state 格从「强刷 0 + 全套写后钩子」回「静默拒绝」**。旧病灶（b759e60 引入）：命令 PlaceBlock(id, state=0) 落在「已持有同 id 且 state≠0」格（楼梯朝向/上半砖/耕地湿度态）被当作 state 变化落入完整写路径——命令放置从 MC 口径的「静默拒绝」变成「改写既有方块朝向/形态」。

**fix B（#2 生命周期边缺 kind 门）**：generationjob.h 同步 pump 与 pumpAsync 两路共**六处**边驱动点（①②⑨ × 2 路）全部加 `job.kind != GenerationJobKind::Mesh` 内联门（选型 = 三处内联门非抽函数：零 `setLifecycle(` 计数变化 → r2011d/r2010bb 既有源码钉免同变更修订，且边①在执行前/边②⑨在投递后的两相结构使单函数收口需引入模式参数反增复杂度）；头注互锁契约段扩写立此存照（「r2017 kind 门 = 契约的代码兑现」+ 两个坏面登记：Absent chunk 被 Mesh 请求推入 Generated 后对 mesher 三门永久不可见 / 异步交接窗内 Mesh 失败交付经边⑨把 Generate 在途 chunk 打回 Absent = 状态与数据失配）。Mesh 类 job 成功/失败全路径生命周期表逐位不动；Generate/Load 三边照旧（kind 域门非全局摘除）。

**fix C（#3 tick 外编辑被抹）——选型②（窗口语义迁移），头注立证**：去掉 runOneTick 开头 `m_edits.clear()`，清账唯一落点移到收口发布**之后**（emit tickCompleted 完再 clear——emit 栈内消费者按同帧快照读 lastDirtyChunks()，清账在信号栈外）——tick 窗口语义从「start-clear 到收口」变为「**上收口到本收口**」，tick 收口后、下一 tick 开始前到达的编辑（World 全局信号 → noteEdit → m_edits）自然归入下一窗、在下一收口照常作为 WorldDelta + BlockChanged 发布，绝不静默丢失（event.h「事件丢弃必须被看见」纪律）。选型依据：候选①（noteEdit 检测 tick 栈外走旧逐条入队）会破 R20.09「收口单点发布」纪律且 out-of-tick 编辑脱离 EditBuffer 合并面/脏 chunk 集合面（渲染调度漏格）；候选③（残余计数）只是「可见的丢弃」不满足「发布」验收。**约束核验**：Tick 内零通知、收口单点发布不破（out-of-tick 编辑与 tick 内编辑走同一合并面同一发布点）；**同变更修订（纠偏非放宽）= r2009b/c**——「收口后 lastDirtyChunks() 仍见本 tick 脏集」旧读数随窗口语义退役，集合面单源断言移入 tickCompleted 信号栈内（gamesession.h lastDirtyChunks() 注释自述的「tickCompleted 同帧快照」域，review #12 同口径），修订后断言在两种窗口语义下都成立 = NEG-C 不敏感相对恒等，恰红面保持 {r2017c} 单腿。

**fix D（Info setParams 不规整）**：规整唯一路径 = GenerationPolicyParams 的 public static `normalized(...)`（落位在 Params 本体 = 规整即该值类型的构造契约），4 参构造与 GenerationPolicy::setParams 同走一路——杜绝「手改字段破不变量 → decide 的 priority=quint8(cheb) 回绕」（gen>255 原病灶：窗口 ±300 越界扫描 + 优先级模 256 回绕）。合规入参逐位恒等（r2016d 构造规整回归柱不动，本体绿色）。

**MC 引证三元组（fix A）**：行为 = 放置需要目标格为可替换方块（Air 等）——minecraft.wiki/w/Block_properties「replaceable」属性原文（2026-09-16 实读）；版本 = Java 版现行机制（1.21.x 系列 wiki）；出处 = https://minecraft.wiki/w/Block_properties 。

**矩阵（602→606，四新腿 + 两条同变更修订行，filter 词 r2017）**：**r2017a（section11）PlaceBlock 三相**——①同 id 异 state 格（WoodStairs 朝向 s1，选址显式布 state≠0 方块）+ state=0 命令 → no-op（id/state 保留、零事件零 delta、命令被消费[FIFO 后继 Dirt marker 照常执行]）；②同格显式 state=5 → 5 参全写生效（state 落 5；同 id 仅 state 变不发 broken/placed = 既有接线语义如实钉[门开合口径]）+ 异格显式 state=1（id 变 → 有事件，5 参权威 + 信号面双钉）；③回归柱：state=0 命令在 Air 格首放（WoodSlab 下半）与异 id 替换（Stone→Cobble）上落 (id,0) 与旧权威逐位一致；diag 带 pre/post id+state。**r2017b（section14）Mesh kind 门双路**——同步路：Absent 格 Mesh 成功（worker 执行时刻实测 Absent[边①被门] + 终态停 Absent[边②被门] + 表逐位）/ Loading 格 Mesh 成功停 Loading（边②被门——合法边只有 kind 门能拦，最锐判别）/ Loading 格 Mesh 失败停 Loading（边⑨被门）；异步路（ScriptedMeshWorker 测试替身，r2010bb 先例）：交接相停 Absent（边①被门）+ 收割成功停 Absent（边②被门）+ 在途账本双消；Generate 对照柱同路由边①②照旧驱动 + 失败柱边①⑨照旧（kind 域门实证）；源码钉 kind 门 minCount=6。**r2017c（section11）out-of-tick 可见性**——tick 1 稳态收口 → 注入对（直写 World：放 Stone@chunk(2,2) + 破表面@chunk(0,2)，破位顶上两格净空自证防级联第三笔）→ 单点发布钉（注入后事件队列恒空 + 账面挂起恰 2 chunk）→ tick 2 收口完整发布（delta aff/chg 2 + 两 BlockChanged）→ tick 内回归柱（tick 3 命令编辑在自身收口发布）。**r2017d（section20）setParams 双生 twin**——①越界 setParams(true,300,-2,400) ≡ 同参构造：字段逐位（gen→255/render→0/scan→255）+ 决策逐位（大缝 = |cx|,|cz|≤300 全 Loaded 自定义 lambda[勿用 tableSeam——缺席默认 Absent]、(300,0) Absent = 仅未规整 ±400 窗可见的回绕判别格、(255,0) Absent = 规整后恰窗界 priority 255 无回绕钉）；②合规 setParams(true,2,1,4) 生效（字段读回 + 与构造 twin 决策恒等，双列表非空）；③全负双生；④默认关形状经 setParams 存活（decide 恒空）；⑤源码钉规整路径三站（normalized( minCount=3 = 定义+构造调用+setParams 调用）。

**验证链（全绿）**：分段增量构建（-j 4）→ binary mtime (12:46 epoch 1789530398) > 全部改动源 → filter **r2017 4P** 首跑绿（首跑三轮腿文坑即修见过程坑）→ 前置回归 **r2007 5P / r2008 5P / r2009 4P[修订后语义绿] / r2010 6P / r2010b 3P / r2011 4P / r2012 4P / r2013 4P / r2014 4P / r2015 4P / r2016 4P** 全绿 → 阴性轮（见下）→ 全矩阵 **606 PASS / 0 FAIL ×2**（matrix_r2017_pos.log + matrix_r2017_final.log 双日志在案，EXIT=0 两轮齐全后才写本关单文；**602 权威 PASS 行 diff = +4 恰 r2017a-d** + 漂移仅登记类：t813 构建戳 09:45→12:06 与哈希、t830 采样计数 41↔40、t997 墙钟 0.1789↔0.2177ms、t979 采样 16↔15[final]；**t1023c 零漂移**[本单零新增 src 文件]；pos/final 腿名 diff 仅 t979 计数 + t997 墙钟数字，腿集合恒等 606）→ app voxelsandbox 重建 EXIT=0 + offscreen 冒烟 **EXIT=124** + logs/voxelsandbox_r2017_tail20.log 稳态 60fps（prof[1s] 60fr / frame main*16.0(61) / act ct 全零稳态；构建戳 2026-09-16 12:29 @ 903be0b）。

**阴性轮（三轮实跑存证 matrix_r2017_neg_a/b/c.log，恰红面设计先于腿文定稿；实测红面 == 声明面）**：**NEG-A** = 摘 A 路由（`if (c.blockState == 0)` 前置 `false &&` = state=0 也走 5 参）→ rebuild → **恰红 {r2017a} 单腿**[diag = 被摘语义本体：楼梯 state 1 被强刷；r2017b/c/d 绿] + r2007/r2009 全绿（PlaceBlock 消费族在 Air 格/显式 state 面与 5 参逐位一致——非交叉感染解析论证 + 实跑双证）→ Edit 反向还原回绿。**NEG-B** = 摘 kind 门——**首版变异形态错误即发现**：`false &&` 前缀合取项把整个驱动条件恒假 = 摘掉的是全部生命周期驱动（Generate 对照柱 r2010ba/bb/r2011d/r2012d 连红 = 假红面过宽，t1051「摘守卫勿 false && 前缀」敏训的合取式变体）→ 废弃；**正确形态 = 删合取项**（六处 `&& job.kind != Mesh` 删除 = Mesh 恢复驱动、Generate/Load 原样）→ rebuild → **恰红 {r2017b} 单腿**（r2010/r2011/r2012 全绿）→ Edit 反向还原（replace_all 三模式逐站重立，首笔漏异步边②[注释文本不同未被 replace_all 命中]被 r2017b 复跑红揪出补齐 → 7 处计数复核）→ 回绿。**NEG-C** = 复原 runOneTick 开头 start-clear（收口清账保留 = 最小语义复原形态）→ rebuild → **恰红 {r2017c} 单腿**（r2007/r2009 全绿——修订后的 r2009b/c 集合面断言移入信号栈内 = 双语义不变量不误伤，恰红面收缩达标）→ Edit 反向还原回绿。

**登记非目标**：review #4（骑乘门 sneak 抑制 = t1054）/ #5~#12 与其余 Info（随对应相位清偿）/ lastDirtyChunks 值拷贝化（#12，注释已如实登记活引用有效域）/ EditBuffer::takeDelta 语义（const 投影不动——clear 归 GameSession tick 边界管）/ EditBuffer 本体（r2009a 纯类型腿全绿零触碰）。

**过程坑**：①r2017a 相 0 未清事件队列——相① pop 到 tick1 遗留事件（evM=16 楼梯而非 Dirt marker）→ 补相 0 双事件收账断言；②r2017c 破位 Q 上方有附着植物 → 级联清块 = 第三笔编辑（chg=3≠2，r2009c chg 首红同源教训）+ K 格 h+1 有 flora（kAir=0）→ Q 选址加顶上两格净空扫描、K 改 placeCol 选列；③r2017d 大缝误用 tableSeam（缺席默认 Absent）→ toRequest 261k 条 4MB diag（d=0 红且输出爆量）→ 改自定义 lambda 默认 Loaded；④QVector 无 .left() 成员 → diag 改 mid 逐条拼接；⑤NEG-B 首版 `false &&` 前缀合取项 = 假红面过宽（上已记）；⑥NEG-B 还原 replace_all 漏异步边②（注释文本不同）→ r2017 复跑红 1 揪出补齐——**计数复核（7 = 6 代码 + 1 头注）纳入还原验证步骤**；⑦app 冒烟 stderr 恒空非环境问题——voxelsandbox 自定义 logHandler 把日志写 logs/voxelsandbox.log（矩阵无 handler 才走 stderr），tail 取应用自有日志文件。

**待实机确认**：无新增（无头面清偿单；#1 属命令路径当前生产零触发——GameSession 生产零实例化，四修全部为 P2 接线前的潜伏债清偿，观感零变化由 app 冒烟稳态 60fps 实证）。

**下一任务**：**t1054（parity 第 14 门：骑乘门 sneak 抑制 + #11 helper 化）**，随后 **§29.4-P2 玩家移动驱动**（r2017 已清偿其接线面三条「中」债）。

**主控复核（r2017，2026-09-16）**：三修源码形态逐一实读核验（A 双路由 / B 六处 kind 门内联零钉数变化 / C 清账唯一落点移收口 / D normalized 唯一路径）；pos/final 606/0 两轮日志在案、+4 恰 r2017a-d。**证据面勘误**：关单时 build/ 下 neg_a.log 实为绿色还原运行（NEG-A 红跑被误写进 %TEMP%/negA_r2017.log，neg_b/neg_c 亦混装红跑+还原摘要）——主控从 Temp 抢救归位为 `matrix_r2017_neg_{a,b,c}_red.log` + `_restore.log` + NEG-A 的 r2007/r2009 对照族日志（恰红面逐份核对：{r2017a}/{r2017b}/{r2017c} 各单腿、还原全绿、废弃过宽变体留档 `neg_b_red_wide_variant_discarded.log`），歧义旧名删除。**教训登记（承 P1「×2 两轮在案」后第二条证据面铁律）**：阴性轮红跑/还原日志必须**直接落 build/ 终名**，禁 %TEMP% 中转；neg 文件名与内容一致（红=red、还原=restore 分文件），混装文件等同证据缺失。

### t1054（2026-09-16，骑乘门 sneak 抑制（同族第 14 门 = 交互抑制面）+ sneakPlaceBlock 合取 helper 化——review0916 #4 + #11 清偿）

  → **✅✅ 落地（2026-09-16）：src/Game/playercontroller.cpp（(a) 骑乘段 sneak 裸门 + 两调用点合取收口）+ src/Game/playercontroller.h（私有 helper heldPlaceableSneak()）+ tools/matrix/section08_recent.cpp（t1054a/t1054b 新腿 + t1050a/t1052a 源钉同变更修订）；矩阵 606→608。**（fix + test + 本 docs 三段提交；输入 = docs/agent-review-2026-09-16.md #4「中」+ #11「低」，主控实锤分诊）

**#4 骑乘门语义选型（头注释立此存照）**：(a) 段 tryMount 前加 **shift 裸门** `!m_keys.value(Qt::Key_Shift)`（先于 tryMount 短路）——潜行一律抑制实体交互，与手持无关。**语义分工**（与 sneakPlaceBlock 合取门的分界，第 14 门 = 交互抑制面）：开箱/扳动类 = 合取门【持可放置方块才旁路到放置，t1050 十二门 + t1052 箱车开箱，helper 一处权威】；骑乘类 = shift 裸门【潜行即不骑——空手 sneak 右键普通矿车 = 无效应（不上车不放物，落 (b) 非矿车物品跳过 + 通用放置 Air 守卫拦）、持方块 sneak = 落 (b) 放矿车/通用放块路径（放置优先）、无 shift = 照旧上车（回归柱）】。t1052 提交注自证矛盾点（旧注释宣称持方块 sneak 落「(a) 骑乘[箱车拒载]」——对普通矿车 (a) tryMount 先于一切放置执行且不查 Shift 会吞掉放置）随本修消除，:4884-4889 注释如实化。**行为在 t1052 前后不变的部分**：箱车面（拒载守卫恰使 t1052a 三相在新旧门下同值——NEG-1 对照柱不红实证）。

**#11 helper 化**：合取 `shift && m_selectedBlock != Air` 收进私有 `PlayerController::heldPlaceableSneak() const`（:3532 定义，.h :1098 声明），两调用点（m_hasHit 块内 12 门判据 + 矿车段 (a0) 开箱门）改读——判据演进一处收口不劈叉，语义逐位不变（m_selectedBlock 经 hotbar 非方块槽归 Air 的既有归一不变）。

**MC 引证三元组（2026-09-16 真实 web 实读）**：①**潜行抑制实体交互/骑乘**——minecraft.wiki/w/Sneaking Effects 段（"While sneaking by using the sneak control" bullet list）原文 "Players are able to interact with a tamed horse (tamed donkey or mule inventories, also) when pressing use item control while holding the sneak key"（潜行按住时 use 对可骑实体 = 背包**交互**而非骑乘——现行 wiki 对「潜行 + use 可骑实体不骑」的表述面；Effects 段无 "does not mount" 字面句，本条为最接近的现行原文，如实登记）；潜行上下一切载具 = 同页 History 段 "Sneaking is now used to get out for all transportation methods, such as boats, horses and minecarts"（Java 1.6.1 / snapshot 13w16a）；②**矿车骑乘 = use 实体交互**——minecraft.wiki/w/Minecart Usage→Transportation 段原文 "Minecarts can be ridden by pressing the \"use\" control on them" + Usage→Dismounting 段 "Players can exit the minecart by sneaking while riding it"（潜行与骑乘互斥的两端）；③**工程口径** = t1050/t1052 潜行旁路家族同门第 14 门统一（源码 :4908-4930 头注释立此存照）。

**矩阵（606→608，section08 矿车族两新腿 + 两条源钉同变更修订，filter 词 t1054）**：**t1054a 骑乘门 sneak 抑制三相 + 回归柱**——rig 照 t1052a 先例（48×48×96 seed 10521 + y=14 Planks 地台）+ **孤轨 (12,15,16) state0 + 普通矿车 on-rail 静止**（pos.y = 15 + kCartRideH ≈ 15.45，体盒 x[12.05,12.95] z[16,17] y[15.0,15.9]；选体射线走 **t983 薄板 sub-AABB 口径**穿轨板上部空气命中地台 (12,14,15)——轨不抢准星；实体射线穿车体盒 s∈[0.5,0.83] 与 t1052a 同几何，**腿内 rig 自证**三重：cartY≈15.45 / ridingIndex==-1 / hit==(12,14,15)）；①持 Planks + shift 右键 → 放置发生（命中面邻格 (12,15,15) 落木板，t1052a(3) 同格先例）+ **零 mount**（ridingIndex==-1 + count==1 + 挥臂恰 1）；②空手 + shift → **无效应**（零 mount、零放置[(12,15,15) 木板不动 + (12,15,14) 恒 Air]、零开箱、零挥臂）；③无 shift → **mount 照旧**（ridingIndex==0 回归柱 + 骑乘挥臂恰 +1 + 不落块）；diag 带 mount 布尔 + 放置事件（格 id）+ heldId（selectedBlock）+ 挥臂/开箱计数。**t1054b helper 化承重钉**（零世界纯源钉，pinSet 剥注释）：①合取唯一权威 = helper 定义行 + `return m_keys.value(Qt::Key_Shift) && m_selectedBlock != BlockRegistry::Air;`（minCount=1）；②两调用点 `const bool sneakPlaceBlock = heldPlaceableSneak();`（**minCount=2** = 12 门判据 + 开箱门）；③反探：旧字面调用行 `const bool sneakPlaceBlock = m_keys.value(Qt::Key_Shift)` 必须消失（miss 非空 = 合规，r2007b 反探先例）。**源钉同变更修订（t1023c 先例，纠偏非削钉）**：t1050a 判据钉旧 needle `const bool sneakPlaceBlock = sneakPlace && m_selectedBlock != Air`（被 helper 化移除的调用行字面）→ 改指 helper return 合取行（合取语义唯一落点，NEG-2 摘合取/&&→|| 同红）；t1052a 开箱门钉 → 改指 `const bool sneakPlaceBlock = heldPlaceableSneak();` minCount=2。

**验证链（全绿）**：分段增量构建（-j 4；playercontroller + section08 重编 + 链接）→ binary mtime (13:46:49) > 全部改动源（playercontroller.cpp 13:46:35 / .h 13:00 / section08_recent.cpp 13:26）→ filter **t1054 2P 首跑绿**（t1050a 源钉红即修后重跑，见过程坑）→ 前置回归（终版 binary 全族重跑）**t1050 6P / t1052 1P / t1013 2 legs(9 PASS 行) / t1046 8P / t1034 3P / t1028 4P / r2007 5P / r2008 5P / r2009 4P / r2010 6P / r2010b 3P / r2011 4P / r2012 4P / r2013 4P / r2014 4P / r2015 4P / r2016 4P / r2017 4P** 全绿 EXIT=0 → 阴性轮（见下）→ 全矩阵 **608 PASS / 0 FAIL ×2**（matrix_t1054_pos.log + matrix_t1054_final.log 双日志在案 EXIT=0，两文件齐全后才写本关单文；**606 权威 PASS 行 diff = +2 恰 t1054a/t1054b** + 漂移仅登记类：t813 构建戳 12:06→13:46 与哈希 903be0b→486a16f[提交前 HEAD]、t997 墙钟；**t979/t830/t1023c 零漂移**；pos/final 腿集合恒等 608[diff 仅 t997 墙钟数 + t979 采样 15↔16]；^FAIL 逐行归类 = 0）→ app voxelsandbox 重建 EXIT=0 + offscreen 冒烟 **EXIT=124** + logs/voxelsandbox_t1054_tail20.log 稳态 60fps（prof[1s] 60fr / frame main*16.0(60/61) / act ct 全零稳态）。

**阴性轮（红跑/还原四日志直接落 build/ 终名，恰红面设计先于腿文定稿）**：**NEG-1**（matrix_t1054_neg1_red.log / matrix_t1054_neg1_restore.log）= 摘 (a) 段 sneak 裸门（Edit 删 `&& !m_keys.value(Qt::Key_Shift)` 行）→ rebuild → **腿级恰红 {t1054a} 单腿**（t1054b 绿 / t1052 1P、t1050 6P 对照族实跑绿——箱车走拒载路径不在 (a)，(a) 门不在 t1050 家族作用域）；**柱级形态偏差留痕**：预判 (1)(2) 红 (3) 绿，实测 diag held/empty/regress 三相全 false——相① 旧门下 mount 后 m_riderCart 残留，相③ ridingIndex==0 断言被残留态连带打 false（**单腿内级联，腿级红面与声明一致**）；还原 = Edit 逆向补回（还原后计数复核：门合取恰 3 项[船互斥 + !shift + tryMount]，`m_keys.value(Qt::Key_Shift)` 全文件 3 处 = helper return + 本门 + :7495 蹲态机既有读点）→ 回绿 2P。**NEG-2**（matrix_t1054_neg2_red.log / matrix_t1054_neg2_restore.log）= 破 helper 合取（`&&`→`||`，判据本体变异）→ rebuild → **全矩阵红跑 599P/9F**；**声明面（实测前先立）** = {t1054b 文钉 + t1050a-d 空手 sneak 臂 + t1052a 相②}；**实测红面 = 声明 ∪ {t1034a, t1034c, t1046a}**——三者为「无 sneak 持方块 = 正常交互」基线柱（合取破坏使持方块无 sneak 也旁路到放置，基线柱红恰属合取语义面），**t1054a 不红**（预判成立：(a) 门读 m_keys 裸键不经 helper，三相 helper 取值均不变）——按 NEG1 t1053 先例改声明留痕（恰红面 = 钉合取语义的腿集合 9 腿）→ Edit 反向还原（还原后计数复核：`&& m_selectedBlock != BlockRegistry::Air` 全文件恰 1 处 = helper 权威行）→ rebuild → filter t1054/t1050/t1052/t1034/t1046 全绿（restore log 20P/0F）。

**登记非目标**：船骑乘段 sneak 抑制（同族骑乘面但 review #4 仅指矿车段，船门另行登记）/ tryMount 本体与 minecartmanager（零改动——门在调用点）/ 浆果丛收获型 useBlock 不让位放置（review0915 Info 既有登记）/ #5~#12 其余项（随对应相位清偿）/ 持矿车物品 + sneak 右键普通矿车的 (b) 放车细分（本 rig 未覆盖，路径守卫未动零风险）。

**过程坑**：①t1050a 源钉红——其判据钉 needle 是被本修移除的旧调用行字面（helper 化波及同族腿钉面），前置回归首跑 t1050 5P/1F + t1046 7P/1F[同腿双 filter 名]抓出 → 同变更修订改指 helper return 行（t1023c 先例），t1050 6P/t1046 8P 回绿；教训：**helper 化/搬移类改动要全矩阵 grep 旧字面钉**（本次 grep 复核仅 t1050a 一处 + 自家两钉）。②矩阵 filter 跑 **QT_QPA_PLATFORM=offscreen 不可用**（build 目录未部署 plugins，评审既登记）——offscreen 环境变量组仅用于 app 冒烟（须配 QT_QPA_PLATFORM_PLUGIN_PATH 指向 Qt 安装 plugins）；矩阵跑用默认 windows 平台 + QT_FORCE_STDERR_LOGGING=1。③NEG-1 相③级联（上记）——「零 mount」断言用绝对 ridingIndex==-1 时，前相污染态会使回归柱连带红；本腿声明按腿级红面成立、柱级形态如实留痕（若需柱级隔离应用每相重建 rig，成本不值）。

**待实机确认（行为修复必列实机）**：①空手潜行右键普通矿车不上车（无效应观感）；②持方块潜行右键普通矿车 = 放置不骑乘（对运行中矿车实测一次）；③无 shift 右键普通矿车照旧上车（回归观感）；④潜行中骑乘矿车时右键不再触发任何骑乘切换（与潜行下车的既有语义共存观感）。

**下一任务**：**治理审计 #10（主控亲自，计数 5/5 满：r2010b/r2016/r2017/t1053/t1054）**，随后 **§29.4-P2 玩家移动驱动**（待主控派单）。

### §29.4-P2（2026-09-16，玩家移动驱动 ChunkStreamDriver——位置沿 → 请求/取消编排 + 风暴背压可见 + 默认关恒惰）

  → **✅✅ 落地（2026-09-16）：src/World/chunkstreamdriver.h（header-only 非 QObject 纯编排值组件，无 AUTOMOC）+ tools/matrix/section21_chunkstreamdriver.cpp（r2018a-d 四新腿 + 三点互钉：CMakeLists 源清单 + matrix_helpers.h 段声明 + runAll 尾调用）；矩阵 608→612。**（fix + test + 本 docs 三段提交；输入 = refactor-plan §29.4「P. 相位拆分」P2 原文「位置沿 → 视距 chunk 请求/取消（GenerationJob 三 kind 已含优先级）；生成风暴背压 = 队列满载拒绝既有语义」+ 治理审计 #10 放行 docs/governance-audit-2026-09-16-b.md）

**设计选型（头注释立此存照）**：①组件形态 = header-only 非 QObject 纯编排值组件（generationpolicy/generationjob 同形态）；②构成 = 持 GenerationPolicyParams + GenerationPolicy（决策权威）+ 生命周期纯缝 `std::function<ChunkLifecycle(int,int)>`（P1 同款缝语义）+ **自持 GenerationScheduler**——选型依据：自持 = 组件自包含可无头测（r2011a-c 纯请求模型腿同门）；生产接线期若需共享世界级实例（epoch/挂点全局面）再演化[登记非目标]；自持实例按纯请求模型形态构造（chunks 挂点为空）——生命周期边①②⑨驱动唯一权威仍 GenerationJob（r2017 kind 门语义不受影响：驱动器只提交 Generate kind 且 pump 纯转发，r2018d 结构钉「头禁 setLifecycle 记号」），ChunkManager 挂点属生产接线面（P3）；③onPlayerChunk 变更沿幂等（同 chunk 重复调用零动作）→ decide → toRequest **逐项** submit（priority 原样传递；r2011 合并别名制天然去重——重复请求零副作用，submitted 计数含别名）→ **离半径取消**：自持「在途提交集」m_pending（packed key → 活别名 id 列表，合并别名制下同 key 可多别名），新沿对切比雪夫距离 > generationRadius 的 key 逐别名 cancel——取消判据是**半径**（位置几何）非 decide 成员判据（在途 chunk 不在 toRequest 也不误取消）；erase-on-cancel 保证每别名至多一次尝试、已执行 id 的 cancel false 不计入账（取消幂等由 r2011 拒重复取消承接）；④背压可见：submit kErrQueueFull 满载拒绝计入 rejectedSubmissions() 并跳过（不抛不堵、不入在途集），下一变更沿 decide 重算天然重发（r2011 完成后重请求 = 新 job 语义承接）；⑤pump 单转发覆盖同步/异步两路（isAsynchronous 分派在底层 pump 内 = R20.12 缝，调用者面零改动即验收④同门）——收割缝 = takeOutcome/outcomeCount/pendingJobCount 三个转发；⑥默认关恒惰 = onPlayerChunk/pump 双短路（P1 承重墙延续，关态在途账逐位不动）；⑦零 World*/ChunkManager* 持有、零线程原语、零 QML 面、QObjectFree 编译期钉照 P1。

**矩阵（608→612，四新腿，全段零世界 = 缝 QMap 合成生命周期表 + 无线程测试替身，filter 词 r2018）**：**r2018a 默认关惰性承重墙**——三构造路径（默认构造/规整构造显式关/setParams 关）× 扫掠（含大幅移动 (40,-30)）提交/取消/拒绝全零、队列逐位不动、worker 零执行、outcome 零；第四相 = **使能装载 25 pending → 中途关断 → 远位扫掠**：关态在途账逐位不动（半径取消环在关态不得触碰在途 job——NEG-1 恰红载体）。**r2018b 请求跟随+取消面（承重）**——①跟随：e1@(0,0) 泵后 executed ≡ twin decide 输出逐项恒等（key+priority+decide 序 = priority 保序）、outcome 全 ok、沿幂等（同 chunk 重复零动作）；②disjoint 行进 (5,0)→(10,0)：D2 全离半径恰被 cancel 一次（canceled==|D2|、泵后 executed 不含 D2 key、D2 别名零 outcome——取消且不执行）、stale 已执行 id 的取消 false 不入账（canceled 不虚增）；③合并别名 dedupe：e1@(0,0)→e2@(1,0) 无泵重叠 key 合并别名 → 48 提交仅 29 pending job（零重复 pending）→ e3@(13,0) 全离半径逐别名取消（canceled==48）→ 泵后 executed 恒空 + outcome 零（全别名死亡整体跳过）；④完成后重入：泵执行 → 世界成熟标记 → (0,0) 复归 Absent → 重入半径重提交 = **新 requestId**（单调分配 newId>firstId）。**r2018c 风暴背压**——gen=4 窗 81 vs 底层 64 容量：远距传送恰拒 17（rejectedSubmissions==17）、受理 64 全执行全交付、被拒 key 零 job 零 outcome（零悬别名零崩溃）；泵排干 + 世界成熟 + 一格重沿 → 收敛（DC2==26 = 17 被拒全量重发承接 + 9 新入窗格；最终提交集 == 决策集序恒等、拒绝计数冻结、累计交付 == 累计提交）。**r2018d 异步收割+结构钉**——ScriptedAsyncWorker 替身（r2010bb/r2017b 先例，零线程零时序）：pumpAsync 交接沿 = handed 记录可见（7 项全 Generate kind + key/priority/decide 序恒等）、收割沿 = outcome FIFO 对账交付（requestId/key/kind 逐项）；结构钉 = 正面 pinSet（driver 类/onPlayerChunk 签名/默认关双短路 minCount=2/Generate-only 提交面/m_jobs.submit/QObjectFree 钉）+ 反探（禁 QObject 基类/Q_INVOKABLE/Q_PROPERTY/QThread/std::thread/QMutex/World*/ChunkManager*/setLifecycle/GenerationJobKind::Mesh/Load 记号——生命周期边唯一权威 stays GenerationJob）+ 零接线反探（world.{h,cpp}/gamesession.h/playercontroller.{h,cpp}/Main.qml 零 ChunkStreamDriver）+ **确定性**：同移动序列（5 沿含无泵沿）双 fresh 驱动器重放 submit/cancel/execute/outcome 轨迹逐位恒等（快照向量 + 轨迹向量全比对，非空转探针只钉变异无关事实[R20.11]）。

**验证链（全绿）**：分段增量构建（-j 4；新 TU section21 + helpers 头变更全矩阵重编 + 链接）→ binary mtime (15:17/15:33 级) > 全部改动源 → filter **r2018 4P 首跑绿**（首跑 r2018c 一处断言笔误即修见过程坑）→ 前置回归（终版 binary 全族重跑）**r2007 5P / r2008 5P / r2009 4P / r2010 6P / r2010b 3P / r2011 4P / r2012 4P / r2013 4P / r2014 4P / r2015 4P / r2016 4P / r2017 4P** 全绿 EXIT=0（r2017 首查 5P = 自家 r2018d 腿名含 "r2017" 词元污染 filter 命名空间，去词元后 4P 达标）→ 阴性轮（见下）→ 全矩阵 **612 PASS / 0 FAIL ×2**（matrix_r2018_pos.log + matrix_r2018_final.log 双日志在案 EXIT=0，两文件齐全后才写本关单文；**608 权威 PASS 行 diff = +4 恰 r2018a-d** + 漂移仅登记类：t813 构建戳 13:46→15:31 与哈希 486a16f→2c705a3[提交前 HEAD]、t997 墙钟 0.1755→0.1767ms 级、**t1023c src 文件计数 146→147**[新增 chunkstreamdriver.h 机制 +1，r2012=139/r2013=141/r2014=143/r2015=145/r2016=146 同款 +N 先例；线程原语白名单语义面零漂移——std::thread 仍仅 backgroundgeneration.h 单落点]；**t830/t979 零漂移**；pos/final 腿集合恒等 612[diff 仅 t997 墙钟数字]；^FAIL 逐行归类 = 0）→ app voxelsandbox 重建 EXIT=0 + offscreen 冒烟 **EXIT=124** + logs/voxelsandbox_r2018_tail20.log 稳态 60fps（prof[1s] 60fr / frame main*16.0(60) / act ct 全零稳态）。

**阴性轮（红跑/还原四日志直接落 build/ 终名，恰红面设计先于腿文定稿；实测红面 == 声明面）**：**NEG-1**（matrix_r2018_neg1_red.log / matrix_r2018_neg1_restore.log）= 摘默认关短路（onPlayerChunk 入口门变异 = disabled 也走编排体）→ rebuild → **恰红 {r2018a} 单腿**[diag 红载体 = 被摘语义本体：ph4 使能装载 25 pending → 中途关断 → 远位扫掠时半径取消环在关态照跑 s=25/c=25 frozen=0；相①-③在途集恒空不可见[绿]、r2018b/c/d 全使能行为逐位不变[绿]] → Edit 反向还原回绿 4P/0F（还原后计数复核：变异字面 0 残留、双短路门在位）。**首版形态偏差留痕**：`false && ` 前缀变异曾连带红 r2018d——其正面钉 needle 含门文本全形（minCount=2 只剩 1），钉面与被摘语义同源红 = 超声明；改形 `&& false` 后缀（保针前缀）+ 钉针去尾括号后恰红收缩达标。**NEG-2**（matrix_r2018_neg2_red.log / matrix_r2018_neg2_restore.log）= 摘出半径取消（取消环条件 `false && ` 前置 = 环体永不迭代零取消）→ rebuild → **恰红 {r2018b} 单腿的取消柱**[diag 仅取消柱失配：ph2 应取消 key 现身 executed + D2 波零 outcome 断言失配、ph3 allDead=0/skipped=0/c=0；提交面柱不误伤——ph1 跟随/idem 幂等/ph4 重入新 id/dedupe pendingJobCount 全绿] → Edit 反向还原回绿 4P/0F（计数复核：变异字面 0 残留）。**首版形态偏差留痕**：NEG-2 首跑连带红 r2018d——其确定性腿非空转探针含 `canceled > 0` 绝对断言（违反 R20.11「取消绝对计数归被测语义腿专钉」）→ 腿文修正去掉取消绝对计数（非空转只钉 submitted/executed）后恰红收缩达标，两轮阴性**按终版腿文重取全部证据**（NEG-1 红/restore 均以终版 binary 重跑落盘）。

**登记非目标**：toEvict 执行（P3 卸载 + Edits-on-evict 落盘——decide 的 toEvict 单驱动器不消费，测试 twin 亦只取 toRequest）/ 生产接线（World/PlayerController/QML 位置源 + GenerationJob 共享世界级实例与 ChunkManager 挂点——P3/P4）/ worker meshing（D6 后续单）/ 参数实值（P5 调参）。

**过程坑**：①r2018c 排干断言笔误——第二波排干期望写成 64+26（首波 64 已排干）→ 实跑红即修为 26 + 累计口径断言（drained 64 + 本波 26 == submitted 90 = 零悬别名）；②r2018d 腿名含 "r2017" 词元污染 r2017 filter 命名空间（前置回归 r2017 首查 5P）→ 去词元（filter 词命名空间纪律：腿名只含本单 filter 词）；③本环境 grep 为 ugrep 7.8.4——日志含控制字节被判二进制，计数必须 `grep -a`（任务纪律既载）；④NEG-1/NEG-2 首版形态均超声明（上已记）——恰红面收缩遵循「变异形变 + 针面前缀化 + 腿文 R20.11 相对化」三工具；⑤twinDecide 返回 GenerationPolicyDecision 需取 .toRequest（本段零消费 toEvict）+ tableSeam 内层 lambda 须捕获 kkey。

**待实机确认**：无新增（无头编排组件 + 零生产接线，观感零变化由 app 冒烟稳态 60fps 实证）。

**下一任务**：**§29.4-P3 卸载 + Edits-on-evict**（视距外 Evicting→Absent 执行器 + 改动块经 SaveCoordinator 落盘回灌；decide.toEvict 消费面 + GenerationJob 挂点生产接线入场的候选面），随后 worker meshing（D6 同批）→ P4 QML 动态化 → P5 调参验收。

**主控接交注（纪律⑦ 遗产接交，2026-09-16 16:0x）**：实现 agent 在全部工作落盘后、**三段提交前**遭遇五小时限额死亡（1308，16:06 窗口重置）——现场盘点（纪律⑦）结论：组件/腿/三点互钉/全矩阵双日志/双阴性红跑还原/烟测 tail20/docs 草稿齐备且与实证一致，唯欠提交。主控逐项核验后接手提交（fix→test→docs 本三笔）并以独立复跑 `matrix_r2018_orch.log` 作终局权威。复核明细：612/0 双轮在案 + 终局复跑绿；608 权威 diff = +4 恰 r2018a-d（其余仅 t813 戳/t997 计时/t1023c 文件计数 +1=新增头机制，登记类）；NEG-1 恰红 {r2018a}、NEG-2 恰红 {r2018b}、双还原 4P 绿（四日志直落 build/ 终名合规）；组件头实读——默认关惰墙（onPlayerChunk+pump 双短路）/变更沿幂等/离半径取消取**半径判据**（非 decide 成员判据，防误伤在途 chunk）/背压 rejected 计数/自持 GenerationJob 纯请求模型形态（挂点留 P3）/QObjectFree 钉，全部合规；driver 消费 decide.toEvict 留 P3 与头注非目标一致。证据面零缺陷——两条铁律已被本单遵守。

---

### §29.4-P3（2026-09-16，卸载 + Edits-on-evict ChunkEvictor——先落盘后转移驱逐编排 + 失败中止 + 驱动器可选 evictor/savedQuery 双注入点）

**任务**：refactor-plan §29.4「P. 相位拆分」P3 原文「视距外 Evicting→Absent，改动块经 SaveCoordinator 落盘、重载回灌（R20.10d 存档往返腿族先例）」+ D5 决策「视距外 **Evicting→Absent + Edits-on-evict 落盘**」。

**结果**：✅✅ 落地（2026-09-16）：src/World/chunkevictor.h（header-only 非 QObject 纯编排值组件，无 AUTOMOC）+ src/World/chunkstreamdriver.h 最小扩展（两个可选注入点，默认 null = P2 逐位等价）+ tools/matrix/section22_chunkevictor.cpp（r2019a-d 四新腿 + 三点互钉：CMakeLists 源清单 + matrix_helpers.h 段声明 + runAll 尾调用）；矩阵 612→616。（fix + test + 本 docs 三段提交）

**设计选型（头注释立此存照）**：①组件形态 = P1/P2 同款 header-only 非 QObject 纯编排值组件，零 World*/ChunkManager*/SaveCoordinator* 持有；②三条纯缝 = `lifecycleTransitionFn(cx,cz,target)→bool`（生产接线期绑 ChunkManager::setLifecycle 等价面/World forwarder，测试用自校验 QMap 缝[chunklifecycle 合法表守卫同构]）+ `dirtyQueryFn(cx,cz)→bool`（生产绑 DirtyChunkSet 等价面，测试 scratch 集）+ `persistFn(cx,cz)→Result<void>`（生产绑真实 SaveCoordinator 调用，测试**真协调器 + fresh 临时库**[r2015 先例，绝触 saves/]）；③**null 缝 fail-safe** = 该项无信息 → 该 chunk 跳过驱逐（宁驻留不误删；persist 缝仅 dirty 候选必需——非 dirty 候选无 persist 缝照常驱逐）；④**顺序铁律：先落盘后转移**（内容存续优先于状态迁移）——dirty 候选 persist 成功才允许任何转移驱动调用，**persist 失败 = 中止该 chunk 驱逐**（保持驻留原态、零转移调用、计入 aborted，绝不「先掉内容再补盘」）；⑤转移驱动 = 合法边路径读表确定三步序：步1 ensure-Loaded（Generated 经边③/Active 经边⑤，已 Loaded 自转移守卫拒 = 预期 no-op 不计失败[GenerationJob best-effort 记账同门]）→ 步2 边⑥ Loaded→Evicting → 步3 边⑦ Evicting→Absent（被拒 → best-effort 边⑧回驻留 + aborted；步2 被拒 = 候选不在可驱逐稳态 → aborted）——**不重试**（重试语义 = 调用方下一变更沿重报候选）；⑥报告小聚合 `Report{persisted,evicted,aborted,skipped}`（每候选恰落一桶，persisted 为落盘成功计数不参与划分）；⑦驱动器最小扩展 = `setEvictor(std::function<void(const QVector<GenerationPolicyEvict>&)>)`（onPlayerChunk 决策沿提交/取消完成后以 d.toEvict 回调，默认 null = 逐位零变化，streaming off 不可达）+ `setSavedContentQuery(std::function<bool(int,int)>)`（submit 前查询：有存档内容 → Load kind[r2011 Load 语义]，null = 全 Generate P2 逐位不变，D5 回灌路径）。

**矩阵（612→616，四新腿，filter 词 r2019）**：**r2019a 惰性承重墙**——默认构造 driver（无 evictor/无 savedQuery）扫掠全零动作（P2 语义逐位不动）+ 双扩展缝已装但 streaming off → 零回调零查询（回调点在默认关短路之后）+ 使能 driver 带记录缝 vs P2 形态 driver 轨迹双生恒等（逐沿 stats + executed + outcomes 全字段；记录面非空转：查询计数 > 0、三沿候选 ≡ twin decide().toEvict 逐沿逐项）。**r2019b 驱逐回灌承重（真 SaveCoordinator + fresh 临时库 + 真世界表 + scratch 脏缝）**——gen=0/scan=2 玩家落 (1,1)：toEvict = 8 半径外驻留（交付单 ≡ twin）；(2,2) 编辑过 → persist 先于转移（缝记录调用序柱：persist 索引 < 首转移索引）、转移序恰 [(Loaded,rej 自转移 no-op),(Evicting,⑥),(Absent,⑦)]、报告 p=1/e=8/a=0/s=0；(2,0) 未编辑 → 零 persist 直达 Absent 同款边序；台账 Clean 1/1（一次统一保存全 9 chunk 随行）；重入 (2,2) → savedQuery true → **Load kind** 提交（非 Generate）→ pump → 回灌读面 = beginLoad 零填充 fresh 世界（store.setWorld 改绑先于 loadChunks——R20.10d rebind 教训钉在腿文，非空转结构性保证）↔ 驱逐前快照三段 blob 逐字节恒等 + 标记块可读 + outcome kind==Load 零错误。**r2019c 失败中止 + null 缝**——persist 注入失败 → 脏候选 aborted、零 accepted 转移、表不动（干净同胞照常驱逐）；同 evictor 实例重报收敛（状态零隐持）；转移缝注入恒拒 → aborted 计数 + 每候选转移尝试恰 2 次（有界无重试风暴）；null 缝三形态（缺 dirty/缺转移 → 全 skipped 零调用；缺 persist → 仅 dirty skipped[宁驻留]、干净照常驱逐）。**r2019d 结构钉 + kind 选择**——QObjectFree/平凡性编译期复述 + chunkevictor.h 正面钉（三缝类型/evict 签名/persist 缝调用行/ensure-Loaded ×2/边⑥/边⑦行）+ 反探（禁 QObject/QML/线程/World*/ChunkManager*/SaveCoordinator/setLifecycle 记号——落盘经缝、转移经缝终点）+ 零接线；driver 头 setEvictor/setSavedContentQuery/if (m_evictor)/m_evictor(d.toEvict)/缝门控 kind 选择行正面钉 + Mesh 仍禁出 + 零 setLifecycle（r2018d 承接）；null savedQuery → 全 Generate 且与无缝 driver 轨迹逐位恒等（r2018b twin 复跑）、恒 false ≡ null 双生、恒 true → 同键同 id 同序 kind 翻 Load；null evictor → 重放恒等、使能 evictor → 候选 ≡ twin toEvict 逐沿；确定性 = 同候选序双 fresh evictor 调用轨迹 + 报告逐位恒等。

**同变更钉面修订（纠偏非削钉，t1023c/t1050a 先例）**：r2018d 原反探 `GenerationJobKind::Load 禁出 driver 头` 随 P3 saved-content Load 路径（D5 回灌，任务单明文要求）退役——同变更改钉「Load 记号恰经缝门控出现 ≥1」（正面计数钉），Mesh 禁出/零 setLifecycle/零接线全保留；r2018d 腿名与 PASS 文本同步如实化（"default Generate-only face (Load only via the P3 seam-gated saved-content query)"），腿名词元零 r2019 污染（filter 命名空间纪律）。

**验证链（全绿）**：分段增量构建（-j 4）→ binary mtime (17:37) > 全部改动源 → filter **r2019 4P 首跑绿**（首跑 r2019b 一处腿文坑即修见过程坑）→ 前置回归（终版 binary 全族重跑）**r2007 5P / r2008 5P / r2009 4P / r2010 6P / r2010b 3P / r2011~r2018 4P×8** 全绿 EXIT=0（r2018 含同变更修订后的 r2018d）→ 阴性轮（见下）→ 全矩阵 **616 PASS / 0 FAIL ×2**（matrix_r2019_pos.log + matrix_r2019_final.log 双日志在案 EXIT=0，两文件齐全后才写本关单文；**612 权威 PASS 行 diff = +4 恰 r2019a-d** + 漂移仅登记类：t813 构建戳 15:31→17:14 与哈希 2c705a3→7887c59、**t1023c src 文件计数 147→148**[新增 chunkevictor.h 机制 +1，r2016=146/r2018=147 同款 +N 先例；线程原语白名单语义面零漂移]；t830/t979/t997 零漂移；pos/final 腿集合恒等 616；^FAIL 逐行归类 = 0）→ app voxelsandbox 重建 EXIT=0 + offscreen 冒烟 **EXIT=124** + logs/voxelsandbox_r2019_tail20.log 稳态 60fps（prof[1s] 60fr / frame main*16.0(61) / act ct 全零稳态）。

**阴性轮（红跑/还原四日志直接落 build/ 终名，恰红面设计先于腿文定稿；实测红面 == 声明面）**：**NEG-1**（matrix_r2019_neg1_red.log / matrix_r2019_neg1_restore.log）= 摘「先落盘后转移」序（evict 循环内 persist 块整体移到步1 ensure-Loaded 转移调用**之后**）→ rebuild → **恰红 {r2019b} 单腿的调用序柱**[diag 红载体 = 被摘语义本体：order=0(22<21)——persist 索引 22 晚于首转移索引 21；候选/计数/⑥⑦三重/回灌恒等/台账面全绿不误伤（r2019b 全候选 Loaded 稳态，步1 自转移守卫拒 = 零 accepted 零表动）] → Edit 反向还原回绿 4P/0F。r2019c 对该变异不敏感 = 腿文设计使 fail-abort 柱与调用序柱正交（r2019c 按「零 accepted 转移 + 表不动」口径断言，兼容被拒 no-op 调用先行）。**NEG-2**（matrix_r2019_neg2_red.log / matrix_r2019_neg2_restore.log）= 摘 persist 失败中止（`if (!pr.isOk())` 前置 `false &&` = 中止分支永不可达，失败仍转移）→ rebuild → **恰红 {r2019c} 单腿**[diag = 失败中止柱：ph1 abort=0/held=0/p/a/e/s=1/0/2/0（persisted 计入但零中止、A 被移 Absent）；ph2 同腿级联红（A 相位①已迁移，重报收敛形态随之改变）——同腿内级联非红面扩散] → Edit 反向还原回绿 4P/0F（还原后复核：变异字面 0 残留、abort 门在位）。r2019b 两轮均绿（全程 persist 零失败面）。

**过程坑**：①r2019b 沿 2 twin 期望沿后快照表 = 陈旧前置态反例（沿回调已把 (1,1) 驱逐 → twin.toEvict 恒空假红）→ 挪到 onPlayerChunk 之前快照（R20.10 态独立成立纪律同源）；②腿名 filter 词污染双例——r2019a/r2019d 腿名描述含 "r2018" 词元（r2018 filter 首查 6P）+ r2018d 腿名修订文本含 "r2019"（会反向污染 r2019 filter）→ 双向去词元后 r2018=4P/r2019=4P 达标（r2018 P2 单同款教训再现）；③本环境 Qt GUI 子系统 binary 经 bash 直跑零输出（Qt 日志走 OutputDebugString 非 stderr）→ 矩阵/冒烟一律带 `QT_ASSUME_STDERR_HAS_CONSOLE=1 + QT_FORCE_STDERR_LOGGING=1` 双 stderr 变量（t1054 先例的落地形态）；④局部类 EvSeamRig 成员函数不可捕获段内 lambda（kkey）→ 键打包收进 static 成员 key()；显式捕获列表 lambda 引用 kkey 须显式 `&kkey`。

**登记非目标**：生产接线（P4/P5：ChunkManager/DirtyChunkSet/SaveCoordinator 真实绑面 + 驱动器/evictor 生产实例化）；worker meshing（D6 后续单）；内容真实丢弃（World 侧 chunk 内存释放归生产接线——本组件只驱动六态转移，r2010「驱逐不销毁 Chunk 对象」最小解释延续）；失败自动重试（重试 = 下一变更沿重报候选，调用方语义）。

**待实机确认**：无新增（无头编排组件 + 生产零接线，观感零变化由 app 冒烟稳态 60fps 实证；P4 生产接线后需复验项：视距推进中远处 chunk 卸载无感[无卡顿/无内容丢失观感]、回灌后地形/编辑一致[存档往返真机]、驱动器使能后 F3 生成排队面）。

**下一任务**：**worker meshing（D6 同批）**（MeshBuilder 快照跨线程 + build 纯函数已就绪，线程化 + 调度接线；t1023c 白名单钉同变更修订面），随后 P4 QML 动态化（全 R20 唯一动 QML 相位）→ P5 实机调参验收。

### D6 worker meshing（2026-09-16，MeshWorker——MeshBuilder 的线程化执行器：单 worker + 有界队列 + 确定性透传，组件先行零生产接线）

**任务**：用户开工单 D6「worker meshing 与流式同批排期」（§29.4 设计草案 R 节「meshing 瓶颈」风险条目兑现）——MeshBuilder 线程化执行器组件；**组件先行零生产接线**（ChunkGeometry 现行同步构建路径零变化）。

**结果**：✅✅ 落地（2026-09-16）：src/World/meshworker.h（header-only 非 QObject 线程化执行器，r2012 backgroundgeneration.h 同门形态，无 AUTOMOC）+ tools/matrix/section23_meshworker.cpp（r2020a-d 四新腿 + 三点互钉：CMakeLists 源清单 + matrix_helpers.h 段声明 + runAll 尾调用）；矩阵 616→620；**t1023c 线程白名单同变更修订为双文件**（backgroundgeneration.h + meshworker.h，纠偏留痕非放宽）+ backgroundgeneration.h 头注「当前唯一」表述如实化。（fix + test + 本 docs 三段提交）

**设计选型（头注释立此存照）**：①形态 = r2012 同门：构造即起单 worker 线程（std::thread + mutex + cv；成员声明序=构造序——锁/队列/标志先于线程），析构 stop + 丢弃计数 + notify_all + join（在途构建跑完、join 有界绝不挂死）；worker 类代码体零 Qt 调用（执行体经 MeshBuilder::build 内触 FrameProfiler 计数 = QMutex 锁保护 + magic-static 单例既设计面，头注如实登记非隐蔽）。②**确定性承重**：worker 执行体 = `MeshBuilder::build(snapshot, Reason::Dirty)` 唯一调用——单一权威禁复制网格逻辑（r2013d 反探继续有效）、零加工零重排；Reason 只影响 FrameProfiler 计数分桶与顶点输出无关（meshbuilder.cpp 实读核验）。③**语义铁律「每个被接受的 request 恰产出一条 built」**：唯一例外 = 析构丢弃且逐条计 droppedCount（禁静默消失——事件可见性同门；已产出未收割项随对象亡 = 消费方自弃非 worker 丢弃）；teardownDroppedSink 构造缝（默认 nullptr = 生产零挂载；SaveFaultHook/ageLifetimeClock 测试缝先例）使丢弃计数越过对象生命周期可见。④容量：请求队列有界 64 满载 kErrQueueFull 可见拒绝（构造缝可压小[生产零参]；负值钳 0 = 拒绝一切的退化上界）；收割队列传递性有界（第三容量形态同门）。⑤快照值越线程安全论证（头注立证）：ChunkMeshSnapshot/ChunkMeshData/MeshBuiltItem 均 QObjectFree owning 值编译期钉——跨线程按值传递安全；**World 读面不越线程**（快照采集恒调用者线程 = 生产接线期契约，本组件只收快照值）。⑥错误码：kErrMeshWorkerStopped = 202（2xx 执行域域内递增，submit-after-stop 防御面）。

**矩阵（616→620，四新腿，filter 词 r2020，section23 置尾）**：**r2020a 确定性等价承重墙**——8 形态快照（世界采集 inland/edge/waterOnly/greedy+AO[fluid rig 四件 r2013a 先例] + 手写异形 cross/流体水池/重负棋盘高 256 + 空快照）worker build ≡ 同步直调逐位恒等三面（顶点/索引/统计）+ 非空快照非空转 + 空快照双路同空 + requestId 零丢零重。**r2020b 队列纪律**——cap=0 退化确定性满载（每提交必 kErrQueueFull 零入账零产出）+ cap=2 重负在途拦截饱和满载（M 出队入途后三连发：第 3 提交可见拒绝 = M 在途 + 队满的确定性证明）拒绝项不入账 + 接受项恰一产出对账{10,11,12} + 默认 cap=64 批量 12 连发恰一{20..31} + 收割后队清。**r2020c 析构卫生**——饱和型积压在途析构（sink 毒值哨 → 析构必覆写）join 有界 deadline 断言 + sink==2 恰等于析构排队数（在途 M 非丢弃产出已发生）+ 析构后零回调零输出（结构性——组件无回调/信号面，r2020d 反探守）+ 再构造再用全周期恰一。**r2020d 结构钉 + 白名单修订自证 + 零接线 + 线程身份**——meshworker.h 正面钉 12 项（submit/takeBuilt/容量 64/单一权威执行体/std::thread/双 cv 唤醒/QObjectFree 钉/析构丢弃计数/构造缝）+ 反探（零 QObject 面/零 Qt 线程设施 QThread/QThreadPool/QtConcurrent/QMutex/零网格逻辑回流记号 kFaces/occludesNeighborFace/MaskEntry/tileFor/零 World 读面 WorldFacade/captureChunkMeshSnapshot）+ 段内 sweep 自证双文件白名单（thread 记号恰 backgroundgeneration.h + meshworker.h 两落点各 ≥1 此外零落点）+ 「MeshWorker」记号全 src 树只在 meshworker.h（chunkgeometry.{h,cpp} 显式反探 + Main.qml 零记号——零生产接线结构性事实）+ 线程身份对账（执行==工作≠调用者，r2012 同款）。

**同变更钉面修订（纠偏留痕非放宽）**：t1023c 白名单腿（section08_recent.cpp）——「唯一受认可落点 backgroundgeneration.h」→ **双文件白名单** {backgroundgeneration.h[R20.12], meshworker.h[r2020/D6，生产零接线]} 且仅 std::thread，新增「每落点在场 ≥1 命中」防空转；F3 行 `threads: 0/0 (sync meshing)` 照常钉（mesher 生产路径仍同步——组件无头）；腿名/PASS 文本如实化（"R20.12 amended, dual-site"），腿名词元零 r2020 污染（filter 命名空间纪律双向核查）；backgroundgeneration.h 头注「（当前唯一）」同步如实化。**agent-state R20.12 纪律行（F3 行）同口径修订**。

**验证链（全绿）**：分段增量构建（-j 4）→ binary mtime (18:20) > 全部改动源 → filter **r2020 首跑** 3P/1F 即修（见过程坑）→ **4P + 三连跑全绿**（r2012 防 flake 先例）→ 前置回归（终版 binary 全族重跑）**t1023c 1P[修订后语义] / r2007 5P / r2008 5P / r2009 4P / r2010 6P / r2010b 3P / r2011~r2019 4P×9** 计数逐一吻合 EXIT=0 → 阴性轮（见下）→ 全矩阵 **620 PASS / 0 FAIL ×2**（matrix_r2020_pos.log + matrix_r2020_final.log 双日志在案 EXIT=0，两文件齐全后才写本关单文；**616 权威 PASS 行 diff = +4 恰 r2020a-d** + 3 条移行全归类：t813 戳 17:14→18:29 与哈希 7887c59→841719f、t997 两墙钟比值 0.292→0.1785 / 0.00666→0.00491[登记类]、**t1023c 同变更修订行**"amended"→"amended, dual-site"如实登记；t830/t979 零漂移；pos/final 腿集合恒等 620、两轮间唯一差 = t997 墙钟登记类；^FAIL 逐行归类 = 0）→ app voxelsandbox 重建 EXIT=0 + offscreen 冒烟 **EXIT=124** + logs/voxelsandbox_r2020_tail20.log 稳态 60fps（prof[1s] 60fr / frame main*16.0(60)）——**零生产接线 → app 侧零变化由冒烟 tail20 实证**。

**阴性轮（红跑/还原四日志直接落 build/ 终名，恰红面设计先于腿文定稿；实测红面 == 声明面）**：**NEG-1**（matrix_r2020_neg1_red.log / matrix_r2020_neg1_restore.log）= 摘「接受项恰一产出」（threadLoop 产出 push 加 `t.requestId != 20` 条件跳过——id 20 的 built 遗漏一条）→ rebuild → **恰红 {r2020b} 单腿**[diag = batch harvest timeout 11 / got=11 = 被摘语义本体；r2020a ids 1-8、r2020c ids 40-52、r2020d id 60 均不含 20 不误伤——恰红面收缩单腿] → Edit 反向还原回绿 4P/0F。**NEG-2**（matrix_r2020_neg2_red.log / matrix_r2020_neg2_restore.log）= worker 执行体加工输出（build 返回后翻转首顶点 r 域 = 模拟「顺手复制/改写网格逻辑」）→ rebuild → **恰红 {r2020a} 单腿**[diag = 7 非空快照 eq=0 且顶点计数逐位相同（vc 20696/20696 等）= 纯逐位扰动的被摘语义本体；空快照经 isEmpty 守卫跳过如设计] → Edit 反向还原回绿 4P/0F（还原后复核：变异字面 0 残留）。

**过程坑**：①**Windows 定时器粒度坑（首跑 r2020c 假红实证）**：QThread::msleep(1) 实际粒度 ~15.6ms——「在途构建时长」级瞬态 pending 窗被整窗漏过（obs=0/sink=0/队已排干）。三路加固：拦截体重负化（monster 高 96→256：~19.7 万面/~52.5 万顶点，构建数十毫秒 ≫ 15.6ms 轮询粒度）+ waitPending 改 busy-yield µs 级轮询 + r2020c 改**饱和型积压构造**（第 3 提交的可见 kErrQueueFull 拒绝 = 「M 在途 + 队满」确定性证明，不追瞬态窗——r2020b② 同门先例）。教训入账：**瞬态窗等待禁 msleep 粒度轮询；时序承重体与观测粒度须差两个数量级以上**。②腿名 filter 词纪律双向核查（t1023c 修订腿名不含 "R2020" 词元防反向污染；r2020d 腿名不含 "t1023c" 防正向污染——r2019 双向去词元同款）。③pinSet minCount 按剥注释后代码位实际出现次数核（kErrQueueFull 代码位恰 1，首版 x2 误设自纠——r2009d 教训再现）。

**登记非目标**：生产接线（ChunkGeometry bake 路径切换 = 后续单/§29.4-P4 前；requestId = chunk key 派生同期落地）；快照采集线程化（World 非线程安全——采集恒调用者线程）；多 worker 并发（单线程起步，扩并发 = 后续单）；优先级调度（FIFO 提交序即序）与结果缓存/去重（requestId 语义全权归调用方）。

**待实机确认**：无新增（组件无头，app 侧零变化由冒烟 tail20 实证；**P4 生产接线后复验项**：编辑重建走 worker 后破/放贴图当帧刷新时延观感、F3 mesh 桶时长占比变化）。

**下一任务**：**§29.4-P4 QML 动态化**（全 R20 唯一动 QML 相位，待主控开工单——r2020 MeshWorker 组件在库待接线、P1-P3 策略/驱动/卸载三件套在库默认关），随后 P5 实机调参验收。

### §29.4-P4（2026-09-16，QML 动态化——ChunkGeometry 固定网格实例化 → 驻留集模型驱动 slot 池 + World 最小驻留集 QML 面，全 R20 唯一动 QML 相位）

→ **✅✅ 落地（2026-09-16）：15bb6c1 fix + d93404c test + 本 docs，矩阵 620→624。**渲染侧数据驱动化（streaming 仍默认关 → 固定世界渲染逐位不变 = 本单零变化承重墙）。**C++ 最小 QML 面（world.h 选型注释立此存照）**：①驻留集 = {Loaded, Active} = chunkLifecycleQueryable（与 mesher 存在门同谓词 →「ChunkGeometry slot 存在 ⇔ mesher 门接受该 chunk」渲染存在面单一口径；专用桥 QObject 更厚弃用——World 已是 QML 唯一世界单件；P1-P3/D6 值组件 QObjectFree 零暴露零接线）；②三成员 = `Q_PROPERTY int residentChunkRevision`（NOTIFY residentChunkRevisionChanged）+ `Q_INVOKABLE int residentChunkCount()` + `Q_INVOKABLE QVariantList residentChunkKeyAt(int)`（[cx,cz]，t1020 structureRegion 返回形态先例）——逐一配矩阵钉（r2021d）；③revision 沿 = 驻留成员集变化（③⑧加入/⑥移出 bump+emit 恰一沿；⑤④集内平移、①②⑦⑨集外沿零 bump——只追渲染集成员不追全转移史）；唯一写点 = World::setChunkLifecycle forwarder（非 Q_INVOKABLE 永久 = r2010d 零决策面延伸，r2021d 反探常驻；生产接线期流式组件转移经此 forwarder 自动携带沿 = 接线缝已留）；④枚举序单一权威 = world.cpp `residentChunkKeysOrdered()`（cz 外 cx 内 = 旧 t276 固定网格循环序 = ChunkManager flatIndex 序；count/keyAt 双消费面共用禁第二份循环）。**Main.qml（只动 chunk 实例化模式段，玩法路径零字节触碰）**：chunkAnchor.onCompleted → `rebuildChunkSlotPool()`（驻留集枚举逐键实例化同 5 段组合；chunksBuilt 一次性守卫同旧）+ revision 沿 Connections（拆池 → 按新枚举整池重派生——保 `_refreshChunkVisibility`/`kickWorldMeshSync` 的「每 segmentsPerChunk 段一组、组首全局对齐」扁平数组不变量，两消费端零触碰 = 变更面集中的承重选择）+ pool 函数落 Window 级（recomputeMeshStats 后 chunk 机器段）；chunkgeometry.{h,cpp} 零触碰（cx/cz 参数本已充分）。**腿（section24 置尾，filter 词 r2021）**：r2021a 固定世界零变化承重墙（3×3 方阵 + 2×3 非方阵序判别枚举 ≡ 旧网格逐项恒等 + revision 初始 0 + 编辑零沿 + keyAt 越界空表 + 幂等 + 枚举键直驱 ChunkGeometry 非空 mesh=QML 消费契约）；r2021b 模型响应性（合法转移 revision/发射精确对账 + 枚举随增删归位）；r2021c QML real-chain 响应性（真 QQmlEngine × 真 World × setData wrapper 池镜像——沿同步直达 handler、池随增删、零警告、rebuilds==发射+初建账面闭合）；r2021d 钉面（world.h 三成员逐一正面钉 + world.cpp 枚举权威钉 + Main.qml 消费面精确计数 + 旧固定网格循环反探退役 + setChunkLifecycle 禁入 QML + 玩法标记在位 + GameSession 零提及复述）。**验证链（全绿）**：分段增量构建（-j 4）→ binary mtime（19:47/20:38）> 全部改动源 → filter r2021 首跑 3P/1F 即修两处腿文坑（wrapper 初参注入消 Connections 创建期警告 + 重加链补 ⑦ 中间步 + rebuilds 对账口径补初建）→ 4P 三连跑全绿 → 前置回归 r2007=5P r2008=5P r2009=4P r2010=6P r2010b=3P r2011~r2020=4P×10 + t874 家族 2P（首跑 r2007=6P/r2010=7P = 本段腿文跨任务词元污染 filter 命名空间——双向去词元后达标，教训入 agent-state 纪律）→ 阴性两轮（NEG-1 摘发射沿 → 恰红 {r2021b,r2021c}=声明面；NEG-2 摘枚举恒等口径 → 实测 {r2021a,r2021b,r2021c} **超首版声明 {r2021a} 改声明留痕**[t1053/t1054 先例]：b/c 断言锚旧网格 cz 主序基线数组 = 被测语义本体非连带伤，逐腿 diag 核对零削钉）→ 双 Edit 反向还原回绿（matrix_r2021_neg{1,2}_{red,restore}.log 直落 build/ 终名；变异字面 0 残留）→ 全矩阵 **624 PASS / 0 FAIL ×2**（matrix_r2021_pos/final.log EXIT=0；620 权威 diff = +4 恰 r2021a-d + 登记类移行 t813/t997；pos/final 腿集合恒等 624；^FAIL 归类 0）→ app 重建 EXIT=0 + offscreen 冒烟 EXIT=124 + logs/voxelsandbox_r2021_tail20.log 稳态 60fps + F3 mesh 行与基线逐字同口径 + 启动日志 `[t276/r2021] built 500 chunk Models from 100 resident chunks`（模型驱动实例化量 = 旧固定网格 10×10×5 逐位同量）。**登记非目标**：增量 slot patch（整池重派生的经济学优化 = 生产接线单）；运行期换网格尺寸池重建（旧守卫同态）；驻留集枚举缓存（沿时刻调用无热路径）；P1-P3/D6 生产接线。

**待实机确认（必列）**：①固定世界观感零变化实机核（地形/水/岩浆/玻璃/冰五段渲染与 P4 前逐像素一致——headless 冒烟只能证实例化量与帧稳态，像素面须真机目视）；②F3 对照（mesh 行顶点/三角汇总、drawCalls、chunk updates 与 P4 前真机数值同口径）；③（生产接线后复验）streaming 开启后视距推进中 chunk 池随增删的观感与重建经济学。

**下一任务**：**生产接线（D2/D3）**——D2 新世界参数开关（旧档/旧世界行为不变）+ 位置源/挂点/bake 路径接线（ChunkStreamDriver.onPlayerChunk 位置源 = 玩家 chunk 沿、lifecycleTransitionFn/dirtyQueryFn/persistFn 三缝生产绑定、ChunkGeometry bake 路径切换[MeshWorker requestId = chunk key 派生同期]）+ D3 就地流式化（SaveCoordinator 代次机制承载）；**设计登记：接线单开工前主控先出接线设计段**；随后 P5 实机调参验收。

### §29.5-W1（2026-09-16，稀疏世界核——World 构造模式分化 + 出生半径预生成 + 未加载域 OOB 等价语义，流式激活接线弧第一单）

→ **✅ 落地（2026-09-16）：5f31dbb fix + ed49154 test + 本 docs，矩阵 624→628。****设计选型（头注释立此存照，§29.5.1 前置事实 + 29.5.4 风险登记为语义依据）**：①**WorldMode{Fixed,Sparse} 显式模式位**（chunkmanager.h；拒「dims 哨兵参数化」备选案——TerrainGen 海域半径/列钳高需要真实 dims，哨兵会破同 seed 恒等承重；width/depth 是 Q_PROPERTY 且被十余处派生面读取，显式模式位 = 「Fixed 分支语句原样 + sparse 全加性」结构性零变化墙）；②稀疏存储 = unordered_map<ChunkKey::packed, SparseSlot{unique_ptr<Chunk>, life}>（负坐标合法 + fromPacked 键逆映射权威；实例与生命周期同槽 = Fixed 稠密 vector 对的键控同构，六态转移图零分叉）；③**统一谓词单点**（禁散落第二份存在性判断）：chunkMaterialized[查询门 = 存在 ∧ chunkLifecycleQueryable，与 mesher 门/r2021 驻留集同谓词] + chunkContentPresent[写门 = 内容在途或可用]——14 门 sparse 分支统一走谓词、未物化一律现行「坐标越界」同值早退（OOB 等价零新增语义分支，y 域两模式同构）；④构造分化：fixed ctor generate() 原样；sparse ctor（SparseWorldParams，spawnPreGenerateRadius 默认 2 = D4 首落[方形切比雪夫同构 + 冷启动 ∝R² + 任务带 2-4 取下限]）→ 零全量生成零 chunk 分配[全 Absent] + 出生半径预生成[5×5 方阵走 terraingen 单列权威 fillTerrainColumn + ①②③生命周期合法链，稳态 = fixed create 终态] + 光场/出生列/结构区域三 generate 尾同门；⑤loadChunkAt 生产缝（幂等 + refloodBox 列种子光 flood；W2 接线用本单生产零调用）；⑥守卫族七入口 isSparse() 早退（尺寸/seed/存档族不适用稀疏域，W5 面）；⑦residentChunkKeysOrdered sparse 分支收集+cz 主序排序（枚举序契约同 r2021）。**worldstore/Main.qml/P1-P3/D6 组件/线程原语零触碰**（app fixed 世界 = sparse 代码零激活）。**腿（section25 置尾，filter 词 r2022）**：r2022a fixed 零变化承重墙（四 setter incantation 全语义恒等 + 越界门逐门 + 裸管理器谓词恒真 + D4 归一化）；r2022b OOB 等价语义族+加载翻转（正/负出界逐门同值且与 fixed 越界对账 + loadChunkAt 翻转真值[内容无关面] + forwarder ⑥驱逐回 OOB/⑧取消驱逐复真值 + 拒写零副作用）；r2022c 同 seed 区域恒等承重（纯函数门三面对账含负坐标 + 5 chunk[含负坐标]对 generateTerrainChunk 权威缓冲逐体素恒等 + 内陆 envelope 锚）；r2022d 钉面（模式枚举/默认参/统一谓词/守卫族/生命周期唯一入口/单列权威正面钉 + worldstore 全盲反探 + QML/组件族零记号）。**验证链全绿**：分段增量构建 → binary mtime > 改动源 → filter r2022 首跑 2P/2F 即修腿文坑[预生成域边界/方阵记账] + 腿名/行文本补词元 → 4P 三连跑全绿 + 相邻 filter 计数零污染 → 前置回归全族绿 → 阴性两轮恰红声明面（NEG-1 摘「可查询」合取 → {r2022b} 单腿[Evicting 相位返真值 = 被摘语义本体；钉面同变更改锚保恰红单腿]；NEG-2 seed ⊕0x5A5A 偏置 → {r2022c} 单腿[r2022b 内容无关面按设计存活]）→ 双 Edit 还原回绿（matrix_r2022_neg{1,2}_{red,restore}.log 直落 build/）→ 全矩阵 **628/0 ×2**（pos/final EXIT=0；624 权威 diff = +4 恰新腿 + t813/t997 登记类；腿集合恒等；^FAIL 归类 0）→ app 重建 EXIT=0 + 冒烟 EXIT=124 + tail20 稳态 60fps + 100 resident 实例化不变。**登记非目标**：跨 chunk 结构 population 重放（树/矿/洞/水 = W2+ 流式 worldgen 设计面——sparse chunk = terraingen terrain-only，与 fixed 全量世界全逐素恒等物理不成立，r2022c 恒等面如实锚权威缓冲 + 纯函数门）；sparse 持久化（W3/W5）；卸载执行（W3，forwarder ⑥⑧即测试缝）；sparse 尺寸/seed 运行期变更（W5）；D4 双半径联动（W2）。**待实机确认**：无新增（sparse 无 UI 入口生产零激活；W5 接线后复验无限世界观感 + D4 首屏调参）。

**下一任务**：**§29.5-W2 位置源 + 驱动接线**（玩家位 → floorDiv16 → GameSession tick 尾 driver.onPlayerChunk；GenerationJob ↔ ChunkManager setLifecycle 挂点首次生产接线；BackgroundGenerationWorker 首个生产消费者；streaming 仅 sparse 世界使能——W1 的 loadChunkAt/统一谓词/setChunkLifecycle 沿即接线缝），随后 W3 驱逐+Edits-on-evict → W4 bake→MeshWorker → W5 D2 开关+D3 转换。

### §29.5-W1b（2026-09-16，sparse population parity——fixed 生成全 pass 的窗口确定性重放，治理审计 #11 放行的流式世界内容正确性前置单）

→ **✅ 落地（2026-09-16）：ffd0cf9 fix + bccc633 test + 本 docs，矩阵 628→632。**审计 #11 第 6 条（W1 内容正确性欠账）清偿：sparse chunk 不再 terrain-only，fixed generate() 的后地形 pass 按窗口确定性重放，流式世界不再比 fixed 世界贫瘠。**逐 pass 处置表（单一权威 = world.cpp sparsePopulateChunk 头注释；全录如下）**：

| pass（fixed generate() 执行序） | 处置 | 理由 / MC 引证 |
|---|---|---|
| fillTerrainColumn 列填充 | (a) | terraingen 单列权威，W1 已重放 |
| placeBedrock | (b)窗 | 逐体素纯 hashVoxel 零状态读，窗口=自身列即精确 |
| scatterOres | (c)替 | 脉形（blob offStone/长条壳/铁立方角）以体素态反馈走向，读域出窗即两序不齐（r2023c 恰红实证）；严格闭合需 C±2 chunk 读域=25 脚手架/chunk 违 sparse 成本纪律 → MC 1.0 per-chunk vein 同构替代（wiki/w/World_generation "Each feature has its own placement rules including the number of placement attempts"），cell 窗+tryOre 读写域双钳窗；块缘 ±8 列跨 cell 脉形交互差异如实登记 |
| placeGravelPockets | (b)全 | 网格 16+抖动±4+半径≤3 → C 影响读 ⊆ 窗；读=写位 Stone 检查 |
| carveCaves(a) 阈值噪声 | (b)窗 | 逐体素纯噪声阈值，读=写位 |
| carveCaves(b) Perlin worm | (b)追 | 路径纯 noise3 位置链零体素读 → 每 chunk 全域 trace 自含重derive；carveSphere 读=写位 skip（Air 上不写=终态同义） |
| carveCaveEntrances | (b)全 | 山坡判定=heightAt 纯 4 邻；cave-air 搜索=候选自身列 ∈ scaffold；开口 3×3 写 ⊆ 中心±4 |
| placeUndergroundWaterPools | (b)全 | 海检/高度纯；disc 读=写位守卫 ⊆ 中心±7 |
| placeLavaLakes | (b)全 | 同上（网格 16） |
| placeDungeons | (c)豁 | 豁口搜空气读域=足迹±12 曼哈顿跨 scaffold；MC 引证：monster room 属 underground_structures decoration step 的 per-chunk feature，结构生成只读自身模板与噪声地形、不读邻块 worldgen 体素态——本 pass 的体素态依赖恰为与 MC 异构之处，如实豁免不硬造 |
| placeMineshaft | (c)豁 | 写域 ⊆ 中心±26 超 scaffold；t1012 地板政策读支撑体素跨界；MC 引证："A structure set determines the placement positions of the structures"（spacing/separation/frequency per-position 确定性布置，starts 由 chunk 坐标种子推导） |
| placeDesertTemple / placeJungleTemple | (c)豁 | 结构族（同 mineshaft 引证） |
| placeStronghold | (c)豁 | 结构族；同页 "strongholds in Java Edition … are placed as concentric rings" |
| carveCanyon | (b)追 | 路径纯 noise2 链+单 worm 全域 trace；carveDisc/排水带/侧洞读=写位（skip 守卫等幂）→ 窗口 bbox 早退只省窗外盘 |
| pruneFloatingSnowLayers / pruneUnsupportedWorldgenRails | (b)窗 | 扫列读自身列正下方一格（矿井豁免 → 无生成轨 → 自然 no-op 与 fixed 自洽） |
| fillWater / freezeSurfaceWater | (b)窗 | 逐列读自身列（seaColumnHeight 纯） |
| placeSurfaceLakes | (b)全 | 低洼判定=heightAt 纯 disc ±rad+1；carve/灌水读=写位 ⊆ 中心±8 |
| placeSwampPools | (b)窗 | 逐列自列读（biome/sea/height 纯） |
| placeTrees / placeJungleTrees | (b)窗 | 树冠 ≤3 IfAir 写 ⊆ scaffold；surf/under1=自列；间距 occupied=窗口扫描（C 的 ±1 检查 ⊆ scaffold） |
| placeTallGrass / placeSwampFlora / placeFlowers / placeSweetBerryBushes | (b)窗 | 逐列自列读 |
| placeDesertFlora | (b)窗 | 仙人掌 4 邻 isFullCube 读 ±1 ⊆ scaffold |
| placeSugarcane | (b)窗 | 邻水判定 4 邻×2 层读 ±1 ⊆ scaffold |
| findSpawnColumn / recomputeLightField / rebuildFluid/IceCells / rebuildStructureRegions | (a/c) | 世界级收口：W1 sparseGenerate 尾同门已处理（light 按需物化=loadChunkAt refloodBox，W1 登记）；索引=population 尾 rebuildPopulationCellIndexes 自身列重建；结构区域表=sites() 纯函数（W1 已落，与体素 population 无关） |

**精确性机制（两轮恰红实证倒逼的两条铁律，头注释立证）**：①**读域时刻一致性**——已物化邻 chunk 的现存内容是终态（含洞穴/树/水），直接读终态会把后序事实泄漏进早期 pass 决策（矿脉 Stone 竞争反序，r2023b 首红实证）→ 已物化邻走快照→数组清零+纯地形回填→population→终态恢复（恢复无损：溢写已在邻块自身 population 时以同值落位）；②**写域两序同空**——全候选域 pass 的远端溢写若落入本 population 时恰已物化的真邻块 = pass-k 数学 applied 到终态 = 按加载次序污染（r2023c 首红实证）→ ChunkManager population 写域钳制（锚 ±1 chunk，fixed/平时恒 off 零开销）：窗外溢写两序同拒。scaffold=自身 ±1 chunk = MC "features … limited in the nearby 3×3 chunk area" 同构；新建脚手架（①②③ 到可查询稳态）population 后经 ⑥⑦ 合法边 + releaseSparseChunk 擦槽（数据不保留，正式物化时全量重放逐位重derive）。**fixed 路径零变化**：窗口参数默认全零=全域原样、m_worldgenQuiet 恒 false（qInfo 逐字原样）、写钳制恒 off——worldgen/determin/re-gen 族与 r2023a 双 generate 逐位恒等墙实证。**腿（section26 置尾，filter 词 r2023，80×80×96 s42 fixtures——5×5 chunk radius 2 预生成恰覆全核心域）**：r2023a fixed 零变化墙（同 seed 双 generate 抽样列全 y 带逐位恒等 + generate() 体 32 条全 pass 调用序字面钉 + population 反探）；r2023b parity 承重墙（9 内部 chunk 全 16×16×H 非 ore 体素逐位恒等 + 逐体素结构区域掩蔽[五类 inside* + xz 9 点偏移栅格 {0,±6,±12} 覆盖地牢豁口 ±12] + ore 掩蔽[scatterOres (c) 替代面] + 无掩蔽全等面 ≥10 万体素 + (b) 签名计数非空转）；r2023c 加载顺序无关（radius 0 双世界两种 loadChunkAt 次序同组 5 chunk[含负坐标 (-1,2)]逐位恒等 + 非空柱顶）；r2023d 结构钉（populate 静默窗+代表窗口调用+scaffold 生命周期/拆卸单点正面钉 + 结构族豁免反探[函数域手工 contains] + generate() 禁出 population）。**r2022c 同变更修订（纠偏留痕非削钉）**：W1 原面「已加载 chunk ≡ terrain-only 权威缓冲」在 population 落地后物理失效——恒等面改锚：负坐标 chunk 纯地形带 [y∈0,5)（bedrock 之上 ore 之下零 population 触达）对权威缓冲逐体素恒等 + 三分支 envelope 锚（纯地形面/特征面/carve 露出自然面含裸露矿脉）；收宽面移交 r2023b。**验证链全绿**：分段增量构建（-j 4）→ binary mtime > 全部改动源 → filter r2023 首跑 1P/3F 迭代三处实现级缺陷（①真邻块终态读泄漏=矿竞争反序；②真邻块溢写污染=次序相关；③ores 走向体素态耦合=两序不齐——各自恰红实证后落地快照恢复/写域钳制/双钳窗三修）→ 4P 全绿 → 前置回归 r2007=5P r2008=5P r2009=4P r2010=6P r2010b=3P r2011~r2021=4P×11 + t874=2P + t1023c=1P + worldgen/determin/re-gen/store/save 0 FAIL → 阴性两轮（NEG-1 摘 placeTrees 窗口调用 → 恰红 {r2023b} 单腿=声明面[r2023c 双世界同缺树仍互等、r2022c 修订面负坐标纯地形带、r2023a/d 不误伤]；NEG-2 树干高混入驻留 revision → 恰红 {r2023b, r2023c}[首版声明 {r2023c} 实测扩面按先例改声明留痕：parity 与顺序无关性是同一确定性机制的两面，次序污染天然双破]）→ 双 Edit 反向还原回绿（matrix_r2023_neg{1,2}_{red,restore}.log 直落 build/ 终名；变异字面 0 残留）→ 全矩阵 **632/0 ×2**（pos/final EXIT=0；628 权威 diff = +4 恰 r2023a-d + r2022c 同变更修订行 + t813 哈希登记类；pos/final 腿集合恒等；^FAIL 归类 0）→ app 重建 EXIT=0 + offscreen 冒烟 EXIT=124 + voxelsandbox_r2023_tail20.log 稳态 60fps（prof[1s] 60fr / frame main*16.0(61)）+ 启动日志 500 chunk Models from 100 resident（app fixed 世界 = sparse population 零激活实证）。**登记非目标**：结构族五员的 sparse 侧替代（region 键确定性布置 = W2+ 流式 worldgen 设计面，随实机观感评估）；ores 块缘跨 cell 交互的精确化（需 C±2 读域，成本纪律优先）；population 溢写对已物化邻块的增量更新（当前恢复丢弃——邻块自身 population 已同值落位）；溢写后光照精度（W1 登记）；fluid/ice 活动索引的邻块增量（同前）。**待实机确认**：无新增（sparse 无 UI 入口生产零激活；W5 接线后复验项追加：流式世界矿/洞/树/水观感 vs fixed 世界对照——本单 parity 面的实机目视）。**下一任务**：**§29.5-W2 位置源 + 驱动接线**（不变，见上）。

### §29.5-W2（2026-09-17，位置源 + 驱动接线——流式激活第一次生产通电：P1 决策 × P2 驱动 × R20.12 真线程件 × W1 稀疏核的首次同台）

→ **✅ 落地（2026-09-17）：fix + test + 本 docs 三段提交，矩阵 632→636。设计选型（头注释立此存照，refactor-plan §29.5.2 W2 原文为契约）**：①**通电条件 = World::isSparse()**（D2 承重门）——GameSession 构造体门：sparse 世界才构造 ChunkStreamDriver + BackgroundGenerationWorker（双 unique_ptr；fixed 世界连构造都不发生 = 零活动墙最强形态，app 冒烟同面实证）；**驱动器归属 GameSession**（R20.07 编排壳纪律——驱动编排归会话壳，World 保持既有职责面；成员声明序 = worker 先、driver 后 → 析构逆序 = 「scheduler 先于 worker 析构」结构性成立[review0916 #8 契约]）；②**位置源零 QML**：PlayerController tickImpl env 桶内 floorDiv16 换格沿检测（ChunkKey::fromWorld 单一权威；t1000/t1020 进入沿先例同位——!m_worldRunning 早退之前；哨兵 INT_MIN 起始沿 + setWorld 重置）→ 新信号 playerChunkChanged(cx,cz) → 生产接线 = QObject::connect 直连 GameSession::notePlayerChunk（QML 全文件零 diff，r2024d 反探钉）；③**组件最小扩展（默认未设 = r2018/r2019 逐位等价）**：chunkstreamdriver.h 四员——setSlotEnsure（submit 前槽位物化缝：sparse 边①的槽位存在性前提，ChunkManager::setLifecycle 无槽位拒）/attachLifecycleSink（r2011 预留 ChunkManager 挂点首用转发，引用形参保 r2018d 指针反探字面不变）/enableStreamingWith + streamingWithDefaultRadii（显式使能参数缝，P1 默认半径 4/4/6 单一来源不复制字面量，GameSession 接线面零策略类型记号[r2016d 反探免修订]）；generationjob.h 一员——setChunks（构造参数形态不变，null = 纯请求模型语义零变化）；④**生命周期三边分工**：边①（交接→Loading）/边②（收割→Generated）由 GenerationScheduler 挂点真实驱动（r2011 预留面首用，r2017 kind 门语义原样；World::streamingLifecycleSink() 提供挂点）；边③（Generated→Loaded）由 World::adoptGeneratedChunk 经 World::setChunkLifecycle 唯一入口收尾（驻留 revision 沿自动携带 = P4 池消费面连通）；⑤**生产执行体**：BackgroundGenerationWorker 原样复用（worker 线程 generateTerrainChunk 纯函数造缓冲，零 World 指针零 QObject）；主线程收割拍 World::adoptGeneratedChunk = 缓冲经 r2012 applyGeneratedChunkData 守卫入口逐格落地 + heightmap 重算 + ③晋升 + **sparsePopulateChunk 主线程窗口重放**（population 写 World 不可越线程；自身 chunk 可查询后运行 = W1b 同门）+ refloodBox 列种子光（loadChunkAt 同门）；⑥**#7 硬契约兑现**（review0916 #7：takeCompletedAsync 与 takeResultData 各自独立 pop，只收一面 = m_data 无界积压）——GameSession::pumpStreamingTick（tick 尾，m_edits.clear() 之后）内**结果面与数据面同拍并列双消**：takeOutcome 循环 + takeResultData 循环同居单一函数体（r2024d 函数域钉）；数据面 = 主线程落位唯一通路；⑦**在途流式邻居守卫**（W2 同变更扩展 sparsePopulateChunk）：邻槽位在而非可查询非 Absent（Loading/Generated = 异步在途）→ 走快照-回填-恢复同门（数组此刻恒空：快照=全零→纯地形回填呈 pass-k 读域→population 窗口写入落快照域→恢复归零→其自身 adopt 落缓冲逐位重 derive），**不走脚手架创建/拆卸**（⑥⑦+擦槽会拆在途槽位）；W1 同步链无在途槽位 = 死分支（r2023 腿族零扰动）；⑧**采纳语义登记**：数据面无条件采纳（在途被取消的 job 照常完成+采纳——内容已算出；离场驻留的回收 = W3 驱逐面）；slotEnsure 先于 submit（被拒请求留 Absent 槽 = 重请求面）。**QML 零改动 / worldstore 零触碰 / 线程原语零新增落点（复用 R20.12 件，t1023c 白名单原样）/ fixed 生成语义零触碰**。**腿（section27 置尾，filter 词 r2024；sparse 80×80×96 s42 radius 0 + fixed 48×48×96 s82 孪生）**：r2024a fixed 零活动承重墙（fixed 会话无驱动器无 worker[streamingActive false + 双 null] + 真 PlayerController 跨 ≥3 次 chunk 边界扫掠 × 12 整 tick → 零生命周期/revision/outcome/adopt 活动 + 九态表/驻留账/抽样列体素+高度图全逐位不动）；r2024b 通电走查承重（真走查 × 真驱动 × 真线程：窗内 8 chunk[中心 radius-0 预生成]①②③真实驱动到 Loaded + OOB 门翻真[界内实值/界外 OOB 同值] + residentChunkRevision 沿发射 + 驻留账恰 +8/+17 + 走离半径恰 9 取消 + 被取消窗收敛 + 抽样全 y 带 id+state 恒等 fixed 孪生[r2023b 同款 ore+结构 9 点栅格掩蔽] + population 非空转签名）；r2024c 真线程收割 + #7 契约 + 背压（线程身份[observedExec==workerThreadId≠调用者线程，r2012a 先例；落位在调用者线程] + 每 job 恰一数据一条 outcome 恰等计数[8/8/8] + 收割拍双面排干零积压[outcomeCount==0 且 resultDataCount==0] + 9×9 窗 81>64 恰 17 拒绝[距离秩升序→字典序末位 (24,24) 必拒保 Absent] + 半径收窄重请求重生成 (24,24)→Loaded + 收尾账面 pending/双面零残）；r2024d 结构钉（会话通电门/双缝接线/固定早退/位置沿喂入/双面 while/单一 adopt 落点正面钉 + #7 函数域同拍钉 + world.cpp adopt 落位链/在途守卫钉 + generationjob setChunks/driver 转发钉 + QML 四 token 反探 + worldstore 四 token 禁触反探 + playercontroller floorDiv16 沿检测/信号/哨兵重置钉）。**同变更钉面修订（纠偏留痕非削钉，t1023c/r2019 先例）**：r2018d 零接线反探——gamesession.h 从「ChunkStreamDriver 禁出」名单退役（W2 授权的会话接线点，plan §29.5.2 原文），改钉 streamingWithDefaultRadii() 正面在场；world.{h,cpp}/playercontroller 双件/Main.qml 禁出原样 + PASS 文本如实化（恰红面零扩散：r2018a-c 全绿）。**验证链全绿**：分段增量构建（-j 4）→ 双 binary mtime > 全部改动源 → filter r2024 首跑 2P/2F 即修两处腿文坑（①radius-0 构造仍预生成中心 chunk (2,2)→请求恰 8 非 9/驻留恰 +8；②抽样柱命中结构族足迹体素→r2023b 同款 ore+结构掩蔽后 cleanEqual≥200 非空转）→ 4P 全绿 → 三连跑全绿 → 前置回归全族 r2007=5P r2008=5P r2009=4P r2010=6P r2010b=3P r2011~r2021=4P×11 + t874=2P + t1023c=1P + worldgen/determin/re-gen/store/save 0 FAIL（终版 binary）→ 阴性两轮恰红声明面（NEG-1 摘位置沿→onPlayerChunk 接线[if(false) 前缀摘调用点]→恰红 {r2024b, r2024c}[任务书候选面 = r2024b 走查收敛红；r2024c 同住生产接线同源红，双面如实声明]；NEG-2 摘 #7 数据面同拍消费→恰红 {r2024b, r2024c}[任务书候选面 = r2024c m_data 积压红[qData=8 实证]；数据面 = 主线程落位唯一通路 → r2024b 物化收敛同源红，扩面按 t1053/r2021 先例改声明留痕]；r2024a[d] 不误伤[fixed null 驱动器不触被摘行；钉面 = 源码文本在位]）→ 双 Edit 反向还原回绿（matrix_r2024_neg{1,2}_{red,restore}.log 直落 build/ 终名；if(false) 变异字面 0 残留 grep 复核）→ 全矩阵 **636/0 ×2**（matrix_r2024_pos.log + matrix_r2024_final.log EXIT=0；632 权威 diff = +4 恰 r2024a-d + r2018d 同变更修订行 + t813 戳/哈希[01:50→03:55、826d4f2→4a40953] + t997 墙钟登记类；pos/final 腿集合恒等 636；^FAIL 归类 0）→ app 重建 EXIT=0 + offscreen 冒烟 EXIT=124 + voxelsandbox_r2024_tail20.log 稳态 60fps + 启动日志 fixed 世界 500 chunk Models from 100 resident（app = fixed 世界 → 零驱动器构造实证）。**登记非目标**：驱逐面（W3——toEvict 决策产出消费回调保持 null；离场驻留回收 = W3 驱逐）；bake→worker 网格化（W4）；UI 开关（W5——app 侧 GameSession 生产实例化随 W5 sparse 世界入口接线）；半径参数实值（P5——W2 用 P1 默认 4/4/6，configureStreamingRadii 即调参缝）；稀疏持久化（W3/W5，worldstore 零触碰）；投递面 outcome 的业务消费（本单只记账，BlockChanged/成就类消费归后续）；在途取消产物的丢弃式采纳优化（当前无条件采纳，内容已算出）。**待实机确认**：无新增（sparse 无 UI 入口 → 生产零激活，实机挂 W5；app fixed 零活动由冒烟实证；W5 复验项追加：流式世界走查观感——chunk 随走查渐次浮现/首屏等待 D4 调参）。**下一任务**：**§29.5-W3 驱逐 + Edits-on-evict 落盘**（driver setEvictor 注入 → ChunkEvictor 生产缝实装：dirtyQuery = World 脏面、persistFn = §29.5.3 选型 (a) per-chunk 附加表、lifecycleTransition = ChunkManager 真转移；实体×卸载竞态语义收口）→ W4 bake→worker 网格化 → W5 D2 开关 + D3 转换 → P5。

### §29.5-W3（2026-09-17，驱逐 + Edits-on-evict 落盘——setEvictor 从 null 转正：P3 执行器 × D5 附加表 × 实体卸载语义的生产实缝）

→ **✅ 落地（2026-09-17）：fix(cc47e1a) + test(526cfc0) + 本 docs 三段提交，矩阵 636→640。设计选型（头注释立此存照，refactor-plan §29.5.2 W3 原文 + §29.5.3 选型 1 (a) 为契约；W2 流式会话 = 接线宿主）**：①**D5 载体 = 新文件 src/World/chunkstore.{h,cpp}**（per-chunk 附加表，r2015 save_coord additive 先例同款）——表 `chunk_edits(cx, cz PK, generation, voxels, states, light)`，**独立命名连接** "voxelsandbox_chunkstore"（savecoordinator kCoordConn 先例：开-用-关无长活连接，与 worldstore kConn 互不占用）；CREATE TABLE IF NOT EXISTS 幂等建表 **零 bump**（user_version/world_version/既有表零变化，无任何不可逆迁移——数据安全硬门自查通过）；旧码读新档 = 未知表全盲（worldstore 读路径不枚举未知表）、新码读旧档 = 缺表 → 建空表 → hasChunk 恒 miss = Fresh 语义（未命中走现行生成路径）；**worldstore.{h,cpp} 一行禁触**（r2025d 反探钉：chunk_edits/ChunkStore/chunkstore 三词元全盲）。**blob 格式自洽** = 驱逐时刻 chunk 三数组（voxels/states/light）原样字节（布局同 Chunk 数组，worldstore chunks 表行同构收窄到单 chunk）。**代次语义**（SaveCoordinator 口径 per-chunk 收窄）：generation = 该 chunk 累计落盘代次，upsert 前读旧行 +1、首行 1、单调不重置；单行 upsert 事务原子 = marker 角色（「没写进 = 旧行原样保留在旧代次」与「没盖戳 = 没成功」同义——事务本身就是 marker，无需独立在途标记行）。②**持久化时机 = 驱逐候选 dirty 时即时落盘**（persistFn 缝内；不走整世界 saveAll——fixed 整存 blob 是全量栅格，per-chunk 表与它正交并存）。③**persist 域脏面（dirtyQuery 权威）**：盘点结论 = 现有两脏面均不可作「未落盘编辑」判据——Chunk 级 markDirty 是 mesh 重建域（worldChanged 后 clearAllDirty 全表清，t155g）、GameSession EditBuffer DirtyChunkSet 是 tick 窗口域（每 tick 收口清）→ **最小 additive 新权威** = ChunkManager persist 域未落盘编辑集合：标记点 = setBlock 5 参守卫入口成功尾部（**单漏斗**——全部内容写路径[玩家 setBlock/静默写族/实体写/红石·铁轨 state 写/worldgen sink/adopt 守卫应用]都经它，不可漏标）；**RAII 抑制窗** UnsavedEditWriteWindow（深度计数可嵌套）罩住三条生成链（fixed generate / sparseGenerateChunk 全链 / adoptGeneratedChunk 应用+population）——生成写入是内容初生非「编辑」；光场单独变化不记（落盘时三数组原样采集，光渗入无内容损失面）；recreate/reinitializeSparse 随网格清空；persist 成功即 clearUnsavedEdits（已落盘不再 dirty）。④**三缝生产绑定**（GameSession sparse 门内，与 W2 通电接线同址）：dirtyQuery = World::chunkHasUnsavedEdits 转发；persistFn = ChunkStore::persistChunk（失败 Result 穿透 → ChunkEvictor 顺序铁律中止驱逐 = P3「宁驻留不误删」生产面；未 bind 存储同路径 fail-safe）；lifecycleTransition = World::setChunkLifecycle 真转移（⑥⑦合法边，驻留 revision 沿自动携带 = P4 池消费面连通）。**转移缝内两项生产语义**：(a) **边⑥前实体移除**（target==Evicting 先调 setEvictionEntitySink(cx,cz)——Entities 层 despawnInChunk 经 std::function 注入，GameSession 零 Entities 类型依赖[分层不变]）；(b) **边⑦后擦槽**（World::releaseStreamingChunk → ChunkManager::releaseSparseChunk——**数据面闭合**：残留槽会让重物化走「applyGeneratedChunkData 空气零写」把陈旧内容复活[守卫入口只写非空格]，擦槽令重载从真 Absent 起步；population 脚手架拆卸同门先例）。⑤**重载路径**：driver savedContentQuery = ChunkStore::hasChunk（行只增不删 → 命中即稳定）→ Load kind job 照常走边①②（r2017 kind 门语义不变；worker 仍产地形缓冲、收割时弃用——浪费工时登记优化非目标）→ 收割拍数据面按表命中路由：**命中 → World::restoreChunkFromBlob 直接物化**（①②③ 合法链 + blob 尺寸守卫[worldstore loadChunks 同门，失败 ⑨ 回 Absent] + **跳过 sparsePopulateChunk**——存档内容已是驱逐时刻**终态**含 population 与玩家编辑，重放会把 pass-k 语义 applied 到终态上 = W1b「读域时刻一致性」铁律的反面 + 编辑丢失；收尾 = 自身列索引重建[rebuildPopulationCellIndexes 空脚手架形态 + 火格自扫] + refloodBox 列种子光）；未命中 → W2 现行 adoptGeneratedChunk 生成路径；命中但 blob 读败 → 降级生成路径 + qWarning（诚实降级，worldstore 尺寸守卫同门）。⑥**实体×卸载语义收口**（P3 登记面「despawn 半径 vs 卸载顺序」）——**选型 = 驱逐候选区活体先于转移移除**（弃「冻结不 tick 留待重载」：需给全部实体 tick 路径加 per-tick chunk 驻留门 + 物理冻结面，且实体不入档口径下「重载恢复」面不可达）。**MC 引证三元组（2026-09-15 实读）**：①minecraft.wiki/w/Chunk「Chunk loading」段 "Unloaded chunks are unprocessed by the game and do not process any of the game aspects."（卸载 chunk 不处理任何游戏面向——实体停 tick 的 chunk 级口径，本项目卸载后 OOB 语义自然承接）；②同页 simulation 段 "Chunk loading caused by a player ticket allows entities to be ticked only if the chunk is in a square around the chunk which has the player ticket" + "Zombies won't despawn because they are unloaded before being 128 blocks away from the player"（实体 tick 资格绑定 chunk 加载态、实体随 chunk 卸载先行）；③minecraft.wiki/w/Chunk_format History（1.17/20w45a）"Entities have been extracted from main (terrain) once they become full chunks, and are now stored in separate entities directory"（MC 实体存档载体 = chunk 存储本身——卸载即随 chunk 离开活动世界）。**局限如实登记**：本项目实体不入档（R20.14/15 登记非目标），chunk blob 无法携带实体 → 卸载时刻从活动世界移除 = 最接近 MC 的形态；MC 重载后实体自 chunk 存档恢复，本项目移除即不再回来（MC 视角 ≈ despawn 而非 unload）；MC despawn 半径（128 块即时 despawn 等）是独立机制，本单零接线。落地 = EntityManager::despawnInChunk + EntityStore::despawnInChunk（模拟权威 = store[R20.14 纪律]，ItemEntityManager 一行委托）——按 ChunkKey::fromWorld(floor pos) 归属 chunk，走既有 releaseSlot 释放语义（t978 幂等守卫 + free list + QML Repeater 槽稳定契约逐位不动），批量 N 移 1 次通知收口。**QML 零改动 / worldstore 零触碰 / 线程原语零新增落点 / fixed 生成语义零触碰**（fixed 世界连 ChunkStore 都不构造 = 零活动墙最强形态，r2025a 同面钉）。**腿（section28 置尾，filter 词 r2025；sparse 80×80×96 s42 radius 0 + fixed 48×48×96 s82 + 真临时 SQLite 库 fresh+用后即删）**：r2025a fixed 零活动墙（fixed 会话 streamingActive false + 驱动器/worker/**附加表**三者连构造都不发生[chunkEditsStore()==null = 「无附加表读写」最强形态] + bind 面 null 拒 + 真 PlayerController 沿扫掠 × 12 整 tick → 零生命周期/revision/驱逐/轨迹活动 + 抽样列逐位不动）；r2025b 驱逐回灌承重（编辑过的半径外 chunk：persist 先于转移**调用序柱**[轨迹 PersistOk < EdgeEvicting < EdgeAbsent，逐事件带 chunk 键] + ⑥⑦到 Absent + **擦槽**[chunk()==null] + residentChunkRevision 沿 + 重入半径附加表命中 → **blob 物化全 16×16×H 逐位恒等**（玩家 Stone 编辑 + population 终态树等结构都在 = 未走重derive，population 跳过面的行为级承载）+ 未编辑 chunk 走重derive 与 fixed 孪生掩蔽恒等[W1b 登记面如实掩蔽：ore 块缘跨 cell + 结构族 9 点足迹 + 块缘列 lx/lz∈{0,15}；内陆 (b) 级内容零差异零豁免 cleanU≥12000] + 重载起点 clean + 二次驱逐零落盘（行数与代次恒定 = persist 仅 dirty 候选负向面）+ store 恰一行代次恒 1）；r2025c 失败中止 + 实体语义（相A：真锁注入[第二连接 BEGIN EXCLUSIVE，t974 先例生产面] → dirty 候选中止：保持驻留 + 内容/脏账原样 + PersistFail 轨迹 + 零转移事件 + aborted 计数，解锁后下一变更沿重报收敛恰一落盘；相B：驱逐区 mob+掉落物经 sink 先移除恰净、gen 窗内见证者不误伤、转移仍达 Absent）；r2025d 结构钉（worldstore 三词元禁触反探 + 附加表 additive 正面钉[幂等建表+upsert] + 零 bump/零破坏 SQL 反探[user_version/PRAGMA/ALTER/DROP] + 先落盘后转移源序钉[chunkevictor.h 内 indexOf 位序] + 会话缝面正面钉[dirtyQuery/persist/清账/转移/实体缝/擦槽/evictor 转正/savedContentQuery/blob 路由/重derive 回退] + 位序钉[实体缝 < 转移；blob 路由 < 回退] + restore 函数域禁 population 记号 + 实体两族单漏斗钉 + Adapter 一行委托钉 + QML 五 token 反探 + fixed 墙[抑制窗恰 3 + 漏斗标记恰 1]）。**同变更钉面修订（纠偏留痕非削钉，r2018d 先例）**：r2019d 零接线反探——gamesession.h 从「ChunkEvictor 禁出」名单退役（W3 授权接线点，plan §29.5.2 原文「ChunkEvictor 生产缝实装」宿主 = GameSession），改钉 ChunkEvictor 成员 + 三缝绑定正面在场；world/playercontroller/Main.qml 禁出原样。**r2016d 免修订留痕**：setEvictor 回调形参用 generic lambda——GenerationPolicyEvict 记号不入 gamesession.h（「接线面零策略类型记号」纪律延续，W2 同款）。**验证链全绿**：分段增量构建（-j 4）→ binary mtime > 全部改动源 → filter r2025 首跑 1P/3F 即修四处（①r2025c 目标沿 cheb((3,2),(6,6))=4 超 scan 3 → 目标不入 toEvict[算术错]，改 (6,5) cheb=3；②r2025b 未编辑 chunk 全逐位柱 vs 快照在内陆差 59 体素——twin 归因诊断证实差异全落块缘列 ore/carve 上下文敏感面[W1b (c) 登记面]，柱改锚 fixed 孪生 + 登记掩蔽[ore/结构/块缘列]，blob 柱保持全逐位；③r2025d restore 函数域钉被自家注释「无 sparsePopulateChunk 调用」字面误伤反探 → 注释措辞改写；④r2016d 零记号反探被 GenerationPolicyEvict 形参字面误伤 → generic lambda 消记号）→ 4P 三连跑全绿 → 前置回归全族 r2007=5P r2008=5P r2009=4P r2010=6P r2010b=3P r2011=4P r2013~r2015=4P×3 r2017=4P r2018=4P r2020~r2023=4P×4 + t874=2P + t1023c=1P + worldgen 24P/determin 37P/re-gen 6P/store 57P/save 45P 0 FAIL（store +3/save +1 = 本单腿文含 store/save 词素 substring 先例，逐腿归因零未解释）→ 阴性两轮恰红（NEG-1 摘 chunkevictor.h「先落盘后转移」序[persist 块后置到转移步后]→实测红 {r2025b, r2025c, r2025d}[任务书候选面 = r2025b 序柱；c 中止语义与序铁律同源红[转移先行即非「失败中止保持驻留」——l32=0 实证]；d 序柱源钉与被摘语义同居一处文本——三面同源如实改声明留痕[t1053/r2021/r2024 先例]；r2025a 不误伤[fixed 零可达]]；NEG-2 摘 gamesession.h 数据面附加表命中路由[false && 前缀]→恰红 {r2025b} 单腿=声明面[identity-blob 柱：命中表仍走重derive → 编辑丢失 eq=24481 实证；a/c/d 不误伤]）→ 双 Edit 反向还原回绿（matrix_r2025_neg{1,2}_{red,restore}.log 直落 build/ 终名；变异字面 0 残留 grep 复核）→ 全矩阵 **640/0 ×2**（matrix_r2025_pos.log + matrix_r2025_final.log EXIT=0；636 权威 diff = +4 恰 r2025a-d + t1023c src 文件计数 148→149[chunkstore.cpp 新增，r2012=139→…同款 +N 先例] + t813 戳[03:55→06:17] + t979/t997 计时登记类；pos/final 腿集合恒等 640；^FAIL 归类 0。**r2012 filter 计数登记**：前置回归实测 r2012=5P（非 W2 记录的 4P）——第 5 计 = r2024d PASS 文本含 "r2012 guarded application" 词元（section27 本单零改动 git 可证，W2 起既有 substring 先例非本单新增））→ app 重建 EXIT=0 + offscreen 冒烟 EXIT=124 + voxelsandbox_r2025_tail20.log 稳态 60fps（prof[1s] 60fr ×2 / frame main*16.0(60/61)）+ 启动日志 500 chunk Models from 100 resident（app fixed 世界 → 零驱动器零附加表构造实证）。**登记非目标**：**跨会话 blob×附加表 overlay 合并**（W5/D3 域——世界读档时 fixed blob 核心区与 chunk_edits 表的合并设计登记 W5：读档链在 finishLoad 同门后按 chunk 键查附加表，命中行覆盖 blob 同键 chunk 三数组[逐尺寸守卫] + 未覆盖区域走 blob/生成；代次取 max 作冲突基准的方案在 W5 设计段定稿，本单只做会话内驱逐+重载闭环）；ChunkStore 生产 bind（W5 存档入口接线——未 bind 时 dirty 候选中止驱逐 fail-safe）；hasChunk 逐查询开闭连接的经济学（W5/P5 优化面——内存键缓存）；Load kind job 的 worker 空跑（重载仍造地形缓冲后弃用——调度面短路归后续优化）；despawn 半径语义（不引入，引证见上）；驱逐轨迹 QVector 无界增长（驱逐沿粒度量级极小，P5 观测面再议）；实体重载恢复面（实体不入档口径下不可达）。**待实机确认**：无新增（sparse 无 UI 入口 → 生产零激活，实机挂 W5；app fixed 零活动由冒烟实证；W5 复验项追加：无限世界走离观感——远处 chunk 卸载后回走内容与编辑仍在[存档往返真机面]）。**下一任务**：**§29.5-W4 bake→worker 网格化**（ChunkGeometry bake 段改提交快照 + 收割应用；同步内联回退开关；F3 mesh 行加 worker 列；QML 零改动）→ W5 D2 开关 + D3 转换 → P5。


### §29.5-W4（2026-09-17，bake→worker 网格化——ChunkGeometry 同步构建转 D6 执行器异步化：快照提交 × 收割拍应用 × 同步内联回退，流式世界 meshing 离开主线程）

→ **✅ 落地（2026-09-17）：fix + test + 本 docs 三段提交，矩阵 640→644。设计选型（头注释立此存照，refactor-plan §29.5.2 W4 原文为契约；性能口径 = §29.5.4 风险登记「meshing 量随视距增长」——固定世界全量重建窗口经济学随视距推进被流式放大，构建段离场主线程是流式激活的吞吐前提）**：①**通电条件 = World::chunkMeshAsyncActive()（World 桥 sink 已绑定 ⟺ sparse ∧ 流式会话在）**——GameSession 构造体 isSparse() 门内构造 MeshWorker（生产零参默认队列 64）并注入提交缝（setChunkMeshBuildSink——std::function，执行器类型零泄漏进 World/Renderer 头）；fixed 世界连 sink 绑定都不发生 → ChunkGeometry::buildMesh 走现行同步内联路径逐位原样（零变化墙；app 冒烟 win 行 worker 列恒 0 + 启动日志 500 Models from 100 resident 同面实证）；②**执行器所有权 = GameSession**（R20.07 编排壳 + W2「驱动编排归会话」同门；「bake 发起点可达 + 收割拍可达」两准绳：发起点 ChunkGeometry 只达 World → 经 World 桥 std::function 缝可达会话侧执行器；收割拍 = pumpStreamingTick 同宿 tick 尾）；③**bake 异步化三段**：ChunkGeometry 主线程定格快照（captureChunkMeshSnapshot 调用与同步路径同一行）→ requestId 派生提交（deriveMeshJobRequestId = hashMix64(ChunkKey{cx,cz}.packed())<<16 | 段位 3bit[terrain0/water1/lava2/glass3/ice4/cutout5——同 chunk 六段独立 bake 快照互异] | 提交代次 13bit[per-geometry 单调 mod 8192 ≫ 在途上界 128——**纯 chunk 键对同段连发会同键互撞，在途互异是最新胜判据与注册表 1:1 交付的承重前提**；基座哈希域 = 64 位单射假设 hashColumn/hashVoxel 同门，极端碰撞最坏退化为一次丢弃+自然重提交，交付回调判据使误应用结构性不可能]）→ GameSession pumpStreamingTick ⑤ 段收割单点（takeBuilt 排干 → World::deliverBuiltChunkMesh 按注册表路由 → 几何交付回调灌注；非阻塞轮询、延迟 ≤ 一个 tick、QML 零改动）；④**requestId 幂等/覆盖双保险（头注释立证）**：MeshWorker 组件不解释不查重不合并（meshworker.h 头注语义原样）；覆盖语义全权落路由层——几何重提交先显式 cancelChunkMeshJob（旧产出交付时 miss → World::droppedBuiltMeshes 可见丢弃）+ 交付回调自证 deliveredId == m_pendingRequestId（latest-snapshot-wins 第二道闸，meshNstale 计数）；几何同时至多一个在途作业；**最新胜语义下 sun/dayMul 账本记采集时刻定格值**（applyChunkMeshData 形参——应用时刻成员可能已前移，误记会破 sun 量化门致软影陈旧）；⑤**同步回退 = 拒绝即内联**（kErrQueueFull 满载 / 202 执行域已停 / 桥未就绪 → FrameProfiler meshNsyncFallback 计数 + qWarning + 落现行同步内联路径，网格仍正确产出；空快照[无 chunk]走同步零成本语义非回退不计数；解绑 sink 回固定门非回退不计数）；worker 析构丢弃 = droppedCount 组件可见、join 有界绝不挂死；W3 驱逐×在途网格 = QPointer 交付守卫自然失效（行为登记）；⑥**World 桥面**（world.{h,cpp}，C++ only 非 Q_INVOKABLE——r2022d 新面禁 Q_INVOKABLE 同门）：setChunkMeshBuildSink/chunkMeshAsyncActive/submitChunkMeshJob（insert_or_assign 注册表）/cancelChunkMeshJob/deliverBuiltChunkMesh（命中即 erase+move 移交；miss 可见丢弃）+ droppedBuiltMeshCount/pendingChunkMeshJobCount 观测读面 + kErrChunkMeshSinkUnbound=203（2xx 执行域续位防御面）；注册表纯主线程单写者零锁，有界性 = 每被接受请求恰一次交付擦除；⑦**F3 worker 列**（frameprofiler.cpp win 行 mesh 括号后 `worker N`——meshNworker = 交付回调应用计数；既有字段逐字保留、列恒在、fixed 恒 0；同步路径不计；worker 构建段按 Reason::Dirty 计入 meshN/meshNdirty——sun/昼夜驱动的异步重建在该分桶如实呈 dirty，worker 列即异步化可见面，计数分桶差异登记）；「mesh」窗口桶分段口径：提交相 = 采集、交付相 = 灌注（不嵌套），构建段在 worker 线程不计入——异步化收益即构建段离开主线程，桶语义如实分账；⑧**两处同变更钉面修订（纠偏留痕非放宽，t1023c/r2018d/r2019d 先例）**：r2020d 全树「MeshWorker」记号反探——白名单 {meshworker.h} → {meshworker.h + gamesession.h[W4 接线宿主]}，chunkgeometry 双件/Main.qml 反探原样保留（几何经 World 桥 token-free 可达）；t1023c F3 `threads: 0/0 (sync meshing)` 互锁叙述如实化（F3 threads 行 = **fixed 世界** GUI 线程同步 meshing 事实[app 现行唯一世界模式]；sparse 流式世界异步 meshing 可见面 = win 行 worker 列；断言集不变、白名单扫描零新落点——W4 复用 meshworker.h 零新增线程原语）。**QML 零改动 / worldstore 零触碰 / MeshBuilder 单一权威零触碰（r2013d 反探常驻，chunkgeometry 仅存适配面+异步提交/交付两半边）/ t1023c 白名单不新增落点（复用 meshworker.h，t1023c 扫描零漂移）/ fixed 生成语义零触碰**。**腿（section29 置尾，filter 词 r2026；sparse 80×80×96 s42 radius 0 + fixed 48×48×96 s82 孪生 + 真线程收敛 deadline 轮询）**：r2026a fixed 零变化墙[无执行器构造 + 桥未绑定 + refreshMesh 返回即应用（apply-before-return）+ 编辑 setBlock 内同步重建 + 字节与直调权威逐位恒等 + worker/回退/stale 计数恒零 + 零收割]；r2026b 异步等价承重墙[真执行器线程 + 3×3 窗物化 + 四段几何（3 terrain + 同 chunk 水段——段位折叠承重）经桥提交 → tick 尾收割拍收敛 → 收割应用 ≡ 同步直调 MeshBuilder::build 逐位恒等（全顶点/索引缓冲 + 统计——组件级等价墙端到端化）+ residentChunkRevision 沿零漂移 + 账面对账恰等（applied==4 + harvested==applied+可见丢弃 + accepted==pending+queued+harvested）]；r2026c 回退与卫生[最新快照胜（真执行器独立实例 + 手工排干——不经会话收割拍，NEG-1 独立性设计：编辑 A 自动提交 A → 编辑 B 自动提交 B 注销 A → 交付 miss 可见丢弃恰 1 + B 应用逐位恒等终态）+ 满载拒绝回退（stub kErrQueueFull → clearMesh 清零防同态 mask → 同步重应用字节恒等 + meshNsyncFallback 恰 +1）+ 已停拒绝同款（kErrMeshWorkerStopped）+ 解绑 sink 回固定门不计数 + clearMesh 注销卫生（注册表清空）]；r2026d 结构钉[F3 worker 列正面钉（frameprofiler.cpp 格式表达式 + chunkgeometry.cpp meshNworker）+ 行为级 flush 报告钉（静默窗含 'w])  worker 0  world'）+ QML 零触碰反探（Main.qml 零桥方法名/零执行器记号/池面在位）+ 收割拍单点收口（takeBuilt 调用面全 src 树恰 {meshworker.h, gamesession.h}）+ fixed 零构造源序钉（make_unique 在 isSparse 门后）+ World 桥面钉 + 单一权威兼容反探（kFaces/occludesNeighborFace/MaskEntry 禁回流）]。**同变更修订（above ⑧）落 matrix diff = r2020d/t1023c 两行文本修订**。**验证链全绿**：分段增量构建（-j 4）→ binary mtime（矩阵 07:57/应用 09:16）> 全部改动源 → filter r2026 首跑 3P/1F 迭代三处腿文坑（①r2026b 收敛判据把水段顶点数作判据——seed 42 该 chunk 无水空网格恒 0 → 改应用计数恰 +4 承载应用事实；②r2026c 相 1 把显式 refreshMesh 当提交面——编辑的 worldChanged 沿才是生产自动提交面，双提交致注销 miss 计数 3 非 1 → 去显式提交归因沿驱动；③r2026c 相 2/3 同态 mask——相 1 终态网格恰等回退产物，clearMesh 清零后回退重应用才可观测）→ 4P 全绿 + **三连跑绿** → 前置回归全族（终版 binary）r2007=5P r2008=5P r2009=4P r2010=6P r2010b=3P r2011-r2019/r2021-r2025=4P + r2012=5P + t874/t875=2P + t1023c=1P[修订后] + worldgen 24P/determin 37P/re-gen 6P/store 57P/save 45P + vertex 15P/t860 2P/t879 2P/t893 1P/t965 3P/t857 1P/t1023 3P 全 0 FAIL；**腿名跨任务词元去污染**（r2021 教训兑现：首版 r2026a/b/d 腿名含 r2013a/r2013d/r2020a 词元 → r2013/r2020 filter 计数 +2/+1 漂移；双向去词元后 r2013=4P/r2020=4P 复位）→ 阴性两轮恰红声明面（NEG-1 摘收割应用[pump ⑤ 段 if(false) 前缀——built 永不交付]→恰红 {r2026b} 单腿=声明面[收敛超时；r2026a 零 worker 不可达/r2026c 独立执行器+手工排干不经会话收割拍/r2026d 源钉剥注释 token 在位均不误伤]；NEG-2 摘同步回退[拒绝路径 return false→return true 谎报受理]→首跑 4P 未红=被 mask[相 1 终态网格恰等回退产物，陈旧顶点蒙混]→**腿文加固后再验**：clearMesh 清零使回退重应用可观测→恰红 {r2026c} 单腿[fb 计数照走但零重应用；a/b/d 不误伤]——阴性恰红面「先于腿文定稿」纪律的二次兑现：首跑未红即 mask 实证，加固后恰红面收止单腿）→ 双 Edit 反向还原回绿（四日志 matrix_r2026_neg{1,2}_{red,restore}.log 直落 build/ 终名：neg1_red 3P/1F/neg1_restore 4P/0F/neg2_red 3P/1F/neg2_restore 4P/0F；变异字面 0 残留 grep 复核）→ 全矩阵 **644/0 ×2**（matrix_r2026_pos.log + matrix_r2026_final.log EXIT=0；640 权威 diff = +4 恰 r2026a-d + r2020d/t1023c 同变更修订行 2 条 + t813 戳登记类[06:17→08:43]；pos/final 腿集合恒等 644[前 110 字符键 diff IDENTICAL]；^FAIL 归类 0）→ app 重建 EXIT=0 + offscreen 冒烟 EXIT=124 + voxelsandbox_r2026_tail20.log 稳态 60fps[prof[1s] 60fr ×3 / frame main*16.0(60/61)] + F3 win 行 `mesh 0.00(0reb [0d 0s 0w])  worker 0  world 0.00` 与 r2025 基线**既有字段逐字同值 + 新列 worker 恒 0**（本单声明格式变化面；fixed 世界零激活实证）+ 启动日志 500 chunk Models from 100 resident（fixed 世界连执行器都不构造实证）。**登记非目标**：多 worker 扩并发/优先级调度（后续单）；W3 驱逐×在途网格竞态（在途网格按现有可见性门自然失效——QPointer 守卫交付丢弃，行为登记）；QML 任何改动（F3 worker 列在 frameprofiler.cpp win 行 C++ 面；W5 UI 开关为 §29.5 显式登记例外）；结果缓存/去重优化（requestId 语义全权归路由层）；收割拍节流/合帧（tick 尾单点已满足延迟有界）；在途取消产物的丢弃优化（注销面已可见）。**待实机确认**：无新增（app fixed 世界 = 异步路径零激活由冒烟实证；**W5 复验项追加**：无限世界实机走查中破/放方块贴图刷新时延观感[同步内联 → worker 异步的 <1 帧承诺变化为一 tick 内交付]、F3 worker 列在流式走查中的计数量级、mesh 桶时长占比变化[P5 调参输入]）。**下一任务 = §29.5-W5 D2 参数开关 + D3 opt-in 转换**（新世界 UI「无限世界」开关[默认关；QML 例外登记——§29.5 显式登记的最小 QML 新增面] + 世界 meta streaming 标志 additive 零迁移 + D3 世界菜单 opt-in 转换；blob 区成核心区境外按需生成经 W3 落盘）→ P5 实机调参验收。

### §29.5-W5 后端（2026-09-17，流式世界持久化 + D2/D3 标志 + overlay 合并——W5 后端半：QML 零触碰，UI = 下一单 W5b 登记例外面）

→ **✅ 落地（2026-09-17）：fix + test + 本 docs 三段提交，矩阵 644→648。设计选型（头注释立此存照，refactor-plan §29.5.2 W5 原文 + §29.5.3 选型 2 为契约；接线宿主 = W2/W3/W4 流式会话[R20.07 编排壳纪律]；worldstore.{h,cpp}/src/ui/fixed 生成语义/线程原语零触碰）**：①**D2 后端 = stream_worlds 附加表**（chunkstore 域 additive 路线，r2015/r2025 先例同门）——DDL `(world TEXT PRIMARY KEY, streaming INTEGER, core_w INTEGER, core_d INTEGER, base_gen TEXT, save_gen TEXT)`，CREATE TABLE IF NOT EXISTS 幂等建表**零 bump**；世界行本身经现有 Q_INVOKABLE 面创建（生产 = createWorld/openWorld，本域零涉），streaming 标志与 core_w/core_d（出生区界）只住附加表；一库一行（一 .sqlite = 一世界，world 列 = 显式标识）；**fixed 世界零标志零活动**（构造门在 GameSession：fixed 会话连 ChunkStore 都不构造，W3 零活动墙同门延续，r2027a 库面级实证 = fixed 全链后库中无 stream_worlds/chunk_edits/save_coord 任何流式表）。②**chunk_edits 行代次契约 W5 修订（纠偏留痕非破坏）**：W3 原版 per-chunk 累计落盘代次 → 世界保存代次刻度（写入时读 save_gen 盖 stamp = save_gen + 1）——修订依据 = 读档仲裁要求「行代次」与「世界存代次」**同刻度可比**（两个从 1 数起的不可互比刻度是 r2025 前会话内闭环无此需求的盲区）；**观测值兼容性立证**：无 meta 行的库上 save_gen 读 0 → stamp 恒 1，与 W3「首写=1、单写后不重写」全部已观测值逐位同值（r2025b/c 的 gen==1 断言零扰动实证）。③**保存时冲洗（流式世界持久化主通路）= GameSession::flushResidentEditsForSave**：保存链前置挂点（盘点：Main.qml saveAndExitToWorldList → persistWorldState 三同步写[saveAll+savePlayerData+saveProgress，t974 saveOkCount 面]——W5b 在三写前置调用并 gate 成功上报；本单 QML 零触碰，矩阵直驱 C++ API）；**拍序 = 先冲洗、后代次推进**（persistResidentChunk 盖写 save_gen+1 → advance save_gen := 同值 = 「行代次 = 本次保存代次」精确成立；推进失败行已在场不失效——仲裁只对 base_gen，marker 同门）；冲洗域 = 驻留集（ChunkManager::sparseResidentKeysOrdered——与驻留集同谓词 cz 外 cx 内序）× dirtyQuery（W3 persistFn 同缝，persistResidentChunk 唯一执行体 = 驱逐与冲洗共用）；任一 chunk 失败或推进失败 = 聚合 false 上报不谎报（r2015 complete 戳同门；账不清 = 重报收敛）；**流式世界持久化形态 = 玩家/进度 blob（现行走）+ chunk_edits 全量**——已生成未编辑的地形块不存（dirtyQuery 负向面 r2027b 实证：9 驻留恰 3 行），读档按 seed 确定性重生成（W1b parity 是正确性根基）。④**读档合并（overlay 语义）= GameSession::loadStreamingWorld**：sparse 构造（W1）+ 出生半径预生成之后的**行全量回灌**（restoreChunkFromBlob——跳 population W3 语义）+ **D3 核心区 blob 物化经现有只读 Q_INVOKABLE 面**（hasChunks + loadChunks——worldstore 零改动纯消费；核心域铺槽[仲裁胜者除外→槽缺席使 loadChunks 跳过该键] → loadChunks memcpy 进槽 → 逐槽取字节 → 已物化占位[出生预生成]经 ⑥⑦+擦槽让位 → restoreChunkFromBlob 全链物化；**全零槽 = 部分 blob[尺寸守卫跳过/缺行]降级 → 留 Absent 走重生成**，worldstore 尺寸守卫降级同门）；**仲裁 = 代次对代次**：行代次 ≥ base_gen → 行胜（新者胜；同代 = 表胜）/ < base_gen → blob 胜（老流式时代行让位再转换锚）/ 无 blob 在场 → 无对手方行全胜（原生流式 chunks 表恒空）；**代次刻度 = stream_worlds 双键**：base_gen（建代次 = 标志置位时刻锚：max[save_coord complete_generation 只读消费——r2015 权威缺台账=0，既有 save_gen]）+ save_gen（保存代次，每冲洗推进 max(save,base)+1）；**限制如实登记**：fixed 生产保存（QML saveAll）不推进 save_coord（r2015 零生产接线）→ 首转换 base_gen 恒 0（行恒胜 = 时序真值：转换后事实压转换前 blob）；「翻回期 fixed 重存不可见」面 = 保存流统一（W5b+）收敛的登记项。⑤**D3 转换（后端）= 纯元数据零搬迁**（§29.5.3 选型 2 原文）：markStreamingWorld（D2 创建与 D3 转换同一落点：置位 + core dims + 代次锚，SQL 失败面 = 旧行原样）/ clearStreamingWorldFlag（仅清标志位，行保留 = 境外数据不可达但不清除）；标志翻回 = fixed 语义（fixed 读档链 beginLoad/loadChunks/finishLoad 结构性不识 chunk_edits——worldstore 冻结域；r2027c 逆翻后 fixed 读档逐位恒等 + chunks 表行数恒 9 实证流式时代零 blob 触碰）；**仲裁双相位直证**：抬锚（base_gen=5 > 行代次 1）→ blob 胜（老行让位）；平锚（base_gen=1 == 行代次）→ 表胜（同代=表胜）。⑥**world 零 diff 登记**：world.{h,cpp} 本单零改动（restoreChunkFromBlob/ensureStreamingChunkSlot/releaseStreamingChunk/setChunkLifecycle 全既有面即可组装修读档链——W3 面即接线面的结构性兑现）。**QML 零改动 / worldstore 零触碰 / 线程原语零新增 / fixed 生成语义零触碰**。**腿（section30 置尾，filter 词 r2027；sparse 80×80×96 s42 radius 0 + fixed/sparse-core 48×48×96 s82 + 真临时 SQLite 库 fresh+用后即删+saves/ 零触碰[openWorld 绝对路径 = r2015 段先例]）**：r2027a fixed 零活动墙[fixed 保存/读档全语义逐位不动：元数据域 API 全 fail-safe false + 三写链 saveOkCount==3 + beginLoad/loadChunks/finishLoad 往返栅格逐位恒等 + raw 库面零流式表]；r2027b 流式存读往返承重墙[三区编辑（含构造期预生成中心 chunk = 弃槽路径承载选址）→ 冲洗恰 3 行/行代次=1/账清 → 销毁重建 → 编辑块表回灌逐位恒等 + 未编辑块走查重生成逐位恒等（含 population 树签名）→ 追加生成走 W2 驱动路径 → 二次编辑冲洗行代次=2 → 三读四区恒等（二次幂等）]；r2027c D3 转换往返[fixed 三编辑 → saveAll blob → 标志翻转 → 生产同构首进（blob 9 全物化逐位）→ 流式编辑冲洗 → 再读档（仲裁：编辑区 overlay 双编辑逐位在列 / 无行区 blob 逐位 / 境外驱动生成 ≡ 同参孪生含 population）→ 抬锚 blob 胜 / 平锚表胜 → 逆翻 fixed 读档逐位 + chunks 表零触碰]；r2027d 结构钉[worldstore/world 对新词元族禁触反探 + additive 正面钉（双表幂等建表 + 元数据 upsert + 零破坏 SQL）+ 冲洗失败不谎报钉（真锁 BEGIN EXCLUSIVE → false + 零部分冲洗 + 账原样 + 解锁后代次未动）+ 未登记世界 fail-safe + QML 零触碰反探 + 会话缝面正面钉（冲洗枚举/persist 单一执行体/仲裁胜者面/blob 只读消费面）+ 位序钉（冲洗体：落盘循环 < 代次推进收口；读档体：blob 物化 < 行回灌）]。**验证链全绿**：分段增量构建（-j 4）→ binary mtime（12:00）> 全部改动源 → filter r2027 首跑 0P/4F 迭代三处腿文坑（①placeTop 用 World::heightAt——生成器纯噪声面，海洋列与体素顶脱节 → 改 heightmapAt 体素真实顶 + 植被冠层整列换扫[r2009c 教训延伸]；②`ok && setup` 链短路使后续会话 setup 未执行 → 绑定面改无条件执行 + 独立断言；③r2027b 会话 2 未驱动走查就比对未编辑块——未编辑块按需生成是语义本位，读档只回灌行 → 补走查收敛后比对）→ 4P 全绿 + 三连跑绿 → **腿名跨任务词元去污染**（r2021 教训三次兑现：首版腿名/PASS 文本含 save/store/determin 词元 → save 45→49、store 57→61、determin 37→38 漂移；双向去词元后 45/57/37 精确复位）→ 前置回归全族 r2007=5P r2008=5P r2009=4P r2010=6P r2010b=3P r2011-r2019/r2021-r2026=4P + r2012=5P + t874/t875=2P + t1023c=1P + worldgen 24P/determin 37P/re-gen 6P/store 57P/save 45P + vertex 15P/t860 2P/t879 2P/t893 1P/t965 3P/t857 1P/t1023 3P 全 0 FAIL → 阴性两轮恰红声明面（NEG-1 摘保存时冲洗[flush 落盘循环 if(false) 前缀——代次推进拍保留]→恰红 {r2027b, r2027c}[b：flush 谎报 true 而行未写 → 销毁重建编辑块重derive 丢失；c：流式时代零行 → 仲裁无行可胜 → overlay 柱红；a fixed 无冲洗域可达/d 失败侧钉在锁面成立不误伤——恰红面 = 任务书候选 {r2027b} + c 同住冲洗通路同源红，如实双面声明]；NEG-2 摘代次仲裁改旧胜[仲裁行 `||`→`&&`——有 blob 对手方时行恒败]→恰红 {r2027c} 单腿=声明面[b 原生无 blob 分支不经被变异 rhs/a fixed 不可达/d 钉面不锚被变异字面]）→ 双 Edit 反向还原回绿（四日志 matrix_r2027_neg{1,2}_{red,restore}.log 直落 build/ 终名：neg1_red 2P/2F/neg1_restore 4P/0F/neg2_red 3P/1F/neg2_restore 4P/0F；变异字面 0 残留 grep 复核）→ 全矩阵 **648/0 ×2**（matrix_r2027_pos.log + matrix_r2027_final.log EXIT=0；644 权威 diff = +4 恰 r2027a-d + t813 戳/哈希登记类[08:43/f896c14→12:00/3414b0c]；pos/final 腿集合恒等 648[前 110 字符键 diff IDENTICAL]；^FAIL 归类 0；chunkstore removeDatabase 在用告警为 W3 既有登记噪声，本单冲洗/元数据读频次增高使计数 127→242，登记类）→ app 重建 EXIT=0 + offscreen 冒烟 EXIT=124 + logs/voxelsandbox_r2027_tail20.log 稳态 60fps[prof[1s] 60fr ×3 / frame main*16.0(60)] + F3 win 行 `mesh 0.00(0reb [0d 0s 0w])  worker 0  world 0.00` 与 r2026 基线逐字同值（fixed 世界零激活实证）+ 启动日志 500 chunk Models from 100 resident。**登记非目标**：UI（W5b——新世界「无限世界」开关 + D3 世界菜单「转为无限世界」+ 保存链三写前置冲洗接线 = §29.5 登记的 QML 例外面）；删除流式世界的清表策略（登记）；despawn 半径接线（既有登记）；性能调参（P5——hasChunk 逐查询连接经济学/冲洗批量面同 P5）；保存流统一经 SaveCoordinator（W5b+——base_gen 的「fixed 重存不可见」限制随统一收敛）；读档部分 blob 降级的逐槽粒度优化（当前全零槽整块降级）。**待实机确认**：无新增（本单后端零 UI 入口 → 生产零激活由冒烟实证；W5b 接线后复验项：无限世界走查观感 + 走离回走内容与编辑仍在[存档往返真机面] + D3 转换菜单动作观感）。**下一任务 = §29.5-W5b UI**（新世界 UI「无限世界」开关[默认关；照 P4 钉纪律守护的最小 QML 新增面] + D3 世界菜单 opt-in 转换接线 + 保存链冲洗接线；UI 文案与默认值是产品面——用户可 md 改）→ P5 实机调参验收。

### §29.5-W5b UI（2026-09-17，流式界面接线——新世界「无限世界」开关[默认关] + 世界菜单 opt-in「转为无限世界」+ 保存链前置冲洗：§29.5 登记的 QML 例外单，治理审计 #12 放行，流式弧收官单）

→ **✅ 落地（2026-09-17）：fix(r2028) 91daf84 + test(r2028) 9f4c0ca + 本 docs 三段提交，矩阵 648→651。设计选型（头注释立此存照，refactor-plan §29.5.2 W5 UI 半 + §29.5.3 选型 2 为契约；P4 钉纪律先例守护——变更面集中/禁字面反探/real-chain 全绿；worldstore.{h,cpp}/两段之外 QML/fixed 语义/线程原语/git 历史零触碰）**：①**桥形态 = Game 层 StreamingBridge**（src/Game/streamingbridge.{h,cpp}；QML 单例 BuildInfo/FrameProfiler 同款 QML_SINGLETON+instance——两段 QML **零元素实例化**即按类型名调用 = 变更面集中的结构性前提；**QML 面仅五 Q_INVOKABLE**：flagNewWorldStreaming[创建链标志落库]/saveIsStreaming[标志读面，转换按钮显隐 + 分流内用]/convertSaveToStreaming[D3 opt-in 转换]/enterWorld[进入分流]/flushForSave[保存链前置冲洗]；**值组件零暴露**：桥头零 Q_PROPERTY、ChunkStore/StreamWorldMeta/会话类型不出 C++ 面（r2022d「新面禁 Q_INVOKABLE 形态」同门延伸——本桥是 W5 会话在 QML 面的唯一投影，零复制游戏/存档逻辑[R20.07 编排壳纪律]）；**有状态面只有会话所有权**：会话生命周期 = enterWorld(true) 起至下次 enterWorld/detachWorld，退出世界不拆[世界列表态无泵拍源 worker 空闲挂起；跨世界切换先拆后建，线程 join 有界]）；②**变更面（逐 hunk 清单，全部登记）**：WorldList.qml 四 hunk = (a) :19 convertingFile 属性[锚定文件名而非布尔——切选世界即隐确认行免重置钩子] + (b) :270 勾选行[自绘 20×20 复选，「无限世界」标签，**默认关**=D2] + (c) :318 创建链传递[勾选 → flagNewWorldStreaming(file, store.world.width, store.world.depth)——core dims 与宿主世界栅格同口径] + (d) :428 转换动作+确认行[「转为无限世界」按钮（未转换 fixed 世界显/已转换隐藏）+ 三点确认文案 + 确认/取消]；Main.qml 两 hunk = (e) :765 enterWorld 分流[问桥一次：fixed 恒 false → 既有 beginLoad/regenerate 分流逐字节原样；true = 跳过分流（sparse 七入口守卫本就早退——冗余保险）] + (f) :1066 runExitSave 首行冲洗[非流式恒 true 零动作；失败短路三写不谎报]；**「存档按钮本体/输入/交互/HUD/其余 15k 行 QML」零字节触碰（r2028c 桥调用精确计数钉：WorldList x3/Main x2）**；③**进入分流语义**：fixed 存档恒 false[桥内顺带清上一局流式残留：会话终结 + World::reinitializeAsFixed 空网格归位——既有链照走]；流式存档 true[五拍主线程同步完成：sparse 重构 → 会话构造 → 绑定 → overlay 读档 → 挂钩，返回即「进入即通电」事实]；④**最小 C++ 新增面**：World::reinitializeAsSparse[sparse ctor 体单一权威委托——构造与运行期共用 sparseGenerate，禁第二份初始化逻辑；静默不 emit，QML 池重建走驻留 revision 沿]/World::reinitializeAsFixed[模式位 + 稠密空网格最小归位；**调用序契约 = caller 随后必接 beginLoad/regenerate 全量卫生重置**——侧表族卫生由既有链承担刻意不做]/ChunkManager::reinitializeFixed[recreate 体复用对偶面]/GameSession::pumpStreamingFrame[**生产只泵 tick 尾流式收割拍**——World 模拟家族仍由 QML tick 桥独占驱动，「玩法路径零改动」R20 不变量的会话侧承重面；无头全语义入口 stepTick 不变]；均 C++ only 非 Q_INVOKABLE（模式决策零 QML）；⑤**worldstore 零触碰**：saves 目录三级解析在桥内逐行镜像（登记的镜像面）+ 绝对路径直通（矩阵临时库先例）——两处一致性由 r2028b 真链行为锚定；chunkstore.cpp 自本单入 app 目标（会话首度生产实例化，W3-W5 期间仅矩阵目标直编）；⑥**文案（§29.5 登记：用户可 md 纠偏）**：开关名「无限世界」；动作名「转为无限世界」；确认弹窗 = 「转为无限世界？/核心区原样保留；境外按需生成、编辑自动保存；已卸载区的实体不保留。/（可翻回固定世界，界面暂不提供入口）」（三点 = 核心区保留 / 境外按需生成+编辑自动保存 / 卸载区实体不保留[W3 局限如实告知] + 可翻回说明[翻回不设 UI 登记]）。**腿（section31 置尾，filter 词 r2028；真临时库 fresh+用后即删 saves/ 零触碰 + 真线程收敛 deadline 轮询）**：r2028a 默认关零变化墙[真链进入分流恒 false + 会话零构造 + 标志读面 false + raw 库面零流式表 + 冲洗放行恒 true + 三写链 saveOkCount==3 + fixed 往返栅格逐位 + 二次分流复归墙 + 引擎零警告]；r2028b 开关+转换通电面[real-chain 单腿双柱：真 QQmlEngine × 真桥单例（C++/QML 同对象）× 真 World/WorldStore/WorldClock/PlayerController——QML→C++ 类型化指针转换链与生产同构；[toggle 柱] 勾选 → 标志行在（core 48×48/baseGen=saveGen=0）→ 进入即 sparse（W1 默认半径 2 = 25 预生成驻留）+ 会话通电 + 真玩家走查（loadSavedState 落 (4,4) + pc.tick() 起始沿 → 桥挂钩喂入）→ 生产泵拍收敛 (4,4) Loaded + revision 沿移动；[convert 柱] fixed 两编辑 → saveAll blob → 转换（标志翻转/core dims/代次锚 0/chunk_edits 零行 = 零搬迁）→ 首进 blob 核心区 9 全物化逐位 → 流式编辑冲洗恰 1 行/行代次 1 → 再进行回灌仲裁胜利（流式编辑 + fixed 时代编辑双在 + (1,1) 全量逐位 = 编辑后快照）]；r2028c QML 面钉[变更面集中：桥调用精确计数 + 五 Q_INVOKABLE 逐一正面钉 + 桥头零 Q_PROPERTY 反探 + world/会话/网格最小新增面正面钉（两法/ctor 委托/reinitializeFixed/pumpStreamingFrame）+ 玩法标记在位（enterWorld/onTicked/startGame）+ r2007b 站立钉复述 + W5 冻结域词元双 QML 禁触反探 + 值组件零暴露]。**验证链全绿**：分段增量构建（-j 4；矩阵 + app 双 binary mtime > 全部改动源）→ filter r2028 首跑 2P/1F 即修一处腿文坑（wrapper JS 函数缺 return——落库本身已成功[row 断言全过]，断言面误读 undefined）→ 3P 三连跑绿 → 前置回归全族（终版 binary）r2007=5P r2008=5P r2009=4P r2010=6P r2010b=3P r2011-r2019/r2021-r2027=4P + r2012=5P + t874/t875=2P + t1023c=1P + worldgen 24P/determin 37P/re-gen 6P/store 57P/save 45P + vertex 15P/t860 2P/t879 2P/t893 1P/t965 3P/t857 1P/t1023 3P 全 0 FAIL 计数与 r2027 基线逐项吻合 → **腿名跨任务词元去污染**（腿名首轮即避 save/store 词元[create-enter-save-load→persist-load、restored→reloaded、saved→persisted]；PASS 回显文本与腿名漂移[旧词元残留]NEG-1 轮发现即修——回显与 filter 名恒等纳入自检）→ 阴性两轮恰红声明面（**NEG-1 摘开关→创建链标志传递**[flagNewWorldStreaming 体 if(false) 包裹落库——谎报已传]→恰红 {r2028b} 单腿[toggle 柱：flag=1 谎报而 row=0 → enter 走 fixed → sparse/通电/走查同源红；a 不误伤 fixed 墙不走标志传递/c 钉声明面不锚函数体——变异不可达性设计]；**NEG-2 摘转换动作→API 接线**[convertSaveToStreaming 体 if(false)]→恰红 {r2028b} 单腿[convert 柱：conv=1 谎报而 row=0；a/c 不误伤同门]；恰红面先于腿文：NEG 落点选桥函数体——行为级红面单腿精确[摘 QML 行只会红源钉]）→ 双 Edit 反向还原回绿（四日志 matrix_r2028_neg{1,2}_{red,restore}.log 直落 build/ 终名 2P/1F+3P/0F ×2；变异字面 0 残留）→ 全矩阵 **651/0 ×2**（matrix_r2028_pos.log + matrix_r2028_final.log EXIT=0；648 权威 diff = +3 恰 r2028a-c + t813 戳/哈希登记类[12:00/3414b0c→13:59/b04aa06]；pos/final 腿集合恒等 651[前 110 字符键 diff IDENTICAL]；^FAIL 归类 0）→ app 重建 EXIT=0 + offscreen 冒烟 EXIT=124 + logs/voxelsandbox_r2028_tail20.log 稳态 60fps[prof[1s] 60fr ×多窗 / frame main*16.0(60-62)] + F3 win 行 worker 0 与 r2027 基线同值 + 启动日志 500 chunk Models from 100 resident（fixed 默认路径零激活实证）。**登记非目标**：翻回 UI（后端 clearStreamingWorldFlag 已备，界面不设入口）；流式参数调试面板（P5——configureStreamingRadii 即调参缝，桥不暴露）；文案润色迭代（用户 md 通道）；世界删除时的流式表清理策略（既有登记）；保存流统一经 SaveCoordinator（既有登记）；worldChunksPerSide 运行期变更（当前常量 10，登记）。**登记边缘（如实）**：标志读败（库锁/病）→ 诚实降级走 fixed 链 + qWarning 留痕；新流式世界跨世界 spawn pristine 边缘与 fixed 同 seed 连续进入既有行为同面。**待实机确认**：本单新增五项（见下节汇总）。**下一任务 = P5 实机调参验收（用户设备会话，无代码排队单）**。

### t1055（2026-09-17，残余清偿合集一——agent-review-2026-09-16 #8/#12/Info「worker 默认」三修 + Review_2026-09-15 #7 weather 0 值诊断：四处潜伏债一次清偿，filter 词 r2029；矩阵 651→655）

→ **✅ 落地（2026-09-17）：fix(t1055) 856c810 + test(t1055) d0f6639 + 本 docs 三段提交，矩阵 651→655。设计选型（四修各自头注释立此存照；worldstore.{h,cpp}/src/ui/线程原语/QML/git 历史零触碰；初代实现 agent 于全矩阵绿后、阴性轮前遭遇限额接交，阴性轮/终局/烟测/关单由接交 agent 按「Edit 反向还原」纪律完成——现场盘点即清单）**：①**fix A（#8 worker/scheduler 析构序无守卫）= GenerationScheduler::detachWorker() 守卫缝**：m_inFlight 按「已放弃」销账（inFlightJobCount() 归零——不残留永无 outcome 的僵尸账）+ m_worker 摘指针（后续 pump 走空门首行返回 = 恒安全 no-op——悬垂 UB 面结构性摘除）；幂等；**刻意不动 pending 域**（未交接 job 从未经 worker 之手，重挂新 worker 可继续服务）；析构序契约「scheduler 先于 worker 析构**或**显式 detachWorker」写进 setWorker/成员注释（生产路径 = GameSession 流式会话成员声明序结构性满足，本缝服务非成员序消费方）；BackgroundGenerationWorker 析构（队内丢弃 + 至多一个在途跑完后 join）不变——本缝纯主线程账面操作，线程原语零触碰（t1023c 白名单纪律）。②**fix B（#12 lastDirtyChunks 契约与实现不符，取建议①值拷贝）**：lastDirtyChunks() 返回 DirtyChunkSet **值拷贝** = 真快照（lastDelta() 同门）——旧形态活引用自称「同帧快照」而随账面活变，r2017 窗口语义下跨帧持有读到「已清空/下一窗进行中」集合；值拷贝后任意时刻可安全持有（栈内读 = 本 tick 快照，r2009b/c 断言域不变；栈外持有 = 上一窗终态）；DirtyChunkSet 平凡可拷贝固定容量（editbuffer.h 编译期钉，64 键 ≈1KB）；生产零消费者（grep 复核）= 纯加性安全。③**fix C（Info「GenerationWorker 基类默认实现自相矛盾」）= submitAsync 基类默认 ok() → fail(kErrAsyncNotImplemented=204)**（2xx 执行域 201/202/203 同域递增下一位；**选型偏差如实登记**：审查建议借用 kErrWorkerStopped——「协议缺失」与「worker 已停」是两语义，取新码不挪用旧码）；pumpAsync 交接相失败面两分：kErrQueueFull 背压重试语义原样（r2024c 收敛面零扰）/ 其他码 = 永久性交接失败 → **失败 outcome 可见穿透**（每活别名一条、Error 原样穿透 = 事件丢弃可见性同门）+ job 就地销账（不滞留不重试风暴；**边①未取**、chunk 恒 Absent 零转移——与「失败实投取边⑨」不同域）——漏覆写的异步 worker 从「静默永久滞留 m_inFlight（三账同谎）」变可见失败；同步 worker（execute 纯虚面不经异步协议）与覆写正确替身（ScriptedAsyncWorker 族/BackgroundGenerationWorker）零影响。④**fix D（Review_2026-09-15 #7 weather_timer_ms 恰 0 边界）**：setWeatherRemainingSec 对 sec≤0 拒面从静默升级为**恰一条 qInfo**（区分「精确续跑」= sec>0 受理零噪声，与「0 值回落随机重抽」= 拒 + 诊断——审查建议原文照落）；行为零变化（计时照旧不动，t1046c 非法拒柱回归绿），纯日志可见性（事件丢弃可见性同门）。**顺带核（#5①「SaveRequest 一次性构建+停 tick」现状核）**：W5b 接线后 runExitSave = flushForSave → savePlayerData → saveAll → saveProgress 全同步 C++ 调用（调用返回即写已完成、SQLite commit 同步落盘、链上无 deferred/分帧写），QML 单线程无事件循环重入 → 保存链内 tick 结构性不可插入——「一次性构建 + 停 tick」契约由**同步链序**事实成立，无需停 tick 机制，登记为该低危项的现状核报告（维持「recover() 开库失败静默 Fresh」另一半登记不变）。**腿（section32 置尾，filter 词 r2029；r2029a/c 零世界 = 裸 scheduler + 零线程脚本化替身 worker 族[r2010bb/r2017b 先例]，r2029b fresh 48×48×96 s82 ×1，r2029d 最小 32×32×64 s2029 ×1——rig 世界零接触）**：r2029a detachWorker 五柱[①正常交接柱（双 job 入在途账/pending 清空/零 outcome——hand-out 记账先于 detach 自证完好）②销账+摘指针③detach 后 pump 恒安全 no-op（死缝零咨询：不 take 不交接、账面稳定）④detach 后提交保留 pending（worker 零触）⑤重挂新 worker 复服（幸存 job 走正常交接+收割收成功 outcome——销账后账本仍一致可用）]；r2029b lastDirtyChunks 值快照语义[信号栈内取拷贝 → stepTick 返回（收口已清账）后活账空而持拷贝满员（值语义本体）→ 窗 2 第三 chunk 新快照只携带本窗编辑、旧拷贝逐位不动（含插入序）]；r2029c fail-fast 默认三柱[漏覆写替身：204 失败 outcome 可见穿透 + 就地销账（零 pending/零 in-flight/chunk 恒 Absent/边①未取）+ 重泵零重试风暴；覆写正确替身照常交接+收割；同步 execute-only worker 零影响 + kErrAsyncNotImplemented==204 static_assert 编译期钉]；r2029d weather 0 值诊断三相[精确续跑（sec>0）静默受理落值精确 / 0 与负值各恰一条含「zero-value fallback」诊断且计时逐位不动 / 重抽新窗后 0 值写仍保持新窗（回落语义不变）——qInfo 捕获 = 文件尾 static 消息槽 + qInstallMessageHandler 窗口（用后即还原）]。**验证链全绿**：分段增量构建（-j 4）→ filter r2029 首跑 4P/0F（matrix_r2029_first.log）→ 全矩阵 prelim 654P/1F 即修一处**记号面坑**（fix C 头注释初稿直引 202 号常量名——内嵌 D6 执行器记号被 r2020d ④ 全树白名单扫略红[wiring elsewhere=World/generationjob.h]；改语义描述不引字面，注释内「记号面如实登记」留痕[t1023c/r2020d 扫略纪律]）→ 全矩阵 prelim2 **655/0**（matrix_r2029_prelim2.log）→ 阴性两轮恰红声明面（**NEG-1 摘 detachWorker 销账**[m_inFlight.clear() 行 if(false) 前缀]→恰红 {r2029a} 单腿=声明面[diag 四柱同源 fl=2 僵尸账：detach fl=2 / noop fl=2 / pend fl=2 / re fl=2——被摘语义本体即销账面；b/d 不经 scheduler、c 不调 detachWorker 不误伤]；**NEG-2 摘 fail-fast 默认**[submitAsync 默认体回 ok()]→恰红 {r2029c} 单腿=声明面[diag leak out=0/code=0/fl=1/life=1 = 静默滞留本体复现（零 outcome + in-flight 卡 1 + 边①照取推 Loading）+ storm fl=1；a 的替身族覆写正确 / b/d 不走默认体不误伤]）→ 双 Edit 反向还原回绿（四日志 matrix_r2029_neg{1,2}_{red,restore}.log 直落 build/ 终名[neg1 3P/1F EXIT=1 → 4P/0F EXIT=0；neg2 3P/1F EXIT=1 → 4P/0F EXIT=0]；变异字面 0 残留——全 diff grep 复核 if(false)/mutation 标记/ok() 默认体三面全零，含初代 WIP 一并扫描）→ 全矩阵 **655/0 ×2**（matrix_r2029_pos.log + matrix_r2029_final.log EXIT=0；651 权威[r2028_final] diff = **+4 恰 r2029a-d** + t813 戳/哈希登记行[13:59/b04aa06→17:33/5633a7c]；651-only 除 t813 登记行外零；pos/final 腿集合恒等 655[前 110 字符键 diff 空]；^FAIL 归类 0；腿名/PASS 文本跨任务词元去污染自检通过——无 save/store/r20xx 旧词元）→ app voxelsandbox 重建 EXIT=0（binary mtime > 全部改动源）+ offscreen 冒烟 EXIT=124 + logs/voxelsandbox_r2029_tail20.log 稳态 60fps[prof[1s] 60fr ×2 窗 / frame main*16.0(60)] + F3 win 行 `mesh 0.00(0reb [0d 0s 0w]) worker 0 world 0.00` 与 r2028 基线同值 + 启动日志 500 chunk Models from 100 resident（fixed 默认路径零激活实证）。**登记非目标**：detachWorker 的自动 RAII 守卫形态（显式缝即契约面，调用方显式调）；submitAsync 失败的自动重试（永久性失败面销账即终局——重试归调用方新 submit）；204 码的跨子系统统一登记册（result.h 百位段约定既有口径续位即登记）；weather 诊断的消息类别升级（qWarning）与节流（概率面毫秒级窗口，qInfo 零噪声足额）。**待实机确认**：无新增（A/C 无头面[裸值组件生产零调用面 detachWorker + 默认体仅漏覆写替身可达]；B 行为面零变化[生产零消费者，值拷贝语义面]；D 纯日志可见性[app 侧仅在「存档瞬间剩余 <0.5ms」毫秒级窗口可见一条 qInfo，行为零变化]——四修无一改生产可观测行为，冒烟稳态 60fps + F3 基线同值实证）。**过程坑**：①记号面白名单扫略红（上记 prelim 1F——src 注释引白名单管辖词元字面即红，t1055 后续 src 注释一律语义描述不引字面）；②NEG-1 变异形态 = m_inFlight.clear() 行整行 if(false) 前缀（销账是语句非守卫——t1051 教训对齐：摘调用本体不摘邻行 m_worker=nullptr）；③接交现场纪律：初代 fix 内容零返工（读 diff 全核：A/B/C/D 与头注释/腿注释一致），阴性轮/终局/烟测由接交 agent 补齐。**下一任务 = t1056（agent-review-2026-09-16 #9 软影列顶快照域差 1 列：kTopDim 21→22 + 静态断言补钉 PCF 半格触达 kMaxShadow+1 ≤ 域半宽 + 列顶域边界源码钉；顺带 #6 biome memo 首循环回填——generate() 首循环拿 TerrainColumnSummary.biome 回填 m_biomeCache 消 fBm 双算[fa5c3aa 引入的 ~一成级一次性成本]，两修同窗同族[渲染域+worldgen 域各一]、互不纠缠）→ P5 实机调参验收。

### t1056（2026-09-17，残余清偿合集二——agent-review-2026-09-16 #9 软影列顶快照域差 1 列 + #6 biome memo 首循环回填：两修同窗互不纠缠，filter 词 r2030；矩阵 655→657）

→ **✅ 落地（2026-09-17）：fix(t1056) 05c7182 + test(t1056) f13d703 + 本 docs 三段提交，矩阵 655→657。设计选型（两修各自头注释立此存照；src/ui/、worldstore.{h,cpp}、线程原语、QML、生成结果语义[#6 零变化墙]、git 历史零触碰）**：①**修一（#9）= 列顶快照域 kTopDim 21→22（域 [origin-2, origin+19]）**——推导补全：顶点 x/z ∈ [origin, origin+kChunk]（右/下边界面顶点恰在 origin+16），太阳水平单位向步进 ≤ kMaxShadow=2，PCF 每步采样 floor(落点) 与 floor(落点)+1 两列（半格列贡献 0.5）→ 前向最深探测 = origin+16+2+1 = origin+19，旧 21 宽域 [origin-2, origin+18] 恰差此 1 列（columnTopAt 域外 -1 不遮挡而 World 真值路径照常遮挡 = chunk 边缘极端姿态影偏亮）；后向最小探测 = origin-2（floor 不下探，恰落 -kMaxShadow）→ 域两端恰紧。**静态断言选型（本单核心权衡，断言处留痕）**：meshbuilder.cpp 增两条**松耦合式**编译期断言（kMaxShadow+1 ≤ 域前向余量 kTopLo+kTopDim-kChunk；kTopLo ≤ -kMaxShadow）——守「kMaxShadow 增大须同步扩域」的耦合（kMaxShadow 增 2 即编译红）；**紧覆盖式**（last index ≥ kChunk+kMaxShadow+1）会令 NEG-1 恰红轮的 kTopDim 回退（21）直接变编译错误——阴性轮须红在测试面而非断编译，紧推导由 r2030a 以运行时常量承载；域外 -1 分支保留为纯防御（正常路径恒不触）。头注「列触达」算术重写（+1 半格写进算术）+ 采集成本注释如实化（21×21→22×22）。②**修二（#6，取建议①回填案）= generate 首循环以 TerrainColumnSummary.biome 回填 m_biomeCache**——memo 预置 0xFF 哨兵（biomeAt 未填语义原样）+ 首循环同遍逐列写（键序与懒填一致），值同源 = biomeComputeAt 结果 → 逐位一致 = 行为零变化；懒填路径保留为 beginLoad/读档世界兜底（beginLoad 清表语义原样）；**heightAt 维持现状**不回切 memo 化 biomeAt（选型头注立证：双算病灶 = 首循环计算不落 memo + 后续 pass 的 biomeAt 首查再各算一次，回填后已消除；heightAt 委托链是出生解析/结构选址等一次性生成期路径的既有形态，若未来成热路径另单评估）；新增**判别探针** biomeMemoFilledCount() / biomeMemoMissCount()（C++ only 非 Q_INVOKABLE 生产零调用——r2022d 同门；miss 计数 = biomeAt 懒算次数，generate 随 memo 重置归零，powerRecomputePasses 同族口径）。**腿（section32 置尾追加，filter 词 r2030；两腿各自 fresh 世界，rig 零接触）**：r2030a 软影列顶域[常量柱 kTopDim==22/kTopLo==-2 + 紧覆盖推导（运行时承载——编译期紧式会令 NEG-1 变编译错误，选型与 meshbuilder.cpp 断言处互证）+ 行为柱：世界边 (0,0) 与内陆 (1,1) 真实快照逐探测列 columnTopAt == World::columnTopSurfaceY 且世界内列恒 ≥0（极端列 ±19/-2 = 被测病灶本体）+ 域外防御柱恒 -1]；r2030b memo 回填[全列已填 filled==width×depth + **判别面 miss==0**（回填在位 → 后续 pass 的 biomeAt 全命中；见下 NEG-2 实证注记）+ biome/height 采样 vs TerrainGen 纯函数直连（零变化面）+ 命中稳定柱 + beginLoad 懒填兜底柱（清表后 0 → 随查 3 列 filled 恰 3 且 miss 恰 +3）]。**同变更钉面修订（纠偏留痕非削钉，t1023c 先例）**：r2013d 字面钉 kTopDim = 21→22；section23 rig 注释 21×21→22×22（rig 本体 kTopDim 符号引用零字面）。**验证链全绿**：分段增量构建（-j 4）→ binary mtime > 全部改动源 → filter r2030 首跑 2P/0F（matrix_r2030_first.log）→ 前置回归 40 族全 0 FAIL（r2007-r2029 全族 + t874/t1023c/t879/t893/t860/t1023/t1050/t1052/t1053/t1054 + worldgen 15P/determin 28P/re-gen 5P/store 50P/save 32P/vertex 15P/mesh 36P——计数为腿名同词素 substring 口径）→ 阴性两轮恰红声明面（**NEG-1 回退 kTopDim 22→21**→恰红 {r2030a, r2013d}[a diag dim=21 last=18 fwd=19 + 极端列 got=-1 want≈70..75；d 的修订钉 miss——受影响源钉腿]；**NEG-2 摘 memo 回填**[assign 回 clear + 摘循环内回填行]→恰红 {r2030b}[diag filled 2304/2304 **miss=2304** = 每列首查各付一次 fBm 的双算本体复现]）→ 双 Edit 反向还原回绿（四日志 matrix_r2030_neg{1,2}_{red,restore}.log 直落 build/ 终名[neg1_red 全矩阵 655P/2F EXIT=1；neg1_restore r2013 4P + r2030 2P 双 EXIT=0；neg2_red 全矩阵 656P/1F EXIT=1；neg2_restore r2030 2P + r2024 4P 双 EXIT=0]；变异字面 0 残留全 diff grep 复核）→ 全矩阵 **657/0 ×2**（matrix_r2030_pos.log + matrix_r2030_final.log EXIT=0；655 权威[r2029_final] diff = **+2 恰 r2030a/b** + t813 戳/哈希登记行[17:33/5633a7c→20:29/de07cc9] + t997 墙钟登记行；pos/final 腿集合恒等 657[前 110 字符键 diff 空]；^FAIL 归类 0）→ app voxelsandbox 重建 EXIT=0（binary mtime > 全部改动源）+ offscreen 冒烟 EXIT=124 + logs/voxelsandbox_r2030_tail20.log 稳态 60fps[prof[1s] 60fr ×多窗 / frame main*16.0(60/61)] + F3 win 行 `mesh 0.00(0reb [0d 0s 0w]) worker 0 world 0.00` 与 r2029 基线同值 + 启动日志 500 chunk Models from 100 resident（fixed 默认路径零激活实证）。**过程坑（恰红面实证两则，均留痕）**：①**「全列已填」无判别力**——NEG-2 首跑 r2030b 反而绿（656P/1F 红面在别处）：generate 后续 pass（placeTrees/placeTallGrass 等逐列群系判定）的 biomeAt 懒填同会把 memo 填满，「generate 后全填」对摘回填不敏感；重构腿加 biomeMemoMissCount 判别面（回填在位恒 0 / 摘回填飙升）后恰红收缩单腿——任务书预认的兜底断言面被实证推翻，探针计数即判别正解；②**NEG-2 首跑红面混入 r2024b**[diag cancel +1]——真线程走查腿的负载敏感 flake（同轮变异语义不可达 sparse 路径；NEG-1 全轮同腿绿、重构后 NEG-2 轮绿、还原轮 filter r2024 4P 绿三面互证），按 t1054 NEG 先例以复跑绿+语义不可达双证归类 flake 留痕。**登记非目标**：列顶域的紧覆盖编译期断言（松耦合 + 运行时紧推导的组合即选型，留痕在两处）；sparse 路径的 memo 回填（任务书落点 = fixed generate 首循环；sparse 物化链的懒填面登记不改）；biomeMemo 探针的 QML/F3 暴露（矩阵探针即终态）；heightAt 的 memo 化（选型留痕在定义处）；软影 PCF 参数调参（P5 实机域）。**待实机确认**：修一 = 纯视觉极端姿态列实机核（「软影边缘极端视角偏亮消除」——headless 只能钉「探测列 ∈ 域」推导面，观感面须真机目视 chunk 边缘 + 太阳低角度姿态）；修二 = 零变化性能面（无观感新增，冒烟稳态 60fps 实证）。**下一任务 = legacy residual sweep（review0913 #2 半径 2 运行期残余 / #3 t1033c 文案——老登记清偿波延续）→ P5 实机调参验收。

### t1057（2026-09-17，SaveCoordinator 生产接线——r2015 组件 marker-first/代次/恢复状态机/FaultHook 缝的生产通电：SaveBridge QML 例外单 + #5② 可区分错误态，audit #9 候选表「已建未用」大件收官，filter 词 r2031；矩阵 657→661）

→ **✅ 落地（2026-09-17）：fix(t1057) + test(t1057) + 本 docs 三段提交，矩阵 657→661。设计依据 = refactor-plan §29.6（主控执笔设计段）；设计选型（savebridge.h/savecoordinator.h 头注释立此存照；worldstore.{h,cpp} 冻结域一行禁触、线程原语零触碰、src/ui/ 仅两处登记例外、git 历史零触碰、数据硬门 = save_coord 纯 additive 零格式迁移[旧档无表=Fresh、旧码盲读——r2015 ④语义生产化]）**：①**SaveBridge（src/Game/savebridge.{h,cpp} 新件）= QML 单例**（StreamingBridge/BuildInfo 同款 QML_SINGLETON + instance() 模式；两个 Q_INVOKABLE = `saveViaCoordinator(store, file, name, chests, furnaces, dispensers, worldTime, bedSpawn, playerData, progress)` 一口进 + `recoveryState(file)` 四态字符串读面）。**签名选型 = 分参而非打包 QVariantMap**（立证：打包 map 键是字符串——拼错键静默变空 map，而「playerData 空 = 该部分不请求」是 coordinator 协议语义，拼错 = 静默丢该部分写不可接受；分参由 QML 引擎做形参数量/类型检查错传响亮，且旧 saveAll 本就是六分参形态同族）；store/file 逐调用传入（enterWorld 同款对象传递先例——桥自身无状态零悬垂面）；db 路径 = savesDir 三级解析镜像（worldstore 一行禁触的登记镜像面，StreamingBridge::savesDir 同款）。内部序 = coordinator 写 attempt 标记（代次 g+1）→ 三写照旧调 worldStore 现有 Q_INVOKABLE（逐字节原样）→ 全成才盖 complete 戳；任一败 = 标记留档（下次读档 Interrupted）+ 返回 false——与旧 runExitSave「三写全成才真」返回语义逐位同；saveOkCount 观测面语义不变（成功 = 三部分各 +1 = +3；失败不计数，t974 契约原样）。SaveFaultHook 生产零挂载（桥内钩子缺省恒空；C++ 面 setFaultHook 仅供矩阵腿，Main.qml/main.cpp 零挂载 = r2031d 反探）。②**#5② 修复（review0916 #5 指认面）= recover() 可区分错误态**：新枚举值 `SaveRecoveryState::OpenError`（选型 = 新枚举值而非 error 字段：SaveGenerationInfo 已是纯值读数结构，枚举序稳定追加对既有 Fresh/Clean/Interrupted 三态零扰动；error 字段把「读数」与「读数时的故障」混一结构且 message 指针对值纪律多余）。**关键实现发现（如实登记）**：SQLite open 是惰性的——t974 式 BEGIN EXCLUSIVE 锁占下 `db.open()` 通常仍成功、炸在**读面**（SELECT busy）；故 ② 的两面目 = open 失败 + SELECT busy 同归 OpenError，与「save_coord 表真缺席 = 旧档合法 Fresh」用 **sqlite_master 只读探针**辨析（探针成功且无行 → NoTable→Fresh；探针也失败/有行但主 SELECT 失败 → Unreadable→OpenError 不谎报）。**saveAll 对 prior==OpenError 拒存**（kErrSaveCoordSql、零部分尝试）——禁在不可知台账上从 0 重编 newGen（「generation 从如 5 回写 1」的台账历史失真 = review0916 #5 指认病灶本体）；锁下可观测量与旧 coordUpsert 失败路径逐位同（r2015c 真锁柱零扰动实证）。③**Main.qml 两处例外（登记）**：(a) runExitSave 三写段置换为 `SaveBridge.saveViaCoordinator(...)` 一口进（flushForSave 前置不动 = W5b 序保留，流式 blob 语义零变化；返回值门/caller 完成门/saveOkCount 观测面逐位不变；参数 = 旧三写所需全集九参透传）；(b) enterWorld openWorld 后 `SaveBridge.recoveryState(file)` → Interrupted → toast「上次保存未完成，已载入最后完整数据；建议立即重新保存」+ qInfo（文案登记 md 可纠偏）；Clean/Fresh 静默；open-error qWarning 诚实降级不弹窗（台账读不了 ≠ 载入数据不可用）。其余 15.5k 行零 diff。④**冻结机制不接线选型（§29.6 转抄）**：r2015 ①冻结-持久化分离的 freeze 缓冲在生产冗余——保存链同步单线程无重入（t1055 核），世界不可能在写中途变化；生产只取其簿记面（marker/代次/Interrupted/complete 权威）。freeze 面随 coordinator 协议原样保留（字节面恒等无行为差），不做生产开关不另建第二份冻结实现；freeze 面独立行为证明留矩阵测试域（r2015b）。成本登记：coordinator 首保存对宿主 dims 惰性重建冻结缓冲（new World + dims setter ≈ 4 次一次性 worldgen 成本，跨保存复用；§29.6 原文登记的后续单优化面）。⑤**CMake**：savecoordinator.{cpp,h} + savebridge.{cpp,h} 自本单入 voxelsandbox 目标（review0916 #5 指认的「生产目标根本不编译它」随本单清偿）；矩阵目标 + section33 + savebridge。**腿（section33 置尾，filter 词 r2031；真 QQmlEngine real-chain × 真桥单例[r2028b 同门] + fresh 临时库 fresh+即删+绝触 saves/，rig 世界零接触）**：r2031a Clean 路径零变化墙[真链三写段置换：返回 true + saveOkCount 恰 +3 + 九参逐位透传读回[meta name/player 位姿血饥 xp/progress/worldTime 四键/bed 锚/chests 容器表] + 重读档栅格逐位含 marker + 引擎零警告 + 旧档 Fresh 读档回归[无台账旧库与裸 store 保存均 fresh + 旧读路径照旧]——**墙零台账断言**（恰红面设计前置：戳权威归 r2031b 专钉，旧三写语义本就不含台账）]；r2031b marker→complete 往返 + 中断恢复[两键 raw 直读 1/1→2/2 代次单调 + Player 段经桥钩注入 → false + 计数只动已写部分 + Interrupted 3>2 + 旧 player 数据重读照旧 + Finalize 段注入 = 三部分全写（+3）仍不得报成功（戳 = 成功唯一权威）+ 清钩收敛 Clean 5/5（中断号消耗不重置不复用）]；r2031c ②可区分[真锁占 BEGIN EXCLUSIVE → open-error 非 fresh（字符串+枚举双面）+ 锁下保存拒存零部分尝试计数零动 + 锁释放后台账历史不失真 clean@1/1 + open 级失败（库路径为目录）同归 open-error——两面目可区分且都不谎报]；r2031d 结构钉[worldstore 零触碰反探（SaveBridge/save_coord 禁入 + kSchemaVersion=2/saveOkCount/saveAll 面原样）+ additive 正面钉（save_coord 幂等建表/独立命名连接/sqlite_master 探针/OpenError 守卫）+ 桥面钉（QML 单例 + 两 Q_INVOKABLE + 钩子透传唯一缝 + 零 Q_PROPERTY）+ QML 两处例外面钉（saveViaCoordinator 恰 1/recoveryState 恰 1/StreamingBridge 恰 2 复述/GameSession·SaveCoordinator·SaveRequest·setFaultHook 禁入/toast 文案在位）+ 生产零挂载反探（main.cpp 零桥记号）]。**验证链全绿**：分段增量构建（-j 4）→ 双 binary mtime > 全部改动源 → filter r2031 首跑 1P/3F 即修三处腿文坑（①legacy 库期望 chunk 数错——裸 saveAll 保存了绑定世界 9 块非 0；②QML wrapper 函数名 recState() 与同名 property 冲突致编译异常 → 改名 recStateOf；③r2031d 中文 contains 用 QLatin1String 编码失配 → fromUtf8[t974 的 QStringLiteral 先例是编译期 UTF-16 转换，QLatin1String 是按 Latin-1 重解 UTF-8 字节必失配——记号面新坑]）→ 4P 三连跑绿（run3/4/5）→ 前置回归 33 族全 0 FAIL（r2007-r2030 全族 + t874/t1023c/t974 + worldgen 15P/determin 28P/re-gen 5P/store 52P/save 36P——substring 口径）即修一处**既有钉随字面重排**（t1016 qml-exit-chain-5th 钉锚旧 runExitSave 单行 worldTime 字面——置换时被拆行断钉 → Main.qml 侧还原单行字面 = 钉免修订原样成立[t1016 是 t974 前老钉，置换面零削钉]）→ 阴性两轮恰红声明面（**NEG-1 摘 complete 戳**[收尾戳条件 false && 前缀 = 写完不盖且 receipt 仍报成功]→声明红面 = r2031 族内 {r2031b, r2031c} + **跨族同源 {r2015a, r2015b, r2015c, r2019b}**（complete 戳是共享语义：r2015a/b/c 与 r2019b 的「成功保存后台账 Clean/complete_generation」断言同锚被摘语义本体——按 t1053/r2021 改声明先例如实预声明非连带伤；r2031a 不红 = 墙零台账断言兑现；r2015d/r2031d 不红 = 文本钉非行为钉；实测与声明逐腿吻合[neg1_red log：r2031 2F + r2015 3F/r2015d 1P + r2019 恰 b 1F]）；**NEG-2 摘 ②错误态**[recover() OpenError 派生改回 Fresh]→恰红 {r2031c} 单腿（r2031a legacy 走 NoTable→Fresh 合法路径不误伤；r2015 真锁柱锚 error 码+计数零动——新守卫跳过落回 coordUpsert 失败旧路可观测量逐位同，实测 0 FAIL））→ 双 Edit 反向还原回绿（四日志 matrix_r2031_neg{1,2}_{red,restore}.log 直落 build/ 终名[neg1_red 三 filter 全红 EXIT=1 ×3 恰声明面；neg1_restore 三 filter 全绿 EXIT=0 ×3；neg2_red r2031 1F EXIT=1；neg2_restore 三 filter 全绿 EXIT=0 ×3]；变异字面 0 残留全 src grep 复核）→ 全矩阵 **661/0 ×2**（matrix_r2031_pos.log + matrix_r2031_final.log EXIT=0；657 权威[r2030_final] diff = **+4 恰 r2031a-d** + 登记类[t813 戳/哈希 20:29/de07cc9→23:03/fda962a、t997 墙钟、t1023c src 文件计数 153→155 = savebridge 两件同款 +N 先例]；pos/final 腿集合恒等 661[前 110 字符键 diff 空；全文本唯一差 = t997 墙钟数字登记类]；^FAIL 归类 0）→ app voxelsandbox 重建 EXIT=0（binary mtime > 全部改动源；NEG 轮动过 savecoordinator.cpp 的 mtime，还原后强制重建对齐）+ offscreen 冒烟 EXIT=124 + logs/voxelsandbox_r2031_tail20.log 稳态 60fps[prof[1s] 60fr ×2 窗 / frame main*16.0(60)] + `root objects after load: 1`（Main.qml 加载成功 = SaveBridge 单例注册/解析成立）+ 启动日志 500 chunk Models from 100 resident（fixed 默认路径零变化实证）。**登记非目标**：Interrupted 的自动修复/自动重存（仅提示 + 建议重存——恢复动作 = 调用方重存收敛，r2015 语义原样）；冻结缓冲重建成本优化（§29.6 登记后续单）；open-error 的 UI 弹窗（qWarning 诚实降级——台账读不了 ≠ 数据不可用）；桥的多实例/多线程（单实例假设 = kCoordConn 连接名全局唯一 + 每次 add/removeDatabase，r2015 已登记）；SaveBridge 的 Q_PROPERTY 值暴露面（两 Q_INVOKABLE 即终态）。**待实机确认（必列）**：①真机保存→退出→读档→改动完整（桥路径的完整退出存档往返观感——headless 已钉返回语义/计数/往返逐位，真机观感面待核）；②杀进程重进 → Interrupted toast 出现 + 重存后收敛（下次进入不再提示）；③旧档（无 save_coord 表）首次经桥保存 → save_coord 表自动补建无感 + 二次进入无提示。**下一任务 = residual sweep 3（t1033c 文案 + review0913 #2 半径 2 运行期残余 misc 收尾）或 D6 多 worker 扩并发（按主控队列派单）**。

### t1058（2026-09-17/18，嵌入 mob 水平脱出——agent-review-2026-09-16 #10 + t1033c 文案对账 review0913 #3，filter 词 r2032；矩阵 661→664）

→ **✅ 落地（2026-09-18）：fix(t1058) + test(t1058) + 本 docs 三段提交。设计选型（entitymanager.cpp 头注释立证；src/ui/、worldstore.{h,cpp}、线程原语、玩家侧 extrudeEmbedded/launchUnburyUpward 禁复用[同型机制参考]、t973 活埋门语义、git 历史零触碰）**：①**修一（#10）= 嵌入态水平脱出**（`mobEmbeddedHorizontalEscape` 匿名域帮手 + tick resting 复探嵌入顶起分支的**净空失败 else 面唯一调用点**）。**触发域刻意收窄 = 嵌入态 ∧ 顶起被净空门挡**——顶起优先（MC 向上 = 支撑在脚下的最小脱出向，t1053 口径零变化墙：净空足照旧顶起、正常站立/合规越障零波及），净空门挡下才轮到水平向；#10 病灶三连环的最后一环（豁免生效[整层不跳 isJumpObstacle t1053②] + 顶起被挡[本 else] + 水平 AI 移动被 mobAabbHitsSolid 撤回[嵌入格 top>miny 不享 review26 #1 脚位格豁免] → 冻结直到外部挖块）。**脱出机制选型**：±2 列邻窗扫描（有界脱出登记语义；自身列跳过 = 顶起分支已试过）× 支撑层探 [feetCell+1..feetCell-2]（与 resting 复探两格支撑/落地承接语义同族，容 1-2 格落位；首支撑即定层同柱不复试；更高层落位自然被净空门滤除——封顶场景高位必无净空）× 候选双判【落位后全高净空 mobAabbHitsSolid（与顶起同门）+ 支撑可站 mobSupportTopY ≥ 0】；列中心落位（±0.5 与 spawn/stuck-escape 同式——halfW < 0.5 = footprint 整入候选列天然离开嵌入列）**直接位移贴新支撑**（extrudeEmbedded 玩家先例同型确定性收口，非物理弹射）；候选序 = XZ 距离最近优先 + 同距 (tx,tz) 字典序（任务书钦定确定性序）；fallPeakY 基准随新位 + vy 清零（顶起先例同门——横移非腾空，防旧低位基准给下次落地注假落差触发 t1045 踩踏掷骰）；脱出后嵌格态下 tick resting 复探贴新支撑自然解除。**无候选列（真围死）→ 冻结维持**（登记非目标：MC 同款困死场景，外部挖块/头嵌窒息兜底承接）。**MC 引证**：minecraft.wiki/w/Entity General behavior 节 "free to move out of the solid block but not back in"（t1053 引证沿用 + 2026-09-17 真实 web 实读复核确认原文在位；向上 = t1053 已实现的最小脱出向，水平向 = 本单补全）。②**修二（review0913 #3）= t1033c 文案对账**（纯测试侧文案零行为）：t1033c 腿名 + PASS 回显 + 腿头注释三处水毁族清单随 t1012④ isAttachableBlock 现行集逐项对账更新（"torches, cobweb, ladder" → "torches, redstone torch, cobweb, ladder, rail family"——t1043 裁-1 后 rail 三族入清单；断言本体零触碰）。**腿（section02 置尾，filter 词 r2032；固定选址 + 工作盒先清空 = 地形零耦合画布；wander 冻结缝 + playerTargetable=false 双门 AI 全惰性[位置断言零 RNG 承受] + setTrampleRollOverride(0) 确定性缝）**：r2032a 正常路径零变化墙[嵌入+净空足 → 顶起照旧（finalFeet ≈ +0.9375）+ x 零漂移（横移分支结构性未触）+ 正常站立逐 tick 位置零漂移]；r2032b 水平脱出承重三相[(a) 嵌入+头顶封顶 → 恰脱出最近可站列 + **同距字典序确定性断言**（farmland 行全可站 → (bx0+2)/(bx0+4) 距离同 1.0 → 字典序取小 bx0+2）+ 无跳弧 + 耕地全存；(b) 最近列被堵（kRigY+1..+2 两格石柱 = 支撑探窗首层假支撑顶 + 落位体格实心）→ 净空门拒 → 次近 (bx0+4)；(c) 无候选（farmland/封顶只留嵌入柱）→ 冻结维持（登记语义腿）]；r2032c 文案对账钉[更新后文案在文案 TU 恰 2 处（pinSet 剥注释计——字符串字面保留；**坑：PASS 回显字符串字面把 needle 拆行 → pinSet 不拼接相邻字面 = count 1<2 首跑红 → 回显改写保 needle 单字面内连续**）+ 陈旧文案 0 处（原始文本）+ 活谓词现场对账（火把/红石火把/蛛网/木梯/铁轨三族 ∈ 族，床白/棕与满格石不在族）]。**验证链全绿**：分段增量构建（-j 4）→ binary mtime > 全部改动源 → filter r2032 首跑 2P/1F 即修一处腿文坑（上记 r2032c 回显拆行）→ 3P/0F EXIT=0 → 前置回归全族 0 FAIL（r2007-r2031 全族[5/5/4/6/3/4/5/4/4/4/4/4/4/4/4/4/4/4/4/4/4/4/3/4/2/4P] + t1053=1P/review26-1=7P/t1045=3P/t947=2P/t1033=4P/t1042=5P + worldgen 24P/determin 38P/re-gen 6P/store 59P/save 49P/vertex 15P/mesh 37P；r2012=5P 含 r2024d 既有同词素腿非本单污染；腿名/回显零跨任务词元实测[t1053/t1033 等 filter 计数逐一复位]）→ 阴性两轮恰红声明面（**NEG-1 摘水平脱出分支**[调用条件 false && 前缀]→恰红 {r2032b}[A/B 相冻结=旧行为，diag x 停 23.5/feet 41.5；C 相照旧绿]；**NEG-2 摘候选列净空门**[false && 前缀]→恰红 {r2032b}[脱进实心落位柱，diag B x=22.5 feet 44 = 被摘语义本体；A 相最近列本就合法零扰动/C 相无候选不敏感，双实测吻合声明]）→ 双 Edit 反向还原回绿（四日志 matrix_r2032_neg{1,2}_{red,restore}.log 直落 build/ 终名[neg 均 2P/1F EXIT=1；restore 均 3P/0F EXIT=0]；变异字面 0 残留 diff grep 复核[现存唯一 false && 为既有注释非本单]）→ 全矩阵 **664/0 ×2**（matrix_r2032_pos.log + matrix_r2032_final.log EXIT=0；661 权威[r2031_final] diff = **+3 恰 r2032a/b/c** + t813 戳/哈希登记行[23:03/fda962a→00:42/a49b346]；pos/final 腿集合恒等 664[前 110 字符键 diff 空]；^FAIL 归类 0；全文本 diff = vo.light/vo.entity 微计时与 RNG 诊断类[与 r2031 pos/final 同 binary 34k 行同类漂移互证 = 既有噪声类非本单引入]）→ app voxelsandbox 重建 EXIT=0（binary mtime > 全部改动源）+ offscreen 冒烟 EXIT=124 + logs/voxelsandbox_r2032_tail20.log 稳态 60fps[prof[1s] 60fr ×多窗 / frame main*16.0(61)] + F3 win 行 `mesh 0.00(0reb [0d 0s 0w]) worker 0` 与 r2031 基线同值 + 启动日志 500 chunk Models from 100 resident。**登记非目标**：真围死的进一步脱困（1 格头顶挖凿 AI/玩家交互域——冻结维持即登记语义，MC 同款困死）；越窗脱出（±2 列外；有界窗 = 有界成本）；同柱更低层落位复试（首支撑即定——越挂空支撑本不可站）；脱出动画/插值（直接位移 = 与顶起/stuck-escape 同族的确定性瞬移收口）；t1033c 断言本体（文案零行为）。**待实机确认（必列行为面）**：①嵌入+封顶 mob 横移脱出观感（脚下换高支撑 + 头顶 ≤2 格封顶 → mob 平滑移到邻列可站面、无蹦跳/无踩踏级联）；②正常顶起不受影响（净空足场景照旧被顶上新支撑顶行走）。**下一任务 = D6 multi-worker 或按主控队列派单**。

### t1059（2026-09-18，P5 观测前置 + dt 计数小修——F3 流式观测行 + 负/NaN 丢弃分域计数，filter 词 r2033；矩阵 664→667）

→ **✅ 落地（2026-09-18）：fix(t1059) + test(t1059) + 本 docs 三段提交。目的 = §29.4/§29.5 风险登记「生成风暴：……W2 实装后 F3 加排队观测行，P5 调参」兑现前置 + review0916 Info「负/NaN 丢弃无计数」清偿——P5 实机调参（等用户真机）开工前把流式观测数据面备齐。两修皆纯加性观测面，生产行为零变化。设计选型（gamesession.h/frameprofiler.cpp 头注释立此存照；禁触面 = worldstore.{h,cpp}/线程原语/driver-worker 组件语义零触碰，git 历史零触碰）**：①**修一 = F3 stream 行**（行格式 `stream s=N sub=N can=N rej=N out=N adopt=N mesh=N ev[P=N E=N S=N F=N R=N]`，挂在 win ms 行族——报告第 3 行，既有六行逐字不动故 r2026d 行为钉 `w])  worker 0  world` 免修订）。**行内容选型**：(a) 行恒在、流式关时值全零（与 W4 worker 列恒 0 先例同门——不做条件显行，格式恒定可 grep，「行在而零」与「行缺席」可分辨）；(b) 窗口增量语义（与 win 族全行同门：flush 界定 1s 窗——P5 调参视角要每秒速率/背压尖峰非会话总量，总量经日志差分可得）；(c) 数据流 = GameSession 在事件落账点把增量推入 FrameProfiler 计数桶（键前缀 stream*，meshNworker 先例同构——Core 叶子可被任意上层 include 向下推数），flush 拼行读桶清窗，本层零语义。**数据源映射表（零新增聚合源）**：s = 泵拍心跳（streamPump——驱动器在场的泵拍每拍 +1，窗内 ≥1 拍即 s=1）；sub/can/rej = streamDriver()->stats() 三累计对推送基线差分（新成员 m_streamStatsPrevPushed = F3 差分底账非第二账本，聚合权威仍在驱动器）；out/adopt/mesh = streamingOutcomeCount/streamingAdoptedCount/meshHarvestedCount 三收割拍计数的逐拍增量（outcomesDrained/adoptedDrained/meshDrained 本拍局部量）；ev 五分域 = 驱逐轨迹五类事件逐条推送（P=PersistOk E=EdgeEvicting S=EdgeAbsent F=PersistFail R=TransitionRejected——成功三域+失败两域，失败可见 = P3「宁驻留不误删」实机表现面）。**实现发现（首跑红实证即修）**：ev[R] 域口径收窄——驱动器 ensure-Loaded 步对已 Loaded 候选的自转移守卫拒 = 预期 no-op（chunkevictor.h 步1 注释原文「不计失败」），首版追踪 lambda 全量入域致 evR=4 恒噪（r2033b winE 恰红实证）→ 收窄为只记 Evicting/Absent 目标被拒（步2/步3 真失败面），m_evictionTrace kind-4 同步收窄[既有腿零 kind-4 断言零扰动实测]。**QML 例外面 = Main.qml F3 显示段注释一条**（盘点结论：HUD 对报告是整串 Text 渲染 `text: FrameProfiler.report`——新行自动显示，无需逐字段 QML 代码；注释补 stream 行字段释义；Main.qml 禁触词元表[GameSession/streamingActive/MeshWorker/ChunkStore/…十六词元]全避开，r2007b/r2024d/r2026d/r2028c 反探全绿实测）。②**修二 = dt 负/NaN 分域计数**（stepTick 入口既有钳制分支 `!(deltaSecs >= 0.0)` 内 qIsNaN 判据分域：`++m_droppedNanDts` / `++m_droppedNegativeDts`）。**双计数选型**（任务书「两计数或单计数分域」）：负值 = 墙钟回拨族、NaN = Adapter 计算病族，两失败族根因不同 P5 实机须分辨，成本可忽略故不做单计数分域。**行为零变化**：丢弃语义零触碰（仍零累积整体丢弃，review0915 #1 既有腿回归墙 = r2007e 族全绿）；读面 = 与 droppedDtCount 同门纯 C++ const 访问器（droppedNegativeDtCount/droppedNanDtCount，生产零消费可接受）。**腿（section34 置尾，filter 词 r2033；sparse 80×80×96 s42 radius 0 + fixed 48×48×96 s82 + 真临时库 fresh+即删 + ChunkGeometry 桥提交，rig 零接触）**：r2033a fixed 全零墙[静默窗报告含全零 stream 行逐字 + 字段解析器整行可解析（格式正面钉）+ 拼行层/推送面源码正面钉（键读面 ×11+行标+win 族挂载位 / 推送全站点 ×12，剥注释）]；r2033b 流式行计数恒等承重墙[四窗 flush 界定：窗 A 初载 3×3（sub==8/out==adopt==8 精确 + 全字段恒等）/ 窗 B 取消走查（走离拍提交 9→跳回拍半径取消恰 9→被取消窗收敛）/ 窗 B2 网格收割（几何桥提交→tick 尾收割拍，mesh 域==收割增量 ≥1）/ 窗 E 驱逐（会话 2 真临时库 + r2025b 同址编辑 (3,2)→走离拍同步驱逐恰 4：ev[P=1 E=4 S=4 F=0 R=0]）——每窗每字段 == 权威面窗口增量（driver stats 三差分/会话三收割计数/轨迹五分域自基线下标起数）+ s 恒 1]；r2033c dt 计数[负/NaN/超界三类 → 负=1/NaN=2/超界=1 各域精确互不串账（旧 >1s 计数回归柱不动）+ 零变化墙（三类丢弃后 0.11s×3 → 恰 3 tick 无追账）+ 源码钉（两计数语句+两读面+qIsNaN 判据门）]。**验证链全绿**：分段增量构建（-j 4）→ 双 binary mtime > 全部改动源 → filter r2033 首跑 2P/1F 即修一处实现级坑（上记 evR 域 ensure 拒误入）→ 3P/0F 三连跑绿 → 前置回归全族 0 FAIL（41 filters：r2007-r2032 计数逐族同基线[5/5/4/6/3/4/5/4/4/4/4/4/4/4/4/4/4/4/4/4/4/4/3/4/2/4/3P；r2012=5P 既有同词素先例] + t1007=3P/t1020=4P/t1053=1P/t1045=3P/t947=2P/t1033=4P/t1042=5P/review26-1=7P + worldgen 24P/determin 38P/re-gen 6P/store 60P[+1]/save 49P/vertex 15P/mesh 39P[+2]——store/mesh 增量 = 自家腿文词素 substring 先例逐腿归因[r2033b "temp store"/r2033a-b 格式字面与 "mesh"]，零跨任务污染）→ 阴性两轮恰红声明面（**NEG-1 摘 streamSub 推送**[调用体 if (false) 前缀]→恰红 {r2033b} 单腿[窗 A 行字段 sub=0 ≠ 权威 stats 增量 8 = 被摘语义本体；r2033a 不误伤——sub 恒 0 与 fixed 零行同值+源码钉 needle 前缀下在场；r2024/r2026 实跑 EXIT=0 零误伤]；**NEG-2 摘负/NaN 两计数**[两 ++ 语句各自 if (false) 前缀——丢弃语义零变化只摘可见性]→恰红 {r2033c} 单腿[负/NaN 恒 0 ≠ 1/1/2；r2033c 源码钉前缀下在场；r2007 超界族实跑 EXIT=0 零误伤]）→ 双 Edit 反向还原回绿（四日志 matrix_r2033_neg{1,2}_{red,restore}.log 直落 build/ 终名[neg 均 2P/1F EXIT=1；restore 均 3P/0F EXIT=0]；变异字面 0 残留 grep 复核）→ 全矩阵 **667/0 ×2**（matrix_r2033_pos.log + matrix_r2033_final.log EXIT=0；664 权威[r2032_final] diff = **+3 恰 r2033a/b/c** + 登记类[t813 戳/哈希 00:42/a49b346→02:26/a9015b9 + t997 墙钟]；pos/final 腿集合恒等 667[前 110 字符键 diff 空；全文本唯一差 = t997 墙钟数字]）→ app voxelsandbox 重建 EXIT=0（binary mtime > 全部改动源）+ offscreen 冒烟 EXIT=124 + logs/voxelsandbox_r2033_tail20.log 稳态 60fps[prof[1s] 60fr ×多窗 / frame main*16.0(60)] + **流式关 = 新行全零与基线同构**（`stream s=0 sub=0 can=0 rej=0 out=0 adopt=0 mesh=0 ev[P=0 E=0 S=0 F=0 R=0]`）+ win 行 worker 0 与 r2032 基线同值 + 500 chunk Models from 100 resident。**登记非目标**：流式行 QML 逐字段面板（HUD 整串已显行——P5 若要独立面板另立单）；累计总量入行（日志差分可得）；dt 分域计数 Q_PROPERTY 化/信号化（droppedDt 同门纯读面）；冲洗/读档计数（flushPersisted 等save 域稀有事件）入帧窗行（既有会话读面已可观测）；stream 行节流/采样（10Hz 推送在 FrameProfiler 既设计锁保护面内，成本登记可忽略）。**待实机确认（必列）**：①fixed 行全零 = 无新增观感（headless 冒烟已证，实机 F3 目视复核）；②流式世界走查中 stream 行数值合理性（sub/can/rej 随走查速率、out/adopt/mesh 随物化节奏、ev 域随卸载批——P5 真机核数据合理性与调参读数口径）。**下一任务 = 治理审计 #13（主控亲自，计数 5/5：r2029/t1055、r2030/t1056、r2031/t1057、r2032/t1058、r2033/t1059）→ P5 实机调参验收（等用户设备会话）**。

### t1060（2026-09-18，fixed 世界 bake 异步化——C1 完全体，filter 词 r2034；矩阵 667→671）

→ **✅ 落地（2026-09-18）：fix(r2034) + test(r2034) + 本 docs 三段提交。目的 = vulkan-rhi-and-simd-survey-2026-08-21 路线 C1（worker meshing）的完全体：W4（r2026）已把 MeshWorker 接入流式世界，fixed 世界（当前唯一生产模式）bake 仍全同步——用户 2026-08-22 亲测确诊的黎明 sun 重烘风暴（dayMul 跨门 → ~225 段同步重烘 ≈ 100ms 主线程尖刺 → 10fps）正是残留面，本单把它后台化摊平。设计依据 = refactor-plan §29.7（主控执笔）。设计选型（world.{h,cpp}/streamingbridge.{h,cpp} 头注释立此存照；禁触面 = worldstore.{h,cpp}/Main.qml 玩法路径/线程原语新落点零触碰）**：①**MeshWorker 实例归属（fixed）= World 侧惰性单例**（`std::unique_ptr<MeshWorker> m_fixedMeshWorker`，首次异步提交时 `ensureFixedMeshWorker()` 构造 + 自绑定提交缝——lambda 捕获执行器裸指针，受理计数在缝内推 F3 streamSub[fixed 侧]）。**与 W4 流式会话执行器的关系 = 模式互斥的两份所有权、并存不可能**：fixed 世界连流式会话件都不构造（D2 零活动墙，无可复用对象），sparse 世界本类惰性执行器恒 null（reinitializeAsSparse 随模式切换销毁 join 有界 + 使能标志仅 fixed 可置）→「fixed 与流式不重复起线程」结构性成立。ChunkGeometry 共享静态案被否（跨世界活状态 + 生命周期脱钩世界 + 主线程单写者契约面破坏）；流式会话执行器复用案被否（fixed 无会话）。②**收割宿主 = StreamingBridge::pumpTick 挂 WorldClock::ticked 既有拍的薄收割槽**（候选一胜出）：fixed 进入分支（enterWorld 内 `!known || !meta.streaming` 路径——W5b 既有 C++ 面，QML 零改动）调 `world->enableFixedAsyncBake()` + `ensurePumpHook(clock)`（复用与流式泵同一 ticked 连接）+ `m_fixedWorld = world`（QPointer——世界随 QML/矩阵腿存亡，悬垂自动归零）；pumpTick 无会话时拍内调 `World::harvestBuiltChunkMeshes()`。**单拍应用有界 = 风暴摊平静量 kFixedMeshHarvestPerBeat=16**（16 × ~0.1ms 灌注 ≈ 1.6ms ≪ 16.6ms 帧预算；10Hz 拍 → ≤160 段/s 渐进——进世界首建数百段 ~数秒渐显、黎明 ~225 段风暴 ~1.5s 渐变 = MC 天光/光照更新渐进同款观感；余量留队 FIFO 序不变，下一收割拍续排）。候选二「ChunkGeometry 帧驱动拉取」被否：QQuick3DGeometry 无每帧回调沿，500 段各自拉取与 W4「收割拍单点收口」纪律相悖。**MC 引证三元组（2026-09-17 实读）**：minecraft.wiki/w/Light History「Sunlight now has its own light array and optimizations to make dawn and dusk smoother.」+ 同页 Beta 1.8「Day/night cycles no longer require chunk updates and have a smooth transition.」+ modrinth.com/mod/phosphor「works to optimize one of game's most inefficient areas -- the lighting engine」/「nastier frame stutters caused by loading chunks on the client simply vanish」（+ 1.14 光照引擎线程化/排队渐进背景）。③**通电门修订**：`chunkMeshAsyncActive()` = `bool(m_chunkMeshBuildSink) || m_fixedAsyncBakeDesired`——两判据模式互斥（reinitialize* 双归零点），任一为真走异步提交，全假同步内联（裸 fixed 世界默认态 = r2026a 零变化墙延续，实跑绿）。④**env 回退**：`QTVOXEL_SYNC_BAKE≠0` → `enableFixedAsyncBake` 拒绝（qEnvironmentVariable 进程初读一次 static local）→ 门恒关 → bake 全同步内联（旧路径逐位——实机异常自救面）。⑤**双归零点 = reinitializeAsSparse / reinitializeAsFixed**：使能标志清零 + 解绑 sink（先于执行器销毁——自绑定缝捕获裸指针的成员序承重；同时结构性收口「流式会话消亡后 sink 悬垂残留」的既有潜在面）+ 执行器 reset（stop + 丢弃计数 + join 有界，meshworker.h 退出语义）+ 清在途交付注册表。⑥**F3 fixed 侧推送面**：streamSub 在自绑定缝内受理计数（只记真正经由本执行器的受理——sparse 会话 sub 域 = 驱动器 stats 差分独占不串账）；streamMesh 在 harvestBuiltChunkMeshes 按拍应用数推送；meshNworker（win 行 worker 列）由几何交付回调自动携带 → fixed 世界 sub/mesh/worker 三面皆动，行格式与既有字段语义零变化。**腿（section35 置尾，filter 词 r2034；fixed 96×96×96 s82 + 48×48×96 s82 小世界族 + 真临时库）**：r2034a fixed 异步≡同步逐位等价承重墙[四形态（地形/流体水段/异形 cross 并入 PASS1/世界边界）经惰性执行器 + 有界收割拍应用后与裸同步孪生世界直调 capture+build 逐位恒等（全顶点/索引缓冲 + 统计）+ 单实例线程身份（执行==工作≠调用者）+ F3 sub/mesh/worker 三面非零 + 驻留零沿]；r2034b 风暴摊平[6×6 chunk 36 段：首建收敛 → dayMul 跨门（|Δ|≥0.03）→ streamSub 窗增量恰 = 非空段数 36（≤ 队列 64 恒不触满载）→ 收割拍不启动等 built 账恰 36（构建全完成阈值锚）→ 单次收割恰 kFixedMeshHarvestPerBeat=16（上界断言）→ 后续拍每拍 ≤ cap 排干至 totalApplied==36 账面双清 → 终态网格 @dayMul0.5 与直调权威逐位恒等（三抽样）]；r2034c 回退与卫生[env 门（双模确定性：env 令跑 enable 拒绝 + 门关 + 同步 apply-before-return + 执行器缺席；常态跑 enable 成功 + 门开 + 惰性零线程）/ 容量缝 0 退化满载（4 提交全拒 meshNsyncFallback==4 恰等 + 回退字节恒等 + 零在途注册——零时序依赖，r2020b 退化满载同门）/ 析构 join 有界 + 再构造再用]；r2034d 结构钉[env 缝正面钉 + 惰性单例源序（enable 置标先于构造 / 构造在 null 门后 / 提交缝先查门后 ensure）+ 单实例双烘同指针 + sparse 会话不双起线程（gs.meshWorker 非 null ∧ World 侧恒 null）+ 收割宿主单点收口（harvestBuiltChunkMeshes( 全 src 恰 world.h/world.cpp/streamingbridge.h/streamingbridge.cpp 四文件）+ 真桥生产链行为柱（StreamingBridge::enterWorld fixed 分支使能 + pumpTick 薄收割交付——真 WorldStore 临时库）+ MeshBuilder 单一权威反探兼容（world.cpp 零 MeshBuilder::build/kFaces/occlude）+ Main.qml 零新面记号]。**同变更钉面修订（纠偏留痕非削钉）**：r2026d 三处[门表达式 needle 随 `|| m_fixedAsyncBakeDesired` 修订 / takeBuilt 收割落点 2→3 文件（+world.cpp fixed 收割拍）/ 执行器记号 token 反探改白名单正面钉（world.h ownership member 在场 + chunkgeometry 双件仍禁出）+ 腿文如实化]；r2020d ④[执行器记号白名单 {meshworker.h, gamesession.h} → + world.h/world.cpp（fixed 归属宿主转正）；线程原语双文件白名单零扩（world.h 注释剔除 std::thread 字面后 t1023c 实跑绿）]。**验证链全绿**：分段增量构建（-j 4）→ 双 binary mtime > 全部改动源 → filter r2034 首跑 0P/4F 迭代四处腿文坑（B 孪生漏 refreshMesh / 水域 Air-over-Air 写失败 / r2034b F3 断言忽略 flush 基线 / r2034c 几何先挂致 12k 编辑沿重烘 + 棋盘重负段逐顶点 PCF 列扫秒级徒增时长 → 弃用重负拦截改容量缝 0 退化满载 + db 路径错 + 全角括号致 drain 扫略失配 + enable decl needle 容量默认值漂移）→ 4P/0F → 三连跑绿（终版 binary）→ 前置回归全族 0 FAIL（43 filters：r2007-r2033 计数逐族同基线[5/5/4/6/3/4/5/4/4/4/4/4/4/4/4/4/4/4/4/4/4/4/3/4/2/4/3/3P；r2012=5P 既有同词素先例] + t1007=3P/t1020=4P/t1053=1P/t1045=3P/t947=2P/t1033=4P/t1042=5P/review26-1=7P + worldgen 24P/determin 39P[+1]/re-gen 6P/store 60P/save 49P/vertex 16P[+1]/mesh 43P[+4]——增量 = 自家腿文词素 substring 先例逐腿归因[determin←r2034c "deterministic"/vertex←r2034a "vertex/index buffers"/mesh←r2034a-d 全腿 "mesh"]，零跨任务污染；r2034 腿 PASS 行词元自检 = 仅 r2034x 本任务词元）→ 阴性两轮恰红（**NEG-1 摘收割应用**[world.cpp harvestBuiltChunkMeshes 排干循环 while 条件 false&& 前缀]→任务书候选面 {r2034a,b}，实测红面 {r2034a,b,c,d} 四腿[r2034c 相 1/3 收敛面与 r2034d ③⑥ 柱同住收割通路——收割即被测语义本体，diag 逐腿核对均为 pend=N ∧ harv=N 排干超时/bound first=0 的被摘语义本体非连带伤，按 r2024/r2027 先例改声明留痕；r2026 实跑 4P/r2020 实跑 5P 零误伤]；**NEG-2 篡改 worker 输出**[交付前位扰动：requestId 段位域 bits13-15==1 水段且非空时首顶点 u+=0.25f——段位选点红面精确收缩]→恰红 {r2034a} 单腿 = 声明面精确兑现[diag bytes water va=180 vb=180 vm=180——计数面存活，值级扰动非结构级；r2034b 全 terrain 段位 0 不触实跑绿；r2026 4P/r2020 5P 零误伤]）→ 双 Edit 反向还原回绿（四日志 matrix_r2034_neg{1,2}_{red,restore}.log 直落 build/ 终名；变异字面 0 残留 grep 复核）→ 全矩阵 **671/0 ×2**（matrix_r2034_pos.log + matrix_r2034_final.log EXIT=0；667 权威[r2033_final] diff = **+4 恰 r2034a/b/c/d** + r2020d 同变更修订行（110 字符键前缀随腿文如实化变化）+ t813 戳登记类（02:26/a9015b9→04:37）；pos/final 腿集合恒等 671[前 110 字符键 diff 空]）→ **env 回退令跑**（QTVOXEL_SYNC_BAKE=1 --filter r2034 = 4P/0F EXIT=0，matrix_r2034_envsync.log——双模腿文：env 态 enable 拒绝/门关/同步 apply-before-return/退化满载计数恒 0 为 env 态正确面）→ app voxelsandbox 重建 EXIT=0（binary mtime > 全部改动源）+ offscreen 冒烟 EXIT=124 + logs/voxelsandbox_r2034_tail20.log 稳态 60fps[prof[1s] 60fr / frame main*16.0(60)] + 启动 500 chunk Models from 100 resident + **menu 态 stream 行全零与 r2033 基线同构**（boot 世界先于使能点 = boot 零变化墙保持；fixed 使能挂 enterWorld 进入链，app 主菜单态不可达——F3 sub/mesh 非零实证由 r2034a F3 断言面 + r2034d⑥ 真桥生产链腿承载，实机进世界目视复核列入待确认）。**登记非目标**：C4（dayMul/sunDir 挪 shader uniform——根治但大改）/C2/C7/SIMD/多 worker 扩容/流式世界收割语义变化（pumpStreamingTick ⑤ 段排干语义原样）/应用侧单拍常量调参（kFixedMeshHarvestPerBeat=16 出厂值，P5 按实机数据另调）/boot 世界使能（主菜单背景世界保持同步零变化墙——使能挂真实进入链）。**待实机确认（必列）**：①**黎明时分帧率不再 10fps 尖刺**（用户原病灶：dayMul 跨门 ~225 段重烘——预期分帧渐变 ~1.5s 完成无单帧尖刺；debugFast 30s 天可加速复现）；②**天光渐变观感**（星→昼过渡分帧应用 = MC gradual 同款，无跳变）；③**env 回退自救**（QTVOXEL_SYNC_BAKE=1 启动 → 全同步内联旧行为）；④破/放贴图刷新从「<1 帧」放宽为「≤1 tick（100ms）」观感核验（与 W4 sparse 语义对齐——C1 异步化代价，如实登记）；⑤进世界首建渐显观感（数百段 ~数秒渐进 vs 旧单帧 ~百 ms 级 hitch）；⑥进世界后 F3 stream 行 sub/mesh 非零 + win 行 worker 非零目视复核。**下一任务 = P5 用户设备会话（dawn storm recheck 为主项）+ 剩余 survey 项按数据定夺（C4/C2/C7/SIMD/多 worker）**。

### t1061（2026-09-19 立项，P0——无限世界生产入口 livelock：resident 集合永久振荡，世界无法进入；filter 词 r2035）

→ **🚧 开工（2026-09-19，主控接棒 GLM5.3/ZCode 串行队列）**：用户真机首验流式入口即栽——新建无限世界（saves/新世界_2.sqlite，stream_worlds = streaming=1/core 160×160/base_gen=0/save_gen=0，~30 秒强退故无 save_coord）→ enterWorld 后 **resident 集以 ~1 秒周期永久振荡永不收敛**（logs/voxelsandbox.log 10:40:26 起：早期 5↔12，振幅随时间增大至 14↔22；预生成 25 chunk 从未稳定存在；同批 chunk 毫秒级 [dirty]→[sun] 重建且顶点数递减 25500→128→76→0 = 物化→驱逐擦槽→再物化循环痕迹；每次 resident 沿触发 Main.qml 整池重建）。固定世界不受影响（菜单背景世界正常 500 Models/100 resident）。**根因假设（按优先级）**：①驱逐器 keep-set 中心 vs 驱动器请求中心分歧（W1 预生成中心=核心域中心 chunk (5,5)，玩家出生 chunk 由 findSpawnColumn决定可≠(5,5)→预生成/新收养 chunk 反复被"半径外"驱逐再请求——r2019/r2024/r2025 全用 rig 显式一致中心，此生产组合从未被测）；②位置源生产链首个有效沿之前驱逐器就开跑。**修法方向（主控预判，子 agent 读码定选型）**：驱逐 keep-set 与驱动请求集同一位置权威 + 首个有效位置沿之前禁驱逐。流程 = 先真链复现腿（预期红）→ 最小修复 → r2035a-d 四腿（fixed 零变化墙/入口收敛承重/预生成-出生分歧场景/走离走回+中心同源钉）→ 阴性两轮 → 全矩阵 ×2 → 冒烟。状态 = IN_PROGRESS（串行 voxel-dev 实现中）。

## 全弧待实机确认汇总（流式弧收官，2026-09-17——P5 实机会话的清偿清单）

> 各关单「待实机确认」的累计汇总 + 流式批次新增五项。headless 冒烟只能证实例化量/帧稳态/库面语义；**观感面必须真机目视**（dev-spec 惯例）。清偿方式：实机逐项核 → 用户 md 反馈 → 需要改行为的开新单。

### A. 流式批次（§29.5 W1→W5b，本弧新增——P5 主清单）
1. **创建无限世界观感**：新建世界勾选「无限世界」→ 出生区渐次浮现（D4 首屏等待 = P5 调参输入）；未勾选路径与旧行为零差异对照。
2. **走查生成**：无限世界走离出生区，境外地形随走查按需生成（chunk 池随走查渐次浮现的观感、生成速度/排队无卡顿感；F3 生成排队面 + worker 列量级）。
3. **卸载无感**：远离后 chunk 卸载（走离半径外渐次消失无卡顿/无可见突跳）；走离回走**内容仍在**（未编辑区 seed 重生成一致、编辑区在）。
4. **存读往返**：无限世界编辑 → 保存退出 → 重进——改动面完整恢复（存档按钮 + 关窗双路径）；「上次退出未保存」角标不出现。
5. **转换往返**：旧 fixed 世界菜单「转为无限世界」→ 确认弹窗三点文案可读 → 重进核心区原样保留 → 转换后继续编辑再存再进仍一致；（后端翻回已备，界面无入口——如需翻回观感验证走后端单）。
6. 流式世界破/放方块贴图刷新时延（W4：同步 <1 帧 → worker 一 tick 内交付）观感。

### B. P4 相位（§29.4，随 W5b 后首次真机复验）
7. 固定世界观感零变化：进世界地形/水/岩浆/玻璃/冰五段渲染与 P4 前逐像素一致（headless 只证实例化量 500 Models from 100 resident）。
8. F3 对照：mesh 行顶点/三角汇总、drawCalls、chunk updates、worker 列（fixed 恒 0 / 流式走查中量级）与基线同口径。

### C. 历史存量（指针——此前各关单已列，未清偿项随实机会话一并核）
9. t1054 四项矿车 sneak 门：空手潜行右键普通矿车不上车 / 持方块潜行右键 = 放置不骑乘 / 无 shift 照旧上车 / 潜行骑乘中右键不触发骑乘切换。
10. R19.22 16 项 + R19.21 12 项 + parity 波各项观感 + t1048 骑车 500m 节奏待用户数据（详见历史关单「待实机确认」节；agent-state Recovery Point 存量条目）。
11. R20.14 掉落物：合并动画/拾取吸附/爆炸批量掉落/焚毁瞬灭/冰面长滑·薄支撑贴面；R20.15 存档：真实存档往返/异常退出重进/重命名删除世界与封面/退出存档 toast 完成门（生产接线后复验项随 W5b 保存链接线一并复验）。
