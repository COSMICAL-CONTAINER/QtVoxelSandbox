...
- **体素世界**：Perlin 噪声地形、昼夜循环与光照重烘焙；每个世界独立 SQLite 存档（保存代次台账 + 中断恢复提示）
- **无限世界（可选）**：新世界可勾选「无限世界」——按视距流式生成/卸载区块（后台线程生成与网格构建、编辑块随存档落盘、旧固定世界可一键「转为无限世界」且可翻回）；网格构建全程后台线程化，黎明光照重烘焙分帧渐变不再卡顿
- **三模式**：生存 / 创造 / 观察者
...
- **工程化**：671 项无头行为探针全绿（`tools/matrix/`，R20.03 起分层 8 段 TU + `--filter` 子串选腿）、构建版本戳、零警告构建
