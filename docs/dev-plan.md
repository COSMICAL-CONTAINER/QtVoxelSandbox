...
### 🅳 矿井逐方块重建（t1001）
**t1001** 废弃矿井对齐考据：起点厅 10×10 拱顶（四向 1-4 出口）替代现 7×7×4 交叉洞室；巷道支撑**木板+栅栏每 4 格一道**（部分缺失随机）；5×5 双层木柱交叉口（新 piece）；斜坡巷道对角下切（现 L 形折线改造）；**新增洞穴蛛网室 piece**（蛛网高密度围 1 刷怪笼，state=MobSpider 按偏差 1）；铁轨残缺不连续（挖段）；箱落地轨旁（偏差 2）；火把部分巷道。频率经济保留（36 网格×40%，登记）。验收：piece 化生成（起点厅+巷道+交叉口+斜坡+蛛网室按考据组合）。探针 P-t1001：5-seed rig（巷道截面 3×3、支撑间距 4、起点厅 10×10 拱顶、蛛网室存在+蛛网密度+笼 state、轨残缺率窗、箱位）+ 源码钉 piece 表。矿井/沙漠/丛林**当前零矩阵探针**，本探针从零新建（t995 rig 模式）。
