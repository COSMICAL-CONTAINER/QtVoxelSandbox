...
Item {
    ...
    Rectangle {
        ...
        Column {
            ...
            //   等级/冲突提示行包进 opBlock 内容块（width 随操作区、height = 内容自然高 + 冲突多行
            //   余量 120——review0830 #7：108 只容单行冲突文案（冲突行 top=96 剩 12px），2-3 行红字
            //   越过 134 操作区底 ~11px 泼进主物品栏行），整块
            //   anchors.verticalCenter 钉操作区中线 → 上下留白均分（108 时各 13px / 120 时各 7px）。块内锚关系逐字未动
            ...
            Item {
                ...
                //   review0830 #7：块高 108→120——冲突行（块内 top=96）块内可用高 12px 只容单行，
                //   合并冲突文案 2-3 行即越过操作区底；120 给足最坏 2 行实测文案（24px）且 3 行
                //   理论最坏仍不泼进下方主物品栏行（矩阵 t958 文本余量腿运行期量测）。
                Item {
                    id: opBlock
                    width: parent.width
                    height: 120
