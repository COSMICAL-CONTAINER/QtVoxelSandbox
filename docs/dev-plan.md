...
### 🅴 杂项（t991-t995）
**t991** 栅栏几何对齐 MC：木栅栏/石头栅栏「太难受了、完全不符合预期」——**先对照 MC 的样子**再修：木栅栏 = 中心柱（约 1.5 格高）+ 朝连接方向横杆；石栅栏（墙）= 中心柱 + 顶部凸缘 + 低连接杆；相邻栅栏/贴墙自动连接。世界渲染与查看器预览同源修（一处修两面）。验收：孤立栅栏 = 单柱形态；相邻栅栏横杆连接、贴方块面连接；与门/栅栏门衔接自然。
**t992** 按钮几何统一：石头/木头按钮**放地上和放墙上大小不统一——以墙上为准统一**；且墙上形态**比例也不协调**（自查对照 MC 按钮比例修）。验收：同种按钮地面/墙面尺寸一致（墙上口径为基准）；比例协调；按下/弹起动画（如有）零回归。
**t993** 玻璃更实：玻璃方块**透明度还是太透明**——调「更实」一档（边框/面配比或面透明度调整，保持可透视但实体感明显）。pack-on/pack-off 两态同调。验收：玻璃墙实体感明显提升、仍可透视后方景物（用户目视口径）。
**t994** 刷怪笼迷你生物贴图：刷怪笼放生物蛋进去后**能看到生物在笼内悬浮飘动（对）——但只有模型没有贴图、纯灰色**。修笼内迷你 mob 的贴图绑定（t782 三消费端同源纪律：一处修，delegate/图鉴/笼三面一致）。验收：笼内迷你生物带正确贴图旋转飘动；世界内 mob / 图鉴预览贴图零回归。
