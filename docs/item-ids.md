...
> 注：羊毛在两个段都有 —— 方块 `0x1B (27)` = 可放置的羊毛方块（t834/review #7 起杀羊按毛色掉羊毛**方块段**：白→27、有色→63..77）；材料 `0x20E (526)` = 羊毛物品（**已退役**——不再是任何掉落源 / 合成配料，仅旧档物品可显示，见下方「mob 掉落」）。橡树树苗同理：方块 `0x1C (28)` 可放置生长，物品 `0x21B (539)` 是种植原料（破叶 / 破树苗方块掉此物品）。
...
| 0x20E | 526 | `wool` | 羊毛（物品，已退役） | ~~羊~~（t834/review #7 起改掉方块 27/63..77） | 35（block） |
