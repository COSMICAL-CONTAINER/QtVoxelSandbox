...
public:
    ...
    //   收窄到指定槽）。命中 → 返命中距离 tmin（>0）；起点在盒内 / 未命中 / 越界 / 空槽 → -1
    //   （review0906 #13：旧版盒内返 0，「0 ≤ 任何 mobDist」恒真 = 眼位落入车厢盒时点乘员恒判
    //   载具胜——改 -1 视为未中，甄别落回乘员本体。findCartHit（登乘 / 开箱寻的）不改：登乘语义
    //   「挨着车即可上」依赖盒内命中返 0，两条射线口径就此分叉）。
...
private:
    ...
    //   且进谷能量不足以爬升对面坡（t1019 能量判据折算谷底账：v² + 2g·d < 2·kCartSlopeGravity·
    //   (h_对面坡升 + 余量)，d = 到谷心剩余下行水平距离 —— review0906 #6：旧式当前 v² 直比阈在下行半幅
    //   早评漏算剩余下行收益，存在 ~19.8 v² 宽错杀带）且非通电金轨（review0906 #7 收窄：通电 boost 语义
    //   优先；断电金轨谷格照捕 —— t939③ 刹车只管静置车，滑行车旧豁免 = t981 往复复发）→ 以「制动到谷心」
    //   速度律接管本 tick 坡向
    //   物理：a = v²/(2·d)（d = 沿运动向到谷心剩余距离，每 tick 重算 = 自校正常减速），v 与 d 同步归零 →
    //   车一次平滑减速停驻谷底中心（谷心梯度 0 = 静置闸稳定不动点，停驻后所有闸门一致静止，无往复）。
    //   能量足（谷底账 ≥ 2g·h 对面坡升 + 余量）→ 放行交还既有上坡物理按速度爬坡穿过（用户口径「通过或单调
