...
- **日志噪音治理**：`QLoggingCategory::setFilterRules("qt.qpa.*=false\nqt.scenegraph.*=false")` 关掉最啰嗦 debug category，日志从 ~1.2MB 降到几 KB。
  - 证据：`main.cpp`。
- **贪婪网格化（greedy meshing）与纹理图集是天然冲突——图集路径下合并 quad 的贴图必然拉伸，逐格清晰平铺需纹理数组**：经典 0fps greedy 算法按面方向逐层建 2D mask、合并同纹理共面连续格为单个大矩形，顶点/三角数大幅下降（平坦地面 16×16=256 quad → 1）。但本项目走**纹理图集**（一张 atlas PNG，每瓦片子区 UV + 半纹素内缩）：图集采样是 CLAMP，UV 一旦越过 `[u0,u1]` 就采到**相邻瓦片**（不是同瓦片重复），所以无法用「UV 超 [0,1] + REPEAT」实现逐格平铺。结果：合并了 w×h 格的大 quad 只能把**一张瓦片拉伸**铺满整块（近看糊/块状），而非每格各显一张清晰瓦片。**判别信号**：开了 greedy 后平坦地面/大面墙体贴图变糊变大块 = 即此权衡（非 bug）。**通用形态**：凡体素引擎在「图集 + 固定功能材质」路径上做 greedy meshing，必须二选一——(a) 接受拉伸（本工程 t178 初版取此；**t183 实测拉伸不可接受后默认 greedy 关**——`greedyMeshing` 属性默认 false，可开做性能对比 / 纹理数组落地后切回，chunkgeometry.h `m_greedyMeshing = false` 为代码真值）；(b) 迁移到**纹理数组**（per-face `tileIndex` 顶点属性 + `sampler2DArray` 采样、UV 在 [0,1] 内 per-layer REPEAT）= PLAN §2-I 顶点格式 / 自研 RHI 路径，届时 greedy 可兼得「合并 + 逐格清晰」。在迁移前，**把 greedy 做成可关开关**而非硬替换 culled，让贴图清晰度与顶点预算可按场景权衡。
...
- **RHI 后端日志**：`qputenv("QSG_INFO","1")` 启动时打印所选 RHI 后端（走 scenegraph 日志 → 文件）。后端事实（survey 2026-08-21 §1.2 勘误）：**main.cpp 从未调 `setGraphicsApi`，实际后端 = Qt Windows 默认 D3D11**（日志实证 `Creating QRhi with backend D3D11`）；旧记「已确认 Vulkan 生效」为误记（`git log -S setGraphicsApi` 无任何提交）。
...
- **谓词权威迁移（如 isSolid → 碰撞真顶）是「全消费面」迁移，不是「落定链」迁移——凡在新权威产出的稳态（脚位/静息位）下**继续运行**的旧口径谓词（水平碰撞 / 越障 / 采样窗），都会在稳态落进矮支撑格内部的那一刻自锁；且「旧语义不变」的注释承诺只对迁移前的前提成立**：t865 把支撑/落定收口到 supportTopYAt 真顶后，mob 脚位第一次落进支撑格内部（下半砖 +0.5），而 mobAabbHitsSolid 的 y0=floor(feet) 扫描行恰是支撑格自身 → 水平试探恒命中 → 原地冻结；同提交的注释却写着「碰撞实体仍当整格墙挡——旧语义不变」。**判别信号**：diff 引入「真顶/真值」语义的落定链（snap-to-X）时，枚举一切**消费实体稳态坐标**的谓词（碰撞试探的 floor(底面) 行、越障探针的前方脚位格、摩擦面采样格），逐个问「稳态坐标落进格子内部后，这个 floor 还指向我以为的那一格吗」；旧版靠「稳态坐标恒在格边界上」（feet 恒整数）成立的谓词，在真顶语义下全部悬空。**通用修法**：把「贴面排除」原则推广到新权威——格子真顶 ≤ 实体底面 + 容差 = 无严格重叠 = 视穿透（与 XZ 侧 ceil(max)−1 同原则，mobAabbHitsSolid 脚位格豁免 / 掉落物 hBlocked / isJumpObstacle 的 feetY 参三处同判）；豁免只对「正站其顶」的格生效（走向矮墙时底面−格底 < 盒高 → 真顶 > 底面 → 不豁免），墙语义不回归。**自检**：写「收口到真值」类提交时，grep 全部 floor(实体坐标) 消费点画一张「稳态坐标 ↔ 格边界」对照表——任何一行的成立前提写着「坐标恒整数/恒在格上方」就是半改态。
  - 证据：Review_2026-08-26 #1/#2/#3 同根修复（d2d4c22）——三豁免 + isJumpObstacle feetY + 掉落物 hBlocked；若只修 #1 不修越障口径，解冻后的 mob 会在矮支撑地面上全程兔跳（fy=脚位格恒命中自身同类）——「解冻」与「不跳」是同一收口的两个面。

- **物理 tick 里的静置早退分支（resting→continue）会静默杀死**后加在它下方**的特性块——特性块的放置位置必须核它对「每个早退态」的可达性；而钉住「意外冻结行为」的探针（终点位断言）在冻结被修复后会连坐，须重锚到真契约点**：t468 水平滑行块加在 resting 复探的 continue 之下 → 着地后滑动只剩落定那一拍，「冰面长滑」从出生即死（摩擦注释声明的 ~0.5s 停/冰面数秒从未兑现）；修复后 t886 获物探针（断言终点位 <2.2 格）连坐——获物触底后按摩擦合法滑出 rig 短平台。**判别信号**：(1) 给物理循环新增特性块后问「resting/停靠/冻结态的实体执行得到它吗」——上方有同态 continue 即不可达；(2) 引擎语义修复后既有探针红，先分辨探针钉的是「真契约」还是「意外行为的快照」——终点位在冻结时代 = 触底点，解冻后终点位合法漂移，真契约（弹道解准确性）锚在触底点。**通用修法**：静置态早退改为「跳过重力、保留水平/摩擦等与该态兼容的段」（groundedRest 门控），而不是整段 continue；连坐探针改锚首次触底时刻（resting 首翻 true 的采样）而非最终静止位。**自检**：diff 在某早退分支**下方**新增代码块时，列出该分支早退时被跳过的全部新段；修「冻结类」缺陷时列出钉住冻结副产物（终位/终速）的探针清单。
  - 证据：Review_2026-08-26 #2 下半（d2d4c22）——resting 有支撑改 groundedRest 门控（免重力、保水平积分+摩擦），t886 探针改钉首次触底点距玩家 <2.2（触底点 = 旧冻结终点位，语义等价迁移），矩阵 294 全 PASS。

- **输入动作的重路由（A 目标改判 B 目标）必须「值门 + 落回」而不是无条件转移——重路由调用的返回值不消费 = 在「射线够到 A 但够不到 B」的几何里点击被静默吞掉（反馈已发、效果为零）**：t866 把「打乘员」重路由到「打车」时无条件 return，而乘员 AABB 高出车盒一大截——瞄乘员可见上身时射线命中 mob 却不与车盒相交 → 既不打 mob（回归）也打不到车（目标落空），冷却+挥手照发 = 吞击。**判别信号**：任何「改判/代理/转发」形态的输入分流，grep 被转发调用的返回值是否进了条件——弃值调用 + 随后 return 就是本坑；两目标命中域不重合（一个 AABB 含另一个）时必然存在只够到原目标的射线。**通用修法**：重路由调用改成「原门控 && 返回值」联判，返回 false 落回原路径（原路径自带冷却/反馈门，不重复实现）；重路由射线长度用**已命中的那次测试的距离**（m_hitDist）而非全程 reach——与「实体比方块近才优先」口径对齐，顺带消掉隔墙打远处目标。**自检**：写重路由时画两目标 AABB 的不重合区，问「射线落在这个区里点击去哪了」；探针同构——源码钉必须钉「返回值被消费」的形态（如 `&& ...hit*FromRay(...)` 前缀），裸调用名匹配钉不住无条件 return 回归。
  - 证据：Review 2026-08-26 #4（5c79ee7）——值门 + attackMob 落回 + m_hitDist 射线（船分支同构同修）；t866(a3) 源码钉改钉值门形态 + (a4) 行为几何钉（Shambler 上身高度水平射线 findMobHit 命中 / findCartHit -1），阴性验证回退旧码恰 a3 红。

- **根因定位为「守卫 G 对数据形态 D 恒误判」时，修复必须是守卫族的全量清点——只修出过症状的两处 = 疾病在其余 N 处原样存活；且清点要分「读半边」（显示，truthy 即可）与「写半边」（数据保全，必须归一）两种修法**：t874 定根因「C++ 序列对象 Array.isArray 恒 false」却只改 AnvilUI+Main.qml 两处，其余 21 处光晕恒灭/攻击行恒缺原样（review 清单），grep 全量清点再 +7 处写半边守卫（含创造中键复制附魔静默清白板——比显示缺失更重的数据丢失向）。**判别信号**：根因陈述里含「恒 false/恒 true/恒空」类形态词时，grep 该守卫的全部出现点逐个分类「输入是否可能是 D 形态」——凡输入源是 Q_INVOKABLE 返回 / property var 绑定 VM 调用的，全是患者；输入源是 QML 字面量 JS 数组 / 已归一化输出的，是合法用法别误改。**通用修法**：读半边（visible/text 类消费）降为真值守卫（序列下标/length 可读）；写半边（写入存储前的守卫）必须在边界归一成真数组（list4 模式）——写半边的兜底臂方向是**数据销毁**（带附魔输入被兜成空），比读半边的「不显示」重一等。**自检**：任何「X 恒 Y」根因的修复提交，验收标准写成可 grep 的源码钉（目标形态清零/只剩白名单），并在白名单注释里写明每类为什么合法——下一个读到的人才知道边界在哪。
  - 证据：Review 2026-08-26 #9（5c79ee7）——28 处收口（21 清单 + 7 新发现），9 处合法用法保留；阴性验证改回一处 isArray → grep 源码钉即红。

- **同一帧内多个呈现通道（车 delegate 与其乘客 delegate）刷新频率不一致时，锯齿不在「数据旧」而在「采样节拍不同」——修法是给需要高频的那**一条**绑定开专用低触发面 revision，而不是把共享 revision 的发射频率升回去，也不是把高频通道降到低频对齐**：矿车每帧 notifyChanged（60Hz）而乘客走 EntityManager 的 20Hz 相位门 → 快速车载乘滞后 ~0.4 格、每 50ms 跳变；直接复用 entitiesChanged 每帧发射会复活 t500 的「count × ~12 条 revision 绑定激活 + MobModel 几何重建」卡顿（review25 #3 才收口），把车降到 20Hz 则拖累玩家骑乘观感。**判别信号**：两个 delegate 读同一物理量（乘客 pos 钉自车 pos）却挂不同管理器的 revision/NOTIFY，且其中一个管理器每帧发射——频率差 × 移动速度 > 半个身位即肉眼可见锯齿。**通用修法**：低频侧加专用 revision（如 rideRevision），发射条件 = 「需要高频的语义事件真发生」（钉位值真变 = 载客载具移动帧），QML 侧只让目标那一条绑定触碰它——非目标 delegate 单帧重采样读到相同/静息值 = 廉价 no-op，卡顿面（多条绑定 + 几何重建）不被触碰；seat 公式等游戏常量仍单源于 C++，不在 QML 重抄。**自检**：给任何「节流 emit」收口时，列出被节流实体中「跟随后端每帧移动源」的子类（骑乘/牵引/附着），问它们的前端呈现是否需要与源同帧——需要就走专用 revision 旁路，别动共享门。
  - 证据：Review_2026-08-26 #10（131f263）——rideRevision/ridersChanged + mob delegate position 绑定独占触碰；探针 review26-10 钉「移动帧恰 +1 / 停驻帧零 bump」，阴性验证禁用 emit 恰 1 FAIL，矩阵 300 全 PASS。

- **纯视觉 Timer 的暂停语义必须挂「世界模拟总闸」（worldRunning）而非「应用状态」（appState/active）——硬暂停（ESC）下 appState 仍是 playing，只绑 appState 的粒子流/翻书帧/循环动画在「一切停」的声明下照跑；且这类漏网成族出现，修一处必须全仓清点同族**：t873 字形流、t649 符文流、t465 碎屑池、t489/t724/t725 材质翻书帧、附魔书翻页、t878 爱心动画六处同病（t884 自己记得 gate、其余全漏）。**判别信号**：任何 Timer/循环 Animation 的 running 条件只含 appState/active/visible 类「界面在不在」谓词、不含世界模拟谓词，而它驱动的又是**世界锚定**视觉（世界坐标粒子、世界方块材质帧、世界内实体挂件）——ESC 试一遍即漏。**清点判据**（避免误 gate）：问「这个 Timer 驱动的是世界锚定视觉还是 UI chrome」——世界锚定（输出随世界时间流动：发射/飞行/翻帧/循环动画）→ gate worldRunning（冻结语义 = MC Java 单机暂停粒子冻结，恢复自然续飞，比「暂停期继续飞完」更贴切）；UI chrome（toast/聊天淡出/面板闪光/预览窗）或「输入冻结期输出必静态」（玩家位冻结下的 atan2 朝向、世界钟停下的图标帧）→ 豁免并写明理由。**注入形态**：Loader 加载的组件经 onLoaded Qt.binding 注入 window.worldRunning（同 active/world 注入先例），不在加载文件里直引跨上下文 id；imperative start() 常开的 Timer 改声明式 running 顺带消掉菜单期空转。**自检**：实现任何「两档暂停」语义时，grep 全仓 Timer/Animation 的 running/active 条件逐一归类，清点表落进 commit/Review——漏网是族不是个例。
  - 证据：Review_2026-08-26 #11（131f263）——6 处 gate + 10 处豁免（含 t884 已 gate 的 bobber）；探针 review26-11 源码钉（running 契约 + 注入点 + imperative start 退役），阴性验证拆一处 gate 恰 1 FAIL，矩阵 300 全 PASS。

- **行为级探针的假绿有两类结构性来源：辅助驱动器与断言目标未被物理隔离（旧代码的缺陷形态本身会给驱动器开新通道），和基线采样落在触发窗口之内（效果已发生才取「前值」）——两者都在「修好的代码看起来对」时埋雷，只在阴性验证（回退旧代码期望红）时暴露**：(a) 驱动器污染——review26-14 首版用「推手追着行进车走」驱动撞击，旧代码下车互穿自由 → 行进车穿进停驻车后推手进入其 0.8 格推动半径 → 推手直接推走目标车，旧代码也绿；修法 = 给辅助驱动器加距离门（推手只在行进车距目标 ≥1.35 格时跟进，推手-目标距恒 ≥1.75 > reach），并验证「修好绿 / 仅回退该项恰红」双向。**判别信号**：rig 里任何会移动实体的辅助源（pusher/跟踪器/发射器）与位移断言的目标之间，问一句「旧代码的缺陷形态（互穿/冻结/绕过）会不会让辅助源以新方式够到目标」——会就加距离/次数门。(b) 基线时点——t856(d1) 把 items/primed 基线取在「拉杆激活 + tickN(4)」之后，而发射恰恰发生在那 4 tick 里 → 退化掉落物被算进基线，+1 断言恒假红；修法 = 基线永远在**装填完成、触发器放置之前**取，触发后的世界只剩增量可断言。**通用自检**：写「计数差/位移差」类断言时，画出 rig 时间线标出「效果发生 tick」与「基线捕获点」「断言读取点」三者的先后；做阴性验证不是走过场——它同时校验探针的两个假绿/假红源，本批 #14 探针正是靠它发现自己被污染的。
  - 证据：Review 2026-08-26 低危批阴性验证——六个行为修复全量回退跑一轮（恰 5 探针红）暴露 review26-14 假绿，加固推手距离门后单独回退 #14 恰其独红（36f6e75）；t856(d1) 基线时点修正随 #24 探针同步（6e06e2a）。

- **给已有「批量收口」的写入链新增触发源（新钩子 / 新级联 / 新 BFS）时，收口只保护旧路径——新路径每次触发都自带全套 per-cell 昂贵汇点，等于把收口消灭掉的风暴从侧门重新引入；判别法是把「一次用户操作 → 昂贵汇点调用次数」写成公式，次数含 O(格数) 且格数能被放大到数百即风暴**：t320 把爆炸收口成「一次 refloodBox + 一次 worldChanged」，但 t930 给每个破坏格挂的 26 邻域级联里，dropGravityColumn 对**每格**调 recomputeLightAround（每次 = ±15×到顶全盒重 flood）+ **每柱**发一次 worldChanged QML 扇出——沙坑爆炸 = 数百次全盒重 flood + 数百次扇出 = 8FPS，而爆炸本体（t320 路径）零嫌疑。**判别信号**：review 或实现任何「编辑钩子 / 连锁传播 / 批量破坏」类新触发时，沿它到昂贵汇点（光场重 flood、mesh 重建请求、QML 信号扇出、**qInfo 落盘**——logHandler 锁 + flush 是隐性每调用磁盘同步）数调用次数；同一汇点若已有批量收口先例（destroySphereSilent / flushPendingLightEdits），新路径没走它就是回归。**通用修法**：批内联合盒（m_fluidAct* 活动盒同模式）——per-cell 昂贵汇点收口到批末一次，范围 = 全部被写格外接盒 ± 影响半径；等价性论证固定为「每格影响 ⊆ 其 ±R 盒 ⊆ 联合盒，盒外不受影响 → 边界种子法终态与逐格重算一致」，并配行为级终态钉（见天格 ==15 / 遮挡格 <15）防盒算错留暗格。**自检**：diff 给写入链加触发源时，grep 该链上每个「编辑即调」的昂贵函数，确认新触发路径要么复用批量入口、要么自带批末收口。
  - 证据：t933（bc887c6）——dropGravityColumn 柱末一次 refloodBox + cascadeGravityAround 级联末一次联合盒 reflood + 一次 worldChanged；探针钉 200 格坍落 reflood ≤4 / worldChanged ≤3，阴性轮（去级联批）rf 101 / wc 101 恰红。per-call qInfo 同罪：退役为 1s 窗聚合计数（>3ms 才落日志）。

- **headless「跨世界泄漏」排查 rig 的三要素：稳态零断言电池（tick 全系统后全零）、换世界真实路径后同零、以及「先让世界自然瞬态收敛再找位 / 断言」——否则合法的世界自愈（worldgen 水系首轮全量扫描后才稳定）会冒充泄漏残留**：t933 探针首两轮假红均来自 rig 自身——(1) 固定坐标假定「高空必空」被到顶山体证伪（staging 静默失败 → 断言空转）；(2) 找位在沉降前：首轮全量扫描会把 worldgen 未稳水系重排（泉眼 / 瀑布落在沉降后才出现的位置），rig 平台压上即堵水柱 → 断言窗内合法排水写入被当泄漏。**判别信号**：零断言电池报非零时，先用逐系统归因 + 写入坐标日志定位写入者空间位置——坐标在 rig 区内 = rig 与世界瞬态相互作用（改 rig），坐标远离 rig 且随窗递减 = 真残留（修代码）。**通用修法**：rig 位程序化搜索（干盒 / 空气盒谓词扫描，不信任「地形最高 ~N」类经验值——种子换了就翻车）；断言前跑沉降循环（推进 tick 直到连续 N 次零写入）；电池逐系统分立计数使 FAIL 自带归因。**自检**：任何「稳态必须全零」的断言，问一句「世界从生成 / 读档到稳态的合法瞬态清单（水重排 / 红石重算 / 附着物复检）是否都被预热窗或沉降循环吸收」——没有吸收通道的瞬态迟早 flakes。
  - 证据：t933（bc887c6）——先沉降后找位顺序契约 + findDryBox/findAirBox 回退搜索 + 逐系统 sysSame/sysRegen/sysLoad 归因数组；修复后矩阵 348 全绿（347→348）。
