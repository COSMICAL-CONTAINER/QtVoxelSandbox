...
### 🅱 矿车与铁轨（t981-t985）
**t981** V 形轨谷矿车振荡：矿车放在 V 形铁轨（两侧斜坡相对成谷）上滑落到底部时**卡住、来回振荡**最后才停下。谷底应一次平滑减速停驻（或按速度通过），不允许往复振荡。注意 review0831 #28「embedded-cart sampling-lost freeze escapes the exemption contract」登记项同域——本任务实证其是否即此症并一并清算。验收：V 谷场景矿车滑底一次停驻、无往复振荡；平轨/常规坡道行为零回归。
**t982** 铁轨转弯+上坡互斥（用户口径：**一格铁轨绝对不可同时转弯和上坡，只能做到一点**）：现在构造坡上转弯场景，**转弯贴图被拉伸 45° 兼作上坡**。修轨道形状判定——turn 与 ascend 互斥（坡上转弯处按 MC 口径落成平转弯段或直上坡段之一）；斜坡段几何/贴图不再借用转弯贴图拉伸。验收：坡顶接转弯场景逐格形状符合互斥表；无 45° 拉伸贴图；直线坡/平转弯各自零回归。
**t983** 铁轨平行吸附 + 站轨选块：① 三种铁轨**平行放置时吸附作用怪异**、「完全不遵守规则」（连接判定对齐 MC：平行相邻轨不互连，只有端点相对才连接）；② **站在铁轨上朝前放置前向铁轨**：准星已指向目标位置却选不中——实际选中的是**脚底下的铁轨**。选块必须指哪指哪（疑非完整方块命中盒用整格 AABB 抢命中，准星视觉所指与命中盒不一致）。review0830-B #12「under-line parallel-track tradeoff」登记项在本任务翻案清算。验收：平行轨互不吸附改按 MC 连接规则；站轨上前向放置命中准星所指格；探针钉射线命中优先级。
