...
### 🅳 资源查看器（t989-t990）
**t989** 移除生物蛋（t967 局部翻案，用户 9-01 口径：「生物蛋纯属多余，上面已经有生物的查看了」；且小僵尸蛋与别的蛋**风格不统一**）：查看器**整段移除**生物蛋分区/入口（t967「生物蛋并入生物类」决策按「用户后来的话优先于旧任务结论」翻案），三分类归属、单选中收口、预览链随之同步。验收：查看器无生物蛋面；三分类/单选中/预览全链零回归（P-t967 探针按 P-t949(d) 先例合法演化）。
**t990** 方块默认视角回归俯视：查看器方块默认视角**从俯视变成仰视了**（用户记得之前不是这样）。git -S 定位回归源（t966 pitch 父拆层的 -22 基偏疑翻号/丢失）后修复——默认应**自上向下俯视**；t877/t966 拖拽定律与四相位行为不动。验收：打开查看器任一方块默认俯视；拖拽交互零回归（P-t966 行为腿全绿不改判）。
