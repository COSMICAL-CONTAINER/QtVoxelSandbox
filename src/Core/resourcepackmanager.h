...
public:
    ...
    // 复审 #8（2026-08-22；Review 2026-08-23 #1 修正探测区）玩家皮肤 slim（仅臂宽 3px）布局查询（呈现层
    //   PlayerSkinBox.slim 联动）：pack 命中的皮肤按臂区尾 alpha 探测——右臂条带 classic 宽 16px 占 u40..56、
    //   slim 宽 14px 占 u40..54 → 探测区 u[54,56)×v[20,32)：classic 该区 = 臂背面必不透明、slim 该区 = 条带外
    //   必透明；高度不足 32 行的残图保守 classic（缓存 skinSlimFlags，apply() 重建时清）。pack 未启用 / miss →
    //   false（classic 4px——程序皮肤按 4px 绘制）。QML 函数内读 active/skinName → 绑定依赖齐全。
