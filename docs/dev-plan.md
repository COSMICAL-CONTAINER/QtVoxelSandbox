...
### 🅵 沙漠神殿逐方块（t1003）
**t1003** 沙漠神殿 15×15 → **21×21 逐方块**：四面安卡纹样（**橙/蓝羊毛**，偏差 3 时代口径）+ 正面主入口 + 后侧两副入口 + 顶窗；地面风玫瑰棋盘纹 + 中心蓝羊毛块；蓝块正下石压板接 **3×3 TNT×9**（现已有 TNT 陷阱雏形，按考据重摆）；地下暗渠**藏 4 箱**（现 4 箱保位按考据坐标）；切制砂岩家族替代刻纹（偏差 6）。频率/群系门保留。探针 P-t1003：5-seed rig（21×21 足迹、安卡纹样对称面、风玫瑰+中心蓝块、压板→9TNT 链、4 箱暗渠、副入口/顶窗在场）+ 源码钉。沙漠神殿首个行为级探针从零新建。
