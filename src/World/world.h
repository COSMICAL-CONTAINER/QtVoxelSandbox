...
//    ——r2022d「新面禁 Q_INVOKABLE 形态」同门；fixed 世界零绑定零调用 = 零变化墙）────────────
    ...
    //   —— 剩余时长单独由 setWeatherRemainingSec 精确续跑，见下）。态真翻才 emit weatherChanged
    //   （驱动 QML 天空变暗 / 粒子切换）。
    ...
    //      避免双重掉落。）④：本格新放了**整立方方块（isFullCube，shape==ShapeFull）** → 水平 4 邻任一为
    //      Cactus 即「邻接方块」→ 该 Cactus 整柱掉落。**t984 口径翻案**（用户 9-01「我的口径是能放下来，
    //      而不是仙人掌会掉落」）：门槛由「非 Air」收紧为谓词门槛 —— 轨族 / 火把 / 压力板 / 雪层 / 半砖等
    //      非整立方邻接仙人掌不触发（放置成功、仙人掌不动）；**review0903 #1 定为 isFullCube**（isSolid 代理
    //      对 Glass / Ice / Spawner 等 solid=false 但 ShapeFull 的整立方漏放），与 t503 worldgen 4 邻守卫
    //      同谓词同源（worldgen / 放置 / 挖除口径一致）。
