...
### 🅲 狼与豹猫（t986-t988）
**t986** 项圈一圈：狼/豹猫驯服/未驯服时**脖子上的项链完全不明显**——「差不多看到两个红点的样子」，用户要**围绕脖子一圈的项链**。先查 pack 资产/材质包内有无现成 collar 贴图区（wolf 类 entity 贴图），有则贴图驱动；无则几何环带程序自绘（IP 门：不抄 MC 资产）。驯服/未驯服显隐语义按现状保留。验收：驯服狼/豹猫脖子可见完整一圈项圈、远看可辨；未驯服态语义正确。
**t987** 坐姿返修（兔子病）：狼/豹猫**趴下（坐）姿态完全像兔子**——后脚直接多出好长一节接触地面的部分、腿凭空长高一节，「完全不像趴下的样子」。修坐姿肢体变换（四足坐姿：臀部落地、后腿前折、前腿立撑），消灭穿地长腿。验收：坐态臀部着地、后腿折叠贴身、无穿地伸出段；站/走/坐切换无跳变。
**t988** 狼战斗 AI 缺陷：狼帮我打僵尸时 **AI 还是不会走路**（应主动接近目标），且**遇到要跳跃才能上的格子不会跳、卡在那里**。修 tamed wolf 攻击/跟随态的移动驱动 + 越障跳（review0830-C #14 修的是咬范围内禁跳——本任务是其反面：移动途中 1 格障碍必须跳）。验收：指狼打僵尸，狼奔跑接近、途中 1 格台阶自动跳上、抵达后撕咬；平地跟随/待机零回归。
