...
### 🅶 丛林神殿逐方块（t1004）
**t1004** 丛林神殿单层 → **3 层苔石建筑**（考据口径）：底层**三拉杆谜题**（3 输入块上 3 拉杆/按钮【拉杆若无则按钮，实现工核 palette】，正确组合 → 开墙显主层箱——开墙机制走局部方块事件链，**偏差 4：无活塞**）；**2 组发射器陷阱**（走廊中段 + 转角箱前，压力板触发替代绊线【偏差 4】，藤蔓/蛛网遮蔽，箭 2-14 发【Dispenser 已有】）；2 箱（陷阱走廊尽头 + 谜题后）；cobble/苔石混排按考据。探针 P-t1004：5-seed rig（3 层形制、谜题组合唯一正确+开门腿、陷阱位置+发射器箭数、2 箱位）+ 源码钉。首个行为级探针从零新建。
