...
**t1024 床的完整语义**：床方块已存在（bedmodelgeometry/放置）。补 MC 核心两件事：①**睡觉跳夜**——夜晚右键床进入睡觉（视角锁定+黑屏渐变），天亮唤醒；雷暴/敌对近身（8 格内）时"你不能休息，附近有怪物"；②**重生点设置**——成功睡过的床=重生锚（存档持久化，deathScreen 重生回床+「重生点已设置」提示；床被挖→重生点失效回世界出生点）。探针：睡→跳夜时间面/重生点 round-trip/敌对近身拒绝/挖床失效。
