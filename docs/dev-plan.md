...
**t1046 parity 小修合集**：①拉杆/按钮补 `!sneakPlace` 旁路（t1034 同式）；②音符盒 glass→hat 第四族（倍移近似；Glass 脱 GroupStone 前先盘点 materialGroup 影响）；③幼崽血量回成体上限（繁殖+蛋孵两沿）；④天气剩余时长持久化（world_meta 追加键，存档契约尾追加）；⑤小麦种子基准钉死（wiki 三元组引证：Java 现代 1-4 / 旧版 0-3，按项目基准版本取值，探针随钉）；⑥ride_minecart 对齐 MC 1km 累计 + 发明成就 UI 标注「原创」（既往批准功能不删除）。
